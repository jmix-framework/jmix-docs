...
Управляет созданием файла `app-info/app-info.json` в <<jmix.core.conf-dir,каталоге конфигурации>>.

Инструменты разработки используют этот файл, чтобы получить сведения о запущенном приложении: его URL, активных профилях Spring и времени запуска. Jmix создаёт файл при запуске приложения и удаляет при штатном завершении работы. Если приложение запущено фреймворком тестирования, файл не создаётся.

Чтобы отключить создание файла, установите для этого свойства значение `false`.
