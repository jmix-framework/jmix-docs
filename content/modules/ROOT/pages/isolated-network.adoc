...
Для Jmix версии 2.x+ на изолированный компьютер также нужно перенести архив с npm зависимостями, необходимыми для работы frontend'а.
...
. На компьютер с доступом в интернет, помимо ранее установленной утилиты `deptool`, также необходимо установить Node.js с npm менеджером пакетов - инструкция по установке https://docs.npmjs.com/downloading-and-installing-node-js-and-npm.
. Откройте терминал, перейдите в подкаталог bin утилиты и запустите команду: 
+
--
[source,bash]
...
PATH=$PATH:$JAVA_HOME/bin:/home/$USER/work/gradle-8.2/bin
----
...
Откройте терминал, перейдите в каталог `~/work/idea-IC-232/bin` и запустите команду:
...
Для работы плагина Jmix требуется наличие файла `sif.dat` в конфигурационном каталоге IDE. При работе онлайн он создается автоматически при активации Jmix Studio. В изолированной сети его необходимо перенести вручную в каталог `~/.config/JetBrains/IdeaIC2023.2` (соответствует версии вашей IntelliJ IDEA, см. подробнее в https://www.jetbrains.com/help/idea/directories-used-by-the-ide-to-store-settings-caches-plugins-and-logs.html[документации IntelliJ^]). Данный файл можно либо скопировать с компьютера с уже активированной Jmix Studio, либо получить по email, направив запрос по адресу `info@jmix.io`.
...
* После сохранения настроек Gradle начнется импорт проекта и завершится с ошибкой вида `org.gradle.api.plugins.UnknownPluginException: Plugin [id: 'io.jmix', version: '2.1.0'] was not found ...`
+
...
* Для доступа к npm артефактам

** Создайте в корне проекта файл `.npmrc` и добавьте строчку `registry=http://localhost:8081/repository/jmix-npm/`. Информацию о других возможных настройках этого файла можно найти в документации https://docs.npmjs.com/cli/v10/configuring-npm/npmrc.
