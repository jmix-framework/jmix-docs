...
下载 JDK 21 或 25（例如 https://adoptium.net/temurin/releases/?version=21[Eclipse Temurin^]），然后根据安装包的提示进行安装。这对于从 IDEA 中运行应用程序已经足够。

NOTE: Jmix {latest-version} 支持使用任何供应商提供的 Java 21 或 25 构建应用程序。但是我们不推荐使用基于 OpenJ9 的 JDK，因为之前出现过问题。
...
可以通过使用 AI 编码智能体来加速 Jmix 应用程序的开发。

为了使 AI 代理能够有效地处理 Jmix 范式、架构和最佳实践，我们建议使用 https://github.com/jmix-framework/jmix-agent-toolkit[Jmix Agent 工具包^]。其中提供：

* 架构概述：深入理解 Jmix 特定的模式。
* Agent Skills：用于开发实体、UI 视图、数据访问等的专业知识。
* 开发工作流：为 AI 代理优化了遵循 Jmix 特定强制性规则的指令。
* 配置推荐的 MCP 服务和 CLI 工具。

使用 Studio 的 xref:studio:tool-window.adoc#settings[AI Agents Toolkit] 操作或 https://github.com/jmix-framework/jmix-agent-toolkit#quick-cli-installation[CLI 脚本^] 安装工具包。
