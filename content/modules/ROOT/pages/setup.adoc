...
Download JDK version 21 or 25 (for example, https://adoptium.net/temurin/releases/?version=21[Eclipse Temurin^]) and install it to a directory suggested by the installer. This is enough for creating and running applications from the IDE.

NOTE: With Jmix {latest-version}, you can build applications using Java 21 or 25 from any vendor. However, we don't recommend an OpenJ9-based JDK due to the problems it has caused in the past.
...
The command should return the Java version, e.g. `21.0.2`.
--
...
To enable an AI agent to work effectively with Jmix patterns, architecture, and best practices, we recommend using the https://github.com/jmix-framework/jmix-agent-toolkit[Jmix Agent Toolkit^]. This toolkit provides:
...
Use the xref:studio:tool-window.adoc#settings[AI Agents Toolkit] action in Studio or the https://github.com/jmix-framework/jmix-agent-toolkit#quick-cli-installation[CLI script^] to install the toolkit.
