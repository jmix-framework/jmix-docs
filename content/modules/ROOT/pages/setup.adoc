...
*Jmix Studio* is implemented as a plugin for IntelliJ IDEA. The minimal supported IntelliJ IDEA version is 2022.1.
...
. Start IntelliJ IDEA (2022.1 or newer).
...
Download JDK version 11 or 17 (for example, https://adoptium.net[Eclipse Temurin^] or https://bell-sw.com/pages/downloads[Liberica^] JDK) and install it to a directory suggested by the installer. This is enough for creating and running applications from the IDE.

NOTE: With Jmix {page-component-display-version}, you can build applications using Java 11 and 17 from any vendor. However, we don't recommend an OpenJ9-based JDK due to the problems it has caused in the past.
...
The command should return the Java version, e.g. `11.0.14`.
--
