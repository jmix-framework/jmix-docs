...
Скачайте JDK 11 с https://adoptopenjdk.net[https://adoptopenjdk.net^] и установите в каталог, предложенный установщиком. Этого достаточно для создания и запуска приложений в IDE.

NOTE: Jmix поддерживает Java 8 и 11. Обратите внимание, что OpenJ9 JVM не поддерживается.
...
Команда должна вернуть версию Java, например `11.0.9.1`.
--

CAUTION: Если вы подключены к Интернету через прокси-сервер, некоторые системные свойства Java должны быть переданы в JVM, на которой запущены средства разработки и Gradle. Настройте эти свойства с помощью Jmix Studio.
