...
You should register a Jmix account to use Jmix Studio or to manage your subscriptions. You can do it both on the website and xref:studio:subscription.adoc#sign-in-to-account[in Studio].
...
+
When using the *Wire transfer*, select the currency. Verify the total sum and click on the *Complete Purchase* button. The bank transfer invoice will be sent to your email. Also, you can find it on your <<posted-invoices,account page>>. Please make your payment to the bank account specified in the invoice.

. After successful payment, visit the <<subscription-management,subscription management page>>. Assign licenses to your colleagues or manage the parameters of your subscription.

[[individual]]
=== Individual

An Individual license is an option for private individuals who purchase a license with their own funds and solely for their own use. Individual licenses must not be purchased, refunded, or financed in any way by companies.
...
. As soon as the Jmix team validates your data, they will email you with further instructions. Usually, it takes no more than one working day.
...
To view details of a subscription, change a plan, or cancel renewal:
...
** _Admin_ is a role with full access. An admin sees all the keys in the subscription in the company profile and can manage payments.
+
...
* Re-assign this key
* Send an email to the user with instructions to activate a license
...
NOTE: It is not possible to change the number of users and the plan at the same time. You should do this in two steps.
...
. To delete the user, click on the *Delete* button next to this user. If the user has a license key, you should unassign the license key from this user before deleting the user from team members.
