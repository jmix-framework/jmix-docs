...
NOTE: При реализации пользовательского маппера OIDC убедитесь, что пользователь Jmix сохраняет исходные данные пользователя OIDC, включая ID токен. ID токен необходим для обработки выхода из системы. Если ваш класс пользователя расширяет `DefaultJmixOidcUser` или `JmixOidcUserEntity`, вызовите `setDelegate(oidcUser)`, как показано в примере выше.

Обратите внимание, что в приведенном выше примере сопоставление клэймов от пользователя OIDC к полномочиям (granted authorities) Jmix делегируется интерфейсу `ClaimsRolesMapper`. Реализацией `ClaimsRolesMapper` по умолчанию является `DefaultClaimsRolesMapper`, который получает клэйм с именем "roles" из ID токена. Этот клэйм должен содержать коллекцию имен ролей. Затем для каждой роли из значения клэйма выполняется поиск ресурсной роли и роли уровня строк в Jmix. Если они найдены, соответствующие полномочия будут добавлены пользователю. Имя клэйма для ролей можно настроить с помощью следующего свойства приложения:
...
[[jmix.oidc.filter-chain.force-api-scope-enabled]]
=== jmix.oidc.filter-chain.force-api-scope-enabled

Переключатель для совместимости с версиями до 2.8. Установите данное свойство в `false`, чтобы игнорировать xref:security:resource-roles.adoc#role-scope[области действия ролей] при аутентификации пользователей в REST API.

Значение по умолчанию: `true`

[[jmix.oidc.filter-chain.force-ui-scope-enabled]]
=== jmix.oidc.filter-chain.force-ui-scope-enabled

Переключатель для совместимости с версиями до 2.8. Установите данное свойство в `false`, чтобы игнорировать xref:security:resource-roles.adoc#role-scope[области действия ролей] при аутентификации пользователей в UI.

Значение по умолчанию: `true`

[[jmix.oidc.jwks.connect-timeout]]
=== jmix.oidc.jwks.connect-timeout

Задает таймаут подключения для HTTP-запросов, которые загружают набор ключей JWK от поставщика OpenID. Jmix применяет этот таймаут к загрузке набора ключей JWK стандартным декодером ID-токенов при входе через OIDC и JWT-декодером сервера ресурсов OAuth 2.0.

Значение по умолчанию: `3s`

[[jmix.oidc.jwks.read-timeout]]
=== jmix.oidc.jwks.read-timeout

Задает таймаут чтения для HTTP-запросов, которые загружают набор ключей JWK от поставщика OpenID. Jmix применяет этот таймаут к загрузке набора ключей JWK стандартным декодером ID-токенов при входе через OIDC и JWT-декодером сервера ресурсов OAuth 2.0.
