...
NOTE: When implementing a custom OIDC user mapper, make sure the Jmix user keeps the original OIDC user data, including the ID token. The ID token is required for logout processing. If your user class extends `DefaultJmixOidcUser` or `JmixOidcUserEntity`, call `setDelegate(oidcUser)` as shown in the example above.
...
A toggle for compatibility with pre-2.8 versions. Set this property to `false` to ignore xref:security:resource-roles.adoc#role-scope[role scopes] when authenticating users in REST API.
...
A toggle for compatibility with pre-2.8 versions. Set this property to `false` to ignore xref:security:resource-roles.adoc#role-scope[role scopes] when authenticating users in UI.
...
Defines the connection timeout for HTTP requests that load the JWK Set from the OpenID Provider. Jmix applies this timeout to JWK Set retrieval by the default ID-token decoder used for OIDC login and the JWT decoder used by the OAuth2 resource server.
...
Defines the read timeout for HTTP requests that load the JWK Set from the OpenID Provider. Jmix applies this timeout to JWK Set retrieval by the default ID-token decoder used for OIDC login and the JWT decoder used by the OAuth2 resource server.
