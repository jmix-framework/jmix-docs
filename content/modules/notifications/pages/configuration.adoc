...
The recipient email address is taken from a property of the xref:security:users.adoc#entity[user] object. By default, the add-on assumes that email is stored in the `email` property. If you want to use another property, defined its name in the `application.properties` file, for example:
