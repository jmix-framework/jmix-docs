...
To do this, add `notificationIndicator` within the main view's `navigationBar` component:
...
The recipient email address is taken from a property of the xref:security:users.adoc#entity[user] object. By default, the add-on assumes that the email is stored in the `email` property. If you want to use another property, define its name in the `application.properties` file, for example:
...
You can also create a Spring bean implementing the `UserEmailResolver` interface to define more complex logic for getting the user's email address, for example:
...
* *Notifications: In-app notifications reader* – a user that receives notifications sent through the in-app channel and can read them in a special dialog.
* *Notifications: sender* – a user who can create and send notifications.
