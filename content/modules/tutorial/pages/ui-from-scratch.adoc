...
Во-первых, добавьте контейнер данных, который предоставит набор сущностей `UserStep` для UI-таблицы. Перетащите элемент `Data components` -> `Collection` из палитры компонентов в элемент `window` иерархии компонентов, выберите `UserStep` в диалоговом окне и нажмите кнопку *OK*:
...
Перетащите `Containers` -> `VBox` (контейнер с вертикальным размещением) из палитры компонентов в элемент `layout` иерархии компонентов перед `userStepsTable`. Затем добавьте три компонента `Label` в `vbox`:
...
. Существующего обработчика `BeforeShowEvent`:
+
[source,java]
----
@Subscribe
...
Сначала выберите `Containers` -> `HBox` (контейнер с горизонтальным размещением) из палитры компонентов и поместите его между `userStepstable` и `spacer`. Затем добавьте в него две кнопки:
...
Сгенерируйте обработчики нажатия кнопок с помощью вкладки *Component Inspector* -> *Handlers*:
...
В окне инструментов *Jmix* нажмите *New* (image:common/add.svg[]) -> *Advanced* -> *Custom Theme*:

image::ui-from-scratch/theme-1.png[align="center", width="488"]
...
TIP: Вы также можете сгенерировать обработчик на вкладке *Handlers* окна инструмента *Component Inspector*.
