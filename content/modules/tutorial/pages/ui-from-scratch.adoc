...
Нажмите на кнопку *Add Component* на панели действий и перетащите `Layouts` -> `VBox` (контейнер с вертикальным размещением) в элемент `layout` на панель иерархии *Jmix UI* перед `userStepsDataGrid`. Затем добавьте три компонента `HTML` -> `Label` в `vbox`.
