...
从本质上讲，入职步骤是新员工需要在一定时间内完成的操作。应用程序存储操作的名称及其最长期限。具有管理员访问权限的用户将使用特定的视图管理步骤列表、创建新步骤、编辑或删除步骤。步骤创建后，HR 经理可以将步骤分配给员工（用户）。
...
所有这些选项都可以后续在创建完成的视图中修改，此时，我们直接使用建议的默认值，点击 *Next*。

然后，可以为实体列表视图设置 _fetch plan_：

image::simple-crud/create-screens-5.png[align="center"]

NOTE: 简单来说，fetch plan 决定了视图需要加载实体的哪些属性和关联实体。

由于 `Step` 实体非常简单，因此无需调整它的 fetch plan。后面我们处理稍微复杂的实体和视图时，再详细了解关于 fetch plan 的内容。

点击 *Next*。

这里为实体详情视图设置 fetch plan：
...
Studio 会生成两个视图：`Step.list` 和 `Step.detail`，并打开相应的源代码。

NOTE: 每个视图由两部分组成：描述和控制器。描述是一个 XML 文件，用于定义组件和布局。控制器是一个 Java 类，可以包含事件处理器和其他逻辑。

Studio 视图设计器会展示生成的 `Step.list` 视图的 XML 描述文件：

image::simple-crud/create-screens-8.png[align="center",width="1110"]

设计器的主要部分已经在上图中重点提示了。在本向导的后面部分，我们会进一步详细了解。

[[run-app]]
== 运行应用程序

在创建了实体的 CRUD 视图后，你可以运行应用程序查看我们添加的新功能。
...
点击主菜单的 *Application* -> *Steps*，打开 `Step.list` 视图：

image::simple-crud/run-app-4.png[align="center"]

点击 *Create*，打开 `Step.detail` 视图：

image::simple-crud/run-app-5.png[align="center"]

我们使用列表和详情视图创建一些入职步骤，参数如下：
...
* Studio 可以使用模板为实体生成 CRUD 视图。
* 实体的列表视图会被添加至应用程序的主菜单。
