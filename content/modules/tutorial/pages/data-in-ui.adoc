...
然后在 `data` 元素下会创建 id 为 `departmentsDc` 的 `collection` 元素。通过 *Component Hierarchy* 和 XML 代码都能看到：
...
可以在 XML 中直接编辑查询语句，或者通过 *Component Inspector* 中 `query` 属性打开 JPQL 设计器进行编辑：
...
在 *Component Hierarchy* 选中 `departmentField`，然后在 *Component Inspector* 中为 `optionsContainer` 属性选择 `departmentsDc`：
...
然后从 *Component Palette* 中将一个 `Button` 组件拖放至 *Component Hierarchy* 中的 `buttonsPanel` 元素。选择新创建的 `button` 元素并在 *Component Inspector* 中设置其 `id` 为 `generateButton`，`caption` 为 `Generate`。切换至 *Handlers* 标签页，创建一个 `ClickEvent` 的处理器方法：
...
此时，UserSteps 表格提供的信息并不是很有用。我们需要移除 `Version` 和 `Sort value` 列并添加展示步骤名称的列。

删除列很简单，在 *Component Hierarchy* 中选择这些列并按下 *Delete*，或直接从 XML 代码里删除相应的元素即可。
 
添加列时，在 *Component Hierarchy* 选择 `columns` 元素并在 *Component Inspector* 中点击 *Add* -> *Column*，会出现 *Add Column* 弹窗：

image::data-in-ui/columns-2.png[align="center"]

可以看到，这里并不允许添加步骤名称。这是因为 `step` 属性是一个关联属性，而我们没有定义一个合适的 fetch plan 去加载这个实体。

在 *Component Hierarchy* 中，选择 `userDc` 数据容器，然后点击 fetch plan 的 *Edit*（image:common/edit.svg[]）按钮，可以在 *Component Inspector* 中 `fetchPlan` 属性处点击或者直接在 XML 编辑器的装订线栏中点击：
...
在 *Component Hierarchy* 选择 `columns` 元素并在 *Component Inspector* 中点击 *Add* -> *Column*。现在 *Add Column* 弹窗中包含了关联的 `Step` 实体和属性了：
...
将 `step.name` 移到最前面，可以直接在 XML 中修改或者在 *Component Hierarchy* 中拖动元素：
...
在 *Component Hierarchy* 中选择 `columns` 元素并在 *Component Inspector* 中点击 *Add* -> *Column*。显示 *Add Column* 对话框：
...
* xref:ui:vcl/components/button.adoc[] 组件的 `ClickEvent` 处理器用来处理按钮的点击事件。可以通过 *Component Inspector* 工具窗口的 *Handlers* 标签页自动生成。
