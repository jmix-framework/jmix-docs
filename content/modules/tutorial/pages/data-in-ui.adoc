...
然后在 *Jmix UI* 层级面板中选择 `buttonsPanel`，右键点击节点，在菜单中选择 *Add Component*，添加一个按钮。在组件工具箱中找到 `Components -> Button`，并双击。然后在组件属性面板中，设置其 `id` 为 `generateButton`，`text` 为 `Generate`。切换至 *Handlers* 标签页，创建一个 `ClickEvent` 的处理器方法：
