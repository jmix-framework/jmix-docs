...
Выберите контейнер данных `userDc` на панели структуры *Jmix UI* и нажмите кнопку *Edit* (image:common/edit.svg[]) либо в свойстве `fetchPlan` на панели инспектора *Jmix UI*, либо в маркере строки редактора XML:
...
Выберите элемент `columns` на панели структуры *Jmix UI* и нажмите *Add* -> *Column* на панели инспектора *Jmix UI*. Диалоговое окно *Add Column* теперь содержит связанную сущность `Step` и ее атрибуты:
