...
----
include::example$/ex1/src/main/java/sample/screen/newsitem/NewsItemEdit.java[tags=just-created-1;just-created-2]
----
...
[source,java,indent=0]
----
include::example$/ex1/src/main/java/sample/screen/newsitem/NewsItemEdit.java[tags=inject;post-commit-event]
----
...
[source,java,indent=0]
----
include::example$/ex1/src/main/java/sample/screen/newsitem/NewsItemEdit.java[tags=resources;emailer;send-by-email]
----
...
<3> Recipients.
<4> Email subject.
<5> The `from` address will be taken from the xref:configuration.adoc#jmix.email.fromAddress[jmix.email.fromAddress] application property.
<6> Attachments.
...
jmix.email.fromAddress = info@company.com
----
...
To send an email, choose it in the *Administration > Email History* screen and click *Resend email*. You can change or add recipients before sending. After successful sending, you will see a new line in the table with `Sent` status.
