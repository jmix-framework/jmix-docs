...
The add-on provides the default configuration of Quartz Job Scheduler. To process the email queue regularly, do the following:

. Include Quartz in your project as described in the xref:ROOT:quartz-setup.adoc[] section.

. Change scheduling as CRON expression if necessary using the <<jmix.email.email-sending-cron,jmix.email.email-sending-cron>> property.
+
[source, properties,indent=0]
----
...
The add-on has a built-in cleaning  Quartz Job Scheduler that is switched off by default. You can use it for periodic cleaning emails as described below.

If you need to keep some emails, use the "important" flag and set the maximum age for important and non important emails. You can set this flag using the `setImportant()` method of the `EmailInfoBuilder`.

[[cleaning-default-quartz-configuration]]
=== Default Quartz Configuration

The add-on provides the default configuration of Quartz Job Scheduler. To clean emails regularly, do the following:
...
. Change scheduling as CRON expression if necessary using the <<jmix.email.email-cleaning-cron,jmix.email.email-cleaning-cron>> property.
+
[source, properties,indent=0]
----
