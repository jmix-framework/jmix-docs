...
Go to the *Email -> Email history* view of your application. You will see a record with the `Queue` status. It means that the email is in the queue and not yet sent. Email information is displayed on the right.

To send all queued email periodically, configure the xref:configuration.adoc#email-sending-scheduler[Quartz Scheduler].

To send an email from the UI, choose it in the *Email -> Email history* view and click *Resend email*. You can change or add recipients before sending. After successful sending, you will see a new line in the table with the `Sent` status.
