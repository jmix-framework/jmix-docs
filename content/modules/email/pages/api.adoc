...
----
include::example$/ex1/src/main/java/sample/screen/newsitem/NewsItemEdit.java[tags=emailer;email-info1;email-info2]
----
+
...
+
* `sendEmailAsync()` – asynchronous message sending. This method returns `SendingMessage` instance in `SendingStatus.QUEUE` status, which was created in the database. The actual sending is performed later by the `Emailer.processQueuedEmails()` method which should be called by a xref:configuration.adoc#quartz[scheduler].
...
[source,java,indent=0]
----
include::example$/ex1/src/main/java/sample/screen/newsitem/NewsItemEdit.java[tags=create-attachment]
----
...
[source,java,indent=0]
----
include::example$/ex1/src/main/java/sample/screen/newsitem/NewsItemEdit.java[tags=text-attachment]
----
