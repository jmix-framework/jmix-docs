...
To adhere to the full-stack development xref:principles.adoc#full-stack-development[principle], Jmix uses the https://vaadin.com/[Vaadin^] framework for user interface development. In this section, we describe features that Jmix adds on top of Vaadin to maximize the productivity of creating enterprise applications with large data models and UI.
...
* The Jmix security subsystem is highly extensible. Its authentication part plays on the Spring Security rules. The authorization mechanism allows you to build your own attribute-based access control (ABAC) if needed.
