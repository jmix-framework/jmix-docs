...
These aspects should actually be attributed to the Vaadin framework which is used by Jmix under the hood. They are considered in detail in our blog post: https://www.jmix.io/blog/simplicity-focus-through-server-driven-web-ui-development[Simplicity & Focus Through Server-Driven Web UI Development^]. If you are new to this concept, we recommend reading the article. Here we'll just briefly repeat the conclusions.
...
*Lean data model means that you don't have to create layers of data transfer objects (DTOs) and mappers to and from the persistence model. The UI controls and business logic can work directly with persistent entities.*
...
To better understand how Jmix provides a unified data model containing different elements, and what you can do with this model, please refer to the xref:inside.adoc#_data_model_and_metadata[Data Model and Metadata] section.
...
This category also includes high-level abstractions and declarative approach for UI building, data access and security.
