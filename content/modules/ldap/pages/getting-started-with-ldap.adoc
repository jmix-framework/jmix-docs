...
So, let's set the xref:ldap:properties.adoc#jmix.ldap.synchronizeRoleAssignments[jmix.ldap.synchronizeRoleAssignments] to `false` in order to manage user roles manually in the application.

After the synchronization strategy is declared, users will be synchronized on every login. If you want to disable user synchronization on login, set the xref:ldap:properties.adoc#jmix.ldap.synchronizeUserOnLogin[jmix.ldap.synchronizeUserOnLogin] to `false`.
