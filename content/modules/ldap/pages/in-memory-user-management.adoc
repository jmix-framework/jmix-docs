...
当用户主要由 LDAP 管理时，需要能选择用户，或通过 `SuggestionField` 查找用户。为此，该扩展组件提供了 `UserRepository` 的 LDAP 实现。这个 Repository 支持从 LDAP 访问用户，但是不提供 `system` 或 `anonymous` 用户。另外，你也许还希望数据存储的用户也能支持查找。
