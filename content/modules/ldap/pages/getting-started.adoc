...
Let's assume we will use LDAP authentication and maintain users in the application.
...
Now, add the main LDAP properties. We will use a test LDAP server, `\ldap://ldap.forumsys.com:389/`, that has some https://www.forumsys.com/tutorials/integration-how-to/ldap/online-ldap-test-server/[test users^].
...
Next, describe how to synchronize users.

We will implement the default scenario when the add-on authenticates users from the application against LDAP. If a user is successfully authenticated in LDAP, but does not have `UserDetails` in the application, it will be automatically synchronized, and `UserDetails` will be created in accordance with the corresponding LDAP entry.

Declare a bean implementing the `LdapUserDetailsSynchronizationStrategy` interface. The add-on provides the abstract `AbstractLdapUserDetailsSynchronizationStrategy` implementation, so you can declare the strategy by specifying a concrete `User` class and implementing a mapping:
