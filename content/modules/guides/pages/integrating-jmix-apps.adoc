...
This guide demonstrates how to leverage the xref:rest:index.adoc[] and xref:rest-ds:index.adoc[REST DataStore] add-ons to create an integrated distributed system consisting of interconnected web applications.

Key features of the project built in this guide:
...
- **Seamless User Experience**: Provides a unified interface for users to interact with products, customers, and orders, despite the distributed nature of the system.

This guide will walk you through the process of setting up the REST API, configuring the REST DataStore, and implementing the necessary components to create a fully functional distributed system using Jmix. By the end, you'll have a comprehensive understanding of how to leverage these powerful features to build scalable and modular enterprise applications.
...
. Download or create the sample project.
+
You can download the https://github.com/jmix-framework/jmix-integrated-apps-sample[completed sample project^], which includes all the examples used in this guide. This allows you to explore the finished implementation and experiment with the functionality right away.
+
--
* https://github.com/jmix-framework/jmix-integrated-apps-sample/archive/refs/heads/main.zip[Download] and unzip the source repository.
* Or clone it using git: `++git clone https://github.com/jmix-framework/jmix-integrated-apps-sample.git++`
--
+
Alternatively, you can start with an empty Jmix project and follow the step-by-step instructions in this guide to implement the features yourself.

== What We Are Going to Build

The distributed system built in this guide comprises three distinct Jmix applications, each responsible for a specific domain:
...
Add the following entries to your `hosts` file (located in `/etc` on macOS and Linux and in `C:\Windows\system32\drivers\etc` on Windows) to point `host1` and `host2` to `localhost`:
...
+
The Orders application will open its views for DTO entities mapped to the data of the Customers application. You can use these views to manage customers as you would in the Customers application, with a single exception: you cannot create new regions.
+
...
+
If you click *Close and select* button in the dialog, the new tab will close and the imported customer will be selected in the order:
+
...
The second approach, which redirects users between applications, doesn't require the implementation of customer management views in the Orders application, and saves you from possible future problems if the data models for customers diverge between the applications.
...
These views were created using the `DTO entity list view` and `DTO entity detail view` templates provided by Studio. Following the creation of the new DTO views, the delegate method stubs were removed, as they are unnecessary when the entity is associated with a data store. The standard Jmix CRUD functionality operates through the `DataManager` for these entities.
