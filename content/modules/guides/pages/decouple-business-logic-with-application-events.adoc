...
In this guide, we will enhance the https://github.com/jmix-framework/jmix-petclinic-2[Jmix Petclinic] example with custom business logic driven by application events. We will cover the following scenarios:
...
The most common approach for communication in application logic is direct method invocation, where components interact via Java objects, Spring beans, or services. Direct method invocation is primarly used in the first guide on business logic: xref:guides:create-business-logic-in-jmix.adoc[]. While this pattern is often effective, it can lead to tight coupling between components, making the system harder to maintain and extend as it grows.
...
Event listeners should also be named according to the specific action they perform. Instead of creating a broad listener, like `PetCreatedListener` that handles multiple tasks after a pet is created, it is better to focus on a single responsibility. For example, `RoomKeycodeToOwnerSender` specifically describes the listener's role of sending the room keycode to the pet's owner.
...
TIP: Events are typically named in the simple past tense, such as "Entity *Changed* Event." This emphasizes that an event represents an immutable fact—something that has already occurred and cannot be altered. Event listeners, on the other hand, are named in the present tense.

=== Custom Application Logic Events

In this example, we use custom application events to communicate between different parts of the application in a loosely coupled way. When a pet recovers and checks out, the visit is marked as complete. This event can trigger various downstream processes. In this case, the event initiates the invoicing process.
...
In Jmix, custom events are published via Spring's `ApplicationEventPublisher`. This enables different parts of the application to communicate without direct dependencies.
...
The event is published with `applicationEventPublisher.publishEvent(new TreatmentCompletedEvent(this, visit));`. Other components can listen for this event and react without directly interacting with the `VisitStatusService`.
...
On the UI layer, there are two main types of events: framework-provided UI events and custom application UI events. Both types of events are always scoped to a single instance of the UI, meaning they normally only affect the current user.
...
This approach differs from standard Spring events, as UI events in Jmix are typically tied to specific user sessions or browser instances. They ensure that events are only delivered to the relevant UI (or browser tab) without affecting other sessions.

The `UiEventPublisher` supports different variants of event publishing:

* **For the Current UI**: If you need the event to be handled only within the current UI (e.g., the current browser tab), use the `publishEventForCurrentUI()` method.
* **For All UIs in a Session**: To broadcast the event across all UIs (tabs or browser windows) within the user's session, you can use the `publishEvent()` method.
* **For Specific Users**: You can also target specific users by publishing events for all of their active sessions using `publishEventForUsers()`. This is particularly useful in multi-user applications where events need to be scoped by user.
...
When a treatment is started in the `MyVisitsView`, the badge in the `MainView` is updated by counting the number of active treatments assigned to the current user. This keeps the UI views decoupled from each other, as the components do not need to directly invoke each other.
...
Event-driven logic reduces coupling between components, improving testability and resilience. However, it lacks immediate feedback compared to method calls, which can make it harder to manage asynchronous processes. Despite these challenges, event-based communication ensures better modularity and scalability.
