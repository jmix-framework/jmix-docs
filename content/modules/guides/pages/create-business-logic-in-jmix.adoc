...
This approach works well when the business logic is simple and only used in one place. However, as the application grows or if the logic becomes more complex, it might be better to extract the logic into a separate service or shared class for better code reusability and maintainability.

TIP: This approach is acceptable if the logic is invoked from a single point, and it is not too complex to fit into a couple of short methods. For more complex logic or logic that needs to be reused across multiple views, consider moving it into a dedicated service.
