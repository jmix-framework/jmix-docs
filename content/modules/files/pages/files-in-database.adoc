...
For uploading the file from a user interface, use the xref:jmix-ui:vcl/components/file-upload-field.adoc[] component bound to the entity attribute. If the file is an image, you can use the xref:jmix-ui:vcl/components/image.adoc[] component to display the image on the screen. For example:
