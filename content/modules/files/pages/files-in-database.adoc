...
Для загрузки файла из экрана UI используйте компонент xref:flow-ui:vc/components/fileUploadField.adoc[], привязанный к атрибуту сущности. Если файл является изображением, можно использовать компонент `image` для его отображения на экране. Например:
