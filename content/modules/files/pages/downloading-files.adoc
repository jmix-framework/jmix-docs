...
Бин `Downloader` предназначен для выгрузки файлов на компьютер пользователя. Он имеет ряд методов `download()`, которые принимают различные параметры, определяющие выгружаемый контент. Внутри бин использует `DownloaderExportHandler`, реализующий Vaadin https://vaadin.com/docs/latest/flow/advanced/downloads[DownloadHandler API^].
...
[NOTE]
====
Чтобы управлять поведением кэширования в браузере для загружаемых файлов, вы можете использовать свойство приложения xref:flow-ui:ui-properties.adoc#jmix.ui.file-downloader-cache-max-age-sec[jmix.ui.file-downloader-cache-max-age-sec]. Установка его в значение `0` гарантирует, что браузер всегда будет запрашивать последнюю версию с сервера, что особенно полезно для динамически генерируемых файлов, таких как PDF.
====
...
* `iframe` позволяет встраивать содержимое внутрь HTML-элемента `<iframe>`.
Смотрите примеры в онлайн демо-приложении https://demo.jmix.ru/ui-samples/sample/iframe-pdf[UI Samples^].
