...
The `Downloader` bean is designed to download files to the user's computer. It has a number of `download()` methods that accept different parameters defining the downloaded content. Internally, the bean uses `DownloaderExportHandler`, which implements Vaadin's https://vaadin.com/docs/latest/flow/advanced/downloads[DownloadHandler API^].
...
[NOTE]
====
To control browser caching behavior for downloaded files, you can use the xref:flow-ui:ui-properties.adoc#jmix.ui.file-downloader-cache-max-age-sec[jmix.ui.file-downloader-cache-max-age-sec] application property. Setting it to `0` ensures that the browser always fetches the latest version from the server, which is particularly useful for dynamically generated files like PDF files.
====
...
* xref:flow-ui:vc/html-components/image.adoc[] allows you to display images from various resources using the HTML `<img>` element. The component can be declaratively bound to an entity attribute to display images stored as byte arrays in the database or as files in the file storage.

* `iframe` allows you to embed content inside the HTML `<iframe>` element.
See the examples in the https://demo.jmix.io/ui-samples/sample/iframe-pdf[UI Samples^] online demo application.
