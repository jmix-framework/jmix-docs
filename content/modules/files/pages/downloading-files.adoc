...
`Downloader` bean 支持下载文件至用户电脑。带有一组 `download()` 方法，可以接收不同方式定义的文件内容参数。其内部使用了 `DownloaderExportHandler`，实现了 Vaadin 的 https://vaadin.com/docs/v25/flow/advanced/downloads[DownloadHandler API^]。
...
[NOTE]
====
如需控制浏览器对已下载文件的缓存行为，可以使用 xref:flow-ui:ui-properties.adoc#jmix.ui.file-downloader-cache-max-age-sec[jmix.ui.file-downloader-cache-max-age-sec] 应用程序属性。将其设置为 `0` 可确保浏览器始终从服务器获取最新版本，这对于像 PDF 文件这类动态生成的文件尤其有用。
====
