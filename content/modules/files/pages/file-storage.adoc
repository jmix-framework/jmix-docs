...
File Storage is an abstraction enabling different implementations of how and where the files are stored and providing a unified https://github.com/Haulmont/jmix-core/blob/master/core/src/main/java/io/jmix/core/FileStorage.java[interface^] for accessing files and referring to them from the data model entities.
...
For uploading files from a screen of the user interface, use the xref:jmix-ui:vcl/components/file-storage-upload-field.adoc[] component bound to the entity attribute:
...
The following example shows how to work with the https://github.com/Haulmont/jmix-core/blob/master/core/src/main/java/io/jmix/core/FileStorage.java[FileStorage^] interface directly.
...
The xref:jmix-ui:vcl/components/file-storage-upload-field.adoc[] component has the `fileStorage` attribute to specify a file storage name. If it's not set, the component uses the default file storage.