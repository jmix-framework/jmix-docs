...
The Resource Owner Password Credentials grant has been removed from the OAuth 2.1 specification as it is not secure enough. However, it can be useful in some cases, so Jmix framework provides support for it. You can read more about the Resource Owner Password Credentials grant in the {oauth-2_0-specification-page}#section-4.3[OAuth 2.0 specification^].
