...
NOTE: `META-INF/resources` is one of default places where Spring searches for static resources.
...
Afterward, we need to instruct the application that the new login form must be used instead of the one provided by the Authorization Server add-on. This can be achieved by specifying a corresponding application property.
