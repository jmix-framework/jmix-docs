...
首先，创建一个登录表单模板。认证服务扩展组件包含了 https://www.thymeleaf.org[Thymeleaf^] 模板引擎的依赖，因此可以直接使用。
...
你可能会注意到，模板中使用了位于 `+/my-as-login/styles/**+` 和 `+/my-as-login/icons/**+` 路径的样式和图片。

下载 https://getbootstrap.com/docs/5.3/getting-started/download/[Bootstrap^] CSS 文件，并复制 `bootstrap.min.css` 到 `src/main/resources/META-INF/resources/my-as-login/styles` 目录。

NOTE: `META-INF/resources` 是 Spring 搜索静态资源的默认路径之一。
...
关于 Refresh Token 授权可以阅读 {oauth-specification-page}#name-refresh-token-grant[OAuth 2.1 规范^]。需要为客户端注册 `refresh-token` 授权类型：
