...
When using the xref:authorization-code-grant[Authorization Code Grant], it is possible to replace the standard login form provided by the Authorization Server add-on. To achieve this, you need to follow a several steps.
...
Let's create a new file named `my-as-login.html` and place it in the `src/main/resources/templates` directory. This is a default location where Thymeleaf will search for templates.
...
Take any icon (e.g., Jmix logo), rename it as `as-login-icon.png` and place it in the `src/main/resources/META-INF/resources/my-as-login/icons` directory.

Next, we need to instruct Spring Security to allow access to files with styles and images. Create a Spring configuration that permits access to URLs where login form resources are located.
...
Afterward, we need to instruct the application that the new login form must be used instead of the one provided by the Authorization Server add-on. This can be achieved by creating a `WebMvcConfigurer` that maps the default `/as-login` URL to the new login page template. `/as-login` is the URL where unauthorized requests are redirected by the Authorization Server add-on.

.MyWebMvcConfigurer.java
[source,java]
----
include::example$/rest-ex1/src/main/java/com/company/demo/config/MyWebMvcConfigurer.java[tags=class-header;view-controllers;class-footer]
----
<1> The new `WebMvcConfigurer` must have a lower precedence than the one provided by the Authorization Server add-on.
<2> Maps the default `/as-login` URL to the new login page template `my-as-login.html` that we created previously.


In some cases, resources from the `META-INF/resources` directory may not be automatically served. This can happen if you created your application using the "REST Service" application template. Vaadin won't be in the classpath and you will need to define a static resource handler manually. To do this, override one more method in the `MyWebMvcConfigurer.java` file:

.MyWebMvcConfigurer.java
[source,java]
----
include::example$/rest-ex1/src/main/java/com/company/demo/config/MyWebMvcConfigurer.java[tags=resource-handlers]
----
<1> Specifies the location of static resources required by login page (CSS styles, icons). Files for all requests with URLs starting with `/my-as-login` will be searched inside the `/META-INF/resources/my-as-login` directory in the application resources.
