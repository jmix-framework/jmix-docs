...
TIP: The Entities API takes xref:data-model:soft-deletion.adoc[Soft Deletion] into consideration. Depending on the setting of the entity, the instance is therefore either only soft-deleted or deleted entirely from the application.