...
participant "API 客户端" as Client

box "Jmix"
participant "Services API" as ServicesAPI
end box

box "应用程序开发者" #LightBlue
participant "Service Bean" as ServiceBean
end box

/ note over ServicesAPI : 进行 HTTP 交互

Client -> ServicesAPI: HTTP 请求

activate ServicesAPI

ServicesAPI -> ServiceBean: Service 方法调用
activate ServiceBean

ServiceBean --> ServicesAPI: Service 方法返回值
deactivate ServiceBean

ServicesAPI -> Client: HTTP 响应
