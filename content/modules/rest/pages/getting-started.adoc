...
By default, when you add the REST API add-on to the application, no security is applied to REST endpoints. There are xref:security:custom-endpoints.adoc[multiple ways] to protect these endpoints. One way is to configure authenticated URLs for resource server, which is configured by the Authorization Server add-on.
