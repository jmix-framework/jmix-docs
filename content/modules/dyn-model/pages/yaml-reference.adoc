...
| string
| no
| Physical JDBC store for dynamic entities. Blank and `main` are serialized as omitted. Static entity definitions may declare a store only if it matches the entity metadata store. An existing dynamic entity store cannot be changed.
...
| boolean
| no
| LOB metadata and DB type. Supported only for `java.lang.String` and `byte[]` attributes.
