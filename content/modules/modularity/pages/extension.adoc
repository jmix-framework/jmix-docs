...
Note that the `@ViewController` annotation has the same value as in the base screen. This is important because we actually need to _override_ the base screen, meaning that everywhere in the system the extended screen will be used instead of the base one, as is done for replaced entities.
