...
To extend and override a screen provided by an add-on, select *Override an existing screen* template in the Studio screen creation wizard. Studio will create new XML descriptor and controller files. The XML descriptor will contain the `extends` attribute referring the base screen descriptor:
