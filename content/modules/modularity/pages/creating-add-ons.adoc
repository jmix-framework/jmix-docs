...
通过 Studio 的项目创建向导，可以使用 *Single Module Add-on* 模板轻松创建一个扩展组件。生成的项目会包含两个代码模块：功能模块和 starter 模块。
...
----
include::example$/base/base/src/test/java/modularity/sample/base/BaseTestConfiguration.java[tags=package;class]
----
...
----
+
那么 `@JmixModule` 需要声明对 `EclipselinkConfiguration` 和 `UiConfiguration` 的依赖：
+
[source,java,indent=0]
----
include::example$/base/base/src/main/java/modularity/sample/base/BaseConfiguration.java[tags=jmix-module;class]
----
+
对 Core 子系统的依赖是传递过来的，可以查看 `EclipselinkConfiguration` 或 `UiConfiguration` 类的定义。
...
----
include::example$/base/base/src/main/resources/modularity/sample/base/module.properties[]
----
...
[source,java,indent=0]
----
include::example$/base/base/src/main/java/modularity/sample/base/BaseConfiguration.java[tags=package;property-source;class]
----
...
[source,java,indent=0]
----
include::example$/ext/src/test/java/modularity/sample/ext/ExtApplicationTests.java[tags=module-prop]
----

框架中很多地方都用这种方式获取扩展组件中定义的聚合配置，例如，xref:flow-ui:menu-config.adoc[UI 菜单]、共享 xref:data-access:fetching.adoc#fetch-plan[fetch plans]、REST xref:rest:app-properties.adoc#jmix.rest.authenticated-url-patterns[认证]/xref:rest:app-properties.adoc#jmix.rest.anonymous-url-patterns[匿名] URLs 等。
