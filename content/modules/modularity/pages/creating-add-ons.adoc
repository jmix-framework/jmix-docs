...
框架中很多地方都用这种方式获取扩展组件中定义的聚合配置，例如，xref:ui:menu-config.adoc[UI 菜单]、共享的 xref:data-access:fetching.adoc#fetch-plan[fetch plans]、REST xref:rest:app-properties.adoc#jmix.rest.authenticated-url-patterns[认证]/xref:rest:app-properties.adoc#jmix.rest.anonymous-url-patterns[匿名] URLs 等。
