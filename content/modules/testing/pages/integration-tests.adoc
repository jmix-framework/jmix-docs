...
TIP: 使用 Studio *Jmix* 工具窗口的 *New → Advanced → Integration Test* 操作可以快速创建一个集成测试。
...
----
include::example$/testing-ex1/src/main/java/com/company/demo/app/OrderLineEventListener.java[tags=import-order;import-order-line;import-data-manager;import-id;import-entity-changed-event;import-autowired;import-event-listener;import-component;class;recalculate-order-amount]
----
...
用于新用户无任何角色，`CustomerService` 返回了一个空的 `Optional`。
...
有时，即使是集成测试，也需要模拟（mock）应用程序中的某些部分。此时，可以将 `@SpringBootTest` 的功能与 Mockito 相结合，模拟特定的 bean，但仍然使用整个 Spring 上下文。
