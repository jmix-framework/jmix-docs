...
<5> `UiTestUtils.getCurrentView()` method returns the view currently opened by navigation.
...
[[opened-dialog-view]]
== Getting a View Opened in a Dialog Window

The `UiTestUtils.getCurrentView()` method returns the view opened by navigation. If an action opens a view in a dialog window, for example with `OpenMode.DIALOG`, use `DialogWindows.getOpenedDialogWindows()`.

In the following example, the test clicks the order creation button and obtains the detail view opened in a dialog window:
...
The `getDialogs()` method returns dialog views of the current UI. To get dialogs of a specific UI, use `getDialogs(UI)`. Views are added to the list in the order they are opened, so the last opened dialog is the last item in the list.

For more information about the dialog window API, see xref:flow-ui:views/opening-views.adoc#opened-dialog-windows[Getting Opened Dialog Windows].