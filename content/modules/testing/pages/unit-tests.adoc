...
<2> we define that when the method `now` is called on the `TimeSource`, it should return a particular `ZonedDateTime` from 2020.
<3> when instantiating the counter class, the mock / emulation instance of the `TimeSource` will be passed into the constructor.

TIP: When you want to test your Spring components in a unit test, it is best to use constructor based injection instead of `@Autowired` in the class. Then passing in the mocked instance via the constructor is possible.
