...
单元测试是自动化测试中最小的类型。默认情况下，Jmix 自动引入了 https://junit.org/junit5/[JUnit 5^] 测试框架以及用于模拟依赖的 https://site.mockito.org/[Mockito^]。

NOTE: 术语“单元测试”用于描述不同的概念，包括一般的自动化测试。我们将单元测试理解为一种自动化测试，用于验证特定类或一组没有依赖关系（主要是没有 Spring 上下文和数据库）的行为。
...
此测试类不包含任何 Spring Boot 测试注解（例如，`@SpringBootTest`），与 Spring Boot 集成测试相比，运行速度非常快。

但是测试中缺少 Spring 上下文也意味着无法在测试类中使用 `@Autowired` 来获取 Jmix/Spring Bean 的实例。如果被测类与 Spring Bean 有任何依赖关系，则必须手动实例化这些依赖关系。
...
该类使用了 `@Component` 注解，这样 Spring 能自动注入依赖。

但是，我们希望在单元测试中测试此类的功能，则需要手动实例化 `RecentOrdersCounter` 类。如果我们需要测试这样一个场景：
...
----
<1> `mock` 方法创建一个模拟的实例，可以用于控制该类的行为。
<2> 我们定义当 `TimeSource` 的 `now()` 方法被调用时，需要返回 `ZonedDateTime` 类型的 `2020-03-01`。
<3> 初始化计算类时，构造函数传入的是 `TimeSource` 的模拟实例。

TIP: 如果需要在单元测试中测试 Spring bean，请使用基于构造函数的注入，而非 `@Autowired` 的字段。这样在单元测试时，可以通过构造器传入模拟的实例。

使用 Mockito 的更多内容，请参阅 https://javadoc.io/doc/org.mockito/mockito-core/latest/org/mockito/Mockito.html[Mockito 的文档^]。

[[verify-behaviour-with-assertions]]
== 用断言进行验证

可以通过 https://assertj.github.io/doc/[AssertJ^] 库使用断言。

AssertJ DSL 提供了用于验证测试结果的流式 API。断言方法（例如，`assertThat`）应该静态引入，例如：
...
根据对象的类型不同，AssertJ 提供了不同的断言方法来比较值。例如，在比较列表时，AssertJ 提供了 `hasSize` 和 `contains` 方法：
