...
Vaadin предоставляет https://vaadin.com/docs/latest/flow/routing/exceptions[поддержку^] обработки ошибок навигации: при возникновении необработанного исключения во время навигации, отображается экран ошибки. Например, когда пользователь переходит по несуществующему маршруту, отображается следующая страница (в продакшн-сборке доступные маршруты не показываются):

.Пример страницы по умолчанию для не найденного маршрута
...
.Пример пользовательской страницы для не найденного маршрута
