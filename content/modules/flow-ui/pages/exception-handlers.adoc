...
如需控制处理器能处理的异常类型，可以重写 `AbstractUiExceptionHandler` 基类的 `canHandle()` 方法，或者不使用 `AbstractUiExceptionHandler` 类，而是直接自定义实现 `UiExceptionHandler` 接口。参阅 https://vaadin.com/docs/v25/advanced/custom-error-handler[Vaadin 文档^] 了解如何在更低的级别处理 UI 异常。
...
默认异常处理显示的对话框可以通过提供 CSS 变量和应用程序属性进行自定义，或者通过扩展对话框的 Java 类并实现自己的逻辑。

[[using-css-and-properties]]
=== 使用 CSS 和属性

默认异常处理对话框的尺寸可以通过在 xref:flow-ui:themes/themes.adoc#application-theme-files[应用程序主题] CSS 文件中使用 CSS 变量来自定义。例如：
...
默认情况下，`ExceptionDialog` 以模态窗口形式出现。可以通过在 `application.properties` 文件中设置 xref:flow-ui:ui-properties.adoc#jmix.ui.exception-dialog-modal[jmix.ui.exception-dialog-modal] 属性来自定义此行为。

[[extending-exception-dialog-class]]
=== 扩展 ExceptionDialog 类

下面的示例展示如何添加一个按钮，以支持用户上报异常。
...
参阅 https://vaadin.com/docs/v25/flow/routing/exceptions[Vaadin 文档^] 了解关于路由异常的更多内容。