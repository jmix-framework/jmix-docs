...
Vaadin provides https://vaadin.com/docs/latest/flow/routing/exceptions[support^] for navigation target exceptions. When an unhandled exception is thrown during navigation, an error view is shown. For example, when a user navigates to a non-existent route, the following page is shown:
...
If you want to provide a custom error page, you need to extend the default one, for example:
