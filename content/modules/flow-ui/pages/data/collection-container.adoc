...
* `setSorter()` - 设置此容器的排序器。默认情况下，框架通过 `CollectionContainerSortManager` 创建排序器。首先检查注册的 `CollectionContainerSortProvider` bean，如果没有，则使用 `SorterFactory`，这个工厂会创建一个标准的 `CollectionContainerSorter` 。如果需要，也可提供 xref:flow-ui:data/data-examples.adoc#custom-sorting[自定义实现]。
