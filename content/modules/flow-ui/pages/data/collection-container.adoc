...
* `setSorter()` - sets the given sorter for this container. By default, the framework creates a sorter through `CollectionContainerSortManager`. The manager first checks registered `CollectionContainerSortProvider` beans and then falls back to `SorterFactory`, which creates a standard `CollectionContainerSorter`. You can provide your xref:flow-ui:data/data-examples.adoc#custom-sorting[own implementation] if needed.
