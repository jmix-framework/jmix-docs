...
NOTE: `InstanceLoader` вызывает `DataManager` или <<load-delegate>> / <<load-from-repository-delegate>> только если установлен или `entityId` или `query`.
...
Если загрузка данных выполняется с помощью делегата и загруженные данные отображаются в `dataGrid` с компонентом постраничной загрузки (пагинации), то может потребоваться определить также и пользовательскую логику для подсчёта общего количества строк. См. обработчик xref:vc/components/simplePagination.adoc#totalCountDelegate[totalCountDelegate] компонента `simplePagination`.

[[load-from-repository-delegate]]
=== loadFromRepositoryDelegate

Если вы предпочитаете xref:data-access:data-repositories.adoc[репозитории данных] для доступа к базе данных, рассмотрите возможность использования `loadFromRepositoryDelegate` вместо `loadDelegate`, который описан <<load-delegate,выше>>.

Он принимает аргументы, подходящие для прямого вызова репозитория данных:
