...
NOTE: 仅在设置了 `entityId` 或 `query` 时，`InstanceLoader` 调用 `DataManager` 或 <<load-delegate>> / <<load-from-repository-delegate>>。
...
如果使用代理声明自定义的数据加载，并在带有分页组件的 DataGrid 中显示已加载的数据，那么可能还需要定义计算总行数的自定义逻辑。请查看与表格关联的分页组件的 xref:vc/components/simplePagination.adoc#totalCountDelegate[totalCountDelegate] 处理方法。

[[load-from-repository-delegate]]
=== loadFromRepositoryDelegate

如果更倾向使用 xref:data-access:data-repositories.adoc[data repositories] 访问数据库，请使用 `loadFromRepositoryDelegate` 替代 <<load-delegate,上面>> 描述的 `loadDelegate`。

这个类接收适合直接调用 data repositories 的参数：
