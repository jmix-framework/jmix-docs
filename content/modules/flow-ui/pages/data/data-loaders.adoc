...
NOTE: `InstanceLoader` invokes `DataManager` or <<load-delegate>> / <<load-from-repository-delegate>> only if either `entityId` or `query` is set.
...
If you declare custom data loading with a delegate, and you display the loaded data in the DataGrid with a pagination component, then you may also need to define the custom logic to count total number of rows. Take a look at the xref:vc/components/simplePagination.adoc#totalCountDelegate[totalCountDelegate] handler for the pagination component associated with the table.

[[load-from-repository-delegate]]
=== loadFromRepositoryDelegate

If you prefer xref:data-access:data-repositories.adoc[data repositories] for accessing the database, consider using the `loadFromRepositoryDelegate` instead of `loadDelegate` described <<load-delegate,above>>.

It accepts arguments suitable for calling a data repository directly:

[source,java,indent=0]
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/department/DepartmentListView3.java[tags=delegate]
----
...
[[filter-components-and-loader-condition]]
== Filter Components and the Loader Condition

Filter components — xref:flow-ui:vc/components/genericFilter.adoc[], xref:flow-ui:vc/components/propertyFilter.adoc[], `jpqlFilter`, standalone `groupFilter`, and property filters in xref:flow-ui:vc/components/dataGrid.adoc[] column headers — work by adding their conditions to the condition of the data loader they are bound to. This imposes a few rules on the application code:

* Set the base loader condition before filter components are bound to the loader: declaratively in the `<condition>` element, or programmatically in the `InitEvent` handler of the view. Filters combine their conditions with this base.
* After the view is initialized, do not replace the loader condition with `setCondition()` and do not modify the object returned by `getCondition()`. From this point the filter components own the loader condition: a composite filter (`genericFilter`, standalone `groupFilter`) rebuilds it as the user works with conditions, so a replaced condition or a condition added in place may be overwritten on the next rebuild without any error. Exception: when `genericFilter` is the only filter component on the loader, it picks up a condition set with `setCondition()` as its new base.
* A data loader can have multiple filter components: any number of `propertyFilter` and `jpqlFilter` components and at most one composite filter (`genericFilter` or standalone `groupFilter`). Bind all of them to the loader when the view is initialized, not later at runtime.

To filter data by a condition that changes at runtime — for example, by a session attribute — use loader query conditions with parameters (see above) instead of modifying the loader condition object.
