...
A single instance of `DataContext` exists for a given view. It is created if the `<data>` element exists in the view XML descriptor.
...
If you open a detail view that should save data to the current view’s data context, use the `withParentDataContext()` method of the builder:
