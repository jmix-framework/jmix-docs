...
在 XML 中定义的 xref:data/data-loaders.adoc[数据加载器] 会自动将加载的实体合并到 `readOnly` 属性不是 `true` 的 `DataContext` 中。默认情况下，Studio 生成的 xref:views/view-classes.adoc#standard-list-view[实体列表视图] 会带有该属性，因此，如果需要在实体列表视图跟踪实体改动并且提交改动的实体，需删除 XML 中相关数据加载器的 `readOnly="true"` 属性。

如果 `<data>` 元素的 `readOnly="true"`，则视图中的 `DataContext` 是一个不可操作的实现：`merge()` 会返回未修改的实例，`save()` 也仅仅记录日志，不会写入数据。因此，在这样的视图中添加可编辑的组件，编辑后的改动会静默不保存。如果需要编辑，请先移除 `<data>` 元素的 `readOnly="true"` 属性。
...
`DataContext` 实例可以形成父子关系。如果一个 `DataContext` 有父上下文，它会将改动的实体提交给父上下文而不是直接保存至数据存储。通过这个功能支持 xref:concepts:features.adoc#editing-aggregates[编辑实体聚合关系]，从实体只能跟主实体一起保存到数据库。如果一个实体属性使用 xref:data-model:entities.adoc#composition[@Composition] 注解，框架会自动在此属性的详情视图设置父上下文，从而该属性的改动会保存到主实体的数据上下文。
...
如需了解更多关于 `DataContext` 的使用，请参考 xref:data-modeling-composition-guide:index.adoc[] 指南。
...
参阅 xref:data/entities-in-ui.adoc[] 部分了解如何用自定义的服务保存实体。
