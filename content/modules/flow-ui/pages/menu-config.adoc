...
`MenuConfig` is a special class that loads the main menu structure based on a file specified in the xref:flow-ui:flowui-properties.adoc#jmix.flowui.menu-config[jmix.flowui.menu-config] property.
...
Use the xref:flow-ui:flowui-properties.adoc#jmix.flowui.composite-menu[jmix.flowui.composite-menu] property to switch these modes.
...
`item` is a nested element of `menu`. For each item you can set attributes, <<item-properties,properties>>, <<route-parameters,route>> and <<query-parameters,query>> parameters.
...
An `item` element can declare one or multiple query parameters inside its child `queryParameters` element.
