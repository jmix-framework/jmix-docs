...
Маршрут, который определяет URL-путь к экрану, может включать параметр маршрута. Один из примеров такого параметра - это идентификатор конкретного экземпляра сущности, передаваемый в xref:flow-ui:views.adoc#standard-detail-view[экран деталей]. Следующий элемент меню открывает экран деталей для определенного отдела:
