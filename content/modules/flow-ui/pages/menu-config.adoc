...
* `id` -- a unique identifier of the element. If no `view`, `bean`, `className` attributes are specified, the `id` is used to point to a view with this `id`. When the user clicks on the menu item, the corresponding view opens on the main screen.
