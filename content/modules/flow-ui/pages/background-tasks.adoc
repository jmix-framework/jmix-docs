...
该方法在单独的后台线程中调用，执行一个任务。

CAUTION: 不要在这个方法中读取或更新 UI 组件以及数据容器的状态。
...
仅当主动取消任务时在一个可以访问 UI 组件的线程中调用该方法，例如，在 `TaskHandler` 中调用 `cancel()` 方法时。

[[progress]]
=== progress()

当进度值发生变化时在一个可以访问 UI 组件的线程中调用该方法，例如，调用 `taskLifeCycle.publish()` 方法之后。

[[done]]
=== done()

当任务完成后在一个可以访问 UI 组件的线程中调用该方法。

[[handle-timeout-exception]]
=== handleTimeoutException()

当任务超时后在一个可以访问 UI 组件的线程中调用该方法。如果正在运行任务的窗口关闭，则任务将在没有通知的情况下停止。

[[handle-exception]]
=== handleException()

当有任何异常抛出时在一个可以访问 UI 组件的线程中调用该方法。
