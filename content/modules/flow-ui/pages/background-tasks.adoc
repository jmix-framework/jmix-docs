...
2. `V` - `done()` 方法返回的任务结果类型。结果也可以调用 `BackgroundTaskHandler.getResult()` 方法获得，该方法将等待至任务完成。
...
1. 用 `dialogs.createBackgroundWorkDialog()` 方法展示一个包含进度条和 *Cancel* 按钮的模态窗。该模态窗可以定义进度条的不同类型，以及允许或禁止取消后台任务。
