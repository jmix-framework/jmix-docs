...
可以看到，文本控件通过 Java 中的 `TypedTextField` 类表示。`TypedTextField` 继承自 Vaadin 的 https://vaadin.com/docs/v25/components/text-field[TextField^] 组件，并添加了对不同类型值的支持、数据绑定以及其他 Jmix 特定的功能。

标签是使用了 Vaadin 的 {vaadin-api}/com/vaadin/flow/component/html/Div.html[Div^] 组件，表示 HTML 的 `div` 元素。

一般来讲，Jmix 中的每个可视化组件都由下列部分组成：

. 以 {vaadin-api}/com/vaadin/flow/component/Component.html[Component^] 为基类的 Java 类。可以是一个标准的 Vaadin https://vaadin.com/docs/v25/components[组件^]、组件的扩展版本、还可能是基于 {vaadin-api}/com/vaadin/flow/component/Composite.html[Composite^] 或完全从头创建的新组件。
