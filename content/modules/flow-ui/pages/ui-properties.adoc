...
这个应用程序属性控制是否在 Jmix 应用程序中使用模态窗显示 xref:exception-handlers.adoc#customizing-default-handler[ExceptionDialog]。
...
设置下载文件被浏览器缓存视为新文件的最大时间（以秒为单位）。此属性有助于确保 Google Chrome 内置 PDF 查看器显示文件的最新版本。

该属性设置 Cache-Control HTTP 响应头中的 `max-age` 指令。详细信息请参考 https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Cache-Control#response_directives[MDN 文档^]。

默认值：`3600`。

另参考 xref:files:downloading-files.adoc#downloader[使用下载器]。
...
对于 `Downloader` bean，配置哪些文件后缀类型可以直接展示在浏览器而不下载。当一个文件通过这种方式打开时，生成的 URL 具有 `VAADIN/dynamic/resource/<random_id>` 格式的后缀。
