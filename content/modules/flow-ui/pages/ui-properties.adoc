...
Устанавливает максимальное время (в секундах), в течение которого загруженный файл считается актуальным для кэша браузера. Это свойство особенно полезно для обеспечения отображения самой последней версии файла во встроенном средстве просмотра PDF в Google Chrome.

Свойство устанавливает директиву `max-age` в HTTP-заголовке ответа Cache-Control. Подробности смотрите в https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Cache-Control#response_directives[документации MDN^].

Значение по умолчанию: `3600`.

Смотрите также xref:files:downloading-files.adoc#downloader[Использование Downloader].
...
Для бина `Downloader` определяет список расширений файлов, которые будут отображаться во вкладке веб-браузера вместо автоматической загрузки. При открытии файла таким способом сгенерированный URL получает постфикс `VAADIN/dynamic/resource/<random_id>`.
...
Определяет, будет ли показано диалоговое окно подтверждения, если пользователь попытается закрыть вкладку браузера и при этом соответствующее свойство экрана также установлено в `true` методом xref:views/views.adoc#prevent-browser-tab-closing[setPreventBrowserTabClosing].
...
Определяет xref:keyboard-shortcuts.adoc[сочетание клавиш], которое применяет текущее условие в фильтрах по заголовкам колонок компонентов xref:vc/components/dataGrid.adoc#column-filtering[dataGrid] и xref:vc/components/treeDataGrid.adoc[treeDataGrid].
...
Для отдельного грида это свойство можно переопределить с помощью атрибута xref:vc/components/dataGrid.adoc#headerFilterApplyShortcut[headerFilterApplyShortcut] элемента `columns`.
