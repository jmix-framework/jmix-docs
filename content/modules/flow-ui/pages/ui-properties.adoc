...
[[jmix.ui.file-downloader-cache-max-age-sec]]
=== jmix.ui.file-downloader-cache-max-age-sec

Sets the maximum time (in seconds) that a downloaded file is considered fresh by the browser's cache. This property is particularly useful for ensuring that the built-in PDF viewer in Google Chrome displays the most recent version of a file.

The property sets the `max-age` directive in the Cache-Control HTTP response header. See the https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Cache-Control#response_directives[MDN documentation^] for details.

Default value: `3600`.

See also xref:files:downloading-files.adoc#downloader[Using Downloader].
...
For the `Downloader` bean, specifies the list of file extensions to be shown in the web browser tab instead of downloading them automatically. When a file is opened this way, the generated URL postfix has the `VAADIN/dynamic/resource/<random_id>` format.
...
[[jmix.ui.component.data-grid-header-filter-apply-shortcut]]
=== jmix.ui.component.data-grid-header-filter-apply-shortcut

Defines the xref:keyboard-shortcuts.adoc[keyboard shortcut] that applies the current condition in column header filters of xref:vc/components/dataGrid.adoc#column-filtering[dataGrid] and xref:vc/components/treeDataGrid.adoc[treeDataGrid] components.

For example:

[source,properties]
----
jmix.ui.component.data-grid-header-filter-apply-shortcut = CONTROL-ENTER
----

This property can be overridden for a particular grid using the xref:vc/components/dataGrid.adoc#headerFilterApplyShortcut[headerFilterApplyShortcut] attribute of its `columns` element.
