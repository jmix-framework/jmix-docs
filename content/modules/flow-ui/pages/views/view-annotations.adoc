...
* `@ViewDescriptor` 注解将视图类与 XML 描述进行关联。注解的值指定 XML 文件的路径。如果值仅包含文件名，则表示该描述文件位于与视图类相同的包中。否则，文件路径需要以 `/` 开头，包含自代码根目录开始的全路径，例如 `/com/company/demo/view/some-entity-list-view.xml`。

* `@com.vaadin.flow.router.Route` 定义导航至该视图的 URL 路径。如果视图需要在主视图内打开，`layout` 属性需要指定为主视图类。

* `@DialogMode` 注解定义当视图以对话框形式打开时的对话框窗口大小、位置、模态以及其他参数。
