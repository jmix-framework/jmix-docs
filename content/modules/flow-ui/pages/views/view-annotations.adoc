...
* `@DialogMode` определяет размер, положение, модальность и другие параметры диалогового окна, когда экран открывается как диалог.
...
* `@PrimaryDetailView` указывает, что этот экран является экраном деталей по умолчанию для указанной сущности. Аннотация имеет больший приоритет, чем xref:views/opening-views.adoc#view-inference-conventions[соглашение об именовании] `\{entity_name}.detail`.
