...
* `@ViewDescriptor` annotation connects the view class to the XML descriptor. The value of the annotation specifies the path to the descriptor file. If the value contains a file name only, it is assumed that the file is located in the same package as the view class. Otherwise, the path should start with `/` and contain the full path to the descriptor from the source root, for example `/com/company/demo/view/some-entity-list-view.xml`.
...
* `@DialogMode` annotation defines size, position, modality, and other parameters of the dialog window when the view is opened as a dialog.
