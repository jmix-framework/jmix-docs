...
[[opened-dialog-windows]]
=== Получение открытых диалоговых окон

Бин `DialogWindows` также позволяет получить `OpenedDialogWindows` — объект, который хранит список экранов, открытых в виде окон диалогов. Это может быть полезно, например, в интеграционных тестах UI, когда вам нужно получить доступ к экрану, открытому с помощью `OpenMode.DIALOG`.

[source,java,indent=0]
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/sample/SampleView.java[tags=dialogWindows;openDialogWindows]
----

Метод `getDialogs()` возвращает все экраны-диалоги текущего UI. Метод `getDialogs(UI)` возвращает экраны-диалоги предоставленного UI.

Экраны-диалоги добавляются в список в порядке их открытия, поэтому последний открытый диалог является последним элементом в списке.

NOTE: `OpenedDialogWindows` имеет область видимости сессии Vaadin и доступен только тогда, когда сессия Vaadin активна.
