...
=== Getting Opened Dialog Windows

The `DialogWindows` bean also allows you to get `OpenedDialogWindows`, an object that keeps the list of views opened as dialog windows. This can be useful, for example, in UI integration tests when you need to access a view opened with `OpenMode.DIALOG`.
...
The `getDialogs()` method returns all dialog views of the current UI. The `getDialogs(UI)` method returns dialog views of the provided `UI`.

Dialog views are added to the list in the order they are opened, so the last opened dialog is the last item in the list.

NOTE: `OpenedDialogWindows` has Vaadin session scope and is available only when a Vaadin session is active.
