...
* `beforeLeave()` - callback, который выполняется перед переходом на другой экран. Часть https://vaadin.com/docs/v25/flow/routing/lifecycle/[жизненного цикла навигации^] Vaadin.
...
* Два callback позволяют настраивать заголовок и подвал экранов, открываемых в диалоговом окне. Чтобы использовать эти callback, необходимо переопределить их в классе экрана.

** `configureDialogWindowHeader()` - этот callback настраивает заголовок xref:flow-ui:views/opening-views.adoc#dialog-windows[DialogWindow], когда экран открывается в `OpenMode.DIALOG`. Он предоставляет доступ к компоненту заголовка, позволяя добавлять собственные компоненты, заголовки или элементы стиля.
+
[source,java,indent=0]
----
...
** `configureDialogWindowFooter()` - этот callback настраивает подвал xref:flow-ui:views/opening-views.adoc#dialog-windows[DialogWindow], когда экран открывается в `OpenMode.DIALOG`. Он предоставляет доступ к компоненту подвала, позволяя добавлять собственные кнопки, действия или информационные элементы.
+
[source,java,indent=0]
----
...
----
+
На приведенном ниже скриншоте показан пример диалогового окна с кастомизированными заголовком и подвалом.
+
