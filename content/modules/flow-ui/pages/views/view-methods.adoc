...
* `getPageTitle()` - a callback that returns the title of a view. By default, returns localized value defined in a view descriptor. You can override it to provide the title using your own rules, for example:
+
[source,java,indent=0]
----
...
* `setPageTitle()` method allows you to set the view title dynamically at any moment of the view lifecycle. For example, you can invoke it from the `ReadyEvent` handler or from a data container event handler.
+
For consistency, we recommend using either `getPageTitle()` or `setPageTitle()` within a single view.

* `beforeLeave()` - a callback that is executed before the navigation to another view is performed. Part of Vaadin's https://vaadin.com/docs/v25/flow/routing/lifecycle/[navigation lifecycle^].
