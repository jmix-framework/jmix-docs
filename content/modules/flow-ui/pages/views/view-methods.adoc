...
* `hasUnsavedChanges()` - returns `true` if the view has unsaved changes. The default implementations checks if there are removed or modified entities in the `DataContext` or if only new entities have been registered as modified in the `DataContext`, checks whether they have been modified since the view was opened. You can override this method to provide additional checks or disable checking altogether.
