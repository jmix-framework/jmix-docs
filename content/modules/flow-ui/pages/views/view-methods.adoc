...
* `getPageTitle()` - 返回视图标题的回调方法。默认情况下，返回在视图 XML 中定义的本地化值。可以重写该方法按需提供标题，例如：
+
[source,java,indent=0]
----
...
* `setPageTitle()` 方法可以在视图生命周期的任何阶段动态设置视图的标题。例如，可以在 `ReadyEvent` 处理器或数据容器事件的处理器中调用。
+
为了保持一致性，我们建议在单一视图中使用 `getPageTitle()` 或 `setPageTitle()`。

* `beforeLeave()` - 一个回调函数，在导航至其他视图之前调用。是 Vaadin https://vaadin.com/docs/v25/flow/routing/lifecycle/[导航生命周期^] 的一部分。
