...
* `getViewAttributes()` - returns the `ViewAttributes` object that serves as a storage of values associated with their names. Uses `VaadinSession` as a store. For example, a detail view uses `ViewAttributes` to store *read-only state* and *locked status* and revert these settings if a view is refreshed.
...
* `setSelectionHandler()` - sets a callback that handles selected items. By default, adds items to a xref:data/collection-container.adoc[collection container] if a lookup view is opened for a xref:vc/components/dataGrid.adoc[DataGrid] component or sets a single value if a lookup view is opened for a field, for example xref:vc/components/entityPicker.adoc[EntityPicker].
+
[source,java,indent=0]
----
...
* `setReloadSaved(boolean)` - sets whether edited entity should be reloaded after the xref:views/view-methods.adoc#close-with-save-method[closeWithSave()] method invoked. The default value is `false`. It's set to `true` if a view is opened in a dialog mode.
