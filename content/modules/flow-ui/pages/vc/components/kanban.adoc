= kanban
:page-aliases: kanban:kanban-component.adoc
:table-caption!:

[NOTE.addon-component]
====
This component requires the commercial xref:kanban:index.adoc#installation[Kanban add-on].
====
...
Declare the `kanban` namespace in the view's XML descriptor:

[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=xmlns]
----
...
Then add the `kanban` element and bind it to a xref:flow-ui:data/data-containers.adoc[data container]:
...
[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=data;collection-container;layout;basic]
----

image::kanban:kanban-basic.png[align="center",width="804"]
...
[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=data;key-value-collection-container;layout;key-value]
----
...
[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=programmatically]
----
...
----
include::kanban:example$kanban-ex1/src/main/java/com/company/onboarding/view/kanban/KanbanView.java[tags=kanban;tasksDc;onInit;setItems]
----
...
The following example shows a `kanban` that is mapped to some fields of its task entity:

image::kanban:properties-mapping.png[align="center",width="800"]

[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=properties-mapping-declarative]
----
...
[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=properties-mapping-programmatic]
----

[source,java,indent=0]
----
include::kanban:example$kanban-ex1/src/main/java/com/company/onboarding/view/kanban/KanbanView.java[tags=programmaticPropertiesMappingKanban;onInit;programmaticPropertiesMapping]
----
...
* Change the attribute mapped to <<propertyPriority, priority>> by selecting a different priority using the corresponding icon.
...
[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=data;collection-container;layout;save-delegate;]
----

[source,java,indent=0]
----
include::kanban:example$kanban-ex1/src/main/java/com/company/onboarding/view/kanban/KanbanView.java[tags=kanbanSaveDelegate]
----
...
[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=kanban-actions]
----
...
image::kanban:kanban-actions.png[align="center",width="800"]

[TIP]
====
To add an `action` in Jmix Studio, select the component in the view descriptor XML or in the *Jmix UI* structure panel and click on the xref:studio:screen-designer.adoc#add-action[Add->Action] button in the *Jmix UI* inspector panel.
====
...
image::kanban:hierarchical-columns.png[width=800]
...
[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=hierarchical-columns]
----
====
...
* `TABS` - sub-columns are represented as text tabs. In this case, it makes sense to specify the <<columnSelected, selected>> column attribute.

For example:

image::kanban:hierarchy-mode.png[align="center",width="800"]
...
[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=hierarchy-mode]
----
====
...
image::kanban:task-position.png[align="center",width="800"]
...
[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=task-position]
----
====
...
image::kanban:task-renderer.png[align="center",width="800"]
...
[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=task-renderer]
----
====
...
image::kanban:header-renderer.png[align="center",width="800"]
...
[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=header-renderer]
----
====
...
image::kanban:footer-renderer.png[align="center",width="800"]
...
[source,xml,indent=0]
----
include::kanban:example$kanban-ex1/src/main/resources/com/company/onboarding/view/kanban/kanban-view.xml[tags=footer-renderer]
----
====
...
|If `true`, enables column reordering using drag-and-drop. The state of this attribute can be saved in the xref:flow-ui:facets/settings.adoc[user settings].
|`false`
...
The following shared attributes are supported by `kanban`:
...
|`KanbanColumnClickEvent` is fired when a column header is clicked. Corresponds to the `columnClick` DOM event.
|[[KanbanColumnDoubleClickEvent]]`KanbanColumnDoubleClickEvent`
|`KanbanColumnDoubleClickEvent` is fired when a column header is double-clicked. Corresponds to the `columnDoubleClick` DOM event.
|[[KanbanColumnCollapseEvent]]`KanbanColumnCollapseEvent`
|`KanbanColumnCollapseEvent` is fired when a column is collapsed using the column's actions menu, by double-clicking the column header, or by programmatically changing the <<columnCollapsed, column's collapsed>> property. Corresponds to the `columnCollapse` DOM event.
|[[KanbanColumnExpandEvent]]`KanbanColumnExpandEvent`
|`KanbanColumnExpandEvent` is fired when a column is expanded by double-clicking the column header, or by programmatically changing the <<columnCollapsed, column's collapse>> property. Corresponds to the `columnExpand` DOM event.
|[[KanbanColumnHideEvent]]`KanbanColumnHideEvent`
|`KanbanColumnHideEvent` is fired when a column is hidden by double-clicking the column header, or by programmatically changing the <<columnCollapsed, column's collapsed>> property. Corresponds to the `columnHide` DOM event.
|[[KanbanColumnReorderEvent]]`KanbanColumnReorderEvent`
|`KanbanColumnReorderEvent` is fired when the column order changes. Corresponds to the `columnReorder` DOM event.
|[[KanbanColumnShowEvent]]`KanbanColumnShowEvent`
|`KanbanColumnShowEvent` is fired when a column is shown using the column's actions menu, or by programmatically changing the <<columnVisible, column's visible>> property. Corresponds to the `columnShow` DOM event.
|[[KanbanDragEndEvent]]`KanbanDragEndEvent`
|`KanbanDragEndEvent` is fired when a task card is dropped. Corresponds to the `dragEnd` DOM event.
|[[KanbanDragStartEvent]]`KanbanDragStartEvent`
|`KanbanDragStartEvent` is fired when a task card is dragged. Corresponds to the `dragStart` DOM event.
|[[KanbanTaskClickEvent]]`KanbanTaskClickEvent`
|`KanbanTaskClickEvent` is fired when a task is clicked. Corresponds to the `taskClick` DOM event.
|[[KanbanTaskDoubleClickEvent]]`KanbanTaskDoubleClickEvent`
|`KanbanTaskDoubleClickEvent` is fired when a task is double-clicked. Corresponds to the `taskDoubleClick` DOM event.
|[[SelectionEvent]]`SelectionEvent`
|Fired when the selected task changes. The event provides the previous and current selected task.
|[[saveDelegate]]`saveDelegate`
|Adds a save delegate function that is used to save data updated with Kanban. It is called when a task card is dragged across the board and when the task priority changes using the corresponding icon. See <<save-delegate, Save Delegate>>.
|===

The following shared handlers are supported by `kanban`:
...
The elements within `kanban` control its data mapping, columns, actions, swimlanes, and renderers. It can include <<propertiesMapping,propertiesMapping>>, <<columns,columns>>, <<actions,actions>>, <<swimlanes,swimlanes>>, <<columnHeaderRendererFunction,columnHeaderRendererFunction>>, <<columnFooterRendererFunction,columnFooterRendererFunction>>, and <<taskRendererFunction,taskRendererFunction>>.
...
|`VERTICAL`
|[[columnReorder]]`reorder`
|If `true`, enables the ability to reorder. The attribute works in conjunction with the <<columnReorderingAllowed, columnReorderingAllowed>> Kanban attribute.
|`true`
|[[columnSelected]]`selected`
...
The `swimlanes` element can define a set of swimlanes to display, either explicitly or using an enumeration. Swimlanes are horizontal stripes that divide task cards into areas of responsibility. Nested columns are not available when using swimlanes.
