...
====
This component requires the free xref:pivot-table:index.adoc#installation[Pivot Table add-on].
====
...
`pivotTable` turns a data set into a summary table that users can configure with a two-dimensional drag-and-drop interface.
...
Declare the `pvttbl` namespace in the view's XML descriptor:
...
Then add `pivotTable` and bind it to a xref:flow-ui:data/data-containers.adoc[data container]:
...
To set up the component, provide a data container and specify the properties to show in the table. Define the data for the component as follows:
...
+
In the view XML descriptor, configure the `pivotTable` component to use your custom data model. For example:
+
.Show code
[%collapsible]
====
...
When `unusedPropertiesVertical` is set to `true`, the unused attributes area is always displayed vertically.

When `unusedPropertiesVertical` is set to `false`, the unused attributes area is always displayed horizontally.
...
|`85` (vertical when the combined attribute-name length exceeds this value)
|===

The following shared attributes are supported by `pivotTable`:
...
The following shared handlers are supported by `pivotTable`:
