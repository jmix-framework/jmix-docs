...
|Elements
|<<actions,actions>> - <<columns,columns>> - <<column,column>> - contextMenu
|===

[[basics]]
== Basics

To create the component, use the `dataGrid` XML element and bind it to a xref:flow-ui:data/data-containers.adoc[data container]. The component supports both collection and key-value containers. Then, specify which attributes you want to display:
...
|`column`
|Attributes
|<<autowidth,autowidth>> - <<editable,editable>> - <<filterable,filterable>> - <<flexGrow,flexGrow>> - <<footer,footer>> - <<frozen,frozen>> - <<header,header>> - <<key,key>> - <<property,property>> - <<resizable,resizable>> - <<sortable,sortable>> - <<textAlign,textAlign>> - <<visible,visible>> - <<width,width>>
|Handlers
|xref:vc/common-handlers.adoc#AttachEvent[AttachEvent] - DataGridColumnVisibilityChangedEvent - xref:vc/common-handlers.adoc#DetachEvent[DetachEvent] - <<classNameGenerator,classNameGenerator>> - <<partNameGenerator,partNameGenerator>> - <<renderer,renderer>> - <<tooltipGenerator,tooltipGenerator>>
|Elements
...
The `editorActionsColumn` element can display up to four different buttons, each corresponding to a specific stage in the editing process:
...
Each of the buttons supports a limited set of attributes of a standard xref:vc/components/button.adoc[]. The editor element supports a limited set of attributes and handlers inherited from a standard <<column, column>>.
...
In Jmix there are many xref:vc/common-attributes.adoc[common attributes] that are configured in the same way for all components.
...
|*Default*
|[[allRowsVisible]]allRowsVisible
|If `true`, the data grid will display all rows at once, disabling virtual scrolling. This means that instead of only rendering the visible rows and loading more as the user scrolls, the grid will render all the rows in the DOM simultaneously.
