...
The set of columns for a data grid is defined in the `columns` element. `columns` can have the following attributes:
...
<2> Define the `create` standard action.
...
Then you should start editing of a row either programmatically as described in the https://vaadin.com/docs/v23/components/grid#inline-editing-java-only[Vaadin documentation^], or by defining the `editorActionsColumn` element, for example:
...
<3> Set <<SupportsStatusChangeHandler,SupportsStatusChangeHandler>>.
...
By default, the inline editor uses this handler to set error message of a component as its xref:flow-ui:vc/common-attributes.adoc#title[title].
...
++++
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/v23/components/grid/#sorting-by-multiple-columns" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++
