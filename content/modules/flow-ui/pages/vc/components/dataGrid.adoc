= dataGrid
:page-aliases: vcl/components/data-grid.adoc, vcl/components/table.adoc, vcl/components/group-table.adoc


A `dataGrid` displays structured data in rows and columns and supports efficient browsing of large data sets.

[cols="1h,1", options="autowidth", grid=none, frame=none]
|===
|XML Element
|`dataGrid`

|Java Class
|`DataGrid`
|===

== Overview

[cols="a", options="autowidth", frame=all, grid=none, align=center]
|===
|image::visual-components/components/data-grid-basics.png[Data grid, align="center", width="804"]
|===

[[basics]]
== Basics

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-simple" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

To create the component, use the `dataGrid` XML element and bind it to a xref:flow-ui:data/data-containers.adoc[data container]. The component supports both collection and key-value containers. Then, specify which attributes from the container you want to display:

* To display all attributes as specified in the fetch plan, add the <<columns,columns>> element with `includeAll = true`. To exclude unnecessary attributes, list them in the `exclude` attribute separated by commas.
* To display only certain attributes, add the <<columns,columns>> element nesting individual <<column, column>> elements for each attribute you want to include.
* If the fetch plan contains a reference attribute, this attribute will be displayed according to its xref:data-model:entities.adoc#instance-name[instance name]. To display a specific attribute, add it explicitly to the fetch plan as well as in the `column` element.

The sample binds the grid to a collection container populated by `usersDl`. The XML chooses the entity attributes to display, while the controller supplies generated data to the loader:

.XML
[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/simple/data-grid-simple.xml[tags=basics]
----

.Java
[source,java,indent=0]
----
include::example$ui-samples/main/java/io/jmix/uisamples/view/flowui/components/datagrid/simple/DataGridSimpleSample.java[tags=basics]
----

[[data-binding]]
== Data-aware `dataGrid`

Set `dataContainer` to display entities from a collection container, or use a key-value collection container for scalar and aggregate query results. The grid observes its container, so reloading or changing the container updates the rows.

[[collection-container]]
=== Collection Container

Typically, you bind `dataGrid` declaratively by setting `dataContainer` to a xref:flow-ui:data/collection-container.adoc[collection container]. The <<basics,basics example>> uses this approach.

[[key-value-containers-binding]]
=== Key-Value Container

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-key-value" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

Bind the grid to a xref:flow-ui:data/key-value-containers.adoc[key-value container] when the query returns scalar values or aggregates. The columns in this example correspond to the properties declared by the key-value container:

[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/keyvalue/data-grid-key-value.xml[tags=key-value]
----

[[programmatic-binding]]
=== Programmatic Binding

For a container created in Java, set xref:flow-ui:vc/common-attributes.adoc#metaClass[metaClass] instead of xref:flow-ui:vc/common-attributes.adoc#dataContainer[dataContainer] so the XML columns can resolve entity metadata:

[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/datagrid/data-grid-view.xml[tags=metaClass]
----

Then wrap the programmatic container in `ContainerDataGridItems` and pass it to the grid. The wrapper propagates container changes and selection state through the grid's data-provider API:

[source,java,indent=0]
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/component/datagrid/DataGridView.java[tags=dataGrid;usersDc;onInit;setItems]
----

[[programmatic-columns]]
== Programmatic Columns

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-var-columns" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

`addColumn()` and `addComponentColumn()` let you create columns in Java instead of XML.

Use `addColumn()` with renderer instances such as the `LocalDateRenderer` shown in <<local-date-renderer,Local Date Renderer>>. Use `addComponentColumn()` when each cell should contain a UI component.

The following example creates a data grid with a user-defined number of columns:

.XML
[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/varcolumns/data-grid-var-columns.xml[tags=programmatic-columns]
----

.Java
[source,java,indent=0]
----
include::example$ui-samples/main/java/io/jmix/uisamples/view/flowui/components/datagrid/varcolumns/DataGridVariableColumnsSample.java[tags=programmatic-columns]
----

[[multi-select]]
== Multi-Select Mode

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-selection-mode" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

By default, the component operates in single‑selection mode. When you set <<selectionMode, selectionMode>> to `MULTI`, a checkbox column appears, allowing you to select any number of rows. To select every row, click the checkbox in the header row.

.XML
[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/selectionmode/data-grid-selection-mode.xml[tags=selection-mode]
----

.Java
[source,java,indent=0]
----
include::example$ui-samples/main/java/io/jmix/uisamples/view/flowui/components/datagrid/selectionmode/DataGridSelectionModeSample.java[tags=selection-mode-fields;selection-mode-handlers;selection-mode-items]
----

NOTE: Range selection using *Shift + click*  or other shortcuts is not currently supported.

[[inline-editing]]
== Inline Editing

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-inline-editor" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

The component supports inline editing, allowing users to switch between reading and editing table data. Inline editing can be activated through buttons in the <<actions-column, actions column>> or on <<double-click-editing,double click>>.

[NOTE]
====
Inline editing updates entity attributes in memory. To save changes to the database, use the following methods:

* In a detail view, entities are merged into the xref:data/data-context.adoc[]. Changes will be saved automatically when the user clicks *OK*, triggering `DataContext.save()`.

* In a list view, where `DataContext` is not used, save changes with `DataManager` as explained <<auto-save,below>>.
====

[[actions-column]]
=== Actions Column

Use `editorActionsColumn` to provide *Edit* and *Close* buttons next to each row. These enable users to start and stop editing respectively.

NOTE: The actions column is added relatively to other columns. When `includeAll="true"` is set, the editor column is placed at the far right.

[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/inlineeditor/data-grid-inline-editor.xml[tags=non-buffered-editor]
----

==== Buffered Mode

With <<editorBuffered, buffered mode>> on, users will have to confirm or cancel their edits reducing the risk of accidental modifications. Clicking on a different row discards edits.

[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/inlineeditor/data-grid-inline-editor.xml[tags=buffered-editor]
----

[[double-click-editing]]
=== Edit on Double Click

Sometimes it is more convenient to start inline editing by double-clicking the item.

[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/datagrid/data-grid-view.xml[tags=dblClickTable]
----

[source,java,indent=0]
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/component/datagrid/DataGridView.java[tags=dblClickTable;onInit;addItemDoubleClickListener]
----

[[auto-save]]
=== Auto-Save Edits

In a typical list view, loaded entities are not merged into `DataContext` (if the `loader` XML element has the `readOnly="true"` attribute). Besides, list views usually do not include confirmation actions such as *OK* or *Save*. Therefore, modified entities should be saved to the database explicitly.

In non-buffered mode, you can do it by using `DataManager` in an `EditorCloseEvent` as follows:

In buffered mode, do the same in the `EditorSaveEvent` listener. The sample implements both variants:

[source,java,indent=0]
----
include::example$ui-samples/main/java/io/jmix/uisamples/view/flowui/components/datagrid/inlineeditor/DataGridInlineEditorSample.java[tags=auto-save]
----

[TIP]
====
To generate listeners as annotated methods, use the *Handlers* tab of the Jmix Studio's *Component Inspector*, where editor event listeners are marked with `[Editor]` prefixes.

The listeners can also be added programmatically via `dataGrid.getEditor().addSaveListener()` and similar methods.
====

[[DataGridEditor]]
=== DataGridEditor

The `io.jmix.flowui.component.grid.editor.DataGridEditor` interface provides additional editor functionality: configure an editor, open the editor, save and cancel a row editing, register listeners, and utility methods for defining column edit components.

CAUTION: To support framework mechanisms like data containers, value sources, etc., the column editor component must be added using `DataGridEditor` methods (`DataGridEditor#setColumnEditorComponent()`) instead of direct column API `Column#setEditorComponent()`.

See the example:

[source,java,indent=0]
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/component/datagrid/DataGridView.java[tags=uiComponents;editableUserTable;onInit;DataGridEditor]
----
<1> Get the instance of `DataGridEditor`.
<2> The `JmixComboBox` component instance is created using the `UiComponents` factory.
<3> Set <<SupportsStatusChangeHandler,StatusChangeHandler>>.
<4> The `setColumnEditorComponent()` method returns the visual component to be shown as the column editor component.

[[SupportsStatusChangeHandler]]
=== SupportsStatusChangeHandler

By default, field components (for example, xref:flow-ui:vc/components/textField.adoc[textField], xref:flow-ui:vc/components/comboBox.adoc[comboBox]) display error messages in a label above them. Such behaviour has disadvantages in case of limited area of edit cell. The `io.jmix.flowui.component.SupportsStatusChangeHandler` interface enables to define different way of displaying error messages. Components that implement this interface support error handling delegation.

By default, the inline editor uses `StatusChangeHandler` to set error message of a component as its xref:flow-ui:vc/common-attributes.adoc#title[title].

[[header-footer]]
== Header and Footer

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-header-footer" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

The component can include header and footer sections to show supplementary information. Each section can contain one or more rows, which you can add using the following methods:

[cols="1h,6", grid=rows]
|===
|*Method*
|*Description*
|`appendHeaderRow()`
|Adds a new row at the bottom of the header section.
|`prependHeaderRow()`
|Adds a new row at the top of the header section.
|`appendFooterRow()`
|Adds a new row at the bottom of the footer section.
|`prependFooterRow()`
|Adds a new row at the top of the footer section.
|===

The following example demonstrates a `dataGrid` that includes merged cells in its header and a computed value in the footer:

.XML
[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/headerfooter/data-grid-header-footer.xml[tags=header-footer]
----

.Java
[source,java,indent=0]
----
include::example$ui-samples/main/java/io/jmix/uisamples/view/flowui/components/datagrid/headerfooter/DataGridHeaderFooterSample.java[tags=header-footer]
----

[[column-filtering]]
== Column Headers Filtering

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-header-filter" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

You can filter data by individual columns using xref:flow-ui:vc/components/propertyFilter.adoc[property filters] embedded in column headers.

Filterable columns display a funnel icon (image:visual-components/components/funnel.svg[]) in their headers. Clicking the icon opens the filter dialog. Set the `filterable` attribute to `true` on a `column` element to enable filtering:

[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/headerfilter/data-grid-header-filter.xml[tags=column-filtering]
----

TIP: You can configure a keyboard shortcut for applying column header filters. See <<headerFilterApplyShortcut,headerFilterApplyShortcut>> or xref:ui-properties.adoc#jmix.ui.component.data-grid-header-filter-apply-shortcut[jmix.ui.component.data-grid-header-filter-apply-shortcut] for global configuration.

Like standalone `propertyFilter` and xref:flow-ui:vc/components/genericFilter.adoc[] components, column header filters add conditions to the xref:flow-ui:data/data-loaders.adoc[data loader]. Their state can also be bound to the page URL using the `dataGridFilter` element of the xref:flow-ui:facets/urlQueryParameters.adoc[urlQueryParameters] facet. In the standard data-loading flow, conditions are translated into JPQL and applied at the database level. Conditions from all filters are combined using logical `AND`.

WARNING: Header filters add their conditions to the data loader condition, so the rules from xref:flow-ui:data/data-loaders.adoc#filter-components-and-loader-condition[Filter Components and the Loader Condition] apply. In particular, do not replace the loader condition programmatically after the view is initialized: otherwise the header filter conditions may be excluded from the query while the columns still appear filtered.

[[actions]]
== Actions

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-actions" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

The `dataGrid` component implements the `HasActions` interface and can contain both standard xref:flow-ui:actions/list-actions.adoc[list actions] and custom actions. Actions are invoked by clicking designated buttons or from the <<context-menu, context menu>> that appears on the right click.

[TIP]
====
To add `action` in Jmix Studio, select the component in the view descriptor XML or in the *Jmix UI* structure panel and click on the xref:studio:screen-designer.adoc#add-action[Add->Action] button in the *Jmix UI* inspector panel.
====

.XML
[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/actions/data-grid-actions.xml[tags=actions]
----

.Java
[source,java,indent=0]
----
include::example$ui-samples/main/java/io/jmix/uisamples/view/flowui/components/datagrid/actions/DataGridActionsSample.java[tags=actions]
----

[[context-menu]]
== Context Menu

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-context-menu" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

The context menu offers an alternative way to access <<actions, actions>> through a right click. Each action is represented by its own menu item.

Use the `contextMenu` element to refine the list of items in the menu, organizing them with separators and a hierarchical structure.

.XML
[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/contextmenu/data-grid-context-menu.xml[tags=context-menu]
----

.Java
[source,java,indent=0]
----
include::example$ui-samples/main/java/io/jmix/uisamples/view/flowui/components/datagrid/contextmenu/DataGridContextMenuSample.java[tags=context-menu]
----

[[renderers]]
== Renderers

Renderers customize how column cells are displayed. This section covers renderer options specific to `dataGrid` columns. Shared renderer APIs such as `ComponentRenderer`, `LitRenderer`, and `FragmentRenderer` are described in xref:flow-ui:vc/miscellaneous/renderers.adoc[Renderers].

Renderers can be defined in the following ways:

* *Declaratively in XML*: use predefined renderers such as `numberRenderer`, `localDateRenderer`, `localDateTimeRenderer`, `detailLinkRenderer`, and `detailButtonRenderer` inside a `column` element.
* *Using `@Supply` annotation*: provide a custom renderer from the view controller.
* *Using `fragmentRenderer`*: use the shared fragment-based renderer mechanism described in xref:flow-ui:vc/miscellaneous/renderers.adoc#fragment-renderer[Fragment Renderer].
* *Using `addColumn()` and `addComponentColumn()`*: create columns in Java and configure renderers programmatically. See <<programmatic-columns,Programmatic Columns>>.

[TIP]
====
To add a renderer in Jmix Studio, select the `column` element in the view descriptor XML or in the *Jmix UI* structure panel and click xref:studio:screen-designer.adoc#component-inspector[Add -> Renderer].
====

[[local-date-renderer]]
=== Local Date Renderer

Renders `LocalDate` values in a column.

[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/datagrid/data-grid-view.xml[tags=localDateRenderer]
----

Use the required `format` attribute to define the output format. Use the optional `nullRepresentation` attribute to specify how `null` values are displayed.

For Java configuration, use `LocalDateRenderer` with `addColumn()`:

[source,java,indent=0]
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/component/datagrid/DataGridView.java[tags=usersDtGr;onInit;addJoiningDate]
----

[[local-date-time-renderer]]
=== Local Date Time Renderer

Renders `LocalDateTime` values in a column.

[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/datagrid/data-grid-view.xml[tags=localDateTimeRenderer]
----

Use the required `format` attribute to define the output format. Use the optional `nullRepresentation` attribute to specify how `null` values are displayed.

[[number-renderer]]
=== Number Renderer

Renders numeric values in a column.

[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/datagrid/data-grid-view.xml[tags=numberFormat]
----

Specify exactly one of `format` or `numberFormat`. If neither is specified, or if both are specified together, the framework throws a `GuiDevelopmentException`. Use the optional `nullRepresentation` attribute to specify how `null` values are displayed. `numberFormat` uses `java.text.DecimalFormat` syntax.

[[detail-link-renderer]]
=== Detail Link Renderer

Renders a link that opens the detail view of the current entity.

[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/datagrid/data-grid-view.xml[tags=detailLinkRenderer]
----

Supported attributes:

* `viewClass`, `viewId` specify the target view. The renderer resolves the view in the following order: `viewClass`, `viewId`, default detail view.
* `text` sets the link text. If omitted, the renderer uses the formatted value of the bound column property.
* `target` sets the link target.
* `classNames`, `css` apply styles.

The target view must have a registered route with exactly one route parameter, and the entity instance must already have a non-null id. For routes with zero or multiple route parameters, configure the renderer from Java code and provide a custom URL using `DetailLinkRenderer.withHrefProvider()`.

[[detail-button-renderer]]
=== Detail Button Renderer

Renders a button that opens the detail view of the current entity.

[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/datagrid/data-grid-view.xml[tags=detailButtonRenderer]
----

Supported attributes:

* `viewClass`, `viewId` specify the target view. The renderer resolves the view in the following order: `viewClass`, `viewId`, default detail view.
* `openMode` controls how the view is opened: `NAVIGATION` (default) or `DIALOG`.
* `text` sets the button text. If omitted, the renderer uses the formatted value of the bound column property. Set it explicitly for key-only columns.
* `icon` sets the button icon. You can define it through the attribute or a nested `icon` element.
* `themeNames`, `classNames`, `css` apply styles.

[[detail-renderers-key-only-columns]]
=== Detail Renderers in Key-Only Columns

You can use detail renderers in a column with `key` instead of the `property` attribute:

[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/datagrid/data-grid-view.xml[tags=detailRenderersKeyOnly]
----

In this case, set the link or button `text` explicitly, because there is no bound property value.

[[text-renderer]]
=== Text Renderer

Renders plain text provided from Java code.

[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/datagrid/data-grid-view.xml[tags=textRenderer]
----

[source,java,indent=0]
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/component/datagrid/DataGridView.java[tags=textRenderer]
----

This renderer is typically supplied from Java code by using the `renderer` handler or `@Supply`.

[[component-renderer]]
=== Component Renderer

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-custom-renderer" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

Renders a custom component in each cell. Use this renderer when a cell must contain rich or interactive content, such as a checkbox, image, or button.

NOTE: See xref:flow-ui:vc/miscellaneous/renderers.adoc#component-renderer[Component Renderer] for available constructors, general behavior, and performance considerations.

The following example renders a read-only checkbox in the `active` column and a badge in the `grade` column.

.XML
[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/customrenderer/data-grid-custom-renderer.xml[tags=component-renderer]
----

.Java
[source,java,indent=0]
----
include::example$ui-samples/main/java/io/jmix/uisamples/view/flowui/components/datagrid/customrenderer/DataGridCustomRendererSample.java[tags=component-renderer]
----

[[item-details]]
== Item Details

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-item-details" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

This component allows rows to expand, revealing additional information about each item. To implement this functionality, use the `setItemDetailsRenderer()` method. This method takes a <<component-renderer, ComponentRenderer>> as an argument, which specifies how the details are rendered.

The <<detailsVisibleOnClick, detailsVisibleOnClick>> attribute allows you to control whether these details appear when the row is clicked. If you prefer the details to be triggered by an event other than clicking, set this attribute to `false`.

.XML
[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/itemdetails/data-grid-item-details.xml[tags=item-details]
----

.Java
[source,java,indent=0]
----
include::example$ui-samples/main/java/io/jmix/uisamples/view/flowui/components/datagrid/itemdetails/DataGridItemDetailsSample.java[tags=item-details]
----

[[sorting]]
== Sorting

TIP: This section demonstrates sorting customization for individual `dataGrid` component. For application-wide sorting customization, see xref:flow-ui:data/data-examples.adoc#sorting[Sorting].

Any column can be used for sorting the data displayed. Clicking on the column header activates a special control in the grid that shows which column is currently being sorted and the sorting direction (ascending or descending). Each subsequent click on the same header will toggle the sorting direction.

[NOTE]
====
xref:data-model:enumerations.adoc[Enumeration] attributes are sorted by their `id` values, not by localized captions displayed in the UI.

As a result, the sorting order can differ from the visible order of enum values in a `dataGrid`, especially when enum values are localized.
====

[[multiple-sorting]]
=== Sorting by Multiple Columns

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-multi-sort" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

It is possible to sort by multiple columns simultaneously. When sorting by multiple columns, the first selected column determines the primary sorting criterion, the second column sets the secondary criterion, and so on. You can configure it using <<multiSort, multiSort>>, <<multiSortOnShiftClickOnly, multiSortOnShiftClickOnly>>, and <<multiSortPriority, multiSortPriority>> attributes.

For example:

[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/multisort/data-grid-multi-sort.xml[tags=multi-sort]
----

[[column-comparator]]
=== Column Comparator

Use `DataGridColumn#setComparator()` to define custom sorting logic for a specific column when sorting is performed in memory. In-memory sorting is used when the container has no loader, or when only the first page is loaded and the number of loaded items is less than the page size.

The following example sorts the `Department.num` attribute as numbers even though the attribute type is `String`:

[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/datagrid/data-grid-sorting-view.xml[tags=comparator-grid]
----

[source,java,indent=0]
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/component/datagrid/DataGridSortingView.java[tags=in-memory-comparator]
----

[[sort-builder-delegate]]
=== Sort Builder Delegate

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-sort-delegate-expression" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

If the grid reloads data from the database, configure Sort Builder Delegate. The delegate receives sorting instructions from the grid and returns a `DataGridSort` object, allowing you to override in-memory sorting, database sorting, or both in one place.

The following example sorts the `customer` column by customer name and last name:

.XML
[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/sortdelegateexpression/data-grid-sort-delegate-expression.xml[tags=sort-builder-delegate]
----

.Java
[source,java,indent=0]
----
include::example$ui-samples/main/java/io/jmix/uisamples/view/flowui/components/datagrid/sortdelegateexpression/DataGridSortDelegateExpressionSample.java[tags=sort-builder-delegate]
----

[[sorting-computed-column]]
==== Sorting a Computed Column

Sort Builder Delegate is also useful for columns that are not bound to an entity property. In the example below, the `fullName` column is rendered from `firstName` and `lastName`, and the sort builder delegate maps this synthetic column to both JPQL expressions and an in-memory comparator:

[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/datagrid/data-grid-sorting-view.xml[tags=sort-builder-computed-grid]
----

[source,java,indent=0]
----
include::example$/onboarding/src/main/java/com/company/onboarding/view/component/datagrid/DataGridSortingView.java[tags=sort-builder-computed-column;full-name-renderer]
----

[[aggregating]]
== Aggregating

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-aggregation" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

The component supports aggregating values in its rows. When aggregation is enabled, it will display an additional row containing aggregated values. For example:

[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/aggregation/data-grid-aggregation.xml[tags=aggregation]
----

The `aggregation` element can specify a class with custom aggregation logic through `strategyClass` attribute:

Such class must implement `AggregationStrategy` interface. For example:

[source,java,indent=0]
----
include::example$ui-samples/main/java/io/jmix/uisamples/view/flowui/components/datagrid/aggregation/DataGridCustomerGradeAggregation.java[tags=aggregation]
----

[[empty-state]]
== Empty State

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-empty-state" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

When a data grid has no data to display, the area between the header and footer is blank by default. Use the empty state feature to display a message or UI component informing the user that there are no items to show.

* Use the `emptyStateText` attribute to define the text that appears when the data grid is empty.
+
[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/datagrid/data-grid-features-view.xml[tags=empty-state]
----
+
NOTE: This attribute takes precedence over any empty state component configured using the `setEmptyStateComponent(Component)` method in the view controller or the `emptyStateComponent` element in the view XML-descriptor. Setting `emptyStateText` will remove any existing component.
* Use the `emptyStateComponent` element to define the component that appears when the grid is empty.
+
[source,xml,indent=0]
----
include::example$ui-samples/main/resources/io/jmix/uisamples/view/flowui/components/datagrid/emptystate/data-grid-empty-state.xml[tags=empty-state]
----
+
[source,java,indent=0]
----
include::example$ui-samples/main/java/io/jmix/uisamples/view/flowui/components/datagrid/emptystate/DataGridEmptyStateSample.java[tags=empty-state]
----
+
Once you've set `emptyStateComponent`, you can interact with the nested component as usual. This includes customizing its styling and creating event handlers.
+
NOTE: Setting this element overrides any empty state text defined using the `setEmptyStateText(String)` method in the view controller or the `emptyStateText` attribute in the view XML-descriptor.

[[theme-variants]]
== Theme Variants

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/data-grid-theme-variant" class="live-demo-btn" target="_blank">LIVE DEMO</a>
</div>
++++

Use xref:vc/common-attributes.adoc#themeNames[themeNames] attribute to set a component theme.

[cols="1,4,1", options="header"]
|===
|Variant
|Description
|Supported By

|`column-borders`
|Adds vertical borders between columns.
|Aura, Lumo

|`compact`
|Reduces row height and spacing.
|Lumo

|`no-border`
|Removes the outer border around the grid.
|Aura, Lumo

|`no-row-borders`
|Removes the horizontal borders between rows.
|Aura, Lumo

|`row-stripes`
|Alternates row background colors to improve readability.
|Aura, Lumo

|`wrap-cell-content`
|Allows cell content to wrap onto multiple lines instead of truncating.
|Aura, Lumo
|===

[[attributes]]
== Attributes

The following attributes are specific to `dataGrid`:

[cols="2,5,1", options="header"]
|===
|Name
|Description
|Default
|[[allRowsVisible]]allRowsVisible
|If `true`, the data grid will display all rows at once with no scroll bars, effectively disabling virtual scrolling. This means that instead of only rendering the visible rows and loading more as the user scrolls, the grid will render all the rows in the DOM simultaneously.

*Using this feature is discouraged for a large number of items as it may cause performance issues.*
|`false`
|[[aggregatable]]aggregatable
|If `true`, enables aggregating of columns. See <<aggregating, Aggregating>>.
|`false`
|[[aggregationPosition]]aggregationPosition

|If the data in the component is <<aggregatable,aggregatable>>, determines whether the aggregation row is displayed above or below the other rows. Possible values: `TOP` or `BOTTOM`. See <<aggregating, Aggregating>>.
|`BOTTOM`
|[[columnReorderingAllowed]]columnReorderingAllowed
|If `true`, enables users to change the order of columns.
|`false`

|[[detailsVisibleOnClick]]detailsVisibleOnClick
|If `true`, enables <<item-details,item details>> to be revealed on mouse click.
|`true`
|[[dropMode]]dropMode
|Determines rows where a drop can happen. Possible values: `BETWEEN`, `ON_TOP`, `ON_TOP_OR_BETWEEN`, `ON_GRID`. This feature might be used, for example, to reorder rows and to drag rows between grids.

|–
|[[editorBuffered]]editorBuffered
|If `true`, activates buffered mode for <<inline-editing, inline editing>>,  meaning that the user must confirm making changes by clicking a confirm button. This mode also allows users to cancel their changes. In unbuffered mode changes are applied without the need for confirmation.
|`false`
|[[emptyStateText]]emptyStateText

|The text to display when the data grid is empty. Use null to remove the current empty state content. See <<empty-state, Empty State>>.
|–
|[[multiSort]]multiSort
|If `true`, enables sorting by multiple columns.
|`false`

|[[multiSortOnShiftClickOnly]]multiSortOnShiftClickOnly
|If `true`, multi-sorting is activated only when clicking on the column header while holding down the Shift key.
|`false`
|[[multiSortPriority]]multiSortPriority
|Determines whether the clicked column is added to the end or beginning of the sorted columns list. Possible values: `PREPEND`, `APPEND`.

|`PREPEND`
|[[nestedNullBehavior]]nestedNullBehavior
|Sets the behavior when parsing nested properties which may contain null values in the property chain. Possible values: `THROW`, `ALLOW_NULLS`.
|`THROW`
|[[pageSize]]pageSize

|Determines the page size or the number of items that will be fetched from the data provider at a time.
|`50`
|[[rowsDraggable]]rowsDraggable
|If `true`, enables users to drag rows in the grid.
|`false`

|[[selectionMode]]selectionMode
|Sets the selection mode. Possible values: `SINGLE`, `MULTI`, `NONE`.
|`SINGLE`
|===

== Handlers

The following handlers are specific to `dataGrid`:

[cols="2,5", options="header"]
|===
|Name
|Description

|[[CellFocusEvent]]`CellFocusEvent`
|`com.vaadin.flow.component.grid.CellFocusEvent` is fired when a cell in the data grid is focused. Corresponds to the `grid-cell-focus` DOM event.

|[[ColumnReorderEvent]]`ColumnReorderEvent`
|`com.vaadin.flow.component.grid.ColumnReorderEvent` is fired when the columns in the data grid are reordered. Corresponds to the `column-reorder-all-columns` DOM event.

|[[ColumnResizeEvent]]`ColumnResizeEvent`
|`com.vaadin.flow.component.grid.ColumnResizeEvent` is fired when a data grid column is resized by the user. Corresponds to the `column-drag-resize` DOM event.

|[[GridDragEndEvent]]`GridDragEndEvent`
|`com.vaadin.flow.component.grid.dnd.GridDragEndEvent` is fired when dragging data grid rows ends. Corresponds to the `grid-dragend` DOM event.

|[[GridDragStartEvent]]`GridDragStartEvent`
|`com.vaadin.flow.component.grid.dnd.GridDragStartEvent` is fired when dragging data grid rows starts. Corresponds to the `grid-dragstart` DOM event.

|[[GridDropEvent]]`GridDropEvent`
|`com.vaadin.flow.component.grid.dnd.GridDropEvent` is fired when rows are dropped on the data grid. Corresponds to the `grid-drop` DOM event.

|[[ItemClickEvent]]`ItemClickEvent`
|`com.vaadin.flow.component.grid.ItemClickEvent` is fired when a data grid item is clicked. Corresponds to the `item-click` DOM event.

|[[ItemDoubleClickEvent]]`ItemDoubleClickEvent`
|`com.vaadin.flow.component.grid.ItemDoubleClickEvent` is fired when a data grid item is double-clicked. Corresponds to the `item-double-click` DOM event.

|[[SortEvent]]`SortEvent`
|`com.vaadin.flow.data.event.SortEvent` describes a change in data provider sorting. It is fired by components implementing `com.vaadin.flow.data.provider.SortNotifier`.

|[[dataGenerator]]`dataGenerator`
|Adds a data generator to the data grid. If the generator has already been added, nothing happens. See the `com.vaadin.flow.data.provider.HasDataGenerators` interface.

|[[dragFilter]]`dragFilter`
|Determines which rows can be dragged when <<rowsDraggable,rowsDraggable>> is enabled.

|[[dropFilter]]`dropFilter`
|Determines which rows are valid drop targets.

|[[enterPressHandler]]`enterPressHandler`
|Handles the event when the user presses Enter.

|[[itemSelectableProvider]]`itemSelectableProvider`
|Determines whether users can select or deselect a particular item.

|[[partNameGenerator]]`partNameGenerator`
|Generates CSS part names for cells based on their items, enabling conditional cell styling.

|[[tooltipGenerator]]`tooltipGenerator`
|Generates tooltip text for cells based on their items. See https://demo.jmix.io/ui-samples/sample/data-grid-tooltip-generator[live demo^].
|===

The following shared handlers are supported by `dataGrid`:

xref:vc/common-handlers.adoc#AttachEvent[AttachEvent] -
xref:vc/common-handlers.adoc#BlurEvent[BlurEvent] -
xref:vc/common-handlers.adoc#DetachEvent[DetachEvent] -
xref:vc/common-handlers.adoc#FocusEvent[FocusEvent]

== Elements

Elements of `dataGrid` provide a wide range of options to control the appearance, behavior, and functionality of columns both collectively and individually.

[TIP]
====
To add an element to a selected component click the xref:studio:screen-designer.adoc#add-column[Add] button in the *Jmix UI* inspector panel.
====

[[columns]]
=== columns

The `columns` element can specify a set of attributes to display and behaviors for all columns.

[cols="1h,6", grid=none]
|===
|XML Element
|`columns`
|Attributes
|<<exclude,exclude>> - <<headerFilterApplyShortcut,headerFilterApplyShortcut>> - <<includeAll,includeAll>> - <<resizable-columns,resizable>> - <<sortable-columns,sortable>>
|Elements
|<<column,column>> - <<actions-column,EditorActionsColumn>>
|===

.Attributes
[cols="1h,5,1", grid=rows]
|===
|*Name*
|*Description*
|*Default*
|[[exclude]]exclude
|Excludes specific attributes from being shown. Several attributes must be separated with a comma. For example: `exclude = "id, version, sortValue"`.
|–
|[[headerFilterApplyShortcut]]headerFilterApplyShortcut
|Sets the keyboard shortcut for applying column header filters. For example: `CONTROL-ENTER`. Overrides the xref:ui-properties.adoc#jmix.ui.component.data-grid-header-filter-apply-shortcut[global setting]. See <<column-filtering,Column Headers Filtering>>.
|–
|[[includeAll]]includeAll
|If `true` includes all the attributes specified in the corresponding data container's fetch plan.
|–
|[[resizable-columns]]resizable
|If `true`, all columns are user-resizable.
|`false`
|[[sortable-columns]]sortable
|If `true`, all columns are sortable.
|`true`
|===

[[column]]
=== column

The `column` element defines an individual column. Attributes set for an individual column override those set for all columns.

[cols="1h,6", grid=none]
|===
|XML Element
|`column`
|Attributes
|<<autowidth,autowidth>> - <<editable,editable>> - <<filterable,filterable>> - <<flexGrow,flexGrow>> - <<footer,footer>> - <<frozen,frozen>> - <<frozenToEnd,frozenToEnd>> - <<header,header>> - <<key,key>> - <<property,property>> - <<resizable-column,resizable>> - <<sortable-column,sortable>> - <<textAlign,textAlign>> - <<visible,visible>> - <<width,width>>
|Handlers
|xref:vc/common-handlers.adoc#AttachEvent[AttachEvent] - <<DataGridColumnVisibilityChangedEvent,DataGridColumnVisibilityChangedEvent>> - xref:vc/common-handlers.adoc#DetachEvent[DetachEvent] - <<partNameGenerator-column,partNameGenerator>> - <<renderer,renderer>> - <<tooltipGenerator-column,tooltipGenerator>>
|Elements
|<<aggregating,Aggregation>> - xref:flow-ui:vc/miscellaneous/renderers.adoc#fragment-renderer[FragmentRenderer] - <<local-date-renderer,LocalDateRenderer>> - <<local-date-time-renderer,LocalDateTimeRenderer>> - <<number-renderer,NumberRenderer>> - <<detail-link-renderer,DetailLinkRenderer>> - <<detail-button-renderer,DetailButtonRenderer>>
|===

.Attributes
[cols="1h,5,1", grid=rows]
|===
|*Name*
|*Description*
|*Default*
|[[autowidth]] autoWidth
|If `true`, the column width will adjust to its contents.
|`false`
|[[editable]]editable
|If `true`, the column can be edited. See <<inline-editing, Inline Editing>>.
|`false`
|[[filterable]]filterable
|If `true` enables filtering for this column. See <<column-filtering,Filtering>>.
|`false`
|[[flexGrow]]flexGrow
|Sets the flex grow ratio for this column. When set to `0`, the column width is fixed.
|`0`
|[[footer]]footer
|Sets a footer text to the column. The attribute value can either be the text itself or a key in the xref:localization:message-bundles.adoc[message bundle]. In case of a key, the value should begin with the `msg://` prefix.
| –
|[[frozen]]frozen
|If `true`, freezes (locks in place) the column, so that it remains visible as the user scrolls the table horizontally. It's generally recommended to freeze columns from left to right.
|`false`
|[[frozenToEnd]]frozenToEnd
|If `true`, freezes (locks in place) the column at the end of the data grid, so that it remains visible as the user scrolls the table horizontally. It's generally recommended to freeze columns from right to left. See https://demo.jmix.io/ui-samples/sample/data-grid-frozen-column[live demo^].
|`false`
|[[header]]header
|Sets the column header text. The attribute value can either be the text itself or a key in the xref:localization:message-bundles.adoc[message bundle]. In case of a key, the value should begin with the `msg://` prefix.
| –
|[[key]]key
|Sets the user-defined identifier to map this column. The key can be used to fetch the column later with the `getColumnByKey(String)` method.

The key has to be unique within the data grid, and it can’t be changed after set once.
|–
|[[property]]property
|Specifies the name of an entity attribute to be displayed in the column. This can be an attribute of the root entity `property = "user"` or an attribute of its child entity `property = `user.department.name` (use dot notation to traverse the object graph).
|–
|[[resizable-column]]resizable
|If `true`, the column is user-resizable.
|`false`
|[[sortable-column]]sortable
|If `true`, the column is sortable.
|`false`
|[[textAlign]]textAlign
|Specifies the alignment of the text with the following possible values: `START`, `CENTER`, `END`. See Text Alignment.
|`START`
|[[visible]]visible
|If `true`, the column is visible.
|`true`
|[[width]]width
|Sets the width of the column as a CSS string using units such as `px`, `%`, `rem`, or `em`. Prefer `rem` over `em` for font-relative widths because differences between header, body, and footer font sizes can cause misalignment.
|–
|===

.Handlers
[cols="1h,6", grid=rows]
|===
|*Name*
|*Description*
|[[DataGridColumnVisibilityChangedEvent]]DataGridColumnVisibilityChangedEvent
|Fired when the column visibility is changed through the xref:flow-ui:vc/components/gridColumnVisibility.adoc[] component.
|[[partNameGenerator-column]]partNameGenerator
|Generates parts of CSS class names for this column based on given conditions. This allows for customizing cell appearance based on the data displayed. See https://demo.jmix.io/ui-samples/sample/data-grid-part-name-generator[live demo^].
|[[renderer]]renderer
|Renders column content using text or components. See <<text-renderer,text renderer>> and <<component-renderer, component renderer>>.
|[[tooltipGenerator-column]]tooltipGenerator
|Generates tooltip for the column cell based on given conditions. See https://demo.jmix.io/ui-samples/sample/data-grid-tooltip-generator[live demo^].
|===

[[context-menu-element]]
=== contextMenu

The `contextMenu` element organizes items the right-click menu in a way that differs from their default arrangement. See the <<context-menu, example>>.

[cols="1h,6", grid=none]
|===
|XML Element
|`contextMenu`
|Attributes

|xref:vc/common-attributes.adoc#id[id] - xref:vc/common-attributes.adoc#classNames[classNames] - xref:vc/common-attributes.adoc#css[css] - xref:vc/common-attributes.adoc#enabled[enabled] - xref:vc/common-attributes.adoc#visible[visible]
|Handlers
|xref:vc/common-handlers.adoc#AttachEvent[AttachEvent] - xref:vc/common-handlers.adoc#DetachEvent[DetachEvent] - <<GridContextMenuOpenedEvent,GridContextMenuOpenedEvent>> - <<openedChangeEvent,openedChangeEvent>> - <<dynamicContentHandler,dynamicContentHandler>>
|Elements
|item - separator
|===

.Handlers
[cols="1h,6", grid=rows]
|===
|*Name*
|*Description*
|[[GridContextMenuOpenedEvent]]GridContextMenuOpenedEvent
|Fired when the context menu opened state changes. May return the target item or the id of the target column allowing the menu to display items based on the clicked item.
|[[openedChangeEvent]]openedChangeEvent
|Fired when the context menu opened state changes.
|[[dynamicContentHandler]]dynamicContentHandler
|Handles dynamic updates to the menu when it opens, such as adding menu items or their content. See the <<context-menu, example>>.
|===

[[empty-state-component-element]]
=== emptyStateComponent

The `emptyStateComponent` element defines the component to display when the grid is empty. Use null to remove the current empty state content. See <<empty-state, Empty State>>.

== See Also

* https://vaadin.com/docs/v25/components/grid[Vaadin Grid documentation^]
