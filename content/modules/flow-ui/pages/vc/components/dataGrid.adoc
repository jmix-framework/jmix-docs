...
[TIP]
====
如需在 Jmix Studio 中添加 `column`，可以在视图 XML 或者 *Jmix UI* 层级面板中选择 `columns` 元素，然后点击组件面板的 xref:studio:view-designer.adoc#add-column[Add->Column] 按钮。
====
...
[TIP]
====
如需在 Jmix Studio 中添加 `action`，可以在视图 XML 或者 *Jmix UI* 面板中选择组件，然后点击组件面板的 xref:studio:view-designer.adoc#add-action[Add->Action] 按钮。
====
...
[TIP]
====
如需通过 Jmix Studio 添加一个渲染器，在视图 XML 或 *Jmix UI* 的层级面板中选中 `column` 元素并点击属性面板中的 xref:studio:view-designer.adoc#component-inspector[Add-><Some>Renderer]。
====
