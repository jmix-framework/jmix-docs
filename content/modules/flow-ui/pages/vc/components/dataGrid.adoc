...
[TIP]
====
To add `column` in Jmix Studio, select the `columns` element in the view descriptor XML or in the *Jmix UI* structure panel and click on the xref:studio:screen-designer.adoc#add-column[Add->Column] button in the *Jmix UI* inspector panel.
====
