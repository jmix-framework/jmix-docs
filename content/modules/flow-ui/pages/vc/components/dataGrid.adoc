...
Отображает в каждой ячейке пользовательский компонент. Используйте этот рендерер, когда ячейка должна содержать насыщенное или интерактивное содержимое, например чекбокс, изображение или кнопку.
...
Следующий пример отображает в колонке `completed` чекбокс только для чтения, в зависимости от того, задано ли значение `completedDate`.
