...
`dataGrid` 组件实现了 `HasActions` 接口，可以包含标准 xref:flow-ui:actions/list-actions.adoc[列表操作] 和自定义操作。操作通过右键菜单或相应的按钮调用。
...
[TIP]
====
如需在 Jmix Studio 中添加 `action`，可以在视图 XML 或者 *Jmix UI* 结构面板中选择组件，然后点击组件面板的 xref:studio:screen-designer.adoc#add-action[Add->Action] 按钮。
====
...
|-
|[[editorBuffered]]editorBuffered
|如果设置为 `true`，则启用 <<inline-editing, 行内编辑>> 的缓存模式，只有当用户点击保存按钮时才会提交改动。该模式下用户也可以取消修改。在非缓存模式时，改动会自动提交。
|`false`
|[[multiSort]]multiSort
|如果设置为 `true`，则启用多列排序。
|`false`
...
在 Jmix 中，所有组件都有一些 xref:vc/common-attributes.adoc[通用事件和处理器]，可以按相同的方法设置。
...
|[[GridDropEvent]]GridDropEvent
|`com.vaadin.flow.component.grid.dnd.GridDropEvent` 当拖放事件发生在数据网格或数据行时发送。对应 `grid-drop` DOM 事件。
|[[ItemClickEvent]]ItemClickEvent
...
|-
|[[property]]property
|指定列对应的实体属性。可以是根实体的属性 `property = "user"`，也可以是子实体的属性 `property = "user.department.name"`（并通过 “.” 变量对象图）。
|-
