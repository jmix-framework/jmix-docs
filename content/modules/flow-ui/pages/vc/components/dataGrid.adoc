...
++++
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/v25/components/grid/flow/#using-renderers-in-columns" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++
...
==== Сортировка вычисляемого столбца

Делегат сортировки также полезен для столбцов, которые не привязаны к атрибуту сущности. В следующем примере столбец `fullName` формируется из `firstName` и `lastName`, а делегат сортировки сопоставляет этот синтетический столбец и с выражениями JPQL, и с компаратором для сортировки в памяти:
