...
* The list should be compact. It can be handy when the list of items is too long for `radioButtonGroup` or `listBox`.
...
Don’t use `entityComboBox` if the number of items is large (thousands or more). `entityComboBox` loads the whole list of items to the user’s browser and into the server memory. For selection from an arbitrary large lists of items, use xref:flow-ui:vc/components/entityPicker.adoc[entityPicker].
...
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/v23/components/combo-box/#custom-value-entry" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++
