...
`entityComboBox` 支持用户从一个下拉列表中选择单一 xref:data-model:entities.adoc[实体] 实例，并支持用户单击右侧按钮执行特定操作。
...
* 字段值是实体实例。
* 用户需要选择单一选项。
* 选项列表应该比较紧凑。如果选项列表对于 `radioButtonGroup` 或 `listBox` 太长时，用下拉列表更方便。
* 用户需要过滤功能。
* 用户需要对关联的实体实例做一些操作。
...
如果选项数量很大（数千个或更多），请不要使用 `entityComboBox`。`entityComboBox` 会将全部选项列表加载到用户的浏览器和服务器内存中。如需从大量选项列表中进行选择，请使用 xref:flow-ui:vc/components/entityPicker.adoc[EntityPicker]。
...
[TIP]
====
如需在 Jmix Studio 中添加 `action`，可以在视图 XML 或者 *Jmix UI* 层级结构面板中选择组件，然后在组件面板点击 xref:studio:screen-designer.adoc#add-picker-action[Add->Action] 按钮。
====
