= tooltip 提示窗

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.io/ui-samples/sample/context-help-tooltip" class="live-demo-btn" target="_blank">在线示例</a>
</div>
++++

提示窗是小的弹窗，用于在鼠标悬停或者键盘聚焦时为 UI 元素提供额外的信息。

XML 元素：`tooltip`

== 基本用法

`tooltip` 是基本上所有组件和布局的内部元素。

TIP: 如需通过 Jmix Studio 添加 XML 中的 `tooltip` 元素，请在界面 XML 中，或者 *Jmix UI* 结构面板中选择需要添加提示的 UI 元素，然后点击 *Jmix UI* 组件面板的 xref:studio:view-designer.adoc#component-inspector-add-button[Add]*->Tooltip* 进行添加。

下面的示例定义了一个带 `tooltip` 的 xref:flow-ui:vc/components/button.adoc[button]：

[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/button/button-view.xml[tags=tooltip-btn]
----

image::visual-components/components/tooltip-basic.gif[align="center",width="293"]

== 属性

<<focusDelay,focusDelay>> -
<<hideDelay,hideDelay>> -
<<hoverDelay,hoverDelay>> -
<<manual,manual>> -
<<opened,opened>> -
<<position,position>> -
<<text,text>>

[[focusDelay]]
=== focusDelay

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/latest/components/tooltip/#configuring-delays" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++

定义键盘聚焦后提示窗延迟出现的毫秒数。

[[hideDelay]]
=== hideDelay

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/latest/components/tooltip/#configuring-delays" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++

定义鼠标离开 UI 元素后提示窗延迟消失的毫秒数。

[[hoverDelay]]
=== hoverDelay

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/latest/components/tooltip/#configuring-delays" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++

定义鼠标悬停后提示窗延迟出现的毫秒数。

[[manual]]
=== manual

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/latest/components/tooltip/#triggering-manually" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++

定义 `tooltip` 的 `manual` 属性。

[[opened]]
=== opened

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/latest/components/tooltip/#triggering-manually" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++

定义 `tooltip` 的 `opened` 属性。

[[position]]
=== position

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/latest/components/tooltip/#positioning" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++

定义 `tooltip` 的 `position` 属性。

[[text]]
=== text

++++
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/latest/components/tooltip" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++

定义 `tooltip` 的纯文本内容。

== 参考

{vaadin-docs}/components/tooltip[Vaadin 文档^]
