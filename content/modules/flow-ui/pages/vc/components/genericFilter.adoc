...
[NOTE]
====
Загрузчик данных может иметь собственное _базовое условие_, установленное в элементе `condition` загрузчика или программно через `DataLoader.setCondition()`. `genericFilter` не заменяет его: условие активной конфигурации комбинируется с базовым условием как `база И конфигурация`, включая случаи, когда вы переключаетесь между конфигурациями.
====
...
CAUTION: В настоящее время fluent builder API является экспериментальным и может измениться в будущем релизе Jmix.
...
TIP: Готовые к запуску примеры всех возможностей билдера см. в разделе *GenericFilter* в https://demo.jmix.io/ui-samples[UI Samples^].
