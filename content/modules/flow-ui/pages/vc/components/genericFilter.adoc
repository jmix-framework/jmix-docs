...
The `genericFilter` component is added by default on some standard views, such as a xref:flow-ui:views/view-classes.adoc#standard-list-view[list view] of an entity. A typical filter is shown below:
...
.generic-filter-add-condition.xml
[source,xml,indent=0]
----
...
.GenericFilterAddConditionSample.java
...
+
* *Configurations* - существующие конфигурации фильтра. Они могут быть созданы либо разработчиком во время <<design-time-configuration,разработки>>, либо пользователем во время <<runtime-configuration,выполнения>>.
...
.generic-filter-property-condition.xml
[source,xml,indent=0]
----
...
.GenericFilterPropertyConditionSample.java
[source,java,indent=0]
----
...
.generic-filter-jpql-condition.xml
[source,xml,indent=0]
----
...
.GenericFilterJpqlConditionSample.java
[source,java,indent=0]
----
...
.generic-filter-group-condition.xml
[source,xml,indent=0]
----
...
.GenericFilterGroupConditionSample.java
[source,java,indent=0]
----
...
Конфигурации могут быть созданы либо разработчиком во время <<design-time-configuration,разработки>>, либо пользователем во время <<runtime-configuration,выполнения>>.
...
Создавать и настраивать `genericFilter` в Java можно с помощью fluent API построителей с двумя точками входа:

* `filterComponentBuilder()` создаёт компоненты фильтра – `propertyFilter`, `jpqlFilter` и `groupFilter`.
* `runtimeConfigurationBuilder()` собирает <<runtime-configuration, конфигурацию времени выполнения>> из этих компонентов и регистрирует её в фильтре.
...
CAUTION: В настоящее время fluent API построителей является экспериментальным и может измениться в будущих версиях Jmix.
...
`runtimeConfigurationBuilder()` создаёт <<runtime-configuration, конфигурацию времени выполнения>> и регистрирует её в фильтре.
...
TIP: Готовые примеры всех возможностей построителя см. в разделе *GenericFilter* в https://demo.jmix.ru/ui-samples[UI Samples^].
...
* *Edit* - открывает редактор для текущей <<runtime-configuration,конфигурации времени выполнения>>. Отключено для <<design-time-configuration,конфигураций времени разработки>>. Реализовано с помощью `FilterEditAction` (`type="genericFilter_edit"` в XML).
* *Remove* - удаляет текущую конфигурацию <<runtime-configuration,времени выполнения>>. Отключено для конфигураций <<design-time-configuration,времени разработки>>. Реализовано с помощью `FilterRemoveAction` (`type="genericFilter_remove"` в XML).
* *Copy* - создает копию <<runtime-configuration,времени выполнения>> для текущей конфигурации. Реализовано с помощью `FilterCopyAction` (`type="genericFilter_copy"` в XML).
