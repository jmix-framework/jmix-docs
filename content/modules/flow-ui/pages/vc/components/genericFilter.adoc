...
[NOTE]
====
A configuration registered from code is created anew on every opening of the view: if a user removes it, it reappears the next time the view is opened. Protection from user deletion (`allowDeletion()`) covers only the *Remove* action — the user can still change the configuration via *Edit* and save their own version of it, or clone it via *Save as*.
====

[NOTE]
====
The id of a configuration saved by the user is generated from its name and is used as a reference to the configuration — for example, when a user makes it their default. Renaming an existing configuration changes its id, so such references stop working.
====
