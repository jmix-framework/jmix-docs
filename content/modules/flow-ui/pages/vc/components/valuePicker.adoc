...
By default, a user cannot input the value manually. If you set the `allowCustomValue` attribute to `true`, you enable manual input. Keep in mind the entered value is not set to the data model. To handle user input, use <<CustomValueSetEvent,CustomValueSetEvent>>.
