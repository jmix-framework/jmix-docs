...
Когда Studio генерирует `entityPicker` в экране деталей, он также генерирует два предопределённых стандартных действия: `entity_lookup` и `entity_clear`. Существуют также действия `entity_open` и `entity_openComposition`.
