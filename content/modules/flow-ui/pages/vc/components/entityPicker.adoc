...
In the following example, the view defines the `userDc` data container for the `User` entity having the `department` attribute. In the `entityPicker` element, the `dataContainer` attribute contains a link to the `userDc` data container, and the `property` attribute refers to the `department` entity attribute. The entity attribute should be a reference to an entity. In the following example, it is `Department`.
