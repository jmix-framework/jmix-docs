...
在下面的例子中，视图为具有 `department` 属性的 `User` 实体定义了 id 为 `userDc` 的数据容器。在 `entityPicker` 元素中，通过 `dataContainer` 属性连接到 `userDc` 数据容器，并给 `property` 属性设置了 `department` 实体属性。实体属性是另一实体的引用，在上面的示例中就是 `Department` 实体。

[source,xml,indent=0]
----
include::example$/onboarding/src/main/resources/com/company/onboarding/view/component/entitypicker/entity-picker-view.xml[tags=data;layout;basics]
----
...
[TIP]
====
如需在 Jmix Studio 中添加 `action`，可以在界面 XML 或者 *Jmix UI* 层级面板中选择组件，然后点击组件面板的 xref:studio:screen-designer.adoc#add-picker-action[Add] 按钮。
====
