...
As you can see, `itemsQuery` in `comboBox` does not need `class` and `fetchPlan` attributes because the query is supposed to return the list of scalar values (notice `e.name` in the result set). To work with entities, use the xref:vc/components/entityComboBox.adoc[entityComboBox] component.
...
The `classNames` attribute allows you to add custom CSS classes to the component. You can use these classes to style the overlay parts from the component selector.
...
|This handler only fetches data when it's needed. See <<itemsFetchCallback-handler,Programmatic Items Fetching>>.
...
A `comboBox` can include <<itemsQuery,itemsQuery>>, xref:vc/miscellaneous/prefix-suffix.adoc[prefix], xref:vc/components/tooltip.adoc[tooltip], and <<validation,validator>> as its nested elements.

== See Also

* https://vaadin.com/docs/v25/components/combo-box[Vaadin Combo Box documentation^]
