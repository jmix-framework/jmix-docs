...
当输入自定义字符串时，配置是否启用组件事件。

当启用时，用户输入字符串确认后（光标移出或按下回车键），如果没有匹配上任何选项，则触发 <<CustomValueSetEvent,CustomValueSetEvent>> 事件。
...
++++
<div class="jmix-ui-live-demo-container">
    <a href="https://vaadin.com/docs/v23/components/combo-box/#auto-open" class="vaadin-docs-btn" target="_blank">Vaadin</a>
</div>
++++
...
当用户输入非空值，且不匹配任何已有选项时，发出 `com.vaadin.flow.component.combobox.ComboBoxBase.CustomValueSetEvent` 事件。如需启用用户输入，请设置 <<allowCustomValue,allowCustomValue>> 属性。
...
https://vaadin.com/docs/v23/components/combo-box[Vaadin 文档^]。
