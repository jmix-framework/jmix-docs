...
Атрибут `classNames` позволяет добавлять пользовательские CSS-классы к компоненту. Вы можете использовать эти классы для стилизации частей оверлея из селектора компонента.
...
[source,css,indent=0]
----
include::example$/onboarding/src/main/resources/META-INF/resources/themes/onboarding-aura/onboarding.css[tags=overlay]
----
