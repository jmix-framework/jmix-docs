...
`multiSelectComboBox` 专为存储多个值的属性而设计。当该组件绑定实体属性时，属性类型应为集合，例如 `List` 或 `Set`。
...
此时，`multiSelectComboBox` 支持用户选择多个 `Hobby` 实例，并且该对应关系存储在连接表中。

如需创建数据感知的 `multiSelectComboBox`，请使用 xref:vc/common-attributes.adoc#dataContainer[dataContainer] 和 xref:vc/common-attributes.adoc#property[property] 属性将组件绑定到集合属性。使用 xref:vc/components/entityComboBox.adoc#itemsContainer[itemsContainer] 属性提供可用项列表。以下示例生成一个数据感知的 `multiSelectComboBox`。
...
<4> `multiSelectComboBox` 使用 `userHobbiesDc` 作为选项容器，下拉列表中显示用户的爱好。

[NOTE]
====
`itemsContainer` 定义下拉框中可用的选项。用户选择的值保存在 `dataContainer` 和 `property` 指定的集合属性中。
====

组件的值包含所选的条目。当组件绑定集合属性时，Jmix 会将选定的条目转换为属性集合类型，例如 `List` 或 `Set`。
...
https://vaadin.com/docs/v25/components/multi-select-combo-box[Vaadin 文档^]
