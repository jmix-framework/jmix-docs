...
Наиболее распространенный сценарий использования — выбор связанных сущностей для ассоциации many-to-many:
...
<2> Встроенный фетч-план экземпляра сущности, находящегося в контейнере.
<3> `CollectionContainer` для сущности `Hobby`.
<4> `multiSelectComboBox` получает `userHobbiesDc` в качестве контейнера элементов, чтобы показывать раскрывающийся список увлечений.
