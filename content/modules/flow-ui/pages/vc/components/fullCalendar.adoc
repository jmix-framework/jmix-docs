...
====
Для работы компонента требуется дополнение xref:calendar:index.adoc#installation[Calendar].
====
...
`calendar` отображает диапазоны дат и события календаря в режимах дня, недели, месяца и других.
...
Объявите пространство имен `calendar` в XML-дескрипторе экрана:
...
Затем добавьте элемент `calendar`:
...
Чтобы отображать события, свяжите `containerEventProvider` с xref:flow-ui:data/data-containers.adoc[контейнером данных].

В следующем примере создается базовый компонент `calendar`:
...
* Пользователь делает новое выделение: событие срабатывает до того, как произойдет новое выделение.
...
Полный список генераторов имен классов приведен в разделе <<handlers,Обработчики>>.
...
Например, формат `HH:mm` выводит `00:00`: часы — от `00` до `23`, минуты — от `00` до `59`.
...
Например, формат `ha` выводит час от `1` до `12` с суффиксом `am` или `pm`, например `1 am`.
...
Например, формат `[Week] w` выводит номер недели от `1` до `53`, например `Week 1`.
...
|Определяет задержку (в миллисекундах) перед срабатыванием события одиночного клика. Эта задержка используется для различия одиночного клика от действия двойного клика.
Пожалуйста, обратите внимание, что этот атрибут поддерживается в Jmix 2.7.2 и выше.
|`250`
...
|Отправляется, когда пользователь нажимает навигационную ссылку в заголовке дня. Ссылку можно активировать свойством <<navigationLinksEnabled,navigationLinksEnabled>>.
...
Событие клика в календаре содержит `MouseEventDetails`, предоставляющее информацию о взаимодействии с мышью:

* `clickCount`: количество последовательных кликов (1 для одиночного клика, 2 для двойного клика). Пожалуйста, обратите внимание, что этот атрибут поддерживается в Jmix 2.7.2 и выше.

* `button`: какая кнопка мыши была нажата (`LEFT`, `MIDDLE`, `RIGHT`).

* `pageX/pageY`: координаты клика относительно документа.

* Модификаторы: состояние клавиш `Alt`, `Ctrl`, `Meta` (`Cmd` на macOS) и `Shift` во время клика.

|[[EventDoubleClickEvent]]EventDoubleClickEvent
|`io.jmix.fullcalendarflowui.component.event.EventDoubleClickEvent` возникает, когда пользователь дважды кликает на событие в календаре.

Пожалуйста, обратите внимание, что эта функциональность поддерживается в Jmix 2.7.2 и выше.

`EventDoubleClickEvent` содержит <<EventClickEvent,MouseEventDetails>>, предоставляющее информацию о взаимодействии с мышью.
...
|`io.jmix.fullcalendarflowui.component.event.EventSingleClickEvent` возникает, когда пользователь кликает на событие в календаре. Чтобы отличить его от двойного клика, событие отправляется после короткой задержки. Вы можете настроить эту задержку с помощью атрибута <<eventSingleClickThreshold,eventSingleClickThreshold>>.

Пожалуйста, обратите внимание, что эта функциональность поддерживается в Jmix 2.7.2 и выше.

`EventSingleClickEvent` содержит <<EventClickEvent,MouseEventDetails>>, предоставляющее информацию о взаимодействии с мышью.
...
* Пользователь делает новое выделение. Событие отмены выделения будет вызвано до того, как произойдет новое выделение.
