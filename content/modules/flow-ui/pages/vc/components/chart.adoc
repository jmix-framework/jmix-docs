...
The `chart` component is rendered by Apache ECharts and needs a calculable non-zero size when it is rendered or re-rendered. This is important when charts are placed in responsive layouts, hidden containers, or containers whose size changes dynamically, for example when the application menu is collapsed or expanded.

If several charts are placed next to each other, avoid relying only on `width="100%"` for their parent containers. A chart can expand when the available area grows, but it may not shrink correctly when the surrounding container becomes narrower. In such cases, use a flex layout and apply `flex-grow` to the chart containers:
...
Also make sure that the chart or its parent container has an explicit `height` or `minHeight`. If a chart is placed into a container with `height="100%"`, but none of the parent containers has a calculable height, the chart may be rendered with zero height.

This is especially important for charts or containers that are initially hidden with `visible="false"` and later shown programmatically.
