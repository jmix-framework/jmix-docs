...
Use the `dataContainer` and `property` attributes to bind `select` to an entity attribute. The data container provides the entity instance, and `property` identifies the attribute to edit.
...
<2> The lambda receives the current `Department` item.
<3> The component is created and configured inside the same callback.
<4> The lambda returns the component for the current item.
...
|When `false`, the overlay is shown on top of the input field. When `true`, the overlay does not cover the input field. See this in https://vaadin.com/docs/v25/components/select#overlay-position[Vaadin documentation^].
|`false`

|[[overlayWidth]]`overlayWidth`
|Specifies the overlay’s width. If omitted (default), the overlay automatically adjusts its width to match the widest option or at least the input field’s width. See this in https://vaadin.com/docs/v25/components/select#overlay-width[Vaadin documentation^].
|—
|===

The following shared attributes are supported by `select`:
...
The following shared handlers are supported by `select`:
...
A `select` can include <<fragment-renderer,fragmentRenderer>>, xref:vc/miscellaneous/prefix-suffix.adoc[prefix], and xref:vc/components/tooltip.adoc[tooltip] as its nested elements.

== See Also

* https://vaadin.com/docs/v25/components/select[Vaadin Select documentation^]
