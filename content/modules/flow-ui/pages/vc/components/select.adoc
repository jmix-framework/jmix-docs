...
|
|[[noVerticalOverlap]]noVerticalOverlap
|Когда `false`, оверлей отображается поверх поля ввода. Когда `true`, оверлей не покрывает поле ввода. См. {vaadin-docs}/components/select#overlay-position[документацию Vaadin^].
|`false`
|[[overlayClass]]overlayClass
|Определяет список имён CSS-классов, разделённых пробелами, для установки на элементе оверлея. Смотрите <<overlay, Оверлей>>.
|
|[[overlayWidth]]overlayWidth
|Задаёт ширину оверлея. Если параметр не указан (значение по умолчанию), ширина оверлея автоматически подбирается так, чтобы соответствовать самой широкой опции или, как минимум, ширине поля ввода. См. {vaadin-docs}/components/select#overlay-width[документацию Vaadin^].
|
|===
...
Дополнительную информацию смотрите в {vaadin-docs}/components/select[документации Vaadin^].
