...
|
|[[noVerticalOverlap]]noVerticalOverlap
|当为 `false` 时，覆盖层显示在输入框上方。当为 `true` 时，覆盖层不会覆盖输入框。参阅 https://vaadin.com/docs/latest/components/select#overlay-position[Vaadin 文档^]。
|`false`
|[[overlayClass]]overlayClass
|定义需要在 overlay 元素上设置的 CSS 类列表，以空格分隔。参阅 <<overlay, Overlay>>。
|
|[[overlayWidth]]overlayWidth
|指定覆盖层的宽度。如果省略（默认值），覆盖层会自动调整其宽度以匹配最宽的选项，或者至少与输入框的宽度相同。参阅 https://vaadin.com/docs/latest/components/select#overlay-width[Vaadin 文档^]。
|
|===
