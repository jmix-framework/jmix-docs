...
与 `multiSelectComboBox` 类似，`multiSelectComboBoxPicker` 专为存储多个值的属性而设计，例如 `List` 或 `Set`。有关常见数据结构和用例，请参见 xref:vc/components/multiSelectComboBox.adoc#data-binding[数据绑定]。

要创建连接数据的 `multiSelectComboBoxPicker`，请使用 xref:vc/common-attributes.adoc#dataContainer[dataContainer] 和 xref:vc/common-attributes.adoc#property[property] 属性。xref:vc/components/entityComboBox.adoc#itemsContainer[itemsContainer] 属性用于提供可用项列表。以下示例生成一个数据感知的 `multiSelectComboBoxPicker`。
