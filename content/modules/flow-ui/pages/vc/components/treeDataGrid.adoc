...
|Задаёт сочетание клавиш для применения фильтров по заголовкам колонок. Например: `CONTROL-ENTER`. Переопределяет xref:ui-properties.adoc#jmix.ui.component.data-grid-header-filter-apply-shortcut[глобальную настройку]. См. xref:vc/components/dataGrid.adoc#column-filtering[Фильтрация по заголовкам колонок].
|—
...
|Если `true`, колонка фиксируется в конце таблицы и остаётся видимой при горизонтальной прокрутке. Рекомендуется фиксировать колонки справа налево.
|`false`
|[[header]]header
