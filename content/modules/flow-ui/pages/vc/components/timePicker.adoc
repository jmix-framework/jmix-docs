...
The default interval between the items displayed in the overlay is set to one hour. A custom step value can be set using the xref:vc/common-attributes.adoc#step[step] attribute. This can also be done in the screen controller:
...
To restrict the input to a specific time range, specify a minimum and maximum value using the <<max, max>> and <<min,min>> attributes. This can also be done in the screen controller:
