...
====
This component requires the commercial xref:groupdatagrid:index.adoc#installation[Grouping Data Grid add-on].
====
...
By default, group items display the number of descendants. Use the `displayItemsCount` property of <<groupColumn,groupColumn>> to hide this number.
...
|Builds a `DataGridSort` object from `GroupDataGridSortContext`, allowing component-specific mapping of columns to custom comparators and database sort expressions. See xref:flow-ui:vc/components/dataGrid.adoc#sort-builder-delegate[Sort Builder Delegate].
...
A `groupDataGrid` can include `actions`, <<groupBy,groupBy>>, <<columns,columns>>, `contextMenu`, and `emptyStateComponent` as its direct nested elements. The `columns` element can contain <<column,column>>, <<groupColumn,groupColumn>>, and xref:flow-ui:vc/components/dataGrid.adoc#actions-column[editorActionsColumn]. The shared grid elements work as described for xref:flow-ui:vc/components/dataGrid.adoc#actions[actions], xref:flow-ui:vc/components/dataGrid.adoc#context-menu-element[contextMenu], and xref:flow-ui:vc/components/dataGrid.adoc#empty-state-component-element[emptyStateComponent].
...
The `columns` element contains individual `column` elements and defines a set of attributes shared by those columns. It supports `includeAll`, `exclude`, `sortable`, `resizable`, and `filterable` as described for xref:flow-ui:vc/components/dataGrid.adoc#columns[dataGrid columns].

[[column]]
=== column

The `column` element declares an individual column. Attributes set for an individual `column` override those set for `columns`. It supports the xref:flow-ui:vc/components/dataGrid.adoc#column[dataGrid column] configuration and adds the `groupAllowed` attribute, which controls whether users can group by that column. See <<disable-grouping,Disable Grouping>>.
...
|Sets whether a group header displays the number of descendant items. See <<hide-group-item-count,Hide Group Item Count>>.
|—
