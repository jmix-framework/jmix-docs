...
TIP: 属性值可以是文本或者 xref:localization:message-bundles.adoc#_message_bundles[消息包] 中的键值。
...
在登录表单中，可以使用 <<LocaleChangedEvent,LocaleChangedEvent>> 处理器修改语言而无需重新加载页面。读取 xref:localization:message-bundles.adoc#_message_bundles[消息包] 中本地化的信息并设置：
...
https://vaadin.com/docs/v23/components/login[Vaadin 文档^]。
