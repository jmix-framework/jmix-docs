...
可以通过属性配置是否显示可选的一些元素，例如语言选择框、*记住我* 复选框或 *忘记密码* 按钮：
...
*Remember me（记住我）* 复选框支持用户长时间保持登录状态，甚至比 HTTP 会话的生命周期还长。即使服务器重启，此状态仍然保持。

当用户在登录时选中 *Remember me* 复选框，系统会在浏览器 cookie 中存储一个持久化 token。在后续访问中，用户无需再次输入凭证即可自动进行身份验证。

如需在登录逻辑中处理 `Remember me` 状态，使用 <<RememberMeChangedEvent,RememberMeChangedEvent>>。

更多信息请参考 https://docs.spring.io/spring-security/reference/servlet/authentication/rememberme.html[Spring Security Remember-Me 文档^]。
...
设置是否显示 *Remember me（记住我）* 复选框。设置为 `true` 时，用户可以在浏览器会话级别保持登录状态。
...
https://vaadin.com/docs/v25/components/login[Vaadin 文档^]
