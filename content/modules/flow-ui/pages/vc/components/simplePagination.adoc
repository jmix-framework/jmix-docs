...
Jmix Studio 在为实体创建 xref:flow-ui:views.adoc#standard-list-view[StandardListView] 时，默认生成 `simplePagination`。
...
该列表的默认值通过 xref:flow-ui:flowui-properties.adoc#jmix.flowui.component.pagination-items-per-page-items[jmix.flowui.component.pagination-items-per-page-items] 属性配置。
...
[NOTE]
====
所有实体的最大加载数是通过 xref:flow-ui:flowui-properties.adoc#jmix.flowui.default-max-fetch-size[jmix.flowui.default-max-fetch-size] UI 属性定义。默认值为 10000。某些特定实体可能有不同的加载要求，可以用 xref:flow-ui:flowui-properties.adoc#jmix.flowui.entity-max-fetch-size[jmix.flowui.entity-max-fetch-size] 设置。
====
