...
`propertyFilter` добавляет условие фильтрации на основе конкретного атрибута сущности.
...
<2> Создаёт экземпляр `PropertyFilter` и устанавливает его свойства.
...
`jpqlFilter` позволяет создать условие, которое только позволяет пользователю выбирать, применять его или нет. С точки зрения пользователя, он не принимает параметров, так как задаётся непосредственно в запросе.
...
<1> Добавляет конфигурацию времени разработки. Метод принимает два параметра: `id` конфигурации и название конфигурации.
<2> Создаёт компонент `GroupFilter` и устанавливает его свойства.
...
Все зарегистрированные компоненты фильтра становятся доступны в меню xref:vc/components/genericFilter.adoc#creating-conditions[*Создать*] диалогового окна *Добавить условие*.
...
`genericFilter-components` имеет следующие общие атрибуты:
...
`genericFilter-components` имеет следующие общие обработчики:
