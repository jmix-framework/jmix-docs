...
|The built-in `propertyFilter`, `jpqlFilter`, and `groupFilter` components do not define built-in theme variants. Use `themeNames` for custom styles in your application theme.
|Custom themes
|===

== Attributes

The following shared attributes are supported by `genericFilter-components`:
...
The following shared handlers are supported by `genericFilter-components`:
