...
[NOTE]
====
当 `parameterClass` 是实体类型时，`jpqlFilter` 将所选实体的 *标识符* 作为查询参数传入，而非实体实例本身。与 `where` 子句中的关联实体 ID 进行比较，例如 `\{E}.customer.id = ?` 。直接比较关联实体引用（`\{E}.customer = ?`）不会返回任何结果。
====
...
|内置的 `propertyFilter`、`jpqlFilter` 和 `groupFilter` 组件并未定义任何样式版本。请在应用程序主题中使用 `themeNames` 配置自定义样式。
|自定义主题
|===
