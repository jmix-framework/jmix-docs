...
The wizard lists class names declared in these Java classes. It does not check whether the corresponding CSS is loaded. Use `StyleUtility` instead of `JmixLumoUtility` in new code. `LumoUtility` class names work only with the Lumo theme and the xref:flow-ui:themes/lumo.adoc#utility-classes[Lumo utility stylesheet].
