...
* `io.jmix.flowui.theme.StyleUtility` 类及其内部类。
* `com.vaadin.flow.theme.lumo.LumoUtility` 类及其内部类。
* 废弃的 `io.jmix.flowui.theme.lumo.JmixLumoUtility` 类。仅以兼容性为目的而保留。
* 任何带 `@ThemeUtilityClasses` 注解的自定义类。可以在自己的扩展组件或应用程序源代码中提供共享类名。

向导会列出这些 Java 类中的类名。不会检查对应的 CSS 是否加载。在新代码中，使用 `StyleUtility` 替换 `JmixLumoUtility`。`LumoUtility` 类中的类名仅支持 Lumo 主题和 xref:flow-ui:themes/lumo.adoc#utility-classes[Lumo 工具样式]。
...
为组件设置值更改模式。该模式定义了值更改的发送频率：

* `EAGER` – 每次用户修改时（例如每个键盘输入）。
* `LAZY` – 用户停止输入超过超时时间后。
* `TIMEOUT` – 在输入过程中，每个超时间隔就发送一次。
* `ON_BLUR` – 当组件失去焦点时。
* `ON_CHANGE` – 当值通过更改操作提交时。

对于文本输入，默认情况下通常不会在每个键盘输入时发送更新。
使用 `EAGER` 支持即时验证、实时搜索等类似的响应式行为。
使用 <<valueChangeTimeout>> 配置 `LAZY` 和 `TIMEOUT`。
...
设置 <<valueChangeMode>> 的超时时限。单位为毫秒。

该属性仅影响延迟模式（`LAZY`、`TIMEOUT`），在即时模式或基于操作的提交模式（`EAGER`、`ON_BLUR`、`ON_CHANGE`）中，会忽略该设置。
