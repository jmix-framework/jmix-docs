= Validator
:page-aliases: vcl/miscellaneous/validator.adoc
...
Собственный валидатор можно определить в бине Spring, реализующем интерфейс `Validator`. Например:
...
В XML-дескрипторе экрана пользовательский валидатор используется во вложенном элементе `custom`. Атрибут `bean` должен указывать имя бина валидатора. Например:
...
Создать валидатор можно, унаследовав его от базового класса `AbstractValidator`. Тогда в XML можно указать шаблон сообщения и использовать метод `getTemplateErrorMessage()`, чтобы сгенерировать сообщение об ошибке. В этом случае бин будет иметь состояние (сообщение из XML), поэтому он должен быть прототипом. Например:
