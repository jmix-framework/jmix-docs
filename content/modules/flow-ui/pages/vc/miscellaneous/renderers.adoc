...
Эта страница описывает общие механизмы рендереров для нескольких компонентов Flow UI, таких как `ComponentRenderer`, `LitRenderer` и `FragmentRenderer`.

Используйте эту страницу, когда нужно выбрать или реализовать общий API рендерера. Предопределенные XML-рендереры, поддерживаемые атрибуты и ограничения конкретных компонентов описаны в соответствующей документации компонентов. Например, xref:vc/components/dataGrid.adoc#renderers[Рендереры dataGrid] описывает все рендереры, доступные для столбцов `dataGrid`.
...
Примеры использования `ComponentRenderer` в конкретных UI-компонентах приведены в соответствующих разделах документации:
