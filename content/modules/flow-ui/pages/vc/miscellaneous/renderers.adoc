...
This page describes renderer mechanisms shared by multiple Flow UI components, such as `ComponentRenderer`, `LitRenderer`, and `FragmentRenderer`.

Use this page when you need to choose or implement a shared renderer API. For predefined XML renderers, supported attributes, and component-specific constraints, see the corresponding component documentation. For example, xref:vc/components/dataGrid.adoc#renderers[dataGrid Renderers] describes all renderer options available for `dataGrid` columns.
...
Examples of using `ComponentRenderer` in specific UI components are provided in the relevant sections of the documentation:
