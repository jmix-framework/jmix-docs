...
NOTE: Форматтер можно использовать только с нередактируемыми компонентами, такими как xref:vc/components/valuePicker.adoc[valuePicker], xref:vc/components/multiValuePicker.adoc[multiValuePicker] и xref:vc/components/entityPicker.adoc[entityPicker]. Значения непосредственно редактируемых компонентов, например xref:vc/components/textField.adoc[textField], форматируются механизмом xref:data-model:datatype.adoc[Datatype].
