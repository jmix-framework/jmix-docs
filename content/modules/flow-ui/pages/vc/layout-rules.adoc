...
For simple styling, you can use https://vaadin.com/docs/latest/styling/lumo/utility-classes[Lumo Utility Classes] provided by Vaadin. *Lumo Utility Classes* are predefined CSS class names and stylesheets. They can be used to style HTML elements and layouts without having to write CSS.
...
The https://javadoc.io/doc/com.vaadin/vaadin-platform-javadoc/latest/com/vaadin/flow/theme/lumo/LumoUtility.html[LumoUtility] Java class in contains `String` constants for all utility classes. They're divided into nested category classes, such as `LumoUtility.Margin`.

An example of using Lumo utility classes to add a background color to a container:
...
----
<1> `ms-auto` effectively means `margin-inline-start: auto`, i.e. defines the logical inline start margin of an element, which maps to a physical margin depending on the https://developer.mozilla.org/en-US/docs/Web/CSS/flex-direction[flex-direction]; `me-s` effectively means `margin-inline-end: var(--lumo-space-s)`, i.e. defines the logical inline end margin of an element, which maps to a physical margin depending on the https://developer.mozilla.org/en-US/docs/Web/CSS/flex-direction[flex-direction]
