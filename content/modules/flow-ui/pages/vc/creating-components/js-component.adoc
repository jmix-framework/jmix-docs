...
<4> 为 getter 添加 `@Synchronize` 注解，告诉 Flow 需要在触发 `custom-slide-changed` 事件时同步该属性。在本示例中，当滑块的值变化时就会触发该事件。
...
TIP: 关于创建自定义组件的更多内容可以参阅 Vaadin 文档： https://vaadin.com/docs/v25/flow/advanced/loading-resources#using-annotations[导入 JavaScript & CSS^], https://vaadin.com/docs/v25/flow/create-ui/web-components[集成 Web 组件^], https://vaadin.com/docs/v25/flow/create-ui/element-api[Element API^]
...
<2> 添加 `slider` 元素时，需使用 `app` 命名空间前缀。
