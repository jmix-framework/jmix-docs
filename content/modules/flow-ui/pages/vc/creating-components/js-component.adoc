...
<4> Adding the `@Synchronize` annotation to the getter tells Flow to synchronize the property from the client when the `custom-slide-changed` event is fired. In this example, the event is dispatched from the slider when its value changes.
...
TIP: More information about creating custom components can be found in Vaadin documentation: https://vaadin.com/docs/v25/flow/advanced/loading-resources#using-annotations[Importing JavaScript & CSS^], https://vaadin.com/docs/v25/flow/create-ui/web-components[Integrating Web Components^], https://vaadin.com/docs/v25/flow/create-ui/element-api[Element API^].

[[usage-example]]
== Usage Example

After a component is implemented it can be used in views either in XML descriptors or programmatically.
...
<2> Adds the `slider` element with the `app` namespace prefix.
