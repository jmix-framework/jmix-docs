...
----
<1> Определяет корневой *root* элемент, который автоматически создаётся классом `Component` и доступен через метод `getElement()`. Должен совпадать с экспортируемым Web Component.
<2> Аннотация `@NpmPackage` определяет зависимость от npm-пакета, который включает один или несколько JS-модулей.
<3> Аннотация `@JsModule` определяет импорт JavaScript-модуля.
<4> Добавление аннотации `@Synchronize` к геттеру указывает Flow синхронизировать свойство с клиента при возникновении события `custom-slide-changed`. В этом примере событие отправляется слайдером при изменении его значения.
...
TIP: Более подробную информацию о создании пользовательских компонентов можно найти в документации Vaadin: https://vaadin.com/docs/v25/flow/advanced/loading-resources#using-annotations[Импорт JavaScript и CSS^], https://vaadin.com/docs/v25/flow/create-ui/web-components[Интеграция веб-компонентов^], https://vaadin.com/docs/v25/flow/create-ui/element-api[API элементов^].
...
----
<1> Определяет пространство имён с тем же значением, что и в атрибутах `xmlns` и `targetNamespace` XSD компонента.
<2> Добавляет элемент `slider` с префиксом пространства имён `app`.
