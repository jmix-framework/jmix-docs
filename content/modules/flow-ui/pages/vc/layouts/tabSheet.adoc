...
The component's appearance is customizable with the xref:vc/common-attributes.adoc#themeNames[themeNames] attribute, which allows you to select among ten different options:
...
|`filled`
|Applies a filled background style to the tab strip.
|Aura
...
|`show-scroll-buttons`
|Shows the scroll buttons used to navigate overflown tabs.
|Aura, Lumo
|===
