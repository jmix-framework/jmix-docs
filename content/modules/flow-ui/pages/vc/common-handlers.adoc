...
TIP: Для типизированных компонентов используйте <<TypedValueChangeEvent, TypedValueChangeEvent>>, чтобы убедиться, что входное значение имеет необходимый тип, и избежать дополнительных преобразований типов и проверок.
