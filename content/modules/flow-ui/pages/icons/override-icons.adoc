...
通过重新定义 CSS 变量可以把一个图标重新映射为另一个。例如，在 xref:flow-ui:themes/themes.adoc#application-theme-files[应用程序主题] 的自定义样式文件中添加下面的内容：
...
系统还支持重映射不同图标字体的图标。在 xref:flow-ui:themes/themes.adoc#application-theme-files[应用程序主题] 的自定义样式文件中添加下面的内容：
...
例如，假设按照 xref:icons/custom-icons.adoc#icon-fonts[说明] 添加了 *Material* 图标字体，则可以在的 xref:flow-ui:themes/themes.adoc#application-theme-files[应用程序主题] 自定义样式文件中覆盖图标：
