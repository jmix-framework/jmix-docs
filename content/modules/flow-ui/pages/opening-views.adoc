...
主菜单通过导航的方式打开视图。标准操作 `create` 和 `edit` 也是通过导航方式打开详情视图，但是可以配置使用对话框方式。`entity_lookup` 标准操作始终使用对话框的方式打开列表视图，以便能返回选中的实体。
...
如需通过 URL 打开视图，可以注入 `ViewNavigators` bean，然后使用流式接口指定视图类：
...
TIP: 以导航方式打开视图时，使用流式接口的 `withRouteParameters()` 和 `withQueryParameters()` 方法为打开的视图传递参数。 
...
如需在详情视图中新建一个实体，先指定视图类，然后调用 `newEntity()`。使用 `AfterCloseEvent` 监听器获取创建的实体。实例：
