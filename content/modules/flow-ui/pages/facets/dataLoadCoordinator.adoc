...
In this mode, `dataLoadCoordinator` relies on parameter names with particular prefixes. The prefix denotes a component that produces the parameter value and changes events. If the loader has no parameters in its query text, although it can have parameters in query conditions, it is refreshed on `BeforeShowEvent`.
...
[[containerPrefix]]
* `containerPrefix` - defines a prefix for parameters which refer to xref:data-components.adoc[data containers] from which `DataLoadCoordinator` will take parameter values in automatic mode. The default value is `container_`.
