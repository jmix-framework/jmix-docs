...
You can use the Spring's `ApplcationEvent` to notify about events that have occurred and to respond to them in different parts of the application.

To deliver application events to UI components including opened xref:views/views.adoc[views] you need to use the `UiEventPublisher` bean provided by the framework.
