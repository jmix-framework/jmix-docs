...
Because Jmix is based on Spring, you can use the `ApplcationEvent` in order to notify about events that have occurred and to respond to them in different parts of the application. To deliver application events to UI components including opened xref:views.adoc[views] you need to use the `UiEventPublisher` bean provided by the framework.

To correctly update the UI, the class that implements `AppShellConfigurator` must contain the `@Push` annotation. This can be the main Spring Boot application class:
...
The `UiEventPublisher` bean provides the following methods:
...
Consider the following development task: users need to complete the onboarding process by following the steps. Each step is represented by an entity instance and there is a view that shows information about completed steps. Also, users need a reminder about the number of uncompleted steps, which updates its value every time the steps are updated. For example, a badge in a menu item, so as not to go to a statistics view every time.

Users can update instances of entities in the xref:views.adoc#standard-detail-view[detail view] or programmatically within the application. This means that everything happens in different places, so a global event is needed.
...
<3> Get incomplete steps number from a service.
<4> Set incomplete steps number value as a badge to the menu item.
...
.Menu item with a badge that displays the number of incomplete steps
