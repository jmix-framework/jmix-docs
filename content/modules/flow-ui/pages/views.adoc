...
* `@PrimaryDetailView` указывает, что этот экран является экраном деталей по умолчанию для указанной сущности. Аннотация имеет больший приоритет, чем xref:opening-views.adoc#view-inference-conventions[соглашение] `\{entity_name}.edit`.
