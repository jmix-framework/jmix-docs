...
`StandardMainView` 是在用户登录之后打开的应用程序根视图。包含 https://vaadin.com/docs/latest/components/app-layout[AppLayout^] 组件和主菜单。
...
`StandardListView - 标准列表视图` 是展示实体列表 CRUD 视图的基类。也可以用来作为查找视图（lookup view），在其中的列表选择实体实例后，将选择结果返回给调用方。列表视图通常使用 https://vaadin.com/docs/latest/components/grid[DataGrid^] 或 https://vaadin.com/docs/latest/components/tree-grid[TreeDataGrid^] 组件。
...
`StandardDetailView - 标准详情视图` 是展示单个实体实例 CRUD 视图的基类。详情视图通常使用 https://vaadin.com/docs/latest/components/form-layout[FormLayout^] 组件。
