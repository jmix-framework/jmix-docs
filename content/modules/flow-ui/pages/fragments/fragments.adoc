...
* The fragment class (controller) supports injection of Spring beans and fragment's UI components, as well as the use of annotated methods as handlers.
...
* `actions` - optional fragment actions element (similar to the view `actions` element). Actions defined for a fragment have a shortcut bound to the root component of the fragment. That is, the shortcut can be triggered only if focus is within the fragment layout.
* `data` - optional fragment data element (similar to the view `data` element). A fragment can define its own data containers and loaders defined in the `data` XML element or get them by id from a host view or enclosing fragment as described in the xref:flow-ui:fragments/using-fragments.adoc#using-data-components[Using Data Components] section.
...
* `getFragmentData()` - returns an object defining methods for interacting with the fragment data components. Similar to `ViewData`.
* `getFragmentActions()` - returns an object defining methods for interacting with fragment actions. Similar to `ViewActions`.
...
----
<1> Triggers `load()` method of all loaders registered in the fragment, including the xref:fragments/using-fragments.adoc#provided-data-containers[provided] ones.
