...
This section explains how to define and use fragments. See also xref:flow-ui:fragments/fragments.adoc#fragment-events[Fragment Events] for how to handle fragment lifecycle events.
...
Then you can include it in another view using the `fragment` element with the `class` attribute specifying the fragment FQN:
...
TIP: In Studio xref:studio:view-designer.adoc[], use the *Add Component* action to open the component palette and find the `Fragment` component in it.

[[programmatic-usage]]
== Programmatic Usage

// todo event link when available
The same fragment can be included in a view programmatically in a `InitEvent` or `AfterInitEvent` handler as follows:
...
----
<1> The fragment subscribes to the host view `BeforeShowEvent`, so when the host view is opened, data will be loaded in the fragment.
