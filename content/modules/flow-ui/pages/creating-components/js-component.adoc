...
You can connect a component event to a DOM event that's fired by an element in the browser. To do this, use the `@DomEvent` annotation on your event class to specify the name of the DOM event you want to listen for. Vaadin Flow will automatically add a DOM event listener for the element if a component event listener exists.
...
image::creating-components/slider-usage.gif[align="center", width="350"]