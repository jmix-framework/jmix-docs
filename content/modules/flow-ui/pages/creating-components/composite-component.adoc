...
[[composite]]
In the example below, the `ColorComponent` component is created by combining the `ColorPicker` component, created in the xref:creating-components/using-element-api.adoc[] section and a xref:vc/html-components/span.adoc[span] that displays its value:
