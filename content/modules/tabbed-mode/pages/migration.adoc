...
When a Tabbed Application Mode add-on is added, Vaadin navigation functionality is disabled, so no navigation events are sent to views, such as `BeforeEnterEvent` or `AfterNavigationEvent`. Because of that, you may need to refactor some of your views that rely on these events.
