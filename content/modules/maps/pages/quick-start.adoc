...
* Know the date of creating order by pointing the cursor on the order.
...
<1> Specifies height and width of the map window. Can be set in pixels or per sents.
<2> Coordinates of the initial geographical center of the map — longitude and latitude.
<3> Initials zoom level. The default value is 15.

Do the same in the `order-browse.xml` file.

You can se `geoMap` element on the right side of the screens.

image::qs-add-map.png
...
Lets's start with *Order Browse* screen, declare a vector layer in the XML descriptor and pass a `dataContainer` to the layer:
...
To display a date next to orders on the map on mouseover, we will use the `tooltipContentProvider`.
...
Now, we are ready to run the application. Go to the *Order browse* screen and click *Create*. Let's add a few orders. To point the location of the order, just click on the map.  

image::qs-edit.png

Go back to the *Order browse* screen. To view the date of the order, the order.
