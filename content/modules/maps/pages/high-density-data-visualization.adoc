...
. Include a vector layer in the xref:flow-ui:vc/components/geoMap.adoc[geoMap] component.
...
. Include a heatmap layer in the xref:flow-ui:vc/components/geoMap.adoc[geoMap] component: select the `geoMap` element in the *Jmix UI* structure panel or in the view XML descriptor, then click the *Add* button in the inspector panel. From the drop-down list, choose *Layers → HeatmapLayer*.
+
