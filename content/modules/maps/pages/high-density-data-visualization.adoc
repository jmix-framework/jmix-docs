...
. Include a heatmap layer in the xref:maps:map-component.adoc[geoMap] component: select the `geoMap` element in the *Jmix UI* hierarchy panel or in the view XML descriptor, then click the *Add* button in the inspector panel. From the drop-down list, choose *Layers → HeatmapLayer*.
+
