...
Source for loading tiles from an https://www.openstreetmap.org[OpenStreetMap^] tile server and from xref:maps:map-services.adoc#xyz-tiles[XYZ tile] services. For more details, see https://openlayers.org/en/v8.2.0/apidoc/module-ol_source_OSM-OSM.html[OSM documentation^].
...
Example of showing an image from a URL. This example is taken from https://openlayers.org/en/latest/examples/static-image.html[OpenLayers -> Examples -> Static image^].
...
Additionally, you can achieve this in the Java controller using `DownloadHandler`:
...
`DataVectorSource` allows data binding to Jmix data containers through the `dataContainer` and `property` attributes. The `property` attribute can take both a simple property and a property path.
