...
更多详情请参阅 https://openlayers.org/en/v8.2.0/apidoc/module-ol_layer_Tile-TileLayer.html[TileLayer 文档^]。
...
这个源的瓦片数据是从 https://www.openstreetmap.org[OpenStreetMap^] 瓦片服务和 xref:maps:map-services.adoc#xyz-tiles[XYZ 瓦片] 服务加载。更多详情请参阅 https://openlayers.org/en/v8.2.0/apidoc/module-ol_source_OSM-OSM.html[OSM 文档^]。
...
这个源的瓦片数据是在 URL 模板中定义一组 XYZ 格式的 URL。默认情况下，遵循 Google 矩阵的定义：x 0 和 y 0 位于左上角。`XyzSource` 可以加载 xref:maps:map-services.adoc#xyz-tiles[XYZ tile] 服务提供的瓦片。更多详情请参阅 https://openlayers.org/en/v8.2.0/apidoc/module-ol_source_XYZ-XYZ.html[XYZ 文档^]。
...
这个源的瓦片数据是从 xref:maps:map-services.adoc#web-map-service[WMS]（Web Map Service）服务加载。更多详情请参阅 https://openlayers.org/en/v8.2.0/apidoc/module-ol_source_TileWMS-TileWMS.html[TileWMS 文档^]。
...
这个源展示单一静态图片。更多详情请参阅 https://openlayers.org/en/v8.2.0/apidoc/module-ol_source_ImageStatic-Static.html[Static 文档^]。

这个示例展示使用 URL 的图片。取自 https://openlayers.org/en/latest/examples/static-image.html[OpenLayers -> Examples -> Static image^]：
...
此外，在 Java 控制器中可以通过 `DownloadHandler` 实现：
...
这个源从 xref:maps:map-services.adoc#web-map-service[WMS] 服务加载单一未命名图片。更多详情请参阅 https://openlayers.org/en/v8.2.0/apidoc/module-ol_source_ImageWMS-ImageWMS.html[ImageWMS 文档^]。
...
这个源处理地图要素。更多详情请参阅 https://openlayers.org/en/v8.2.0/apidoc/module-ol_source_Vector-VectorSource.html[VectorSource 文档^]。
