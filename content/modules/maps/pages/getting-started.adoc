...
If you did not create this application, go to the xref:tutorial:index.adoc[Tutorial] section, which contains a step-by-step description for developing the application. Otherwise, download the complete application source code from the https://github.com/jmix-framework/jmix-onboarding-2[jmix-framework/jmix-onboarding-2^] GitHub repository.

Let’s look at the model of the sample onboarding application.

image::data-model-diagram.svg[align="center"]

Let's add a new entity - `Location`. Two types of locations are presented in the application: office and coworking space. When creating a user card, HR manager selects the base location - the office where the user was assigned to work.
