...
We will use the Onboarding application as a basis.

If you did not create this application, go to the xref:tutorial:index.adoc[Tutorial] section, which contains a step-by-step description for developing the application.

Otherwise, download the complete application source code from the https://github.com/jmix-framework/jmix-onboarding-2[jmix-framework/jmix-onboarding-2^] GitHub repository. Then open it in Jmix Studio as described in the xref:studio:project.adoc#opening-existing-project[Opening Existing Project] section.
...
We will add a new entity - `Location`. Two types of locations are presented in the application: office and coworking space. When creating a user card, HR manager selects the base location - the office where the user was assigned to work.

* The user should be able to see all available locations marked with markers on the map.
* Offices and coworking spaces should be marked with different markers.
* The user should be able to change the base location by clicking on the map marker.
...
* `centerX` defines longitude of the initial geographical center of the map.
* `centerY` defines latitude of the initial geographical center of the map.
