...
Let's go back to the `Location` entity and add the following attributes:
...
A vector layer is used to work with geometries. It's possible to add multiple vector layers to a map.
...
The building entrance is shown as a black dot on the map. The path to the building is shown as a dashed line, while the polygon shows the building boundary.
