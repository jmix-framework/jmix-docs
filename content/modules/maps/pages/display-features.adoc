...
To showcase polygons, polylines, and points, you need to append a new vector layer onto a map.

Open `location-lookup-view.xml` and introduce the new `vectorLayer` after `dataVectorLayer`:
...
----
<1> Returns a point feature with a custom point style.
...
----
<1> Returns a polyline feature with a customized linestring style.
...
----
<1> Returns a polygon feature with a customized polygon style. The fill color and the polygon border color are established in variables and are dependent on the location type.
