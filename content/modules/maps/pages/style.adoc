...
如需更改 xref:maps:features-geometries.adoc#point-feature[PointFeature] 的样式，可以定义一个 `Style` 对象，设置所需的外观属性（如 `image`、`fill`、`stroke` 等），然后将该样式应用于 `PointFeature`。
...
如需更改 `LineStringFeature` 的样式，可以定义一个 `Style` 对象，设置所需的外观属性（如 `fill`、`stroke` 等），然后将该样式应用于 `LineStringFeature`。
...
如需更改 `PolygonFeature` 的样式，可以定义一个 `Style` 对象，设置所需的外观属性（如 `fill`、`stroke` 等），然后将该样式应用于 `PolygonFeature`。
