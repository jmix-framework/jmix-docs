...
A geo-object, short for geospatial object, refers to an entity with a property of a geometry type. The add-on supports the following geo-specific types that come from the JTS Topology Suite (JTS) library:

|===
| *Datatype* | *Java type*
| GeoMultiPoint         | org.locationtech.jts.geom.MultiPoint
| GeoMultiPolygon      | org.locationtech.jts.geom.MultiPolygon
...
As you can see, `Location` is a simple entity, and one of its properties, `building`, is of the `org.locationtech.jts.geom.Point` type.
