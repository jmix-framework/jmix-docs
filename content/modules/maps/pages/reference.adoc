...
在本节中，我们介绍一些 Jmix 地图的细节。我们将介绍使用 xref:maps:layers-sources.adoc[图层] 在地图上组织数据，并了解能提供可视化内容的数据源。此外，我们还将介绍 xref:maps:geo-objects.adoc[geo-object] 的概念，以及在 Jmix 地图中的实际用途。
...
坐标参考系（CRS） 通过一组称为坐标的数字来指定地球上的每个位置。有两组主要的坐标系：投影坐标系（也称为笛卡尔坐标系或矩形坐标系）和地理坐标系。
