...
Zooming and centering are essential xref:flow-ui:vc/components/geoMap.adoc[map] features that allow users to adjust the zoom level and focus on specific geographical locations within the map xref:flow-ui:vc/components/geoMap.adoc#view[view].
