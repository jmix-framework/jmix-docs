...
在 *Jmix UI* 层级结构面板或 XML 中选择 `valuePicker`，然后点击组件面板的 *Add* 按钮。在下拉列表中，选择 *Actions → Action*。
...
在 *Jmix UI* 层级结构面板或 XML 中选择 `select` 操作。然后在 *Jmix Inspector* 中切换至 *Handlers* tab，生成一个 `ActionPerformedEvent` 的处理方法：
...
----
<1> xref:flow-ui:opening-views.adoc#dialog-windows[DialogWindows] 提供以对话框的方式打开视图的流式接口。
...
点击操作面板中的 *Add Component*，然后将 `Layouts → VBox` 拖放至 *Jmix UI* 层级结构面板中的第一个 `hbox` 元素。配置 `vbox` 如下：
...
此时，*Jmix UI* 层级结构和 XML 中都会在 `vbox` 元素的下方添加新的 `geoMap` 元素。按照下面的代码配置 `id`、`height` 和 `width` 属性。
...
打开 `LocationLookupView` 控制器，并注入 `GeoMap` 组件。
...
----
<1> 获取 `locationsDc` 关联的 `DataVectorSource`。
<2> 设置一个新的样式，使用了图片和文字的组合。图片根据位置类型变化。
