...
To start, we'll generate a `LocationType` enumeration consisting of two values: `Office` and `Coworking`. The detailed instructions for creating enumerations can be found in the xref:tutorial:enumerations.adoc[Using Enumerations] section of the Tutorial.
...
Studio will run the changelog on your database, then build and run the application:
...
The extent limits the view. You can adjust how the viewport is constrained by the extent using `mapView` extent constraint options. See xref:flow-ui:vc/components/geoMap.adoc#extent-constraint-options[Extent Constraint Options].
...
When an HR manager creates a location, they click a spot on the map, which is then stored in the `Location` entity as a `Point` with coordinates.
...
----
<1> To obtain `GeometryFactory`, use `GeometryUtils` - a utility class with functions related to the JTS library.
