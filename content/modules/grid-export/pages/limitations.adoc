...
If you use custom xref:jmix-ui:vcl/components/table.adoc#table-column-generation[generated columns] in the `Table`, `GroupTable`, or `TreeTable` component, their content is not exported by default.
...
If some export action is invoked for a table with xref:jmix-ui:vcl/components/pagination.adoc[paging], and a user selects *All rows* export, the export action cannot by design output all rows, as it outputs only content of the table.