...
`ExcelExportAction` 操作继承了 xref:actions.adoc[ExportAction] 用于以 XLSX 或 XLS 格式导出表格内容。
...
`JsonExportAction` 操作继承了 xref:actions.adoc[ExportAction] 用于以  格式导出表格内容。 and designed to export the table content in JSON format.
