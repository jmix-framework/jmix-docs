...
* The *All rows* option exports all database records considering the applied filter and the initial data loader query from the view. Data loading occurs in batches, with the batch size configurable through the xref:grid-export:properties.adoc#jmix.gridexport.export-all-batch-size[jmix.gridexport.export-all-batch-size] application property.
+
