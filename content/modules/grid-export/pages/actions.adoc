...
`ExportAction` is a base xref:jmix-ui:actions/standard-actions.adoc#list-actions[list action] for export table content with a defined exporter. An instance of xref:exporters.adoc[TableExporter] is required for this action.

The action should be defined for a list component (xref:jmix-ui:vcl/components/table.adoc[Table], xref:jmix-ui:vcl/components/group-table.adoc[GroupTable], xref:jmix-ui:vcl/components/tree-table.adoc[TreeTable], xref:jmix-ui:vcl/components/data-grid.adoc[DataGrid], xref:jmix-ui:vcl/components/tree-data-grid.adoc[TreeDataGrid], xref:jmix-ui:vcl/components/tree.adoc[Tree]).
...
The action is implemented by the `io.jmix.uiexport.action.ExcelExportAction` class and should be defined in XML using `type="excelExport"` action’s attribute for a list component. You can configure common action parameters using XML attributes of the `action` element. See xref:jmix-ui:actions/declarative-actions.adoc[Declarative Actions] for details.
...
The action is implemented by the `io.jmix.uiexport.action.JsonExportAction` class and should be defined in XML using `type="jsonExport"` action’s attribute for a list component. You can configure common action parameters using XML attributes of the `action` element. See xref:jmix-ui:actions/declarative-actions.adoc[Declarative Actions] for details.
