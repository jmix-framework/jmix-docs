...
In this example, the `Employee` entity contains the `equipment` attribute that is a one-to-many collection. So the simplest way to implement the screen is to load the list of employees with a xref:data-access:fetching.adoc#fetch-plan[fetch plan] containing the `equipment` attribute and use a xref:backoffice-ui:data/property-containers.adoc[property container] to hold the list of dependent equiment lines. Then bind the left table to the master container and the right table to the property container.
