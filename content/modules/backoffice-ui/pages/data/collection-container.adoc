...
* `setSorter()` - sets the given sorter for this container. A standard implementation of the `Sorter` interface is `CollectionContainerSorter`. It is set by the framework automatically when the container is associated with a loader. You can provide your xref:backoffice-ui:data/data-examples.adoc#custom-sorting[own implementation] if needed.
* `getSorter()` - returns the sorter currently set for this container.

[[events]]
== Events

In addition to events of xref:backoffice-ui:data/instance-container.adoc#events[InstanceContainer], the `CollectionContainer` interface allows you to register listeners to the `CollectionChangeEvent` event which is sent when the container items collection is changed: on adding, removing and replacing elements. Example of subscribing to the event for a container defined in the screen XML with `customersDc` id:
