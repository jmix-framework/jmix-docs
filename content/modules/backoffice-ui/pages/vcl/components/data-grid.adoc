...
Ниже показан типичный `dataGrid`:
...
Специальный заголовок для колонки `hobby` отображается в выпадающем меню кнопки управления колонкой.
...
[[column-expand-ratio]]
`expandRatio` - необязательный атрибут, устанавливает соотношение, с которым колонка расширяется. Если хотя бы одной колонке установлено иное значение, все неявные значения игнорируются и учитываются только проставленные.
...
`DataGrid` позволяет задать позволяет настроить заполнитель в виде сообщения и/или ссылки, отображаемый в случае, если источник данных пуст или не задан.
...
Компонент `DataGrid` реализует интерфейс `ActionsHolder` и может содержать специальные действия и xref:actions/standard-actions.adoc#list-actions[стандартные действия с коллекцией].
...
<2> Определение специального действия `about`.
...
Интерфейс `DetailsGenerator` позволяет создать собственный компонент для отображения подробностей.
...
* `no-vertical-lines` - даляет вертикальные разделители колонок;
