...
However, there is no `GroupDataGrid` implementation (but `Table` xref:vcl/components/group-table.adoc[has]).
...
[[exclude]]
* `exclude` contains a comma-separated list of attributes that should not be loaded to the data grid.
+
...
At the same time, the `firstName` and `lastName` columns cannot be made collapsed using user actions.
...
The default value of minimum column width is specified in the xref:app-properties.adoc#jmix-ui-data-grid-default-column-min-width[jmix.ui.DataGrid.defaultColumnMinWidth] property.
...
The default value of maximum column width is specified in the xref:app-properties.adoc#jmix-ui-data-grid-default-column-max-width[jmix.ui.DataGrid.defaultColumnMaxWidth] property.
...
If the `selectionMode` attribute is set to `MULTI_CHECK`, users can select multiple rows using the embedded column with checkboxes.
...
When a column name is checked/unchecked, the value of the <<column-collapsed,collapsed>> attribute of this column is updated.

The column collapsing changes can be tracked with <<column-collapsing-change-event,ColumnCollapsingChangeEvent>>.
...
`ColumnGeneratorEvent` allows you to add generated, or calculated, columns and define a custom representation of data in a column.
...
Please note, the `startDate` field has a `DateRenderer`, and the `endDate` field does not.
...
The `column` element of `DataGrid` has a child `htmlRenderer` element with a non-required `nullRepresentation` attribute.
...
There is also a declarative way of defining what component to use for selecting an entity in generic mechanisms. It is based on the xref:app-properties.adoc#jmix-ui-component-entity-field-type[jmix.ui.component.entityFieldType] and xref:app-properties.adoc#jmix-ui-component-entity-field-actions[jmix.ui.component.entityFieldActions] properties.
...
[[editor-open-event]]
* `EditorOpenEvent` is an event that is fired before the `DataGrid` editor is opened. Provides access to the components used in the editor, allowing the possibility to change their values programmatically.
+
For example:
+
[source,java,indent=0]
...
[[editor-close-event]]
* `EditorCloseEvent` is an event that is fired when the `DataGrid` editor is closed. Provides access to the components used in the editor, allowing the possibility to use their values programmatically.

[[editor-pre-commit-event]]
* `EditorPreCommitEvent` is an event that is fired before the item is updated. Provides access to the components used in the editor, allowing the possibility to use their values programmatically.

[[editor-post-commit-event]]
* `EditorPostCommitEvent` is an event that is fired after the item is updated. Provides access to the components used in the editor, allowing the possibility to use their values programmatically.

The changes are committed to the data container only. The logic to save these changes in the database should be added separately.
