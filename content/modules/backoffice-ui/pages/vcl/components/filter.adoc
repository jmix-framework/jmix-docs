...
The drop-down actions of the *Create* button correspond to the <<filter-component-registration,registered>> filter components. Three filter components named <<property-filter,PropertyFilter>>, <<jpql-filter,JpqlFilter>>, and <<group-filter,GroupFilter>> are available by default:
