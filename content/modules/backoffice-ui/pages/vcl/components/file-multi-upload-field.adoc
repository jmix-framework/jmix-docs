...
The `FileMultiUploadField` component allows you to upload files to temporary storage as `FileRef` objects. This approach expands the possible use cases of the component. For example, you can upload files to xref:files:index.adoc[file storage] or only process and not upload them.
