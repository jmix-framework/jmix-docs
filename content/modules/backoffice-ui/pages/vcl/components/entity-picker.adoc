...
An `EntityPicker` displays an xref:data-model:entities.adoc[entity] instance in a text field and performs actions when the user clicks buttons on the right.

Component's XML-name: `entityPicker`.

== Basics

Use an `EntityPicker` if:

* The field value is a reference to an entity instance.
* The user needs to select or create an entity instance via the lookup screen or enter a specific value.
* The user needs to open the edit screen for a related entity instance.

By default, Jmix Studio generates an `EntityPicker` when creating an entity editor screen with a reference attribute.
...
[WARNING]
====
For an ``EntityPicker``'s proper operation, you need either set the `metaClass` attribute or simultaneously set the `dataContainer` and `property` attributes.
====

[[actions]]
== Actions

You can define custom and predefined actions for an `EntityPicker` displayed as buttons on the right.
...
When Studio generates an `EntityPicker` in the editor screen, it also generates two predefined standard actions: `entity_lookup` and `entity_clear`. There are also the `entity_open` and `entity_open_composition` actions.

Use the `type` and `id` attributes for declaring predefined action in XML.

If you create an `EntityPicker` without actions, the XML loader will define only the `entity_lookup` and `entity_clear` actions. To add other predefined action, for example, the `entity_open`, you should specify the `actions` element as follows:
...
[TIP]
====
You can generate the `Action.ActionPerformedEvent` handler implementation method’s stub using the Studio.
====

[[meta-class]]
== MetaClass

You can use an `EntityPicker` without binding to data container, that is, without setting `dataContainer` and `property`. In this case, the `metaClass` attribute should be used to specify an entity type for an `EntityPicker`. For example:
...
An `EntityPicker` can have an icon on the left. Just install `fieldIconProvider` in the screen controller to implement your custom logic:
...
== Creating an EntityPicker Programmatically

To create an `EntityPicker` in the controller, use the `UiComponents` factory. Keep in mind that the programmatically created `EntityPicker` will get no default actions, and you need to add all necessary actions explicitly:
