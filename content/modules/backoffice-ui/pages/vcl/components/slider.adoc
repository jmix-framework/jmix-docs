...
`Slider` can be bound to a xref:data/data-containers.adoc[data container] using the `dataContainer` and `property` attributes. In this case, the component will define the data type from the entity field contained in the `property` attribute.
...
//todo add link to validator page and uncomment the handler below
////
[[validator]]
=== Validator
