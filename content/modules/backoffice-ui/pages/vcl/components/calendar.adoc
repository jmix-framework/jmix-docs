...
Calendar component is intended to organize and display calendar events.
...
2. Weekly view - displayed when the date range between 1 and 7 days long.
+
[source,xml,indent=0]
----
...
You can also use `weeklyCaptionFormat` to change the caption of weekly days. The rules of the `https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html[SimpleDateFormat^]` class define the format of the `weeklyCaptionFormat` value.
...
`CalendarEventProvider` interface has `ContainerCalendarEventProvider` implementation, which allows you to fill events with data directly from entity fields.
