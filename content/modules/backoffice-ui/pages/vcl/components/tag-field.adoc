...
`TagField` is based on `xref:vcl/components/suggestion-field.adoc[SuggestionField]` and has similar functionality, but one significant difference is that `TagField` allows you to select multiple values instead of a single value.
...
* You need to select a multiple options.
