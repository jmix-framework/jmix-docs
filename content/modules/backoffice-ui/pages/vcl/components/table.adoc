...
The set of columns for a table is defined in the `columns` element. If it is not specified, columns will be automatically determined from the xref:data-access:fetch-plans.adoc[fetch plan] attributes defined in `dataContainer`.
