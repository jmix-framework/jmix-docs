...
`TagPicker` combines `xref:vcl/components/combo-box.adoc[ComboBox]` and `xref:vcl/components/values-picker.adoc[ValuesPicker]` so it has a dropdown list and the ability to work with actions.

image::vcl/components/tag-picker.png[align="center"]
...
To create a `TagPicker` associated with data, you should use the `dataContainer`, `property` and `optionsContainer` attributes:
...
As `TagPicker` is the inheritor of `xref:vcl/components/values-picker.adoc[ValuesPicker]`, it can use all xref:vcl/components/values-picker.adoc#actions[actions] applicable for it.
...
`TagPicker` has the following handlers and events described in `xref:vcl/components/tag-field.adoc#handler-and-events[TagField]`:
