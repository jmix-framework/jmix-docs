...
`EntitySuggestionField` allows searching for entity instances according to a string entered by a user and performs actions when a user clicks buttons on the right.

`EntitySuggestionField` similar to the xref:vcl/components/entity-picker.adoc[EntityPicker] component with the additional functionality: a user enters a few characters, and if several matches have been found, a drop-down list will display all of them. `EntitySuggestionField` refreshes the list of options on each entered symbol.
