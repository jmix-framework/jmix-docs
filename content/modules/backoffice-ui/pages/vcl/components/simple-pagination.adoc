...
`SimplePagination` is a component used to load data by pages. As opposed to xref:vcl/components/pagination.adoc[Pagination], you can use it in `Table`, `DataGrid`, and other list components.

Component’s XML-name: `simplePagination`.

== Basics

`SimplePagination` has a simple view with a count of rows and navigation buttons. It can also have a xref:vcl/components/pagination.adoc#items-per-page[drop-down list] for the number of items per page.

image::vcl/components/simple-pagination.png[align="center"]

To create `SimplePagination` connected to data, use the xref:vcl/components/pagination.adoc#container-loader-provider[containerProvider] or xref:vcl/components/pagination.adoc#container-loader-provider[loaderProvider] nested element.
