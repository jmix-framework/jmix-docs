...
`ColorPicker` allows a user to preview and select a color. The component is a button that opens the color panel when clicked. The button is colored in color selected by the user. `ColorPicker` returns a hexadecimal (HEX) value of the color as a `String`.
...
This attribute allows you to display the HEX value of the selected color as a button caption. To achieve this result, set this attribute to `true` and not specify a value for the attribute `buttonCaption`.
