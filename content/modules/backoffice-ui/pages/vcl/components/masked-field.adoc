...
`MaskedField` is a `xref:vcl/components/text-field.adoc[]` in which you can enter data in a predefined format.
...
`MaskedField` repeats the functionality of `xref:vcl/components/text-field.adoc[]`, but it is convenient to use `MaskedField`, for example, to enter telephone numbers:
