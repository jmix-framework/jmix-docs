...
You can track `PopupButton` visibility changes by subscribing to the corresponding event:
...
Also, you can use the listener, that allows you to intercept the events of the component’s visibility changes.
...
The `PopupButton` component's appearance can be customized using SCSS variables with `$cuba-popupbutton-*` prefix. You can change these variables in the visual editor after creating a theme extension or a xref:themes/custom_theme.adoc[custom theme].
