...
[[null-option-visible]]
=== nullOptionVisible
The `nullOptionVisible` XML attribute sets visibility for the null option in the drop-down list. It allows you to make a `ComboBox` not required but still without the null option.
...
[[text-input-allowed]]
Use the `textInputAllowed` attribute to disable filtering options from a keyboard. It can be convenient for short lists. The default value is `true`.

[[paging]]
== Paging

A `ComboBox` supports a paging mechanism that helps break a long list of items into multiple separate pages reducing load time and increasing performance. The number of pages is determined by the page size and the number of items in the drop-down list.
