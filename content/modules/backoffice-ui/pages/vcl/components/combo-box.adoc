...
Use the `setNewOptionHandler()` method to let users enter a value that is not in the initial drop-down list and to save it to select the item later without entering it again:
...
`setNewOptionHandler()` is invoked if the user enters a value and presses *Enter*. In this case, a new value is added to the list of options and becomes available for later use.
