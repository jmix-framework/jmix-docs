...
`Pagination` is a component used to load data by pages. `Pagination` allows users to navigate through pages easily and organize data according to performance.

`Pagination` cannot be used in `Table`, `DataGrid`, and other list components.
...
[[container-loader-provider]]
To create `Pagination` connected to data, use the `containerProvider` or `loaderProvider` nested element:

[source, xml,indent=0]
----
include::example$/ex1/src/main/resources/ui/ex1/screen/component/pagination/pagination-screen.xml[tags=data;layout-start;pagination-loader-container;layout-end]
----
<1> `CollectionContainer` for the `Customer` entity.
<2> The `customersDl` `CollectionLoader` loads a collection of entities by a JPQL query.
<3> The `Pagination` component is connected to the loader using the `loaderId` attribute of the `loaderProvider` nested element.
<4> The `Pagination` component is connected to the data container using the `dataContainer` attribute of the `containerProvider` nested element.
