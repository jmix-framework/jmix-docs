...
Maximum upload size is determined by the xref:app-properties.adoc#jmix.ui.maxUploadSizeMb[jmix.ui.maxUploadSizeMb] application property and equals 20MB by default. If you select a file of a larger size, a corresponding message will be displayed, and the upload will be interrupted. To change the maximum upload size for concrete `fileStorageUploadField` use the `fileSizeLimit` attribute, which allows you to set the maximum allowed file size specified in bytes.
