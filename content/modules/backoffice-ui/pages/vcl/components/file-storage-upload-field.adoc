...
`FileStorageUploadField` component allows you to upload the file to the server, as a `FileRef` object.
...
In the example below, `image` attribute of the `Person` entity corresponds to the `FileRef` java class.
...
To store the file as an array of bytes instead of the `FileRef` object, you should use the `xref:vcl/components/file-upload-field.adoc[FileUploadField]` component.
...
By default, the `fileStoragePutMode` attribute is set to `IMMEDIATE` value. You can also control the saving of the file programmatically. To do this, set the `fileStoragePutMode` attribute to `MANUAL` value:
...
//todo add link to client tier
The component will upload the file to the temporary storage of the client tier and invoke the `FileUploadSucceedEvent` listener.

//todo add link to FileStorage
`TemporaryStorage.putFileIntoStorage()` method is used to move the uploaded file from the temporary client storage to FileStorage. Parameters of this method are file identifier and file name. Both of these parameters are provided by `FileStorageUploadField`.

[[file-storage]]
=== fileStorage

The `fileStorage` attribute allows you to specify a `FileStorage` where the upload file will be placed.
...
Maximum upload size is determined by the `jmix.ui.maxUploadSizeMb` application property and equals 20MB by default. If you select a file of a larger size, a corresponding message will be displayed, and the upload will be interrupted. To change the maximum upload size for concrete `fileStorageUploadField` use the `fileSizeLimit` attribute, which allows you to set the maximum allowed file size specified in bytes.
...
The `fileSizeLimit` attribute allows you to set the white list of permitted file extensions. The value of the attribute should be a comma-separated list of extensions with leading dots.
...
Also, for all listed above <<attributes, attributes>>, there are corresponding methods:
