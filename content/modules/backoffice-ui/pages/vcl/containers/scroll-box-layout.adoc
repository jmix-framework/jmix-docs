...
[[content-max-width]]
* `contentMaxWidth` - sets maximum CSS width for content. The width of the container's contents can't get any wider, even if the screen size allows it.
...
Up to the values installed in the `contentMinWidth` or `contentMinHeight`, the `ScrollBox` content can reduce its size. As soon as the screen becomes smaller, part of the container’s content will disappear, and a scrollbar will appear at the bottom or the right.
