...
* `vertical` – nested components are placed vertically, by default.
* `horizontal` – nested components are placed horizontally.
+
image::vcl/containers/split-panel-horizontal.gif[align="center"]
...
`settingsEnabled` attribute defines if user settings for the component should be saved/restored. Settings are saved only if the component’s `id` is set. Possible values:
