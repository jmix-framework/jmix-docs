...
[[caption]]
* `caption` - defines group header.

[[orientation]]
* `orientation` -  defines components placement direction − horizontal or vertical. The default value is `vertical`. Horizontal orientation shows below:
+
image::vcl/containers/group-box-horizontal.png[align="center"]

[[collapsable]]
* `collapsable` - if the value is set to true, you can hide the component’s content using a button in the top-left corner.
+
...
xref:vcl/containers/box-layout.adoc#required-indicator-visible[requiredlndicatorVisible] -
