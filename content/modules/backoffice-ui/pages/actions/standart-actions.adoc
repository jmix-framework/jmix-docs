...
List actions work with collections of entities that are displayed in tables or trees. These are xref:actions/standard-actions/add-action.adoc[AddAction], xref:actions/standard-actions/bulk-edit-action.adoc[BulkEditAction], CreateAction, EditAction, ExcludeAction, ItemTrackingAction, RefreshAction, RelatedAction, RemoveAction, SecuredListAction, ViewAction.
