...
== Basics
`AddAction` is a xref:actions/standart-actions.adoc#list-actions[list action] designed to add existing entity instances to a data container by selecting them in a lookup screen. For example, it can be used to fill a many-to-many collection.
...
----
include::example$/ex1/src/main/java/ui/ex1/screen/entity/customer/CustomerEdit.java[tags=add-action]
----
