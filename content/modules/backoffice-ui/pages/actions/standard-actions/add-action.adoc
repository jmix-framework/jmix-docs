...
`AddAction` is a xref:actions/standart-actions.adoc#list-actions[list action] designed to add existing xref:data-model:entities.adoc[entity] instances to a data container by selecting them in a lookup screen. For example, you can use it to fill a many-to-many collection.
