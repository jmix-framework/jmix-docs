...
`ViewAction` is a xref:actions/standart-actions.adoc#list-actions[list action] designed to view and edit xref:data-model:entities.adoc[entity] instances. It opens the edit screen in the same way as xref:actions/standard-actions/edit-action.adoc[EditAction], but it makes all fields non-editable and disables actions implementing interface `ReadOnlyAwareScreen`. If you want to allow users to switch the screen to the editable mode, add a button and link it with the predefined `enableEditing` action:
