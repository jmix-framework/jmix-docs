...
`EntityLookupAction` is an entity picker action designed to select an xref:data-model:entities.adoc[entity] instance from a lookup screen and set it to the xref:vcl/components/entity-picker.adoc[entity picker].

The action is implemented by the `io.jmix.ui.action.entitypicker.EntityLookupAction` class and should be defined in XML using `type="entity_lookup"` action’s attribute. You can configure common action parameters using XML attributes of the action element. See xref:actions/declarative-actions.adoc[Declarative Actions] for details. Below we describe parameters specific to the `EntityLookupAction` class.

== Properties

The following parameters can be set both in XML and in Java:

* `openMode` - the lookup screen opening mode as a value of the `OpenMode` enum: `NEW_TAB`, `DIALOG`, etc. By default, `EntityLookupAction` opens the screen in `THIS_TAB` mode.
* `screenId` - string id of the lookup screen to use. By default, `EntityLookupAction` uses either a screen, annotated with `@PrimaryLookupScreen`, or having identifier in the format of `<entity_name>.browse`, for example, `demo_Customer.browse`.
...
----
include::example$/ex1/src/main/java/ui/ex1/screen/component/entitypicker/EntityPickerScreen.java[tags=inject-customer-lookup;userPicker2;set-entity-lookup-action;end]
----
...
It is a handler that returns `ScreenOptions` object to be passed to the opened lookup screen. For example:
