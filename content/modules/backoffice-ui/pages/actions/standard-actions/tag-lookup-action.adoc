...
`TagLookupAction` is a lookup action designed to select xref:data-model:entities.adoc[entity] instances from a lookup screen and set it to the tag picker.
