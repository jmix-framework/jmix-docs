...
`BulkEditAction` is a xref:actions/standart-actions.adoc#list-actions[list action] designed to change attribute values for multiple xref:data-model:entities.adoc[entity] instances at once. It opens a special screen where the user can enter desired attribute values. After that, the action updates the selected entities in the database and in the data container of the UI component.
