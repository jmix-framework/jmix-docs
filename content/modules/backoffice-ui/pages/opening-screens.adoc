...
Экран может быть открыт из xref:backoffice-ui:screens/root-screens.adoc#main-screen[главного меню], xref:backoffice-ui:actions/standard-actions.adoc[стандартным действием] при работе с экранами просмотра и редактирования, или программно из другого экрана. В данном разделе рассматривается, как открывать экраны программно.
...
----
<1> Инжектирование компонента xref:backoffice-ui:vcl/components/label.adoc[Label].
...
В большинстве случаев экраны редактирования открываются с помощью xref:backoffice-ui:actions/standard-actions.adoc[стандартных действий], таких как xref:backoffice-ui:actions/standard-actions/create-action.adoc[CreateAction]. Давайте рассмотрим примеры, когда можно использовать API `ScreenBuilders` напрямую для открытия экрана из обработчика xref:backoffice-ui:actions/base-action.adoc[BaseAction] или xref:backoffice-ui:vcl/components/button.adoc[Button].

Экран редактирования по умолчанию определяется по следующей схеме:

. Если существует экран редактирования с аннотацией xref:backoffice-ui:screens/annotations.adoc#primary-editor-screen[@PrimaryEditorScreen], будет использован он.
...
Рассмотрим несколько примеров работы с экранами выбора. Как и в случае с экранами редактора, в основном такие экраны открываются с помощью xref:backoffice-ui:actions/standard-actions.adoc[стандартных действий], таких как действие xref:backoffice-ui:actions/standard-actions/entity-lookup-action.adoc[EntityLookupAction]. Приведенные ниже примеры показывают использование API `ScreenBuilders` и могут быть полезны, если стандартные действия не используются.

Экран выбора по умолчанию определяется по следующей схеме:

. Если существует экран выбора с аннотацией xref:backoffice-ui:screens/annotations.adoc#primary-lookup-screen[@PrimaryLookupScreen], будет использован он.
...
Если экран открывается из стандартного действия, такого как xref:backoffice-ui:actions/standard-actions/create-action.adoc[CreateAction], используйте его обработчик `screenConfigurer` для передачи параметров через публичные сеттеры контроллера экрана.
...
В открываемом экране `FancyMessageScreen`, объект параметров может быть получен в обработчиках xref:backoffice-ui:screens/screen-events.adoc#init-event[InitEvent] и xref:backoffice-ui:screens/screen-events.adoc#after-init-event[AfterInitEvent]:
...
Если экран открывается из стандартного действия, такого как xref:backoffice-ui:actions/standard-actions/create-action.adoc#create-action[CreateAction], используйте его обработчик `screenOptionsSupplier` для создания и инициализации требуемого объекта `ScreenOptions`.
