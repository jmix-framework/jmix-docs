...
В данном разделе описаны события жизненного цикла экрана, которые могут обрабатываться в контроллерах. Обратите внимание, что события не отвечают за сохранение настроек компонента; это берет на себя xref:facets/screen-settings-facet.adoc[].
...
* `SHORTCUT` - экран закрыт нажатием горячих клавиш, определенных в свойстве приложения xref:backoffice-ui:app-properties.adoc#jmix.ui.screen.close-shortcut[jmix.ui.screen.close-shortcut].
