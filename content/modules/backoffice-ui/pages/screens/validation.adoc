...
----
+
* `showUnsavedChangesDialog()` - показывает стандартный диалог несохраненных изменений ("Вы действительно хотите закрыть экран?") с кнопками *Yes* и *No*. Используется в `StandardEditor`. Метод `showUnsavedChangesDialog()` имеет обработчик, который реагирует на действия пользователя.

//Пример использования можно увидеть в методе `io.jmix.ui.screen.StandardEditor#preventUnsavedChanges`.
//See ticket https://github.com/Haulmont/jmix-docs/issues/330


* `showSaveConfirmationDialog()` - показывает стандартный диалог подтверждения сохранения измененных данных ("Сохранить изменения перед закрытием экрана?") с кнопками *Save*, *Do not save*, и *Cancel*. Используется в `StandardEditor`. Метод `showSaveConfirmationDialog()` имеет обработчик, который реагирует на действия пользователя.
+
Тип диалога можно настроить с помощью свойства приложения xref:backoffice-ui:app-properties.adoc#jmix.ui.screen.use-save-confirmation[jmix.ui.screen.use-save-confirmation].
