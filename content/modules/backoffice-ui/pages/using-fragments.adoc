...
This section explains how to define and use xref:backoffice-ui:screens.adoc[screen fragments]. See also xref:backoffice-ui:screens/screen-fragment-events.adoc[ScreenFragment Events] for how to handle fragment lifecycle events.

== Declarative Usage of Fragment

Suppose we have a fragment for entering an address. Screen controller:
...
== Programmatic Usage of Fragment

The same fragment can be included in the screen programmatically in a xref:backoffice-ui:screens/screen-events.adoc#init-event[InitEvent] or xref:backoffice-ui:screens/screen-events.adoc#after-init-event[AfterInitEvent] handler as follows:
...
----
<1> Passes a string parameter to the `setStringParam()` method.
<2> Passes a data container to the `setContainerParam()` method.
<3> Pass a `TextField` component to the `setComponentParam()` method.
...
----
<1> Data container that is uded in the fragment below.
