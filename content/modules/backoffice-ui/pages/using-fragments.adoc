...
В данном разделе рассматриваются примеры определения и использования xref:backoffice-ui:screens.adoc[фрагментов экранов]. См. также раздел xref:backoffice-ui:screens/screen-fragment-events.adoc[События ScreenFragment] для получения информации о событиях жизненного цикла фрагментов.
...
Тот же самый фрагмент может быть включен в экран программно в обработчике xref:backoffice-ui:screens/screen-events.adoc#init-event[InitEvent] или xref:backoffice-ui:screens/screen-events.adoc#after-init-event[AfterInitEvent] как показано ниже:
...
----
<1> Передача строкового параметра в метод `setStringParam()`.
<2> Передача контейнера данных в метод `setContainerParam()`.
