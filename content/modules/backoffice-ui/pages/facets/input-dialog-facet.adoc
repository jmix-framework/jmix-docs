...
`InputDialogFacet` is a facet that allows you to pre-configure an xref:dialogs.adoc#input-dialog[input dialog]. Declarative definition of the message dialog replaces existing `Dialogs.createInputDialog()` method.
...
To add input fields in the dialog, use the `parameters` element of the screen XML descriptor to put the following subelements. For each parameter, a corresponding xref:vcl/components.adoc[visual component] will be created for entering the value.
...
* `entityParameter` - will be displayed xref:vcl/components/entity-picker.adoc[]. To define the entity class for the corresponding field, use the `entityClass` attribute.
+
[source,xml,indent=0]
----
...
* `enumParameter` - will be displayed xref:vcl/components/combo-box.adoc[]. To define the enum class for the corresponding field, use the `enumClass` attribute.
+
[source,xml,indent=0]
----
