...
|`false`
|[[showColumnTotals]]showColumnTotals
|`showColumnTotals` 属性控制 `pivotTable` 组件是否显示每列的总计。当 `showColumnTotals` 设置为 `true` 时，透视表将在表格底部额外显示一行每列的汇总数据（例如，总和、均值等）。`showColumnTotals` 属性仅适用于表格类型的渲染器（`Table`、`Table barchart`、`Heatmap`、`Row heatmap`、`Col heatmap`）。参考 https://demo.jmix.io/ui-samples/sample/pivot-table-ui-properties[在线示例^]。
|`true`
|[[showRowTotals]]showRowTotals
|`showRowTotals` 属性控制 `pivotTable` 组件是否显示每行的总计。当 `showRowTotals` 设置为 `true` 时，透视表将在表格最右侧额外显示一列每行的汇总数据（例如，总和、均值等）。`showRowTotals` 属性仅适用于表格类型的渲染器（`Table`、`Table barchart`、`Heatmap`、`Row heatmap`、`Col heatmap`）。参考 https://demo.jmix.io/ui-samples/sample/pivot-table-ui-properties[在线示例^]。
|`true`
|[[unusedPropertiesVertical]]unusedPropertiesVertical
...
参考 https://demo.jmix.io/ui-samples/sample/pivot-table-events[在线示例^]。
|[[PivotTableRefreshEvent]]PivotTableRefreshEvent
