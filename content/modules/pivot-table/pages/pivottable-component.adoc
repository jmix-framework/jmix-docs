...
The new `pivotTable` element will be added in both the *Jmix UI* structure panel and in the XML. You can configure attributes like xref:flow-ui:vc/common-attributes.adoc#id[id], xref:flow-ui:vc/common-attributes.adoc#height[height], xref:flow-ui:vc/common-attributes.adoc#width[width], etc., in the same way as for other xref:flow-ui:vc/components.adoc[UI components].
