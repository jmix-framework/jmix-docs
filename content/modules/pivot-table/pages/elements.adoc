...
A key-value map with a set of properties to be used in the `pivotTable`, where the key is the attribute's name from the xref:jmix-ui:data/data-containers.adoc[data container], and the value is its localized caption.
