...
`ShowPivotAction` is a special action that enables exporting data from the components that extend `ListComponent`, such as xref:backoffice-ui:vcl/components/table.adoc[Table], xref:backoffice-ui:vcl/components/tree.adoc[Tree], and xref:backoffice-ui:vcl/components/data-grid.adoc[DataGrid], to a pivot table.
