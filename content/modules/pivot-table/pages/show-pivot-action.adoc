...
`ShowPivotAction` is a special action that enables exporting data from the components that extend `ListComponent`, such as xref:jmix-ui:vcl/components/table.adoc[Table], xref:jmix-ui:vcl/components/tree.adoc[Tree], and xref:jmix-ui:vcl/components/data-grid.adoc[DataGrid], to a pivot table.
