...
A pivot table in xref:flow-ui:vc/components/pivotTable.adoc#ui-mode[UI Mode] will be displayed in a new tab. By default, all attributes included in the component's data container view will be shown, except for:
...
You can export the displayed Pivot Table data to Excel (if the current renderer is supported). A corresponding button is displayed by default in the opened tab for this purpose.