...
NOTE: The export functionality currently supports the following xref:flow-ui:vc/components/pivotTable.adoc#using-renderers[renderer] types: `TABLE`, `TABLE_BAR_CHART`, `HEATMAP`, `COL_HEATMAP`, and `ROW_HEATMAP`. Note that cell colors are not exported.
