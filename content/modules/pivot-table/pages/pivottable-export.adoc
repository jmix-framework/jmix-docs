...
//     <a href="https://demo.jmix.ru/ui-samples/sample/pivot-table-export" class="live-demo-btn" target="_blank">ДЕМО</a>
// </div>
// ++++
...
NOTE: Функция экспорта в настоящее время поддерживает следующие типы xref:flow-ui:vc/components/pivotTable.adoc#using-renderers[рендереров]: `TABLE`, `TABLE_BAR_CHART`, `HEATMAP`, `COL_HEATMAP` и `ROW_HEATMAP`. Важно отметить, что цвета ячеек не экспортируются.
