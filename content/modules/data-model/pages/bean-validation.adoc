...
* Validation logic is located next to your domain model: defining value, method, and bean constraints is natural.
* The Bean Validation standard gives you tens of https://docs.jboss.org/hibernate/stable/validator/reference/en-US/html_single/#validator-defineconstraints-spec[validation annotations^] out-of-the-box, like `@NotNull`, `@Size`, `@Min`, `@Max`, `@Pattern`, `@Email`, `@Past`, and less standard ones like `@URL`, `@Length`.
...
Bean Validation is automatically invoked in xref:flow-ui:views/view-validation.adoc[UI views] when a user submits data, and in the xref:rest:index.adoc[generic REST API].
...
Let’s check how bean validation runs automatically when a user submits data in the UI.

image::validation-ui.png[align="center"]

As you can see, our application not only shows error messages to a user but also highlights form fields that have failed single-field bean validation with red lines.
...
[NOTE]
Bean validation is inheritable. If you annotate a class, field, or method with a constraint, all descendants that extend or implement this class or interface are affected by the same constraint check.
...
* `jakarta.validation.groups.Default` - this group is always passed, except when saving the entity detail view.
