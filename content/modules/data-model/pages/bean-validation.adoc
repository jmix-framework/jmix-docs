...
Using Bean Validation brings quite a lot of benefits to your project:
...
Bean Validation is automatically invoked in UI views when user submits the data, and in the xref:rest:index.adoc[generic REST API].
...
Entity detail view xref:flow-ui:views.adoc#standard-detail-view[controllers] perform validation against class-level constraints on save if the constraint includes the `UiCrossFieldChecks` group and if all attribute-level checks are passed.
