...
Ограничения Bean Validation определяются с помощью аннотаций пакета `jakarta.validation.constraints` или собственных аннотаций. Аннотации указываются на декларации класса сущности или POJO, на поле или getter-методе, а также на методе сервиса.
...
* `jakarta.validation.groups.Default` - эта группа передается всегда, кроме как при коммите редактора UI.
...
Вы можете выполнить Bean Validation программно, используя метод `validate()` интерфейса `jakarta.validation.Validator`. Результатом валидации является набор объектов `ConstraintViolation`. Например:
