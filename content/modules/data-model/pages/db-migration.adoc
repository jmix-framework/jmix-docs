...
Альтернативно, для основного хранилища данных вы можете настроить это поведение в Studio, используя опцию *Do not generate Liquibase drop changesets for unmapped tables* в диалоге xref:studio:data-stores.adoc[Data Store Properties].

[[ignoring-foreign-key-constraint]]
=== Игнорирование внешнего ключа

Jmix Studio учитывает ограничения JPA при генерации changelog файлов. Если вам нужен столбец без внешнего ключа (например, при работе с устаревшей схемой базы данных), установите `ConstraintMode.NO_CONSTRAINT` в `@JoinColumn` на стороне владельца:
...
Если указан `ConstraintMode.NO_CONSTRAINT` в сгенерированном changelog будут скрипты для создания колонки и индекса, но не для внешнего ключа.