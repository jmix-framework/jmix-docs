...
元数据 API 的主要入口是 `Metadata` bean。它能使你以 `MetaClass` 和 `MetaProperty` 对象的形式访问实体的有关信息。
...
<8> 你可以使用 `Range.asClass()` 方法获取引用的实体的 meta 类。
...
<13> `Range.asDatatype()` 方法返回实体属性的 xref:data-types.adoc[数据类型]。你可以用它格式化和解析属性值。
