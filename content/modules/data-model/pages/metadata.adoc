...
NOTE: 元数据是框架的内部机制，很少直接被应用程序代码使用。

元数据 API 的主要入口是 `Metadata` bean。 它能使您以 `MetaClass` 和 `MetaProperty` 对象的形式访问实体的有关信息。
...
<8> 您可以使用 `Range.asClass()` 方法获取引用的实体的 meta 类。
<9> `Range.getCardinality()` 方法返回引用的关系：`ONE_TO_ONE`、`MANY_TO_ONE`、`ONE_TO_MANY`、`MANY_TO_MANY`。您还可以使用 `Range.getCardinality().isMany()` 来确定属性是否为一个集合。
...
<13> `Range.asDatatype()` 方法返回实体属性的 xref:data-types.adoc[数据类型]。您可以用它格式化和解析属性值。