...
Jmix предлагает аннотации xref:entities.adoc#on-delete[@OnDelete] и xref:entities.adoc#on-delete-inverse[@OnDeleteInverse] для обработки ссылок между мягко удаленными сущностей.
