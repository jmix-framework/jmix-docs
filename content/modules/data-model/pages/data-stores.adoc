...
* `JpaDataStore` stores xref:entities.adoc#jpa[JPA entities] in a relational database. This is the main persistence mechanism in Jmix applications, and when we mention a data store in this manual, we always mean `JpaDataStore` if not explicitly stated otherwise.

* `NoopDataStore` is a stub that is used just for associating xref:entities.adoc#dto[DTO entities] with it. It has no any persistence logic.
...
[source,java,indent=0]
----
include::example$/ex1/src/test/java/sample/EntitiesTest.java[tags=custom-store]
----
+
