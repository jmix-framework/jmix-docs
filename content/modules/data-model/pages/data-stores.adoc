...
当你使用 Studio 创建一个新的 Jmix 项目时，它会有一个名为 _main_ 的数据存储用来连接到一个关系型数据库。连接参数通过以下应用程序属性指定：
...
如果需要处理多个数据库，你需要 additional data stores - 附加数据存储。

TIP: 可以使用 Studio 图形界面定义附加数据存储。

每个附加数据存储都有唯一的名称，该名称在 `jmix.core.additionalStores` 应用程序属性指定, 该属性是一个以逗号作为分隔符的列表。数据库连接的参数以数据存储的名称作为前缀。在下面的示例中，配置了一个名称为 `locations` 的附加数据存储：
...
定制数据存储可以帮助你以处理 JPA 实体相同的方式处理某些 DTO 实体 - 通过使用 `DataManager`。如果你的 DTO 实体与定制数据存储关联，则 `DataManager` 会为你代理定制数据存储的 CRUD 操作，并能处理其他实体对 DTO 的引用。
...
. 现在你可以使用 `DataManager` 保存和加载实体了，它会帮你代理定制数据库存储的 CRUD 操作：
+
[source,java,indent=0]
----
