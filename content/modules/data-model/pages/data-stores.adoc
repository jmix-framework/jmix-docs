...
Jmix provides the following two data store implementations:

* `JpaDataStore` stores xref:jpa-entities.adoc[JPA entities] in a relational database. This is the main persistence mechanism in Jmix applications, and when we mention a data store in this manual, we always mean `JpaDataStore` if not explicitly stated otherwise.

* `NoopDataStore` is a stub that is used just for associating xref:dto-entities.adoc[DTO entities] with it. It has no any persistence logic.
...
----
include::example$/ex1/src/main/java/sample/SampleDataModelApplication.java[tags=main-data-source]
----
...
In order to work with multiple databases, you need additional data stores. You can use Studio interface to define them.
...
[source,java,indent=0]
----
include::example$/ex1/src/main/java/sample/LocationsStoreConfiguration.java[tags=add-data-source]
----
...
[source,java,indent=0]
----
include::example$/ex1/src/main/java/sample/entity/Country.java[tags=store-ann]
----
...
[source,java,indent=0]
----
include::example$/ex1/src/main/java/sample/store/InMemoryStore.java[tags=custom-store]
----
...
[source,java,indent=0]
----
include::example$/ex1/src/main/java/sample/store/InMemoryStoreDescriptor.java[tags=custom-store]
----
...
[source,java,indent=0]
----
include::example$/ex1/src/main/java/sample/entity/Metric.java[tags=custom-store]
----
...
[source,java,indent=0]
----
include::example$/ex1/src/test/java/sample/InMemoryStoreTest.java[tags=custom-store]
----
+
