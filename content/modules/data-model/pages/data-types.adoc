...
Each non-reference entity attribute is associated with an implementation of the `Datatype` interface. This interface defines methods for converting attribute values to and from strings (formatting and parsing) when displaying entities in xref:jmix-ui:index.adoc[] and serializing in xref:rest:index.adoc[].
