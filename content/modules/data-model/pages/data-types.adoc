...
当 UI 组件使用一个数据类型解析字符串输入时，有可能会产生解析异常。UI 组件会处理这个异常并展示 xref:ui:vcl/components/text-field.adoc#data-type-conversion[对用户友好的信息]。这些信息在框架的消息包中通过 `databinding.conversion.error.<datatype-id>` 键值定义。例如：
