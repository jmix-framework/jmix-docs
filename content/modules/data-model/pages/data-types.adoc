...
Each non-reference entity attribute is associated with an implementation of the `Datatype` interface. This interface defines methods for converting attribute values to and from strings (formatting and parsing) when displaying entities in xref:ui:index.adoc[] and serializing in xref:rest:index.adoc[].
...
The data type of UI component can be set declaratively in screen descriptor with the help of the xref:ui:vcl/xml.adoc#datatype[datatype] attribute.

To obtain the data type programmatically from Java code, inject the `DatatypeRegistry` bean into to the controller and use it to get the `Datatype` instance, for example:
...
The default error messages are specified in the main localized message pack. There are three ways to define a custom conversion error message:

. Overriding the default message in your local message pack:
+
...
----
+
Thus, this message will be used on the application level.

. Using the `conversionErrorMessage` attribute in XML descriptor:
+
[source,xml,indent=0]
----
include::example$/ex1/src/main/resources/datamodel/ex1/screen/model/model-edit.xml[tags=conversion-error]
----

. Using the `setConversionErrorMessage()` method in a screen controller:
+
[source,java,indent=0]
----
include::example$/ex1/src/main/java/datamodel/ex1/screen/model/ModelEdit.java[tags=conversion-error]
----
