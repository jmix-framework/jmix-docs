...
|===
|[[id]]id
|A unique ID for a calendar event. It is used for identification on the client side.

|[[groupId]]groupId
|The group object that is shared among other evens. Events with the same group ID will be dragged and resized together automatically.

|[[allDay]]allDay
|Determines if the event is shown in the 'all-day' section of the relevant display mode. By default, the component considers all events as non-all-day.

The `null` value means that it is not an all-day event.

|[[startDateTime]]startDateTime
|Defines the start date-time position of the event. It has the `LocalDateTime` type, and the value should correspond to the system's default time zone. For <<EntityCalendarEvent>>, the value will be automatically transformed from the supported date-time data type.

|[[endDateTime]]endDateTime
|Defines end start date-time position of the event. It has the `LocalDateTime` type, and the value should correspond to the system's default time zone. For <<EntityCalendarEvent>>, the value will be automatically transformed from the supported date-time data type.
...
|[[overlap]]overlap
|Defines whether the event can be dragged or resized over other events, and prevents other events from being dragged/resized over this event.
...
|Defines a shared group identifier. This property limits dragging and resizing to a certain cells in component.
...
|Specifies the background color for the event. Supported values are:
...
|Specifies the border color for the event. Supported values are:
...
|[[textColor]]textColor
|Specifies the text color for the event. Supported values are:
...
In single calendar, you can display data from different data sources and services.

[[calendar-data-provider]]
=== CalendarDataProvider

`CalendarDataProvider` is a base interface for all data providers that the calendar uses.
...
In order to load data using JPQL query, you should specify an <<items-query-element>> element inside the data provider.
...
[[dayGridWeek]]
- `DAY_GRID_WEEK` - shows days of the week, which is useful for focusing on upcoming events.
[[dayGridMonth]]
...
Use the <<initialDisplayMode, initialDisplayMode>> attribute to display the day-grid modes of the `FullCalendar`:
...
Use the <<initialDisplayMode, initialDisplayMode>> attribute to display the time-grid modes of the `FullCalendar`:
...
Use the <<initialDisplayMode, initialDisplayMode>> attribute to display the list display modes of the `FullCalendar`:
...
Use the <<initialDisplayMode, initialDisplayMode>> attribute to display the multi-month display mode of the `FullCalendar`:
...
- Using the <<custom-display-modes-element>> element in an XML.
...
NOTE: The `dayCount` sets the exact number of days displayed in a custom display mode, regardless of <<weekendsVisible, weekendsVisible>> or <<hidden-days-element, hiddenDays>> properties.
...
NOTE: When a date range is specified with a `duration`, hidden days will be omitted, and the display mode will stretch to fill the missing space.
...
`FullCalendar` allows you to edit calendar events by changing the start date of an event (drag and drop) and by adjusting the event duration (resize).
To handle calendar event changes, you should add the corresponding listeners: <<EventDropEvent, EventDropEvent>> and <<EventResizeEvent, EventResizeEvent>>.
...
The event is sent by the `FullCalendar` component when the user moves a calendar event to another position.
...
To enable calendar event resizing, set the <<eventDurationEditable, eventDurationEditable>> property to `true`.
Then, `FullCalendar` will allow you changing the duration (size) of the calendar event by modifying its end date.
To resize the calendar event by modifying its start date, you should enable the <<eventResizableFromStart, eventResizableFromStart>> property:
...
This event is triggered by the `FullCalendar` component when the user resizes a calendar event.
...
The `FullCalendar` allows you to configure limitations for dragging and resizing events.
There are three options to limit calendar event changes:

- Set an <<eventConstraintGroupId, eventConstraintGroupId>> value.
If this property is set, events that are being dragged or resized must be fully contained by at least one of the events linked to the given group ID.
- Enable the default business hours using the <<eventConstraintBusinessHoursEnabled, eventConstraintBusinessHoursEnabled>> property.
Events that are being dragged or resized must be fully contained within the week’s business hours.
If <<defaultBusinessHoursEnabled, defaultBusinessHoursEnabled>> is set, the business hours are Monday to Friday, 9 AM to 5 PM.
Custom business hours are also respected.
- Set custom business hours.
As with the default business hours, events must be contained within the custom business hours.

[[event-overlap]]
=== Event Overlap

The <<eventOverlap, eventOverlap>> attribute is another type of limitation for dragging or resizing calendar events.
This feature controls whether calendar events can overlap each other while being dragged or resized.
...
For instance, if a calendar event has additional property `myEventStatus`, you can retrieve its value as follows: `stillEvent.extendedProps.myEventStatus`.

[[selection]]
== Selection

The selection feature in `FullCalendar` allows users to select multiple days or time slots by clicking or dragging their mouse.
To enable this feature set the <<selectionEnabled, selectionEnabled>> property to `true`:
...
To handle selection clearing, subscribe to the <<UnselectEvent, UnselectEvent>>.
This event is triggered by the `FullCalendar` component when a selection is cleared.
It is fired in the following cases:

- The user clicks away from the current selection.
Note that this does not happen if the <<unselectAuto, unselectAuto>> property is disabled.
- The user makes a new selection.
The event will be fired before the new selection occurs.
...
The selection constraints are similar to <<calendar-event-constraints>>, but apply to a selection.
This feature limits the selected area by the following options:
...
- The <<selectConstraintBusinessHoursEnabled, selectConstraintBusinessHoursEnabled>> property: the selection can be performed within the week’s business hours.
If <<defaultBusinessHoursEnabled, defaultBusinessHoursEnabled>> is set, the business hours are Monday to Friday, 9 AM to 5 PM.
Custom business hours are also respected.
...
The <<selectOverlap, selectOverlap>> attribute is another type of selection limitation.
This feature controls whether the selection can overlap with calendar events.
By default, this property is enabled.
...
NOTE: The end date in the example above is *excluded*.
It selects current and next days.

[[control-selection-by-a-javascript-function]]
==== Control selection by a JavaScript function

There is the ability to control selection over each cell using a JavaScript function.
For instance, the following function enables selection for only three days: yesterday, today, and tomorrow:
...
Business hours refer to the specific times during which calendar events can be scheduled or displayed.
There are two ways of configuring business hours in `FullCalendar` component:

- The <<defaultBusinessHoursEnabled, defaultBusinessHoursEnabled>> property.
- Custom list of business hours definitions.

The configuration of business hours with the attribute looks as follows:
...
image::business-hours-enabled.png[align="center"]

For fine-tuning the business hours, you can set a list of custom business hours, where you can configure different hours for different days of the week or even split hours into segments (for instance, morning and afternoon shifts).

Let's try setting up a duty schedule.
For instance, on Monday, Wednesday, and Friday, the work hours are from 8 AM to 12 PM, while on Tuesday and Thursday, they are from 12 PM to 6 PM:
...
NOTE: the `startTime` and `endTime` properties  follow the ISO-8601 duration format, which is used by the `java.time.Duration` class.

image::business-hours-custom.png[align="center"]

[[context-menu]]
== Context Menu

The context menu for the `FullCalendar` component is provided by the `FullCalendarContextMenu` class.
You can configure the content of the context menu depending on the clicked day cell or calendar event.

However, there are limitations on where the context menu can be opened:

- Context menu for calendar events: available in all display modes.
- Context menu for day cells: available in all day-grid display modes and multi-month year mode.
- Context menu for time slots: not available.
...
<2> Sets the context menu handler: this allows you to configure the menu items.
<3> Removes all menu items that the context menu contained before the current opening.
...
You can style calendar and its events with CSS.
Inspect the DOM tree of calendar using browser’s developer tools and write selectors.
For instance, the following selectors change the background color of Saturday and Sunday in day-grid modes:
...
image::styling-properties1.png[align="center"]

A calendar event has properties that can override the component color definitions:
...
image::styling-properties2.png[align="center"]

Another way to style calendar events is by assigning a class name to an event.
...
image::styling-properties3.png[align="center"]

[[class-names-generators]]
=== Class Names generators

Class name generators help generate class names for calendar UI elements, depending on the generation context.
...
To style the background and text colors, add two selectors:
...
image::styling-day-header-class-names-generator1.png[align="center"]

Refer to the <<Handlers>> section to find the full list of class name generators.

[[localization]]
== Localization

The localization can be changed locally for a single `FullCalendar` component or globally using the messages mechanism.

To configure localization of a single component consider to use component properties and the `FullCalendarI18n` object.
It contains locale-dependent properties, and some of them can affect the component behavior, such as <<firstDayOfWeek, firstDayOfWeek>>, <<direction, direction>> and other properties.
...
To globally change localization, you should add message keys from the calendar add-on to your messages bundle.
See the full list of available messages in https://github.com/jmix-framework/jmix/blob/1e61c4f49db9d31045b421a4d0f33e8137a32542/jmix-translations/content/io/jmix/fullcalendarflowui/messages.properties[jmix/fullcalendarflowui/messages.properties].

For instance, default names for months:
...
NOTE: The translation names should be in the same order because the parser expects weekdays from Sunday to Saturday and months from January to December.
