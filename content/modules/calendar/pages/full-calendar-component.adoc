...
`FullCalendar` 组件通过接口提供了丰富的自定义外观功能，包括日期、时间和文本。

=== 默认格式属性

该组件提供了一组全局的默认属性，用于定义常见元素的格式。可用于在所有日历视图上建立统一的基本格式。

* xref:defaultDayHeaderFormat[defaultDayHeaderFormat]：定义日历列标题的文本格式。

* xref:defaultDayPopoverFormat[defaultDayPopoverFormat]：定义当点击单元格中的“更多”链接时弹出的气泡窗口标题的日期格式。

* xref:defaultEventTimeFormat[defaultEventTimeFormat]：定义每个日历事项上显示的时间文本的格式。

* xref:defaultSlotLabelFormat[defaultSlotLabelFormat]：设置时段（time slot）内显示文本的格式。

* xref:defaultWeekNumberFormat[defaultWeekNumberFormat]：定义周数的显示格式，当启用了 xref:weekNumbersVisible[weekNumbersVisible] 属性时会显示周数。

默认情况下，组件在创建时会根据 `messages` 设置本地化格式。

=== 格式优先级

如果没有在显示模式级别定义更特定的显示格式，则在所有标准显示模式（`dayGridDay`、`timeGridWeek` 等）以及任何自定义模式（xref:custom-calendar-display-mode[CustomCalendarDisplayMode]）中，都使用默认格式属性。

然而，每种显示模式的属性对象默认都会明确指定自己的格式。因此，这些特定于显示模式的配置会覆盖全局默认属性的配置。

例如，在 `dayGridDay` 的属性中配置 `dayHeaderFormat` 将会覆盖该特定视图下的全局 `defaultDayHeaderFormat`。
...
该组件依赖 https://fullcalendar.io/docs/moment-plugin[moment 插件^]。因此，所有格式字符串必须遵循 https://momentjs.com/docs/#/displaying/format/[Moment.js 文档^] 中定义的格式化规则。

例如，使用 `dd` 这个标记会生成两个字母的星期几值，如 `"Mo"`；而使用 `dddd` 则会生成完整的名称，如 `"Monday"`。

[NOTE]
====
对这些属性进行更改（例如通过编程式更新 `defaultDayHeaderFormat`）在组件添加到 UI 并完成渲染后将不会生效。如需在运行时更改格式，需要重建日历的 UI 或完全重新渲染该组件。
====

=== 示例

以下示例演示了如何为不同的显示模式配置日期和时间格式。

*示例 1：周时间网格视图*

本示例配置了一个周时间网格视图（`TIME_GRID_WEEK`）中的日历。设置时段标签为24小时格式，并为日期列标题使用了自定义格式。
...
本示例配置了一个周列表视图（`LIST_WEEK`）中的日历，自定义了列表项的日期格式。
...
|[[defaultDayPopoverFormat]]defaultDayPopoverFormat
|设置点击“更多（more）”链接弹出窗口标题的日期格式。

例如，`MMM D, YY` 生成 `Sep 9, 24` 值。
...
例如，`HH:mm` 生成 `00:00`（01, 2, ... 23 : 01, 02 ... 59）。
...
|[[defaultWeekNumberFormat]]defaultWeekNumberFormat
|设置星期序号的格式，当 <<weekNumbersVisible, weekNumbersVisible>> 为 `true` 时。

例如，`[Week] w` 生成 `Week 1`（1, 2, ... 52, 53）。
...
|[[eventSingleClickThreshold]]eventSingleClickThreshold
|指定触发单击事件前的延迟（毫秒）。这个阈值用于区分单击和双击操作。
请注意，该属性仅在 Jmix 2.7.2+ 中支持。
|`250`
...
日历点击事件中包含 `MouseEventDetails`，可提供有关鼠标交互的具体信息：

* `clickCount`：连续点击次数（单击为 1，双击为 2）。请注意，该属性在 Jmix 2.7.2+ 支持。

* `button`：按下了哪个鼠标键（`LEFT`、`MIDDLE`、`RIGHT`）。

* `pageX/pageY`：点击位置相对于文档的坐标。

* 修饰键：点击过程中 `Alt`、`Ctrl`、`Meta`（macOS 上为 `Cmd`）和 `Shift` 键的状态。

|[[EventDoubleClickEvent]]EventDoubleClickEvent
|`io.jmix.fullcalendarflowui.component.event.EventDoubleClickEvent` 当双击日历事项时发送。

请注意，该属性在 Jmix 2.7.2+ 支持。

`EventDoubleClickEvent` 包含 <<EventClickEvent,MouseEventDetails>>，可提供有关鼠标交互的具体信息。
...
|`io.jmix.fullcalendarflowui.component.event.EventSingleClickEvent` 当用户点击日历事项时发送。为了与双击做区分，该时间会延迟一些发送。延迟可以通过 <<eventSingleClickThreshold,eventSingleClickThreshold>> 属性配置。

请注意，该属性在 Jmix 2.7.2+ 支持。

`EventSingleClickEvent` 包含 <<EventClickEvent，可提供有关鼠标交互的具体信息。
