...
Business hours refer to the specific times during which calendar events can be scheduled or displayed. There are two ways of configuring business hours in `FullCalendar` component:
...
Let’s try setting up a duty schedule. For instance, on Monday, Wednesday, and Friday, the work hours are from 8 AM to 12 PM, while on Tuesday and Thursday, they are from 12 PM to 6 PM:
...
The context menu for the `FullCalendar` component is provided by the `FullCalendarContextMenu` class. You can configure the content of the context menu depending on the clicked day cell or calendar event.
...
You can style calendar and its events with CSS. Inspect the DOM tree of calendar using browser’s developer tools and write selectors. For instance, the following selectors change the background color of Saturday and Sunday in day-grid modes:
...
Depending on the user's locale the `FullCalendar` besides localization of calendar terms additionally can change the text direction and first day of week.
