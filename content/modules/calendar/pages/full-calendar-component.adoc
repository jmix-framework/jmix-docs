...
- Context menu for day cells: available in all day-grid display modes and multi-month mode.
...
The `callbackDataProvider` is a data provider that loads entities using JPQL query or load delegator. It contains attributes for mapping properties from an entity to a calendar event.
...
|fetchPlan
|The fetch plan that should be used for loading entities graph.
|-
