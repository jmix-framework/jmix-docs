...
The xref:flow-ui:facets/urlQueryParameters.adoc[] facet supports URL query parameters for the xref:flow-ui:vc/components/fullCalendar.adoc[]. You can bind two properties to the URL state:

* xref:flow-ui:vc/components/fullCalendar.adoc#calendar-display-modes[Calendar display mode]. This allows you to control which view (for example, day, week, month) is displayed in the calendar.
