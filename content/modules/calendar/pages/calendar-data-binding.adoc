...
* Subscribe to the `BeforeShowEvent`.
...
Then a new `calendar` element will be generated. Configure the `id`, `height` and `width` attributes as shown below.
...
* Select the `admin` user in the *DataGrid* and click the *Edit* button. Then, fill in the *Joining Date* field, generate onboarding steps, and save the changes.
