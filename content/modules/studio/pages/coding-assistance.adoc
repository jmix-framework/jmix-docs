...
** 检测 `@OneToMany` 和 `@ManyToMany` 属性中有可能导致性能问题的集合字段的初始化。Studio 建议使用 `NotInstantiatedList` 或 `NotInstantiatedSet` 替换常规的集合初始化器。
** 检查每个实体字段是否有 JPA 注解。
...
** 检查实体是否有主键属性。
** 检测具有 UUID 特性但不使用 `@JmixGeneratedValue` 作为标识符的实体。Studio 会警告可能导致 `hashCode()` 返回常量问题。
** 检测使用 `@Basic(fetch = FetchType.LAZY)` 注解的 `@Lob` 实体属性。在 Jmix 中，懒加载不适用于基本属性，此类字段会被强制为 `EAGER`；Studio 建议移除该注解，并在不需要 LOB 属性的 fetch plan 中排除。
...
** 检查 `dataContainer` 和 `itemsContainer` 属性。
这些属性应该引用当前视图内的数据容器，或者将当前视图作为 frame 的视图内的数据容器。
** 检查 Jmix 界面描述中的下列内容：
...
** 检测列表视图的集合数据容器使用了 `_base` fetch plan，而视图的 UI 中只使用了一小部分本地属性。Studio 会建议显示定义一个 fetch plan 以减少不必要的数据库负荷。
...
** 检查视图控制器中注入的组件。
如果注入字段没有设置泛型类型，例如，使用了 `DataGrid` 而非 `DataGrid<MyEntity>`，则提供快速修复。
** 检查正确实现了事件处理器：
...
** 检测 Java 和 Kotlin 视图控制器中表格列渲染器内部 `DataManager` 和 data repository 的数据加载调用。此类调用可能导致性能问题，因为渲染器会针对表格的每一行重复执行。

** 检测包含超过三个 `ComponentRenderer` 列的表格。过多的组件渲染器会影响表格性能，因为每个渲染的单元格都会创建服务端组件。

** 高亮可疑的字段赋值，如果此时字段应该使用容器进行注入。
** 检查 GUI 组件是否通过 `ComponentsFactory.createComponent()` 创建。
** 检测 `DataManager.save()` 调用的返回结果没有分配或使用。Studio 建议使用 `DataManager.saveWithoutReload()` 避免不需要的实体加载。
...
** 检查使用了 `org.slf4j.Logger` logger，
而非 `System.out.println()`、`System.err.println()` 和 `java.lang.ThrowableprintStackTrace()`。
...
** 检查项目中是否存在某个视图。
** 检查资源角色中的实体属性策略。
** 检测行级谓词策略中的 `DataManager.load()` 调用。这种调用会导致性能问题，因为在检查实体实例的访问权限时，会多次评估谓词策略。
