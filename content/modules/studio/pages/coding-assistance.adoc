...
image::code-inspections1.png[align="center",width="972"]
...
image::red-bulb.png[align="center",width="883"]
...
image::line-markers.png[align="center",width="971"]

Using line marker, you can edit the fetch plan in the XML descriptor:

image::line-markers-fetch-plan.png[align="center",width="535"]
...
** Detect collection field initializers in `@OneToMany` and `@ManyToMany` attributes that can cause performance issues. Studio suggests replacing regular collection initializers with `NotInstantiatedList` or `NotInstantiatedSet`.
...
** Detect entities with the UUID trait that do not use `@JmixGeneratedValue` for their identifier. Studio warns about a possible constant `hashCode()` problem.
** Detect `@Lob` entity attributes annotated with `@Basic(fetch = FetchType.LAZY)`. In Jmix, lazy loading is not applied to basic attributes and such fields are forced to `EAGER`; Studio suggests removing the annotation and excluding the LOB attribute from fetch plans where it is not needed.
...
** Detect list views where a collection container uses the `_base` fetch plan while the view uses only a small subset of loaded local attributes. Studio suggests defining an explicit fetch plan to reduce unnecessary database load.
...
** Detect `DataManager` and data repository load calls inside grid column renderers in Java and Kotlin view controllers. Such calls can cause performance issues because renderers are executed repeatedly for grid rows.

** Detect grids with more than three `ComponentRenderer` columns. Too many component renderers can affect grid performance because each rendered cell creates server-side components.
...
** Detect `DataManager.save()` calls whose result is not assigned or otherwise used. Studio suggests using `DataManager.saveWithoutReload()` to avoid unnecessary entity reloading.
...
** Detect `DataManager.load()` calls inside row-level predicates. Such calls can cause performance issues because predicates can be evaluated many times while checking access to entity instances.
