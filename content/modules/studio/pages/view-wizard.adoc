...
Studio предоставляет мастер для создания xref:flow-ui:views/views.adoc[экранов] из шаблонов. Чтобы создать новый экран, нажмите *New -> View* в xref:studio:tool-window.adoc[Окне инструментов Jmix].
...
* *Entity list view*
+
Создает экран, отображающий список сущностей с возможностями фильтрации и выполнения действий. Смотрите xref:flow-ui:views/view-classes.adoc#standard-list-view[StandardListView].
+
Ключевые компоненты: xref:flow-ui:vc/components/dataGrid.adoc[dataGrid], xref:flow-ui:vc/components/button.adoc[кнопки] действий (*Create*, *Edit*, *Remove*), xref:flow-ui:vc/components/genericFilter.adoc[genericFilter].

* *Entity detail view*
+
Создает экран на основе формы для создания, просмотра и редактирования отдельных экземпляров сущностей. Смотрите xref:flow-ui:views/view-classes.adoc#standard-detail-view[StandardDetailView].
+
Ключевые компоненты: xref:flow-ui:vc/layouts/formLayout.adoc[formLayout] с привязками полей к данным, кнопки действий *OK* и *Cancel*.

* *Entity list and detail views*
+
Генерирует как экран списка, так и экран деталей в виде отдельных, но связанных экранов.

* *Master-detail view*
+
Создает комбинированный интерфейс со списком сущностей слева и формой деталей справа. Поля формы деталей заблокированы до тех пор, пока пользователь не нажмет *Create* или не выберет сущность для редактирования. Контроллер экрана является _сложным_. Он наследуется от `StandardListView`.
+
Ключевые компоненты: контейнер, содержащий как `dataGrid`, так и `formLayout`.
...
Шаблон `Blank Fragment` предназначен для создания повторно используемых xref:flow-ui:fragments/fragments.adoc[фрагментов], которые могут быть встроены в другие экраны или фрагменты.
...
Используйте xref:studio:view-designer.adoc[дизайнер экранов] для настройки фрагмента.
...
Studio заменит атрибут `layout` аннотации `@Route` во всех экранах и установит новый экран в свойстве приложения xref:flow-ui:ui-properties.adoc#jmix.ui.main-view-id[jmix.ui.main-view-id].