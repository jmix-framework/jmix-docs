...
如需为实体创建 CRUD 视图，可以在树中选中实体，通过右键菜单 *New -> View* 创建。另一个方法是点击 xref:studio:entity-designer.adoc#entity-editor[实体设计器] 顶部面板的 *Views* 按钮。
...
点击 *Finish* 按钮后，会自动创建视图的 XML 描述和控制器，并更新 xref:localization:message-bundles.adoc[消息包] 文件。如果创建了列表视图，则还会添加主菜单项。
