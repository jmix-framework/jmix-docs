...
为了方便访问，所选组件的属性划分了不同的分类。如果需要找到特定的属性，一种方法是展开相应的分类，另一种方法是使用顶部的搜索栏进行查找。可以通过 image:icons/editorConfig.svg[Categories Order] 按钮自定义分类的顺序。image:icons/collapseAll.svg[Collapse All] 和 image:icons/expandAll.svg[Expand All] 按钮可以展开或收起分类中的属性。
