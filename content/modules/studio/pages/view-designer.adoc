...
组件工具箱展示一组可用的视图元素：xref:flow-ui:vc/components.adoc[components]（UI 组件）、xref:flow-ui:vc/layouts.adoc[layouts]（布局组件）、xref:flow-ui:vc/html.adoc[HTML components]（HTML 组件）、xref:flow-ui:data-components.adoc[data components]（数据组件，包括数据容器和加载器）以及 xref:flow-ui:facets.adoc[facets]，facets 里面包括操作和对话框模式设置等。
...
[[add-picker-action]]
* 如需添加一个 xref:flow-ui:actions/entity-picker-actions.adoc[实体选择器] 或 xref:flow-ui:actions/value-picker-actions.adoc[值选择器] 操作，在界面 XML 描述或者 *Jmix UI* 层级结构面板中选择一个合适的组件然后点击 *Add → Action*。此时会显示 *Add Action* 弹窗：
+
...
[[add-action]]
* 如需添加 xref:flow-ui:actions/list-actions.adoc[列表操作]，在出图 XML 描述或者 *Jmix UI* 层级结构面板中选择一个 xref:flow-ui:vc/components/dataGrid.adoc[dataGrid] 或 xref:flow-ui:vc/components/treeDataGrid.adoc[treeDataGrid] 组件，然后点击  *Jmix UI* 组件面板中的 *Add → Action*。此时会显示 *Add Action* 弹窗：
+
...
[[add-column]]
* 如需添加 `column`，在界面 XML 描述或者 *Jmix UI* 层级结构面板中选择 `dataGrid` 或 `treeDataGrid` 组件，然后点击 *Jmix UI* 组件面板中的 *Add → Columns → Column*。此时会显示 *Add Column* 弹窗：
+
