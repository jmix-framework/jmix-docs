...
* *Show product tours* – when enabled, Studio displays interactive guides that highlight UI elements and provide step-by-step instructions for common tasks. This feature helps new users get familiar with the development environment.
+
By default, this setting is enabled for new users and disabled for existing users (who had an earlier version of Studio). As an existing user, you can enable it manually if you want to explore the tours.
