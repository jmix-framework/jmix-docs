...
You can run `gradlew` tasks for subprojects from the composite project's root directory.

First, open `settings.gradle` and find the `includeBuild` instruction for the target application. For example:

[source,groovy]
----
includeBuild 'jmix-subproject'
----

Use the project name as it appears in this instruction when you run a task. To check that Gradle recognizes `jmix-subproject` and see its available tasks, run:

[source,terminal]
----
./gradlew :jmix-subproject:tasks --all
----

For instance, to build the production JAR for this project, run:

[source,terminal]
----
./gradlew -Pvaadin.productionMode=true :jmix-subproject:bootJar
----

If the task needs artifacts from other included projects, Gradle builds them automatically.
