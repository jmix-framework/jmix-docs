...
By default, the *Composite mode* is selected. It means the menu contains items from `menu.xml` of the application, and all included add-ons as well as inherited from the framework. The inherited menu items cannot be edited.
...
After the menu item for opening a view is created, you can configure a set of xref:flow-ui:menu-config.adoc#item-properties[properties] to pass to the view’s public setters.