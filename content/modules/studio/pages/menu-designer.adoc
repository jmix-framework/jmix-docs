...
新建项目时，主菜单的 xref:ui:menu-config.adoc[结构] 保存在 `menu.xml` 文件中。Studio 提供可视化设计器用于配置主菜单。
...
* *Menu* - 包含其他菜单项的菜单。菜单的属性详细信息请参阅 xref:ui:menu-config.adoc#menu-attributes[MenuConfig]。
* *Screen* - 打开应用程序界面的菜单项。
* *Bean* - 调用 bean 方法的菜单项。
* *Class* - 调用类方法的菜单项。
* *Separator* - 菜单项的水平分隔符。仅支持对顶层菜单项分隔。参阅 xref:ui:vcl/components/app-menu.adoc[AppMenu] 组件了解详情。

关于使用界面、Bean、类菜单项的更多信息，请参阅 `MenuConfig` 的 xref:ui:menu-config.adoc#item-attributes[菜单项属性] 部分。

在打开界面的菜单项创建完成后，还支持配置一组 xref:ui:menu-config.adoc#item-properties[属性] 用于传递参数给界面的公共 setter。
