...
image::data-tree.png[align="center",width="364"]

By double-clicking, you can open the properties dialog of the data store. By default, HSQLDB is set as a database type.

[[database-properties]]
image::data-properties.png[align="center",width="666"]

The following properties are available both for the main and <<additional-data-store,additional>> data stores:
...
+
image::data-params.png[align="center",width="526"]
+
...
For the main data store, the *DB Schema Management* section contains the *Do not generate Liquibase drop changesets for unmapped tables* option.

Enable this option when your application shares the main database with other applications and Studio should not generate `dropTable` changesets for tables that are not mapped to the current data model. This option helps avoid complicated configuration of the xref:data-model:db-migration.adoc#ignoring-existing-tables[main.datasource.studio.liquibase.exclude-prefixes] property.
...
image::data-vars.png[align="center",width="666"]
...
To create an additional data store, use <<data-store-actions,an action>> from the context menu. Another way is to select one from *Advanced*:

image::new.png[align="center",width="459"]
...
image::data-schema.png[align="center",width="746"]
...
For additional data stores managed by Liquibase, the *Data Store Properties* dialog also provides the *Do not generate Liquibase drop changesets for unmapped tables* option. Enable it when the additional database is shared with other applications and Studio should ignore tables outside the current data model when generating changelogs.
...
image::data-store-unsupported.png[align="center",width="746"]
...
image::data-actions.png[align="center",width="717"]
