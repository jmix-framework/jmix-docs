...
主数据存储和 <<additional-data-store,附加数据存储>> 都有下列属性：
...
对于主数据存储，*DB Schema Management* 部分包含 *Do not generate Liquibase drop changesets for unmapped tables（不要对没有实体的表格生成 Drop 语句）* 选项。

当你的应用程与其他应用共享主数据库时，并且要求 Studio 不要对未映射到当前数据模型的表生成 `dropTable` changelog 时，需要启用该选项。该选项可以避免配置复杂的 xref:data-model:db-migration.adoc#ignoring-existing-tables[main.datasource.studio.liquibase.exclude-prefixes] 属性。
...
对于由 Liquibase 管理的附加数据存储，在 *Data Store Properties* 窗口也提供了 *Do not generate Liquibase drop changesets for unmapped tables（不要对没有实体的表格生成 Drop 语句）* 选项。当附加数据存储也与其他应用共享时，且 Studio 应该忽略当前模型不匹配的表格，请开启该选项。
...
* *New -> Liquibase Changelog* - 使用正确的名称生成一个新的空 xref:data-model:db-migration.adoc#changelogs[changelog] 文件。
...
* *Generate Liquibase Changelog* - 通过比较数据模型和数据库结构为所选择的数据存储生成更改日志文件。适用于 *Update Only* 和 *Create and Update* schema 类型的主/附加数据存储。
+
TIP: 参阅 xref:data-model:db-migration.adoc#ignoring-existing-tables[数据库结构迁移 -> 忽略已有表格] 部分了解如何在 Studio 中忽略已经存在的数据库表。
+
