...
Studio provides code snippets to improve developer experience. The *Code Snippets* tool window is available on the right side of the IDE when a Java or Kotlin file is opened in the editor. 
...
When you use code snippets that load data by query, the JPQL designer is opened automatically. The JPQL designer allows you to define JPQL queries for loading entities and scalar values from the database.

include::subscription-note.adoc[]

In the example below, the JPQL designer is appeared after dragging the *Load list of entities by query*.
