...
这个类是非持久化实体。实例中包含一个国家 2014 和 2015 年的 GDP 增长率。
...
<2> 图表从 `CollectionContainer` 定义的 `countryGrowthDc` 数据容器获取数据。名称和数值使用 `CountryGrowth` 实体的 `country`、`year2014` 和 `year2015` 属性来展示。
...
<4> `chart:legend` - `chart:serialChart` 元素的内部元素， that defines the chart legend.
...
这种图表从数据容器中获取数据，并且数据会自动更新。当数据容器中添加了新数据时，图表不会完全刷新：数据点每两秒添加一次。此方法非常有用，比如，用来创建自动更新的仪表板。
