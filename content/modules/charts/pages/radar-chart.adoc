...
下一步，在图表中添加雷达序列。在 *Jmix UI* 结构面板或视图 XML 中选择 `chart` 元素，然后点击属性面板中的 *Add* 按钮。从下拉列表中选择 *Series -> Radar*。
...
对于雷达图，可以使用在 xref:charts:bar-chart.adoc#data-binding[] 中配置的 `employeesDc` 数据容器。
