...
An instance of `DataProvider` is passed to the `withDataProvider()` method within the `DataSet.Source` attribute of chart configuration. This approach requires creating instances of `DataProvider` or `DataItem` in a screen controller.
...
=== ContainerDataItems
`ContainerDataProvider` is used to assign a xref:flow-ui:data/collection-container.adoc[CollectionContainer]  to a `Chart` component programmatically.

An instance of `ContainerDataItems` is passed to the `withDataProvider()` method.

Suppose we have a data container with a loader which will load the `VehicleCount` instances. Below is the fragment of the screen XML descriptor:
[source,xml,indent=0]
----
...
The screen controller defines the `onInit()` method that sets a data provider for the chart. The data provider is based on the `vehiclesDc` data container.
[source,java,indent=0]
----
...
These segments demonstrate how to configure a `GaugeSeries` with `dataItem` to bind series-specific data points to the `Gauge` chart.
