...
The xref:flow-ui:vc/components/chart.adoc#data-set[`dataSet`] element accepts a `DataProvider` implementation as its source. An instance of `DataProvider` is passed to the `withDataProvider()` method from the `DataSet.Source` element of chart component.
...
To bind data declaratively, use the xref:flow-ui:vc/components/chart.adoc#data-set[`dataSet`]
