...
Элемент xref:flow-ui:vc/components/chart.adoc#data-set[`dataSet`] принимает реализацию `DataProvider` в качестве источника данных. Экземпляр `DataProvider` передается в метод `withDataProvider()` из элемента `DataSet.Source` компонента графика.
...
Чтобы привязать данные декларативно, используйте элемент xref:flow-ui:vc/components/chart.adoc#data-set[`dataSet`] в файле XML-дескриптора вашего экрана и установить определенный `dataContainer` в качестве источника данных.
[source,xml,indent=0]
----
...
<div class="jmix-ui-live-demo-container">
    <a href="https://demo.jmix.ru/ui-samples/sample/gauge-series-simple" class="live-demo-btn" target="_blank">ДЕМО</a>
</div>
++++
