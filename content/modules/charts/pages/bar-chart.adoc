...
* Интегрируете график с несколькими xref:charts:chart-component.adoc#series[сериями данных]
...
В этом разделе вы создадите график, сочетающую в себе `bar` xref:chart-component.adoc#series[серию] для представления зарплат сотрудников и `line` серию для отображения общих метрик.

[[series]]
=== Настройка Серий Графика

Выберете `user-list-view.xml` в окне *Jmix* студии. Сначала щелкните по элементу `formLayout`, где будет расположена ваша диаграмма.
...
Выберите элемент `chart` в панели иерархии *Jmix UI* или в XML дескрипторе, затем нажмите кнопку *Add* на панели инспектора. Выберите *Series -> Bar* из выпадающего списка.
...
Добавьте атрибут `name` `Зарплата` в элемент `YAxis`, настройте позицию и выравнивание.

Значения зарплаты в базе данных кратные 100 000. Чтобы отобразить эти значения в масштабе, уменьшенном до сотен, добавьте элемент `axisLabel` под `YAxis` и установите атрибут `formatterFunction` следующим образом.
...
Ваша XML конфигурация для столбчатой диаграммы будет выглядеть следующим образом.
