...
* Интегрируете график с несколькими xref:flow-ui:vc/components/chart.adoc#series[сериями данных]
...
В этом разделе вы создадите график, сочетающий в себе `bar` xref:flow-ui:vc/components/chart.adoc#series[серию] для представления зарплат сотрудников и `line` серию для отображения общих метрик.
