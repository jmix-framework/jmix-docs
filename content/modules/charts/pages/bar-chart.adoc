...
В этом разделе вы создадите график, сочетающий в себе `bar` xref:chart-component.adoc#series[серию] для представления зарплат сотрудников и `line` серию для отображения общих метрик.
...
Выберите элемент `chart` в панели иерархии *Jmix UI* или в XML, затем нажмите кнопку *Add* на панели инспектора. Выберите *Series -> Bar* из выпадающего списка.
...
Создайте новый xref:flow-ui:data/data-containers.adoc[`collectionContainer`] под элементом `data` с именем `employeesDc` и соответствующим JPQL-запросом:
