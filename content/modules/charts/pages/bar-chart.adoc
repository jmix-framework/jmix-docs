...
* Integrate a chart with multiple xref:flow-ui:vc/components/chart.adoc#series[series]
...
You will integrate a chart component that combines a `bar` xref:flow-ui:vc/components/chart.adoc#series[series] to represent employees' salaries and a `line` series to display their total assessment scores.
...
To include the `line` series in your chart, choose the `series` element and pick *Add -> Line*. Add a name property for both chart types: `Salary` and `AssessmentTotal` respectively.
