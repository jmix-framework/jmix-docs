...
`ContainerDataProvider` is used to assign a xref:ui:data/collection-container.adoc[CollectionContainer] to a `Chart` component.
...
This option is useful for quick prototyping of charts that are not bound to a data container. The `chart:data` element and its nested item elements enable you to set key-value pairs of data directly in the XML descriptor of the chart, 示例：
