...
你可以根据需要自定义 *Widget editor（部件编辑）* 对话框。

用一个示例了解如何对部件编辑器进行自定义，我们添加一个 `EntityPicker` 组件。然后，该组件选中的实体将作为部件参数使用。

首先，使用 Studio 创建一个空的界面 xref:ui:using-fragments.adoc[fragment]。在 xref:studio:tool-window.adoc#creating-new-elements[Jmix 工具窗口中]，点击 *New -> Screen*，然后选择 *Screen Fragment* 模板。设置界面描述和控制器的名称。
...
----
<1> 自定义部件编辑器的 id 在 `@DashboardWidget` 注解定义。
<2> 部件参数从编辑器中传入。
