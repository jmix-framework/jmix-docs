...
[TIP]
====
如需通过 Jmix Studio 添加一个 `action`，在 *Jmix UI* 结构面板或视图 XML 中选择组件，然后点击属性面板中的 xref:studio:screen-designer.adoc#add-action[Add->Action]。
====
...
任务渲染器由一个回调函数定义 - `taskRendererFunction`。

`taskRendererFunction` 支持自定义任务在看板中的渲染外观。

该回调函数带 2 个参数：

* `task`（HTMLElement）。 任务的 HTML 元素。
* `data`（Object）。任务的数据对象，包含任务的所有属性。
...
参阅 https://www.htmlelements.com/docs/kanban-api/#toc-ontaskrender_any[Kanban JavaDoc^] 查看完整的 API。

[[columnHeaderRendererFunction]]
=== 列表头渲染器

列表头渲染器由一个回调函数定义 - `columnHeaderRendererFunction`。

`columnHeaderRendererFunction` 支持自定义看板列表头的外观。

该回调函数带 3 个参数：

* `header`（HTMLElement）。 列表头的 HTML 元素。
* `data`（Object）。列数据对象，包含列的属性。
* `dataField`（String）。该列的数据字段名称（匹配列定义中的 `dataField` 属性）。
...
参阅 https://www.htmlelements.com/docs/kanban-api/#toc-oncolumnheaderrender_any[Kanban JavaDoc^] 查看完整的 API。

[[columnFooterRendererFunction]]
=== 列脚注渲染器

脚注渲染器由一个回调函数定义 - `columnFooterRendererFunction`。

`columnFooterRendererFunction` 支持自定义看板列脚注的渲染方式。列脚注通常用于显示每列的汇总、统计信息或操作按钮。
...
* `footer` (HTMLElement)。表示列脚注的 HTML 元素。
* `data` (Object)。列数据对象，包含列属性和汇总信息。
* `dataField` (String)。与此列关联的数据字段名称（匹配 `dataField` 属性）。
...
参阅 https://www.htmlelements.com/docs/kanban-api/#toc-oncolumnfooterrender_any[Kanban JavaDoc^] 查看完整的 API。
...
|`false`
|[[taskDueDateVisible]]taskDueDateVisible
|如果为 `true`，显示任务的结束图标。参阅 <<propertyDueDate, dueDate>>。
|`false`
|[[taskPosition]]taskPosition
...
xref:flow-ui:vc/common-handlers.adoc[通用事件和处理器] 对所有组件都是一样的配置。
下面是 `kanban` 的特殊事件和处理器：
...
`swimlanes` 元素可用于定义泳道，可以显式设置值或者使用枚举设置。泳道是将任务水平划分的横向条纹（责任区域），使用泳道时无法使用子列。
