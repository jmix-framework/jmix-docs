...
* `color` - String type. The string value must be formatted as a CSS color.
...
* `tags` - String type. The string value must be a comma-separated sequence of tags without spaces.
...
The dialog window that appears will show all views that display the `KanbanTask` entity. Select only `KanbanTask.detail` view:
...
To map an entity attribute to a Kanban card field, you need to add a xref:flow-ui:vc/components/kanban.adoc#propertiesMapping[propertiesMapping] element to the `kanban` component.
...
Press *Ctrl/Cmd+S* and switch to the running application. The appearance of the cards will change.
