...
Define the `text` attribute for generating an xref:studio:entity-designer.adoc#creating-instance-name[instance name].
...
In the dialog that opens, select the `KanbanTask` entity. The new `collectionContainer` element will be added to both the *Jmix UI* hierarchy panel and the XML.
...
If you want to display a Kanban on the view, you should include the xref:flow-ui:vc/components/kanban.adoc[kanban] UI component in the XML descriptor.
...
To display the `Kanban` board component on the view, you need to define xref:flow-ui:vc/components/kanban.adoc#column[columns]. To do this, select `kanban` in the *Jmix UI* hierarchy panel or in the XML descriptor, then click the *Add* button in the component inspector panel. From the drop-down list, choose *Columns -> Column*.
...
To provide the ability to create, edit and remove tasks, you need to add xref:flow-ui:vc/components/kanban.adoc#actions[actions] to the component.
...
Click the *OK* button. The `action` elements will be added in both the *Jmix UI* hierarchy panel and in the XML. Next, to use these actions, you need to create a buttons panel.
...
To enable automatic saving of the xref:flow-ui:data/data-context.adoc[DataContext], you can use the xref:flow-ui:vc/components/kanban.adoc#autoSave[autoSave] attribute.
...
[NOTE]
====
Remove the `readOnly="true"` attribute from the `loader`; otherwise, changes will not be saved to the database.
====
...
Kanban supports event handling. For more information on available events, please refer to the xref:flow-ui:vc/components/kanban.adoc#handlers[handlers] section.

To enable editing a Kanban task card by double-clicking, you need to implement a xref:flow-ui:vc/components/kanban.adoc#KanbanTaskDoubleClickEvent[KanbanTaskDoubleClickEvent] handler.
