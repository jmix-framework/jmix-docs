...
Если вы хотите отобразить Kanban на экране, вам нужно добавить UI компонент xref:flow-ui:vc/components/kanban.adoc[kanban] в XML-дескриптор.
...
Чтобы отобразить на экране компонент `Kanban`, вам необходимо определить xref:flow-ui:vc/components/kanban.adoc#column[колонки]. Для этого выберите `kanban` в панели иерархии *Jmix UI* или в XML-дескрипторе, затем нажмите кнопку *Add* на панели инспектора компонентов. В выпадающем списке выберите *Columns -> Column*.
...
Чтобы предоставить возможность создавать, редактировать и удалять задачи, вам необходимо добавить xref:flow-ui:vc/components/kanban.adoc#actions[действия] в компонент.
...
Чтобы включить автоматическое сохранение xref:flow-ui:data/data-context.adoc[DataContext], можно использовать атрибут xref:flow-ui:vc/components/kanban.adoc#autoSave[autoSave].
...
Kanban поддерживает обработку событий. Для получения дополнительной информации о доступных событиях обратитесь к разделу xref:flow-ui:vc/components/kanban.adoc#handlers[Обработчики].

Чтобы включить редактирование карточки задачи `KanbanTask` двойным щелчком, необходимо реализовать обработчик события xref:flow-ui:vc/components/kanban.adoc#KanbanTaskDoubleClickEvent[KanbanTaskDoubleClickEvent].
