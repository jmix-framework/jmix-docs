...
The Kanban board component supports dynamic changes to the display and visibility settings of columns. To enable this feature, you need to set the xref:flow-ui:vc/components/kanban.adoc#columnActionsEnabled[columnActionsEnabled] attribute. This attribute adds a column visibility control button to the header. To be able to collapse a column, you need to set the xref:flow-ui:vc/components/kanban.adoc#collapsible[collapsible] attribute.
...
Press *Ctrl/Cmd+S* and switch to the running application. The appearance of the Kanban board will now change:
...
The `kanban` component supports custom JavaScript renderers to style and display the task cards and columns. See xref:flow-ui:vc/components/kanban.adoc#renderers[renderers] section for more information.
