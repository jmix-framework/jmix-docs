...
在同一 Tomcat 服务中部署多个 Jmix WAR 会导致 JMX bean 的命名冲突异常：

----
Caused by: javax.management.InstanceAlreadyExistsException: jmix.ui:type=TemporaryStorage
----

要解决这个问题就需要给每个 WAR 配置一个独有的 JMX 域名。可以按如下配置：
...
有关在 Jmix 应用程序前配置 *Apache HTTPD* 或 *nginx* 的说明，请参考 https://vaadin.com/docs/v25/flow/production/reverse-proxy[Vaadin 反向代理指南^]。该指南包含转发头信息、WebSocket 支持以及针对不同部署布局的路径重写等详细信息。
