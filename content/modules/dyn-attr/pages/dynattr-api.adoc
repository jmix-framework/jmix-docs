...
+
* `CategoryAttribute` 定义某个分类中的动态属性。每个属性使用指定类型定义单一字段。必需的 `code` 字段包含属性的系统名称。`name` 字段包含可读的属性名称。`code` 字段是一个技术性的标识符，只能包含拉丁字母、数字和 `_` 符号。第一个字符必须是小写拉丁字母。
...
TIP: 如果使用 xref:data-access:data-repositories.adoc[data repositories]，也可以在 repository 的方法上使用 `@QueryHints` 和 `@QueryHint` 注解设置同样的 hint。示例请参阅 xref:data-access:data-repositories.adoc#jmix-specific-parameters[Jmix 的特定参数]。
...
动态属性可以使用 Groovy 脚本进行验证、重新计算和加载选项。这些脚本会执行任意 Groovy 代码。可以将 `jmix.dynattr.ui.groovy-enabled` 应用程序属性设置为 `false` 来禁用代码执行。

当禁用时，脚本不会在运行时执行，而会记录警告，并且相应的脚本编辑器在 UI 中变为只读。
