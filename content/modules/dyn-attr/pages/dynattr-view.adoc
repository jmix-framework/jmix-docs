...
当项目中安装了动态属性扩展组件，会在创建新的编辑界面和浏览界面时为界面 XML 描述自动添加 <<using-dynamicattributes-facet,dynamicAttributes>> facet。

在界面展示动态属性的另一个方法就是使用 <<dynamic-attributes-panel,DynamicAttributesPanel>>。
...
然后，需要在 *Category attribute editor（分类属性编辑）* 界面的 xref:dyn-attr:dynattr-managing.adoc#visibility[可见性] 标签页选择此界面。
...
+
* 在界面中添加 `dynamicAttributesPanel` 可视化组件：
+
[source,xml,indent=0]
----
...
最后，你也可以手动在界面中添加动态属性。在 `facets` 中添加 <<using-dynamicattributes-facet,dynamicAttributes>> facet，然后在 UI 组件绑定属性时，使用 `+` 前缀指定动态属性：
