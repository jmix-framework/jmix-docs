...
[NOTE]
====
Dynamic attributes are not supported for entities with xref:studio:entity-designer.adoc#entity-id-type[composite primary keys].

Such entities are not displayed in the entity type selector when creating a dynamic attribute category.
====
...
[NOTE]
====
The *Code* field can contain only Latin letters, digits, and the `_` symbol. The first character must be a lowercase Latin letter.

The code is a technical identifier of the dynamic attribute. Use the *Name* field and the *Localization* tab for user-visible and localized attribute names.
====
...
Within the *Calculated values and options* tab, you can use the *Attribute depends on* field to specify dynamic attributes that the current attribute depends on.

Only other dynamic attributes of the same entity can be selected in this field. Regular entity attributes cannot be selected.

Whenever one of the selected dynamic attributes is changed, either the script for computing the current attribute value or the script for determining the <<options-type-list,list of valid values>> is re-evaluated.
...
[NOTE]
====
If a dynamic attribute uses a lookup component and is selected in a filter condition, the current entity instance is not available when loading options. In this case, the `entity` variable is `null` in a Groovy options loader script and is not passed to SQL and JPQL options loader scripts.
====
