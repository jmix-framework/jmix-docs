...
[NOTE]
====
动态属性不支持具有 xref:studio:entity-designer.adoc#entity-id-type[组合主键] 的实体。

创建动态属性分类时，不会在实体类型选择器中显示该类型的实体。
====
...
[NOTE]
====
*Code* 字段只能包含拉丁字母、数字和 `_` 符号。第一个字符必须是小写拉丁字母。

*Code* 是动态属性的技术性标识符。使用 *Name* 字段和 *Localization* tab 中的设置配置用户可见和本地化的属性名称。
====
...
在 *Calculated values and options* 标签页中，可以使用 *Attribute depends on* 字段来指定当前属性所依赖的动态属性。

此字段中只能选择同一实体的其他动态属性。无法选择常规实体属性。

每当所选的动态属性之一发生变更时，用于计算当前属性值的脚本或用于确定 <<options-type-list,有效值列表>> 的脚本都会被重新计算。
