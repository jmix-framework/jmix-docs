...
// todo flowui
// 下面示例中，为表格组件的 `docLink` xref:ui:vcl/components/table.adoc#table-column-generation[生成] 列创建了 `WebdavDocumentLink` 组件：
//
// [source,xml,indent=0]
// ----
...
`WebdavDocumentUploadField` 的 API 与 xref:flow-ui:vc/components/fileStorageUploadField.adoc[] 类似。

// [[using-filemultiuploadfield]]
// == 使用 FileMultiUploadField
//
// 下面示例展示如何使用通过 xref:flow-ui:vc/components/file-multi-upload-field.adoc[FileMultiUploadField] 上传的文件创建 WebDAV 文档：
//
// [source,java,indent=0]
// ----
