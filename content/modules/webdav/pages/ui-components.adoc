...
Studio 会在 *Jmix UI* 层级面板和 XML 中添加新元素。与其他 xref:flow-ui:vc/components.adoc[UI 组件] 一样，也可以配置各种属性，例如 xref:flow-ui:vc/common-attributes.adoc#id[id]、xref:flow-ui:vc/common-attributes.adoc#height[height]、xref:flow-ui:vc/common-attributes.adoc#width[width] 等。
...
[[how-to-inject]]
可以使用 *Jmix UI* 层级面板中的 *Inject to Controller* 操作将 UI 组件注入控制器：
