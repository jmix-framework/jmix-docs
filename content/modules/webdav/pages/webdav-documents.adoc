...
* *Upload*: Selects and uploads files to the system.
* *Download*: Downloads the most recent or a previous version of a document.
* *Manage Versions*: Opens the xref:version-control.adoc#webdav-document-versions[WebDAV document versions] view.
* *Enable/Disable Versioning*: Controls versioning for a specific document.
+
[[lock-unlock]]
* *Lock*: Locks the selected document so that other users cannot modify it.
+
