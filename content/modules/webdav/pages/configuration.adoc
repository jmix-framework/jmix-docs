...
Advanced use cases like opening WebDav files in LibreOffice or Firefox may not work with self-signed certificates. Trusted
...
. Install `mkcert` from the following repo:
https://github.com/FiloSottile/mkcert[https://github.com/FiloSottile/mkcert]

. You can build it from source (requires golang to be installed) and copy binary to `/usr/local/bin` or use binaries provided by vendor.

. After having `mkcert` in path you can generate and install root certificate
