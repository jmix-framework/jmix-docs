...
You can open screens from the xref:jmix-ui:screens/root-screens.adoc#main-screen[main screen], by a xref:jmix-ui:actions/standard-actions.adoc[standard action] when working with browse and edit entity screens or programmatically from another screen. This section describes how to open screens programmatically.
...
----
<1> Injects xref:jmix-ui:vcl/components/label.adoc[Label] component.
...
In most cases you can open editor screens using xref:jmix-ui:actions/standard-actions.adoc[standard actions] such as xref:jmix-ui:actions/standard-actions/create-action.adoc[CreateAction]. Let's look at the examples when you can use the `ScreenBuilders` API directly to open a screen from xref:jmix-ui:actions/base-action.adoc[BaseAction] or xref:jmix-ui:vcl/components/button.adoc[Button] handler.

The default editor screen is determined by the following procedure:

. If an editor screen annotated with xref:jmix-ui:screens/annotations.adoc#primary-editor-screen[@PrimaryEditorScreen] exists, it is used.
...
Let's look at some examples of working with lookup screens. As with editor screens, you mostly open such screens using xref:jmix-ui:actions/standard-actions.adoc[standard actions] such as xref:jmix-ui:actions/standard-actions/entity-lookup-action.adoc[EntityLookupAction]. The examples below shows the usage of `ScreenBuilders` API and can be useful if you don’t use standard actions.

The default lookup screen is determined by the following procedure:

. If a lookup screen annotated with xref:jmix-ui:screens/annotations.adoc#primary-lookup-screen[@PrimaryLookupScreen] exists, it is used.
...
If you are opening a screen using a standard action such as xref:jmix-ui:actions/standard-actions/create-action.adoc[CreateAction], use its `screenConfigurer` handler to pass parameters via screen public setters.
...
In the opened `FancyMessageScreen` screen, the options can be obtained in xref:jmix-ui:screens/screen-events.adoc#init-event[InitEvent] and xref:jmix-ui:screens/screen-events.adoc#after-init-event[AfterInitEvent] handlers:
...
If you are opening a screen using a standard action such as xref:jmix-ui:actions/standard-actions/create-action.adoc#create-action[CreateAction], use its `screenOptionsSupplier` handler to create and initialize the required `ScreenOptions` object.
