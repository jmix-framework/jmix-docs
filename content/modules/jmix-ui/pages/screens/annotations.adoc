...
* `@UiDescriptor` annotation connects the screen controller to the xref:jmix-ui:screens/descriptors.adoc[XML descriptor]. The value of the annotation specifies the path to the descriptor file. If the value contains a file name only, it is assumed that the file is located in the same package as the controller class.
...
* `@EditedEntityContainer` annotation specifies a xref:jmix-ui:data/data-containers.adoc[data container] that contains the edited entity.
+
