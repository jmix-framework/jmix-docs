...
* A mixin interface can subscribe to xref:jmix-ui:screens/screen-events.adoc[screen events].
* A mixin can save some state in the screen if needed.
* A mixin can obtain screen components and infrastructure beans like xref:jmix-ui:dialogs.adoc[Dialogs], xref:jmix-ui:notifications.adoc[Notifications], etc.
...
The next mixin helps to establish master-detail relationships between data containers. Normally, you have to subscribe to `ItemChangeEvent` of the master container and set a parameter to the detail’s loader, as described in xref:jmix-ui:data/data-examples.adoc#dependencies-between-data-components[Dependencies Between Data Components]. The mixin will do it automatically if the parameter has a special name pointing to the master container.
...
<1> Subscribes to xref:jmix-ui:screens/screen-events.adoc#init-event[InitEvent].
...
<8> Subscribe to xref:jmix-ui:screens/screen-events.adoc#before-show-event[BeforeShowEvent].
