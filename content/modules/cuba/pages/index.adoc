...
+
* If it's required to support traits for Jmix users, follow the steps below:

** Enable traits in the Studio’s xref:studio:entity-designer.adoc[Entity Designer].
** Update column names of corresponding properties:
...
[NOTE]
====
* Changes related to the `SEC_REMEMBER_ME`, `SEC_SCREEN_HISTORY`, and `SEC_SEARCH_FOLDER` tables are safe (but still can be ignored).
