...
The Search add-on adds full-text search functionality to your application. You can configure indexing of entities and uploaded files, use search APIs, and add the xref:flow-ui:vc/components/searchField.adoc[searchField] and xref:flow-ui:vc/components/fullTextFilter.adoc[fullTextFilter] UI components. Search results are filtered based on the xref:security:index.adoc[data access] permissions of the current user.

The add-on utilizes https://www.elastic.co[Elasticsearch^] or https://opensearch.org[OpenSearch^] as its search engine. It can work with only one of them at the same time.
...
NOTE: On AWS, use Amazon OpenSearch Service. The legacy Elasticsearch service is not supported.
