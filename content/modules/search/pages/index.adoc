...
Search add-on uses Elasticsearch Java API client v8.10.4 that supports ES 8.10+ (but also should support older versions till the oldest version is still supported by Elastic - 7.17.x)

If case of OpenSearch add-on uses Java API client v2.10.3.
