...
该扩展组件使用 https://www.elastic.co[Elasticsearch（ES）^] 作为搜索引擎。关于其内部原理和索引构建的更多信息请参阅 Elasticsearch https://www.elastic.co/guide/en/elasticsearch/reference/current/documents-indices.html[文档^]。
...
默认情况下，搜索扩展组件使用的 ES 为 7.11 及以上版本。如需使用 7.11 以下的版本，设置 xref:search-properties.adoc#jmix.search.restHighLevelClientApiCompatibilityModeEnabled[jmix.search.restHighLevelClientApiCompatibilityModeEnabled] 应用程序属性为 `false`。
