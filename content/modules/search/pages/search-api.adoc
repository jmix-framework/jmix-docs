...
The Search add-on includes the following components:

* *Startup Index Synchronizer* - automatically validates and recreates irrelevant or missing indexes on application startup. Starts a background process that enqueues all instances of entities related to created indexes.
...
* `processNextBatch(int batchSize)` - retrieves the next batch (with size equal to `batchSize`) of items from the indexing queue and processes them.
* `processEntireQueue()` - retrieves all items from the indexing queue and processes them.
...
* `SearchResult search(SearchContext searchContext, String searchStrategy)` - performs searching in search indexes according to the provided `SearchContext` and search strategy name.
...
* `synchronizeIndexSchema` - synchronizes the schema of the index related to a provided entity. This may cause deletion of this index with all data, depending on the schema management strategy.
* `synchronizeIndexSchemas` - synchronizes schemas of all search indexes defined in the application. This may cause deletion of indexes with all their data, depending on the schema management strategy.
