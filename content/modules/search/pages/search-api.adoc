...
* 搜索开始前 - 检查 xref:security:resource-roles.adoc#entity-policy[实体策略]，并排除禁止访问实体的相关索引。xref:security:resource-roles.adoc#entity-attribute-policy[属性策略] 也会生效：搜索的查询中仅包含当前用户允许读取的实体属性，不会返回用户无权读取的属性。如果当前用户没有对请求实体的任何访问权限，搜索结果返回空。
