...
This guide provides instructions on getting started with Search in your application.
...
. xref:search:index.adoc#connecting-to-search-engine-service[Configure] the connection based on the service location.
...
Next, xref:search:index-definitions.adoc#creating-index-definition[create an Index Definition] - an interface outlining which entities and attributes are to be indexed. You can create it manually as shown below, or use the Studio wizard from the *Search Indexes* node in the *Jmix* tool window.

Suppose we intend to search orders based on their number, product name, customer status, or customer last name. Within the Index Definition, we will define a method to specify the required attributes.
...
== Configure Index Naming
By default, the index name is formed as `<prefix><entity_name>`. The default prefix is `search_index_`.
...
As an alternative, you can specify the full index name in the `indexName` attribute of `@JmixEntitySearchIndex`.

[[configure-indexing-queue]]
== Configure Indexing Queue Processing

The Jmix application monitors data modifications but does not automatically synchronize them with the search engine service. To ensure regular index updates, simply add the xref:quartz:index.adoc[] add-on to the project. The Search add-on will use Quartz for scheduled processing of the indexing queue with default configuration.
