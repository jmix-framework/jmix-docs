...
Jmix application tracks data changes but does not pass them to Elasticsearch automatically. To update Elasticsearch indexes regularly, include Quartz in your project as described in the xref:ROOT:quartz-setup.adoc[] section. As soon as the application has Quartz on the classpath, the Search add-on will use it for periodic processing of the indexing queue.
