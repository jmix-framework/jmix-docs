...
Add-on allows to configure index settings including analysis.
...
* Create spring bean that implements `OpenSearchIndexSettingsConfigurer` (or `ElasticsearchIndexSettingsConfigurer` in case of Elasticsearch usage).
...
* *Common* - allows to configure settings that will be applied to all indexes. Can be acquired via `context.getCommonSettingsBuilder()`.
* *Entity specific* - allows to configure settings that will be applied only to index for specific entity.
...
Use `.analysis()` to start building chain that allows to create custom analyzers, tokenizers, etc.
