...
The Message Templates add-on provides a flexible and efficient way to manage and use reusable message templates within your Jmix application. It allows developers to define templates with placeholders that can be dynamically populated with data at runtime. This add-on simplifies the creation of personalized messages such as emails or in-app notifications.

[[overview]]
== Overview

This add-on allows you to create and customize message templates that combine static content with dynamic parameters. You can design templates in the visual HTML designer, which provides a wide range of HTML elements.
...
The https://github.com/GrapesJS/grapesjs[GrapesJS^] web component provides a visual editor for crafting HTML templates. You can construct templates using pre-designed HTML blocks by dragging them from the palette onto the canvas.
...
* *Apache FreeMarker Integration*: Uses the https://freemarker.apache.org/[Apache FreeMarker Template Engine^] for placeholders, expressions, loops, and conditional logic in templates.
...
* *Template Preview*: Validate templates in the UI before use by rendering them with current parameter values and checking the result immediately.
...
The *Message Templates* add-on integrates with other Jmix add-ons:
