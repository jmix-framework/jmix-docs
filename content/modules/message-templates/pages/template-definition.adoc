...
** `booking.startDate`: This is a variable reference. In this case, it's assumed that you have a variable named `booking` that is an object (JPA entity) and that object has a property called `startDate`. FreeMarker uses dot notation to access object properties.
