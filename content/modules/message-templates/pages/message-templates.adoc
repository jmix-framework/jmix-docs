...
* *Group* (optional): Assign the template to a specific group for better organization. Groups can be configured in the *Template groups* view.
...
The data used to populate these placeholders can be derived from various sources, such as <<parameters,parameters>> defined within the template itself and parameters passed programmatically from the application code.
...
The button panel at the top of the HTML editor provides essential tools for working with templates:
...
You can preview a message template from the Editor or the *Message templates* list view. If the template defines <<parameters,parameters>>, Jmix first opens the parameter input dialog, then generates the rendered result in a preview window.
...
After you click the *Create* button, the *Template parameter* dialog is displayed.
