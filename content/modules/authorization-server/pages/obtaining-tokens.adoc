...
To exchange this authorization code for an access token, you need to execute another HTTP request to the token endpoint of the Jmix application. We will use the `curl` command-line tool for this. Use the `code_verifier` value from the initial page of https://oauthdebugger.com[^].
...
By default, if xref:configuration.adoc#jmix.authserver.use-in-memory-authorization-service[jmix.authserver.use-in-memory-authorization-service] is `false`, the tokens are serialized and stored in the database. In case of Password grant, the token contains the `User` entity, so you need to define a bean of the `JdbcOAuth2AuthorizationServiceJsonMapperCustomizer` type in your application and implement it as follows:
...
NOTE: Since Jmix 3.0, the customizer receives a `JsonMapper.Builder`, so mixins and other JSON mapping settings should be registered on the builder.

If you get `java.lang.IllegalArgumentException: The class ... is not in the allowlist` when using a token, it means that your user entity contains fields of types not supported by the token serializer by default. Then create a mixin class extending `DefaultOAuth2TokenUserMixin` and use it in the `JdbcOAuth2AuthorizationServiceJsonMapperCustomizer` bean. For example:
...
To exchange the refresh token for a new access token, you need to execute an HTTP request to the token endpoint of the Jmix application. We will use the `curl` command-line tool for this.
