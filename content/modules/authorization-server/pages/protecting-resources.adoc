...
当应用程序中添加了 xref:rest:index.adoc[通用 REST] 扩展组件后，所有的 REST 端点都自动会受到授权服务组件的保护。也就是说，访问任何 REST 端点时，都需要先获取 access token 并在 HTTP 请求的 `Authorization` 请求头中使用。请求头的值必须带有 `Bearer`，后面跟上 token 值。示例：
