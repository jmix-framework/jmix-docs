...
The backend of a Jmix application can have different clients, for example, Jmix UI or REST API. Each client has its own standard authentication mechanism, such as the UI login view and xref:rest:security/authentication.adoc[REST authentication].
