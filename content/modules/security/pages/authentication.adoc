...
Jmix 直接使用 Spring Security https://docs.spring.io/spring-security/reference/servlet/authentication/index.html[servlet 认证^]，因此 i，如果对这个框架比较熟悉，可以很容易地扩展或覆盖 Jmix 提供的内置标准认证机制。
...
* `getUser()` 以 https://docs.spring.io/spring-security/site/docs/current/api/org/springframework/security/core/userdetails/UserDetails.html[UserDetails^] 返回当前认证用户。可以将其转换为项目中定义的 xref:users.adoc#entity[用户实体]。

* `getAuthentication()` 返回当前线程中设置的 https://docs.spring.io/spring-security/site/docs/current/api/org/springframework/security/core/Authentication.html[Authentication^] 对象。可以使用它来获取当前用户的 https://docs.spring.io/spring-security/site/docs/current/api/org/springframework/security/core/GrantedAuthority.html[授权^] 集合。在标准的 Jmix 安全实现中，授权集合包含分配给用户的 xref:resource-roles.adoc[资源] 和 xref:row-level-roles.adoc[行级] 角色对象。
...
NOTE: `CurrentAuthentication` 只是 https://docs.spring.io/spring-security/site/docs/current/api/org/springframework/security/core/context/SecurityContextHolder.html[SecurityContextHolder^] 的包装器，因此能与所有 Spring Security 机制兼容。
...
* UI xref:ui:screens/root-screens.adoc#login-screen[登录界面]
* REST xref:rest:security/authentication.adoc[认证]
...
<4> `InteractiveAuthenticationSuccessEvent` 包含用户实体。
