...
Jmix directly uses Spring Security https://docs.spring.io/spring-security/reference/servlet/authentication/index.html[servlet authentication^], so if you are familiar with this framework, you can easily extend or override the standard authentication mechanism provided by Jmix out-of-the-box.
...
* UI xref:jmix-ui:screens/root-screens.adoc#login-screen[login screen]
