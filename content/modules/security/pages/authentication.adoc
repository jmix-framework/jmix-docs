...
TIP: См. также xref:users.adoc#password-change-at-next-login[Смена пароля при следующем входе] чтобы обязать пользователей изменить первоначальный или временный пароль после входа в систему.
