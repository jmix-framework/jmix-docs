...
* `getAuthentication()` 返回当前线程中设置的 {spring-security-api}/org/springframework/security/core/Authentication.html[Authentication^] 对象。可以使用它来获取当前用户的 {spring-security-api}/org/springframework/security/core/GrantedAuthority.html[授权^] 集合。在标准的 Jmix 安全实现中，授权集合包含分配给用户的 xref:resource-roles.adoc[资源] 和 xref:row-level-roles.adoc[行级] 角色对象。
...
后端 Jmix 应用程序可以支持不同的客户端，例如，Jmix UI 或 REST API。每个客户端都有自己的标准认证机制，例如，UI 登录视图和 xref:rest:security/authentication.adoc[REST 认证]。
