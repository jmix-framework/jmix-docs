...
Each Jmix application by default contains a security configuration that sets up access to internal Vaadin endpoints and delegates all requests authorization to Jmix and Vaadin mechanisms (grants access to views using view controller annotation or by analyzing user's resource roles). `SecurityFilterChain` created by this configuration always has the lowest priority and is always invoked last. The <<static-public-resources>> and <<custom-endpoints-security>> sections below explain how to define your own `SecurityFilterChain` to serve public resources and protect custom endpoints.
...
To define custom security rules for public resources, declare a new `SecurityFilterChain` bean. It is important that the order of this bean must be lower than the order of `SecurityFilterChain` beans provided by Jmix framework.
...
----
include::example$/security-ex1/src/main/java/com/company/demo/security/api/SimplePublicSecurityConfiguration.java[tags=whole-class]
----

The configuration above grants access to all resources that match the `/public/**` pattern.

To completely ignore Spring Security rules for static public resources, define a `WebSecurityCustomizer` bean and register your public URL patterns with the `ignoring()` method of `WebSecurity`:

[source,java,indent=0]
----
include::example$/security-ex1/src/main/java/com/company/demo/security/IgnoringSecurityConfiguration.java[]
----

The configuration above gives public access to all files located inside the `src/main/resources/static/images` directory of your project

TIP: See how to serve static content in the https://docs.spring.io/spring-boot/docs/current/reference/html/web.html#web.servlet.spring-mvc.static-content[Spring Boot documentation^].
...
----
<1> `JmixSecurityFilterChainOrder.CUSTOM` order is lower than any other Jmix security filter chain order, so your filter chain will be used before any filter chain from Jmix.
...
If you come across a 401 Unauthorized or 403 Forbidden HTTP error or have any other issues related to endpoint security, then Spring Security {spring-security-doc}/servlet/architecture.html#servlet-logging[logging^] will likely help you understand what is going on.

To enable logging, add the following application property with `DEBUG` or `TRACE` value to the `application.properties` file:
