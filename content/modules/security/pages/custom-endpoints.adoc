...
// todo restore
// В разделе <<static-public-resources>> приведена информация о том, как полностью обойти Spring Security для некоторых URL.
...
//
// Чтобы полностью игнорировать правила Spring Security для статических общедоступных ресурсов, создайте класс конфигурации, унаследованный от `FlowuiVaadinWebSecurity`, и зарегистрируйте шаблоны общедоступных URL с помощью метода `ignoring()` класса `WebSecurity`:
//
...
// ----
//
// Пример конфигурации выше предоставляет публичный доступ ко всем файлам, расположенным в директории `src/main/resources/public/images` вашего проекта (подробнее о статических ресурсах см. https://docs.spring.io/spring-boot/docs/current/reference/html/web.html#web.servlet.spring-mvc.static-content[документацию Spring Boot^]).
...
Данная конфигурация предоставляет доступ к всем запросам для эндпойнтов, соответствующих шаблону `/public-api/**`.
