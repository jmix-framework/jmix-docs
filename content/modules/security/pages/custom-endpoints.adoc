...
Каждое приложение Jmix по умолчанию содержит конфигурацию безопасности, которая настраивает доступ к внутренним эндпойнтам Vaadin и передает все запросы на авторизацию механизмам Jmix и Vaadin (предоставляет доступ к экранам с помощью аннотации на классе экрана или анализа ресурсных ролей пользователя). `SecurityFilterChain`, созданный этой конфигурацией, всегда имеет наименьший приоритет и всегда вызывается последним. Секции <<static-public-resources>> и <<custom-endpoints-security>> ниже объясняют, как определить ваш собственный `SecurityFilterChain` для публичных ресурсов и защиты пользовательских эндпойнтов.
...
Для определения кастомных правил безопасности для публичных ресурсов объявите новый бин `SecurityFilterChain`. Важно, чтобы порядок этого бина был меньше, чем у бинов `SecurityFilterChain`, предоставленных фреймворком Jmix.
...
Чтобы полностью игнорировать правила Spring Security для статических общедоступных ресурсов, определите бин `WebSecurityCustomizer`, и зарегистрируйте шаблоны общедоступных URL с помощью метода `ignoring()` класса `WebSecurity`:
...
Пример конфигурации выше предоставляет публичный доступ ко всем файлам, расположенным в директории `src/main/resources/static/images` вашего проекта.

TIP: Подробнее о статических ресурсах см. https://docs.spring.io/spring-boot/docs/current/reference/html/web.html#web.servlet.spring-mvc.static-content[документацию Spring Boot^].
