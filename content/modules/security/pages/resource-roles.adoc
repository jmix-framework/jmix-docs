...
UI component policies define permissions on UI components like fields, buttons, and actions in specific views. This policy support is provided by the separate `jmix-uiconstraints` add-on. See the xref:uiconstraints:index.adoc[UI Constraints] section for more information.
