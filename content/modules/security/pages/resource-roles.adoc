...
You can create resource roles at design time using annotated Java interfaces (see xref:studio:role-designer.adoc#resource-role-designer[role designer] in Studio) or at runtime using UI screens available at *Administration -> Resource roles*.
...
[source,java,indent=0]
----
include::example$/ex1/src/main/java/security/ex1/service/NotificationService.java[tags=access-manager;specific-permission]
----
