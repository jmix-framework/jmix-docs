...
You can create resource roles at design time using annotated Java interfaces (see xref:studio:role-designer.adoc#resource-role-designer[role designer] in Studio) or at runtime using UI views available at *Administration -> Resource roles*.
