...
// ------------------
|Backoffice UI data-aware components
|Yes
|Yes
|- <<ui-components,(4)>>
|- <<ui-components,(4)>>
...
In this part, we briefly explain how Jmix authorization works. This information will be helpful if you want to extend or replace the standard system of permissions based on roles and policies.
...
* When `DataManager` loads an entity, it creates an instance of `CrudEntityContext`, sets the `entityClass` attribute, and invokes `AccessManager.applyConstraints()`. After that, it analyzes the value of `CrudEntityContext.readPermitted` attribute and whether continues loading the entity or aborts the operation.
