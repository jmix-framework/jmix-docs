...
[source,java,indent=0]
----
include::example$/ex1/src/main/java/security/ex1/service/NotificationService.java[tags=access-manager;metadata;crud-permission]
----
...
[source,java,indent=0]
----
include::example$/ex1/src/main/java/security/ex1/service/NotificationService.java[tags=access-manager;window-config;all-permitted-screens]
----
...
- `UiShowScreenContext` - 用于检查 xref:security:resource-roles.adoc#screen-policy[界面策略] 的 context。
- `FlowuiShowViewContext` - 用于检查 Flow UI 视图策略的 context。
- `UiMenuContext` - 用于检查 xref:security:resource-roles.adoc#menu-policy[菜单策略] 的 context。
- `FlowuiMenuContext` - 用于检查 Flow UI 菜单策略的 context。
