= 用户

[[entity]]
== 用户实体

Jmix 应用程序的用户使用 `User` 类定义，该类在使用 xref:studio:project.adoc#creating-new-project[Studio] 创建新项目时自动生成。是一个实现了 `JmixUserDetails` 接口的 xref:data-model:entities.adoc#jpa[JPA 实体]，接口中包含框架需要的一些方法：

* `getUsername()` 返回唯一用户名。
* `getPassword()` 返回加密（hash）后的用户密码。
* `isEnabled()`、`isAccountNonExpired()`、`isAccountNonLocked()`、`isCredentialsNonExpired()` 表示用户是否可以登录系统。
* `getAuthorities()`、`setAuthorities()`，在登录时框架用于为用户设置权限。

用户保存在应用程序的主数据库中。默认情况下，`User` 实体和对应的数据库表具有以下属性：

* `id`、`version` 是标准的主键和乐观锁属性。
* `username`、`password`、`enabled` 保存 `JmixUserDetails` 接口中方法返回的值。
* `email`、`firstName`、`lastName` 保存用户的其他属性。

你可以定义应用程序所需的任意数量的附加属性，例如，`department` 或 `position`。

[[management]]
== 用户管理

一个新项目包含 `010-init-user.xml` xref:data-model:db-migration.adoc#changelogs[数据库迁移] 脚本，该脚本创建用户名/密码为 `admin` / `admin` 的用户，通过给用户分配 `system-full-access` xref:resource-roles.adoc[角色] 授予对系统的完全访问权限。

一个新项目还包含用于管理用户的 UI 视图，请查看主菜单的 *Application* -> *Users（用户）*。这些视图支持创建、编辑和删除用户，以及更改和重置用户密码。如需为用户分配角色，点击用户列表视图中的 *Role assignments（角色分配）* 按钮。

NOTE: 新应用程序项目中包含了 `ui-minimal` 角色，该角色授予登录 UI 和打开主视图的权限。新用户需要分配此角色才能与通过 UI 进行交互，否则他们将无法登录。

TIP: 参考 https://github.com/jmix-framework/jmix-samples-2/tree/main/user-registration[User Registration^] 示例项目了解如何实现用户的自注册。

[[password-change-at-next-login]]
=== 下次登录时更改密码

Jmix 可以要求用户在登录后立即更改密码。该行为由 `User` 实体中使用 `@PasswordChangeRequired` 注解的布尔属性控制。

[source,java]
----
@PasswordChangeRequired
@Column(name = "PASSWORD_CHANGE_REQUIRED")
private Boolean passwordChangeRequired = false;
----

如果已认证用户的 `@PasswordChangeRequired` 注解属性设置为 `true`，Jmix 会在登录后强制打开更改密码的标准对话框。此时，用户必须保存新密码后才能关闭对话框。

适合以下场景：

* 对于新创建的用户，如果必须在首次登录时更改初始密码，该注解应设置为 `true`。
* 使用 *Reset Password* 操作并勾选 *Require change at next logon* 时，生成临时密码并强制用户在登录后更改密码。

用户成功更改密码后，Jmix 会自动将该注解属性设置为 `false`。

[[built-in]]
== 内置用户

任何具有标准安全子系统的 Jmix 应用程序都有两个内置的用户对象：

* _Anonymous_ 匿名用户，对应于无认证信息的用户，仅可以使用登录视图。通过自定义该对象，可以在用户未登录时授予某些权限。

* _System_ 系统管理员用户，xref:authentication.adoc#system[系统授权] 机制需要此用户对象。当没有真正的用户与应用程序交互时使用该用户，例如，当应用程序启动时，或者当计划任务执行业务方法时。

内置用户对象没有保存在数据库，而是在应用程序启动时由 `DatabaseUserRepository` 类创建。可以在此类的 `initAnonymousUser()` 和 `initSystemUser()` 方法中对这两个用户进行定制化。

.DatabaseUserRepository.java
[source,java,indent=0]
----
include::example$/security-ex1/src/main/java/com/company/demo/security/DatabaseUserRepository.java[tags=init-built-in-users]
----

<1> 默认情况下，系统用户分配了 `system-full-access` 角色，因而具有所有权限。
<2> 匿名用户没有分配任何角色，因而没有权限。
+
TIP: 为了简化给匿名用户分配访问特定视图的权限，可以使用 xref:flow-ui:views/view-annotations.adoc#common-annotations[@AnonymousAllowed] 注解。

[[user-substitution]]
== 替代用户

系统管理员可以赋予用户替代另一个用户的能力。替代意味着用户拥有被替换用户的全部 xref:security:resource-roles.adoc[资源] 和 xref:security:row-level-roles.adoc[行级] 角色。例如，如果张三代替李四，那么当他以张三的身份登录应用程序时，具有李四的角色。

如需实际了解这些功能，请按照下列步骤：

. 以 `admin` 用户登录，创建至少一个用户，并分配 `UI: minimal access` 角色。
. 在 *Users（用户）* 表选择 `admin`，然后点击 *Additional（更多） -> User substitution（用户替代）*。可以看到 `admin` 已经能替代的用户列表。
. 创建一个 `admin` 能替代的用户添加至列表中。
. 如果项目的主视图包含 xref:flow-ui:vc/components/userMenu.adoc[] 组件，并且该组件带有 xref:flow-ui:actions/user-menu-actions.adoc#sec_userMenuSubstituteUser[sec_userMenuSubstituteUser] 操作，则组件的下拉菜单会显示一个替代用户的菜单项。
. 如果项目的主视图包含 `userIndicator` 组件，当前用户的名称成为一个显示替代用户的下拉列表。
. 当在 `userMenu` 或 `userIndicator` 中选择一个替代用户时，工作区将发生变化，就像再次以该用户身份重新登录。但所有审计功能仍将记录 `admin`（登录用户）。

`CurrentUserSubstitution` bean 可用于获取当前替代的用户、已认证用户或有效用户（在发生替代时为替代用户，否则为认证用户）。

示例：

[source,java,indent=0]
----
include::example$/security-ex1/src/main/java/com/company/demo/view/main/MainView.java[tags=user-substitution]
----

* `CurrentAuthentication.getUser()` 方法总是返回认证用户。

* `CurrentAuthentication.getAuthentication().getAuthorities()` 返回有效用户的授权。即，在替代发生时，这些授权与认证用户的授权不一致。

[[user-substitution-event]]
=== 用户替换事件

如需在代码中跟踪用户的替换，可以监听 `io.jmix.core.security.event.UserSubstitutedEvent` 事件。该事件在当前安全上下文中完成用户替换认证时发出。

[source,java]
----
include::example$/security-ex1/src/main/java/com/company/demo/security/UserSubstitutionEventListener.java[tags=user-substitution-event]
----