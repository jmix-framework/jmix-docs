...
Predicate policies defined as Groovy scripts in runtime roles execute arbitrary Groovy code. You can disable this execution by setting the `jmix.security.data.groovy-enabled` application property to `false`.

When disabled, predicate policies that use Groovy scripts evaluate to deny access, a warning is logged, and the predicate script field becomes read-only in the UI.
