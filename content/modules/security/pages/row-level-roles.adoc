...
在运行时角色中定义的 Groovy 脚本谓词策略会执行任意 Groovy 代码。可以将 `jmix.security.data.groovy-enabled` 应用程序属性设置为 `false` 禁用脚本执行。

禁用时，使用 Groovy 脚本的谓词策略会评估为拒绝访问，会记录警告，并且谓词脚本字段在 UI 中变为只读。
