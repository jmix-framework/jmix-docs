...
可以在设计时使用带注解的 Java 接口（参考 Studio 的 xref:studio:role-designer.adoc#row-level-role-wizard[行级角色创建向导]）或在运行时使用 *Administration（管理） -> Row-level roles（行级角色）* 的 UI 界面创建行级角色。
