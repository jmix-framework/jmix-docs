...
可以在设计时使用带注解的 Java 接口或在运行时使用 *Administration（管理） -> Row-level roles（行级角色）* 的 UI 界面创建行级角色。
...
可以使用 `current_user_<attribute>` 这种模式的参数从当前登录用户的属性中获取值。例如，`current_user_username` 参数从用户的 `username` 属性中获取值。
