...
You can create row-level roles at design time using annotated Java interfaces (see xref:studio:role-designer.adoc#row-level-role-wizard[row-level role wizard] in Studio) or at runtime using UI screens available at *Administration -> Row-level roles*.
