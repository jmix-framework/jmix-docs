...
//The first language in the list becomes the default one. The messages in default language are located in the `messages.properties` file without a language suffix. The framework always looks up for messages in this file if it didn't find the message in the specific `messages_<language>.properties` file. It helps translate your application to additional languages gradually: you can just configure a new locale and start translating in the new `messages_<language>.properties` file. Not yet translated messages will be loaded from the default `messages.properties` file.
