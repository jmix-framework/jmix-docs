...
Let’s move further and create the `Talk` entity and link it to the `Speaker` class. Fields specification table follows. The talk end time will be a calculated value, being the start date and time plus duration. `End time` field is created as a regular field, we will create calculation algorithm a bit later.
