...
In the <<qs-creating-data-model,previous section>>, for the `Speaker` entity, we have defined `lastName` as an entity's instance display name. Now we can define a method that will use a speaker's full name to display them in the UI.
