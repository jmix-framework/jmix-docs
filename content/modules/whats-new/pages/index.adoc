...
`GenericFilter` 组件（XML 元素：`genericFilter`）支持用户通过运行时创建的任意条件对数据进行过滤。当前版本仅支持属性条件。

经典 UI 过滤器的其他功能（JPQL、自定义条件、分组、保存过滤器配置等）将在下一版本提供。
...
. `++jmix.liquibase.*++` 前缀更名为 `++main.liquibase.*++`，目的是为了遵循数据源属性的命名模式（例如，`main.datasource.url`，这里 `main` 是数据源名称）。如果我们引入了一个名为 `second` 的数据源，Liquibase 配置属性则是 `second.liquibase.*`。
