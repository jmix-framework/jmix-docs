...
Теперь вы можете создавать xref:security:row-level-roles.adoc[роли уровня строк] и их политики используя мастер.

Чтобы создать роль, нажмите *New* -> *Row-level Role* в панели инструментов Jmix и введите параметры роли в диалоге:

image::rl-role-1.png[align="center", width="635"]

Studio создаст аннотированный класс роли. Политики можно добавить с помощью действия *Add Policy*:
...
Чтобы устранить возможное влияние на производительность системы, можно настроить кэш для сущности пользователя, например:
[source,properties]
----
...
Такой кэш теперь автоматически создается в новых проектах.
...
Если ваш проект находится в пути, содержащем пробелы, на Jmix 1.4.0 компиляция widgetset может вызывать ошибку. На данный момент единственным решением является перемещение проекта в путь без пробелов. Проблема будет решена в патче 1.4.1, см. https://github.com/jmix-framework/jmix/issues/1162[#1162^].
