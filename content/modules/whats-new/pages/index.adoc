...
Вы можете отключить встроенные фетч-планы для универсального REST в вашем приложении, используя свойство приложения xref:rest:app-properties.adoc#jmix.rest.inline-fetch-plan-enabled[jmix.rest.inline-fetch-plan-enabled].
...
Если вы хотите вернуть хранение токенов в памяти, как в предыдущих версиях, установите свойство приложения xref:authorization-server:configuration.adoc#jmix.authserver.use-in-memory-authorization-service[jmix.authserver.use-in-memory-authorization-service] в `true`.
