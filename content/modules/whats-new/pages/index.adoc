...
Компонент xref:flow-ui:vc/components/html.adoc[html] позволяет вставлять в экраны произвольное HTML-содержимое.
...
Фасет xref:flow-ui:facets/timer.adoc[timer] предназначен для выполнения кода экрана через определенные интервалы времени. Он работает в потоке, обрабатывающем события пользовательского интерфейса, и может обновлять компоненты экрана.
