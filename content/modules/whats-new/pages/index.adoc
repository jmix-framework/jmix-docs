...
* https://vaadin.com/docs/latest/components/tabs/#tab-sheet[TabSheet^] -> `JmixTabSheet` (`tabSheet` в XML)
+
[source,xml]
----
<tabSheet width="100%">
...
* https://vaadin.com/docs/latest/components/multi-select-combo-box[MultiSelectComboBox^] -> `JmixMultiSelectComboBox` (`multiSelectComboBox` в XML)
+
[source,xml]
----
...
* https://vaadin.com/docs/latest/components/upload[Upload^] -> `FileStorageUploadField` (`fileStorageUploadField` в XML) и `FileUploadField` (`fileUploadField` в XML)
+
[source,xml]
----
...
* https://vaadin.com/docs/latest/create-ui/standard-html[Image^] -> `JmixImage` (`image` в XML)
+
[source,xml]
----
<image id="image"
...
* Многие компоненты теперь поддерживают https://vaadin.com/docs/latest/components/tooltip[Tooltip^], например:
+
[source,xml]
----
<textField id="nameField" property="name">
