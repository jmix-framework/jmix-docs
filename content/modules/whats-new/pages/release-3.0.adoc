...
=== Переход на экран редактирования из колонки DataGrid

Компонент xref:flow-ui:vc/components/dataGrid.adoc[dataGrid] теперь предоставляет два предопределенных XML-рендера для перехода на экран редактирования экземпляра сущности прямо из колонки:

* `detailLinkRenderer` отображает ссылку,
* `detailButtonRenderer` отображает кнопку, которая открывает экран редактирования через навигацию или в диалоговом окне.

Теперь действия *Открыть* и *Редактировать* можно добавить в колонку таблицы без написания собственного рендера.
