...
* For distributed tracing, switch to the `spring-boot-starter-opentelemetry` starter and update the OTLP export properties. See xref:features:observability.adoc#tracing[Observability / Tracing] for the updated setup.
...
* The *AI Agents Toolkit* action available in the *Settings* menu of the *Jmix* tool window allows you to automatically install agent skills, instructions, and MCP servers from the https://github.com/jmix-framework/jmix-agent-toolkit[Jmix Agent Toolkit^] GitHub repository into the current project. It supports the following agents: Claude Code CLI, Codex, OpenCode, Junie.

* *Do not generate Liquibase drop changesets for unmapped tables* option has been added to the *Data Store Properties* dialog.

* Inherited attributes can now be displayed in the xref:studio:entity-designer.adoc[entity designer].

* Studio now helps you create xref:data-access:custom-update-services.adoc[custom update services] and use them in views. When you create a JPA entity, select the *Create Update Service* checkbox in the *New JPA Entity* dialog to generate a service class. Later, when you create a view for an entity that has an update service, select the *Use Update Service* checkbox in the *Create Jmix View* dialog to delegate saving and removing to that service automatically.

* If the xref:multitenancy:index.adoc[] add-on is included in the project, the new xref:data-model:entities.adoc#tenant-id-trait[Has Tenant ID] trait allows you to add a `@TenantId` attribute to an entity.

* xref:search:index-definitions.adoc#creating-index-definition[Search] index definitions and xref:quartz:index.adoc[Quartz] jobs are displayed in the *Jmix* tool window and can be quickly created from templates.
...
The xref:flow-ui:vc/components/dataGrid.adoc[dataGrid] and xref:flow-ui:vc/components/treeDataGrid.adoc[treeDataGrid] components now provide two predefined XML renderers for opening entity detail views directly from a column:

* xref:flow-ui:vc/components/dataGrid.adoc#detail-link-renderer[`detailLinkRenderer`] renders a link to the detail view,
* xref:flow-ui:vc/components/dataGrid.adoc#detail-button-renderer[`detailButtonRenderer`] renders a button that opens the detail view by navigation or in a dialog.
...
The new xref:flow-ui:views/view-templates.adoc[] feature lets you generate standard list and detail views from templates declared on entity metadata, instead of creating them manually at design time. You annotate an entity class with `@ListViewTemplate` and `@DetailViewTemplate`, and the framework generates the corresponding views, routes, and menu items at startup.
...
The xref:ROOT:app-properties.adoc#jmix.core.unsafe-runtime-features-enabled[jmix.core.unsafe-runtime-features-enabled] property is a global kill switch: when it is `false`, all the guarded features below are disabled regardless of their own properties. Each feature can also be turned off individually:

* xref:ROOT:app-properties.adoc#jmix.core.hot-deploy-enabled[jmix.core.hot-deploy-enabled] – loading classes from the file system by Hot Deploy.
* xref:ROOT:app-properties.adoc#jmix.core.trigger-files-enabled[jmix.core.trigger-files-enabled] – bean invocation through trigger files.
...
See https://github.com/jmix-framework/jmix/issues/5287[#5287^] for more information.

[[application-information-file]]
=== Application Information File

Jmix now creates the `app-info/app-info.json` file in the xref:ROOT:app-properties.adoc#jmix.core.conf-dir[application configuration directory] for development tools. The file contains the URL, active Spring profiles, and startup time of the running application. Use the xref:ROOT:app-properties.adoc#jmix.core.application-info-file-enabled[jmix.core.application-info-file-enabled] property to disable its creation.
...
Learn more in xref:security:users.adoc#password-change-at-next-login[Password Change at Next Login].

[[message-template-preview]]
=== Message Template Preview

The xref:message-templates:message-templates.adoc[Message Templates] add-on now lets users validate templates directly in the UI before using them. The new *Preview* action helps check rendering with actual parameter values, and exposes FreeMarker or missing-parameter problems earlier.
...
The `AppSettingsEntity` base class of the Application Settings add-on now has `tenantId` attribute to support multitenancy (see https://github.com/jmix-framework/jmix/issues/5144[#5144^]). If you have application entities extending this base class, you need to create the `TENANT_ID` column for this attribute in your database.
...
See xref:studio:data-stores.adoc#data-store-actions[Data Store Actions] for more information about data store actions in Studio.

[[jasperreports-dependencies]]
=== JasperReports Dependencies

The xref:reports:index.adoc[] add-on does not bring JasperReports dependencies transitively anymore. If you use xref:reports:creation/templates.adoc#template_jasper[JasperReports templates], add the following dependencies to your `build.gradle` explicitly:
...
Theme variants support now depends on the application theme. Some available only for Lumo, some only for <<aura-theme, Aura>>, and most work with both themes. Supported variants are listed in each component's *Theme Variants* section. See xref:flow-ui:vc/components/markdownEditor.adoc#theme-variants[markdownEditor Theme Variants] for an example.
...
See https://github.com/jmix-framework/jmix/issues/4371[#4371^] for more information.
...
If you use xref:studio:openapi-client.adoc[], update the configuration in `build.gradle` as follows:
...
See https://github.com/jmix-framework/jmix/issues/5290[#5290^] for more information.
