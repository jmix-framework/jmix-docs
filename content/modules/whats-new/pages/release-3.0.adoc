...
=== DataGrid Detail Renderers

The xref:flow-ui:vc/components/dataGrid.adoc[dataGrid] component now provides two predefined XML renderers for opening entity detail views directly from a column:

* `detailLinkRenderer` renders a link,
* `detailButtonRenderer` renders a button that opens the detail view by navigation or in a dialog.

This makes it easier to add *Open* or *Edit* actions to a grid column without writing a custom renderer.
