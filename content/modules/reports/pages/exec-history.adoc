...
报表执行历史默认不启用，可以设置 xref:configuration.adoc#jmix.reports.historyRecordingEnabled[jmix.reports.historyRecordingEnabled] 应用程序属性为 `true` 启用。
...
该机制提供了将输出文档，即报表结果文件，保存到 xref:files:index.adoc[文件存储] 的功能。该功能会消耗一定的文件存储空间，需要单独配置，并且默认是关闭的。如需开启，可以设置 xref:configuration.adoc#jmix.reports.saveOutputDocumentsToHistory[jmix.reports.saveOutputDocumentsToHistory] 应用程序属性为 `true`：
...
. 设置报表历史清理的应用程序属性：
+
* xref:configuration.adoc#jmix.reports.historyCleanupMaxDays[jmix.reports.historyCleanupMaxDays] - 默认 730 天。
* xref:configuration.adoc#jmix.reports.historyCleanupMaxItemsPerReport[jmix.reports.historyCleanupMaxItemsPerReport] - 默认 1000。
