...
The Reports add-on provides a report execution history management mechanism with the following features:
...
Execution history is also recorded even for reports that are not yet saved to the database but are launched from the report detail view (by clicking the *Run* button).

[[history_output_documents]]
== Output Documents

The mechanism provides the ability to save output documents - report result files - in the xref:files:index.adoc[file storage]. This feature consumes file storage disk space; it is configured separately and is disabled by default. To enable it, define the xref:configuration.adoc#jmix.reports.save-output-documents-to-history[jmix.reports.save-output-documents-to-history] application property to `true`:
