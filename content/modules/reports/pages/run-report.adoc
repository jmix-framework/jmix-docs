...
[[run_report_action]]
* `io.jmix.reportsui.action.list.RunReportAction` – a xref:jmix-ui:actions/standard-actions.adoc[standard action] that displays the list of all available reports. It should be defined for a `Button` or a list component (`Table`, `DataGrid`, etc.).
+
...
* `io.jmix.reportsui.action.list.ListPrintFormAction` – a xref:jmix-ui:actions/standard-actions.adoc[standard action] for printing reports for entity instances associated with a list component (`Table`, `DataGrid`, etc.).
+
