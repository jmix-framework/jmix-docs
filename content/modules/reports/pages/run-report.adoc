...
在任意视图中，用户都可以使用特定操作及相关的按钮或组件右键菜单运行报表。此时，除了用户角色外，还将检查报表在此视图的 xref:creation/permissions.adoc[可用性]。
...
`io.jmix.reportsflowui.action.RunReportAction` 是一个 xref:flow-ui:actions/standard-actions.adoc[标准操作]，可以在当前视图或列表组件运行报表。可以为 `button` 或列表组件（xref:flow-ui:vc/components/dataGrid.adoc[], xref:flow-ui:vc/components/treeDataGrid.adoc[]）定义该操作。
...
`io.jmix.reportsflowui.action.RunListEntityReportAction` 是一个 xref:flow-ui:actions/standard-actions.adoc[标准操作]，用来打印列表组件（xref:flow-ui:vc/components/dataGrid.adoc[], xref:flow-ui:vc/components/treeDataGrid.adoc[]）中的实体集合报表。
...
`io.jmix.reportsflowui.action.RunSingleEntityReportAction` 是与实体详情视图关联的操作。该操作仅选择外部参数类型是 *Entity* 或 *List of entities* 的报表，并且参数实体类型要与编辑的实体类型一样。如果只有一个这样的报表，则会立即调用该报表。如果有多个，则会显示一个列表供用户选择。
...
TIP: 可以使用 xref:jmix:studio:code-snippets.adoc[] 功能生成使用 API 运行报表的代码。
