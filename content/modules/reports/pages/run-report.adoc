...
+
To view the report execution history set the xref:configuration.adoc#reporting.executionHistory.enabled[reporting.executionHistory.enabled] application property to `true` in the **Administration > Application Properties** screen.
+
Below is an example of using the declarative action for the `GroupTable`:
+
[source, xml]
...
To add the cancel option, set the xref:configuration.adoc#reporting.useBackgroundReportProcessing[reporting.useBackgroundReportProcessing] property in the *Administration > Application Properties* screen.
...
You can also set the processing timeout using the xref:configuration.adoc#reporting.backgroundReportProcessingTimeoutMs[reporting.backgroundReportProcessingTimeoutMs] property:
...
The report execution history is disabled by default and can be enabled by setting the xref:configuration.adoc#reporting.executionHistory.enabled[reporting.executionHistory.enabled] application property to `true`. You can do it in the *Administration > Application Properties* screen.
...
Reports with output types xref:templates.adoc#template_chart[chart], xref:templates.adoc#pivotTable_output[pivot table], and xref:templates.adoc#table_output[table] do not have result files, so the execution history of such reports does not save any output documents.
...
** xref:configuration.adoc#reporting.executionHistory.cleanup.days[reporting.executionHistory.cleanup.days] – 730 days by default.
** xref:configuration.adoc#reporting.executionHistory.cleanup.itemsPerReport[reporting.executionHistory.cleanup.itemsPerReport] – 1000 by default.
