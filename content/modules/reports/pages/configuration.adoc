...
Включает исполнение Groovy в отчетах. Это затрагивает запросы наборов данных на Groovy, источники данных JSON на Groovy, форматирование значений на Groovy, скрипты преобразования параметров, кастомные шаблоны на основе Groovy, HTML-шаблоны на Groovy и URL кастомных шаблонов, генерируемые с помощью Groovy.

Если задано значение `false`, перечисленные возможности возвращают дефолтное значение вместо исполнения Groovy, при каждом таком возврате в лог записывается предупреждение, а соответствующие редакторы в UI становятся доступными только для чтения.
