...
Defines whether the TAB key should be handled as the `\t` symbol instead of focus navigation in Script fields of the report detail view.

Default value: `false`

[[jmix.reports.client.show-report-table-view-in-menu]]
=== jmix.reports.client.show-report-table-view-in-menu

Defines whether the *Reports -> Show tables* menu item is displayed.

The view can still be used to show table output after running a report. Set this property to `true` if users need direct access to the table output view from the application menu.

Default value: `false`.

[[jmix.reports.client.table-output-open-mode]]
=== jmix.reports.client.table-output-open-mode

Defines how table output reports are opened in `ReportTableView`.

Possible values are:

* `DIALOG` - opens the table output in a modal dialog.
* `NAVIGATION` - opens the table output as a full page by its route.
...
Enables Groovy execution in reports. This affects Groovy data set queries, Groovy JSON data sources, Groovy value formatting, parameter transformation scripts, Groovy-based custom templates, Groovy HTML templates, and Groovy-generated custom template URLs.

When set to `false`, the listed features return a fallback value instead of executing Groovy, a warning is logged on each fallback, and the related editors become read-only in the UI.
...
Sets the fonts directory for converting reports from HTML to PDF.
...
[[jmix.reports.spreadsheet.output-open-mode]]
=== jmix.reports.spreadsheet.output-open-mode

Defines how the xref:run-report.adoc#spreadsheet-viewer-open-mode[spreadsheet viewer] of the xref:report-spreadsheet:index.adoc[] add-on is opened when a report result is shown in it. `DIALOG` shows the viewer as a modal dialog, `NAVIGATION` opens it as a full page by its route.

This property is provided by the *Report Spreadsheet* add-on and has no effect unless the add-on is installed.
...
Specifies the file extensions for which files are displayed in a browser tab rather than downloaded automatically. In this case, the tab URL contains the `VAADIN/dynamic/resource/<random_id>` postfix.
...
* Configure the <<jmix.reports.office-path,jmix.reports.office-path>> application property and set it to the path to *LibreOffice*, for example:
...
* Configure the <<jmix.reports.office-path,jmix.reports.office-path>> application property and set it to the path to *LibreOffice*:
+
[source, properties,indent=0]
----
