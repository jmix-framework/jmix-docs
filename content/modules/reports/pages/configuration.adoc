...
启用报表中的 Groovy 执行功能。包括：Groovy 数据集查询语句、Groovy JSON 数据源、Groovy 值格式化、参数转换脚本、基于 Groovy 的自定义模板、Groovy HTML 模板以及由 Groovy 生成的自定义模板 URL。

当设置为 `false` 时，上述功能返回一个备用值（fallback）而不会执行 Groovy，每次 fallback 时都会记录警告，并且相关编辑器在 UI 中变为只读。
...
启用 DOCX 报表模板中参数值的多行字符串处理。启用时（`true`），参数值中的换行符（`\n`、`\r\n`）会在生成的 DOCX 文档中渲染为实际的换行。此功能仅在使用 xref:creation/templates.adoc#template_doc[DOCX 模板] 时生效。
...
设置需要在浏览器直接显示而不自动下载的文件类型。对这些文件，浏览器的 URL 包含 `VAADIN/dynamic/resource/<random_id>` 后缀。
...
* 安装软件。
* 配置 <<jmix.reports.office-path,jmix.reports.office-path>> 应用程序属性并设置为 *LibreOffice* 的路径，示例：
