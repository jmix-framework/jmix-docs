...
. Report xref:creation/parameters[parameters].
+
...
To create the report, add a band and select *Crosstab* band orientation on the xref:creation/details.adoc[Report structure] tab of the report editor. This orientation automatically adds three datasets to the band:
...
. Report data xref:creation/details.adoc[structure]. There are three datasets:
+
...
----
+
* The `bi_master_data` dataset returns names and identifiers of the library departments selected by the user as an xref:creation/parameters[external report parameter]:
+
[source, sql,indent=0]
