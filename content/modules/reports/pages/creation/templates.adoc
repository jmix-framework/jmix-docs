...
The band dataset fields are placed in the template using strings in `++${field_name}++` format, where `++field_name++` is the corresponding band field name. 示例：
...
You may use variables for convenience, 示例：
...
----
** `fs://` prefix allows inserting an image by specifying `FileDescriptor` ID. 示例：
+
[source, html,indent=0]
----
...
* Explicitly specify fonts in HTML templates, 示例：
...
* `dataset` –  gets the sub-datasource from the main data source that can be used, for example, in tables or in subreports as subDataset. This method searches among the children of the root band for the band with specified name and creates a new data source with this band data as a new root. 示例：
+
[source, xml,indent=0]
----
...
* `bitmap` – transforms the given byte array into `ByteArrayInputStream` and can be used for embedding images into the report. 示例：
...
The data can be retrieved from the datasource using the following syntax: `$F{<field name>}`. 示例：
