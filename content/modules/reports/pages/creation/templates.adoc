...
[[add_template]]
== Add template
Report templates can be added in Report Editor. You can open Report Editor by click menu item *Reports -> Reports*, then click *create* button, Report Editor screen will show, in this screen, click *Templates* tab:

.Templates tab
image::template-list.png[align="center"]

One of them must be selected as the default template in the *Report structure* tab. 
...
* *Is custom* – the flag to use custom formatting logic not covered by any provided formatter. When it is checked, you need to define 2 more fields:

** *Defined by* – the way the custom template is defined: by a <<template_custom,class>>, script or <<exec_report_on_external_server,URL>>.
...
+
* In the xref:appendix/app-properties.adoc#reporting.curl.path[reporting.curl.path] application property in the `app.properties` file of the *core* module, specify the path to the command line *curl* tool.
...
^1^ - OpenOffice/LibreOffice must be xref:appendix/install-office.adoc[installed] for output.

^2^ - depending on the xref:appendix/app-properties.adoc#reporting.office.docx.useOfficeForDocumentConversion[reporting.office.docx.useOfficeForDocumentConversion] application property, the output can be performed with or without OpenOffice/LibreOffice. In the latter case, you need to provide required fonts, as described in xref:creation/templates/template_html.adoc#html_to_pdf[HTML to PDF].
