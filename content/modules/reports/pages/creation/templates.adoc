...
可以在报表详情视图添加报表模板。在报表列表视图新建或编辑报表时，切换至运行时报表编辑器的 *Templates（模板）* 标签页：
...
* *Is default（默认）* - 将模板标记为特定报表的 “默认模板” 时，除非用户显式选择不同的模板，否则该模板将被用于生成报告。
...
报表带区可以是水平或垂直的。如果报表区是水平的，命名区域将向下扩展；如果是垂直的，将向右扩展。水平报表带区可以用树状结构组织并包含子报表带区（嵌套或子报表带区）。因此，对于子报表带区，需要直接在与父报带表区对应的区域下创建命名区域。
...
在设计时，使用 `@TemplateDef` 注解定义模板。如果报表类包含多个模板，可以多次使用该注解。如需实现额外的逻辑，需要声明一个带有 <<template-delegate,@TemplateDelegate>> 注解的方法。

`@TemplateDef` 注解有以下属性：

* `code` - 模板的唯一代码。在以编程方式引用模板时使用此代码。
* `isDefault` - 是否默认模板。除非用户明确指定其他模板，否则自动使用此模板生成文档。默认值：`false`。
* `outputType` - 报表引擎生成的输出内容类型。输出类型应与 <<output_format_compliance,输出格式兼容性>> 中描述的模板文件类型一致。
...
`band` 属性 - 包含在输出中的一个或多个带区。每个带区由一个单独的表格组件显示。参见 <<table-band-def,@TableBandDef>>。
...
是表格模板定义的一部分，`@TableColumnDef` 注解表示结果表格中的一列，该列通过键值从带区数据中获取值。
