...
[[add_template]]
== Adding Template
可以在报表编辑界面添加报表模板。切换至 *Templates（模板）* 标签页可以创建或编辑报表模板：

.模板标签页
image::template_list.png[align="center"]

一个报表可以有多个模板，但是必须在 *Report structure（报表结构）* 标签页选择一个模板作为默认模板。
...
* `rootBand` - `com.haulmont.yarg.structure.BandData` 类型的对象，提供对带区数据集的访问。
...
CSV 模板中的报表带区只能是水平方向的，相应的命名区域将向下延展。此外，报表带区应属于第一级数据，即根区的第一级子区。其他方面，应使用与 <<template_xls,XLS(X) 模板>> 相同的规则。
...
CSV 模板支持内联编辑。可以直接在 *Template editor（模板编辑器）* 窗口中编辑模板，无需重新上传模板文件即可查看更改。
...
要在文档文本中使用字段变量，可以用 `++${band_name.field_name}++` 格式的字符串，其中 `++band_name++` 是带区名称，`++field_name++` - 带区字段的名称。
...
要控制页面尺寸、页眉和页脚，请使用特殊的 CSS 规则和属性。可以在 xref:sample-reports.adoc#example_html[HTML 示例报表] 中找到每页都显示页面/页脚的报表示例。
...
在模板编辑器中使用 *Template type（模板类型）* 单选按钮切换这两种方法。

HTML 模板支持内联编辑。可以直接在 *Template editor（模板编辑器）* 窗口中编辑模板，无需重新上传模板文件即可查看更改。
...
还可以使用 `getMessage()` 和 `getMainMessage()` 方法获取本地化消息值：
...
* `getMainMessage()` 只能接收一个参数 `key`，返回消息包中的本地化值：
+
[source, groovy,indent=0]
----
...
目前，Jmix 报表扩展在 HTML 报表中不提供类似于 DOCX/XLSX 可以插入图片的功能。但是，图片仍然可以使用 `img` 标签并在 `src` 属性中指定图片链接的方式嵌入。使用下面的任一种方法都能为 HTML 报表添加图片。

* 通过 URL
+
图片可以托管在 Tomcat 服务或者任何外部托管资源中。例如，托管在 `tomcat\webapps\ROOT\images` 文件夹中的图片可以这样嵌入：
...
可以使用 JasperReports 工具（例如，Jaspersoft Studio）或在文本编辑器中创建 JRXML 模板。报表数据结构中定义的每个 xref:creation/data-structure.adoc#report_bands[报表带区] 必须在模板中有相应的 `band` 元素，`band` 元素（在 JasperReports 术语中也称为带区）放置在标准 JasperReports 报表区内：`title`、`pageHeader`、`columnHeader`、`detail` 等。
...
每个 xref:creation/data-structure.adoc#report_bands[报表带区] 只能在模板中使用一次，因此如果需要在一个报表中以不同的形式表示相同的数据（例如，作为表格和图表），需要创建与模板中 `band` 元素一样多的报表带区。不支持嵌套报表带区，所有带区都应该是 *Root* 区的直接子节点。
...
创建一个类并实现 `com.haulmont.yarg.formatters.CustomReport` 接口。定义 `createReport()` 方法，该方法返回一个字节数组并接收以下输入参数：
...
在模板编辑界面中，选中 *Is custom（是否自定义）* 复选框，在 *Defined by（定义方式）* 字段中选择 *Class（类）*，并设置 *custom definition（自定义内容）* 为 Java 类的完全限定名：
...
[[template_chart]]
=== 图表模板
如果应用程序项目 xref:index.adoc#installation[安装] 了 xref:charts:index.adoc[图表扩展组件]，则可以使用图表输出类型。生成的图表显示在应用程序的 *Reports（报表）* -> *Show Charts（展示图表）* 界面中。
...
* *Band name（数据区名称）* - 为图表提供数据的报表带区。
* *Title field（标题字段）* - 带区字段，用于展示饼图中分块的标题。
* *Value field（值字段）* - 带区字段，用于展示饼图中分块的值。
* *Color field（配色字段）* - 带区字段，用于展示饼图分块颜色。颜色值需要以 https://en.wikipedia.org/wiki/Web_colors[web^] 格式指定。如果未定义，将自动选择颜色。
* *Units（单位）* - 数值单位，添加至图例后面。
* *Show legend（显示图例）* - 在图表中展示图例。

[[json_configuration]]
*JSON 配置*:

在底部的 *JSON configuration（JSON 配置）* 字段，可以设置图标的 JSON 配置。这个配置会覆盖自动生成图表配置。
...
* *Band name（数据区名称）* - 为图表提供数据的报表带区。
* *Category field（类目字段）* - 带区字段，用于展示类目轴。
* *Category axis caption（类目轴标题）* - 水平类目轴的标题。
* *Value axis caption（数值轴标题）* - 纵轴标题。
* *Value axis units（数值轴单位）* - 纵轴值的单位。
* *Show legend（显示图例）* - 展示图表图例。

必须为序列图添加至少一个数据行定义：

* *Value field（值字段）* - 带区资源，对应行值。
* *Type（类型）* - 展示图形类型。
* *Color field（配色字段）* - 带区字段，用于展示饼图分块颜色。颜色值需要以 https://en.wikipedia.org/wiki/Web_colors[web^] 格式指定。如果未定义，将自动选择颜色。

序列图也支持 <<json_configuration,JSON 配置>>。

[[table_output]]
=== 表格

当在向导中选择表格输出时，系统会自动创建一个模板。运行报表时，数据会在应用程序中的一个特殊界面展示。

也可以手动为一个已存在的报表创建表格模板，在报表模板编辑界面中选择 *Table（表格）* 作为输出类型。
...
表格列显示 SQL、JPQL 数据集的所有属性。如果使用实体/实体列表数据集，则表格仅显示选择的属性。

表格结果可通过应用程序的 *Reports（报表） -> Show Report Table（展示报表表格）* 界面查看。使用 *Excel（生成 Excel）* 按钮可以将显示的表格导出为 Excel 文件。
...
如果应用程序项目 xref:index.adoc#installation[安装] 了 xref:pivot-table:index.adoc[透视表扩展组件]，则可以使用透视表输出格式。

透视表输出只能作为额外的报表输出类型，这也是为什么在报表  xref:getting-started.adoc[向导] 中没有透视表。如需使用透视，在报表编辑页面切换到 *Templates（模板）* 标签页，点击 *Create（新建）* 然后选择透视表作为新模板的输出类型。

透视表结果可通过应用程序的 *Reports（报表） -> Show Report Table（展示透视表）* 界面查看。

.透视表
image::pivot_template_result.png[align="center"]

报表扩展组件将获取报表带区数据并绘制具有拖放功能、聚合数据和汇总数据的表格。在数据透视报表中只能使用一个报表带区，不支持嵌套区。

Renderer options（渲染器选项）::

此标签页支持定义一组渲染功能，展示在 UI 的可用渲染器列表中，也可以定义一个默认的渲染器。

.Renderer Options（渲染器选项）
image::pivot_template_renderer.png[align="center"]

Aggregation options（聚合选项）::

此标签页支持定义表格聚合功能列表。聚合功能属性：

* *Mode（模式）* 可以设置一个预定义的聚合函数；
* *Caption（标题）* 在 UI 中显示的本地化名称；
* *Custom function（自定义函数）* - 如果不为空，则忽略 *Mode（模式）* 值而使用自定义的 JavaScript 代码。

.Aggregation Options（聚合选项）
image::pivot_template_aggregation.png[align="center"]

Properties options（属性选项）::

透视表显示所有类型 xref:creation/data-structure.adoc[数据集] 的全部属性。为了正确处理这些属性，SQL、JPQL 和 Groovy 数据集中的引用属性的别名不能包含句点，例如，`select u.name as "userName"`。选择的属性应在模板编辑界面的 *Properties options（属性选项）* 标签页上设置为数据透视表的属性：

* *Row（行）*、*Column（列）* - key-value map，包含要在透视表列和行中使用的属性，其中 key 是数据集中属性的名称，value 是其本地化标题。
* *Aggregation（聚合字段）* - 要在每个单元格进行聚合计算的属性。
* *Derived property（派生属性）* - 可用于为原始数据集添加新属性，这些属性派生自已有属性。此元素是 kay-value map，其中 key 是生成的属性的名称，value 是生成此属性的 JavaScript 函数。
...
* *Filter function（过滤函数）* - 用于过滤的 JavaScript 函数。
* *Sorters function（排序函数）* - 用于行和列标题排序的 JavaScript 函数。
* 渲染器配置（取决于选择的渲染器）- 支持设置用于自定义渲染器渲染效果的 JavaScript 函数。实际上，只有两种类型的渲染器可以定制：
+
** 所有的 `heatmap（热力图）`：可以通过 Javascript 代码设置单元格颜色，
+
** 所有的图表：可设置图表的尺寸。
...
[options="header"]
|===============
|模板/输出|XLSX|XLS|CSV|DOCX|DOC|PDF  |HTML |Chart
|XLSX             |+   |   |+  |    |   |+ ^1^|+ ^1^|
|XLS              |    |+  |   |    |   |+ ^1^|     |
|CSV              |    |   |+  |    |   |     |     |
|DOCX             |    |   |   |+   |   |+ ^2^|+ ^2^|
