...
具有 HTML 格式模板和 PDF 输出的报表有时候会显示错误的字体。要解决此问题，将带有所需 `.ttf` 字体的 `jmix/fonts` 子目录添加到 Jmix 配置目录（默认为 `${user.dir}/.jmix/conf/`）。此外，可以通过在 xref:configuration.adoc#jmix.reports.pdfFontsDirectory[jmix.reports.pdfFontsDirectory] 应用程序属性中指定路径来使用已有的操作系统字体。
...
* 设置 xref:configuration.adoc#jmix.reports.pdfFontsDirectory[jmix.reports.pdfFontsDirectory] 应用程序属性：
+
----
...
// 透视表显示所有类型 xref:creation/data-structure.adoc[数据集] 的全部属性。为了正确处理这些属性，SQL、JPQL 和 Groovy 数据集中的引用属性的别名不能包含句点，例如，`select u.name as "userName"`。选择的属性应在模板详情视图的 *Properties options（属性选项）* 标签页上设置为数据透视表的属性：
...
+
* 配置 xref:configuration.adoc#jmix.reports.curlPath[jmix.reports.curlPath] 应用程序属性，设置为 *curl* 命令行工具的路径。
...
^2^ - 根据 xref:configuration.adoc#jmix.reports.useOfficeForDocumentConversion[jmix.reports.useOfficeForDocumentConversion] 应用程序属性的配置，输出可以选择是否使用 LibreOffice。如果不使用 LibreOffice，则需要自己提供所需的字体，参阅 <<html_to_pdf,HTML 转换 PDF>>。
