...
*Entity（单一实体）* 数据集仅包含一行数据，使用单个实体实例的属性和与之相关的实体生成。
...
[CAUTION]
====
由于报表引擎每次都会重新加载 *Entity（单一实体）* 数据集，因此不能用于新实体实例（尚未存储在数据库）。此时，可以使用 <<structure_groovy,Groovy 数据集>>，将实体作为参数传入：
...
*List of entities（实体列表）* 数据集由一组实体实例生成。
