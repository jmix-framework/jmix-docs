...
* *Band name（数据区名称）* - 报表中的唯一带区名称。必须只包含拉丁字母、数字和下划线。此外，如果带区名称以 *header* 开头，则其数据不会在 xref:creation/templates.adoc#table_output[表格] 中输出。
* *Orientation（方向）* - 报表带区方向：*Horizontal（水平）*、*Vertical（垂直）* 或 xref:sample-reports.adoc#crosstab_xls[Crosstab（双向）]。
...
字段名称可以用于报表模板中，并在生成报表时替换为数据集中的相应值。在描述数据集时，可以使用报表的 xref:creation/parameter-and-format.adoc#parameters[外部参数] 以及其他报表带区的字段，这样可以创建关联带区。
...
+
--
*Link field（链接字段）* 用于合并来自一个带区内的多个数据集的数据。当单个查询或 Groovy 脚本无法获取报表所需的完整数据时使用。
--
...
[CAUTION]
====
由于报表引擎每次都会重新加载 *Entity（单一实体）* 数据集，因此不能用于不保存在数据库的 DTO 实体或 JPA 实体。此时，可以使用 <<structure_groovy,Groovy 数据集>>，将实体作为参数传入：
...
[CAUTION]
====
由于报表引擎每次都会重新加载 *List of entities（实体列表）* 数据集，因此不能用于不保存在数据库的 DTO 实体或 JPA 实体。此时，可以使用 <<structure_groovy,Groovy 数据集>>，将实体列表作为参数传入：
...
如需使用这个格式，可以切换到报表编辑器的 **Parameters and Formats（参数和格式）** 标签页并打开 xref:creation/parameter-and-format.adoc#formatters[formatter] 编辑器。例如，对于 `bookPublication.dateTime` 字段，Groovy 脚本如下：
