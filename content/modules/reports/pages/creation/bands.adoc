...
Field names can be used in the report template and will be replaced with corresponding values from the dataset when the report is produced. When describing datasets, you can use xref:creation/parameters[report external parameters] and fields from other bands - this allows creation of linked bands.
...
//TODO узнать, правда ли до сих пор так
[CAUTION]
====
...
//TODO исправить ссылку на актуальную в новом проекте примере
...
//TODO поправить ссылку на новую вкладку с форматами
To do this, switch to the **Value formats** tab of the report detail view and open the xref:creation/value-formats.adoc[formatter] editor. For example, for the field `bookPublication.dateTime` Groovy-script will look like this:
