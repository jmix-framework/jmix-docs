...
报表 REST API 使用 OAuth2 协议进行身份验证，与 xref:rest:access-control.adoc[通用 REST] 相同。
...
如需通过 REST API 提供报表，请在 xref:reports:creation/details.adoc[报表详情] 标签页上选中 *Available through REST API（REST API 可见）* 复选框。
...
<1> 报表 ID，
<2> 报表名称，
<3> xref:creation/details.adoc[报表系统编码]（如果存在），
<4> 报表组 ID。
--
...
* 报表名称
* 报表组 ID
* xref:creation/details.adoc[系统编码]（如果存在）
* xref:creation/templates.adoc[报表模板]
* xref:creation/parameters.adoc[参数] 及其类型。
