...
In order to do this, we need to define a new xref:run-report.adoc#run_report_action[RunReportAction] xref:jmix-ui:actions/standard-actions.adoc[standard action] and a button for it in the `library-department-browse.xml` screen descriptor:
