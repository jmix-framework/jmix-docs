...
CAUTION: The xref:rest:index.adoc[] add-on does not fully support multitenancy. It doesn't separate entity instances between tenants.
...
You can add this attribute using the xref:data-model:entities.adoc#tenant-id-trait[Has Tenant ID] trait in Studio xref:studio:entity-designer.adoc[Entity Designer].
...
+
You also have the option to set a custom name for the URL parameter by using the `jmix.multitenancy.tenantIdUrlParamName` application property.
...
. Add a string attribute to your `User` entity and annotate it with the `@TenantId` annotation. You can also add this attribute using the xref:data-model:entities.adoc#tenant-id-trait[Has Tenant ID] trait in Studio:
+
...
. Add a field for selecting a tenant in `user-detail-view.xml`:
+
[source,xml,indent=0]
----
