...
在 Studio 中，可以通过 xref:studio:entity-designer.adoc[实体设计器] 的 xref:data-model:entities.adoc#tenant-id-trait[Has Tenant ID] 特性添加该属性。
...
. 在 `User` 实体中添加一个属性，并使用 `@TenantId` 注解。也可以通过 Studio 的 xref:data-model:entities.adoc#tenant-id-trait[Has Tenant ID] 特性添加该属性：
+
