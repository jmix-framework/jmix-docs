...
The xref:notifications:index.adoc[Notifications] add-on can be integrated into BPM to allows sending messages for a user task and service task. Install Notifications and set the `jmix.bpm.notification-sending-enabled` property to `true` since it is disabled by default.
