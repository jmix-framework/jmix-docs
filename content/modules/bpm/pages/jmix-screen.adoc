...
Если вам нужна форма процесса со сложной компоновкой и поведением, вы можете использовать существующий экран UI Jmix вместо диалоговых форм ввода. Контроллер экрана должен быть помечен аннотацией `@ProcessForm`, чтобы использоваться в качестве процессной формы.
...
== Параметры формы процесса

Формы экранов процессов Jmix могут принимать внешние параметры, определенные в моделере. Параметры, используемые формой, определяются в атрибуте `params` аннотации `@ProcessForm`:
...
См. <<process-form-with-parameters,пример>> формы процесса с параметрами.

[[output-variables]]
== Выходные переменные

При моделировании процесса может быть полезно знать, какие переменные задаются формой экрана процесса Jmix, чтобы позже использовать их повторно в модели процесса. Для этого используйте атрибут `outputVariabes` аннотации `@ProcessForm`.
...
Информация о выходных переменных отображается в соответствующем разделе панели свойств при выборе формы процесса.

image::forms/output-variables.png[align="center"]

[[restrict-process-form-usage]]
== Ограничение использования формы процесса

По умолчанию все экраны форм процессов доступны в рамках любой модели процесса. Если вы хотите использовать какой-либо экран только в определенных процессах, нужно указать ключи процессов в атрибуте `allowedProcessKeys` аннотации `@ProcessForm`.
...
Вы можете использовать службу `ProcessFormScreens` для создания форм процессов запуска и форм процессов задач, определенных в моделере.

В приведенном ниже примере форма запуска процесса открывается нажатием кнопки на экране браузера.
...
<2> Создание формы запуска процесса с полученным определением процесса.
<3> Отображение формы запуска процесса `order-process`.

Форма запуска процесса может выглядеть как в примере в разделе <<start-form-example,ProcessFormContext>>.
...
Форма процесса задачи может выглядеть как пример в разделе <<task-form-example,ProcessFormContext>>.
...
Давайте рассмотрим пример формы процесса, которая используется в качестве стартовой формы. В форме отображаются два поля:

* текстовое поле для ввода номера заказа.
* выпадающий список пользователей для выбора менеджера. Мнеджер Следующим может быть следующим актором процесса.
...
<1> Аннотация `@ProcessForm` указывает, что этот экран является формой процесса и будет доступен в моделере.
<2> Мы объявляем, что инжектированный компонент UI `orderNumber` является переменной процесса. Поскольку мы разрабатываем форму запуска процесса, переменная еще не имеет значения, но аннотация будет использоваться при запуске процесса.
...
[[task-process-form]]
=== Форма процесса задачи

Рассмотрим пример формы процесса задачи с двумя полями:
...
Кнопки *Approve* и *Reject* завершают задачу пользователя с соответствующим выходом.
...
=== Форма процесса StandardEditor

Этот пример демонстрирует, как использовать `StandardEditor` в качестве формы процесса. Это может быть полезно, если вы храните какую-либо сущность в переменной процесса и хотите просматривать или редактировать поля сущности с помощью формы процесса задачи.
...
Контроллер экрана формы процесса:
