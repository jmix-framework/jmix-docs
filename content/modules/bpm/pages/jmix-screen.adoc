...
如果流程表单是通过 *Start process（启动流程）* 和 *My tasks（我的任务）* 界面打开则可以使用 `ProcessFormContext`。如需通过注入 `ProcessFormContext` 以编程的方式打开流程表单，请使用 <<opening-forms-programmatically,ProcessFormScreens>> bean。
