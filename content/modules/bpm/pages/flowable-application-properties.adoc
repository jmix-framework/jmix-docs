...
# Set this to false to ignore decision table hit policy validity checks. If validation fails, the decision table enters a failed state, and intermediate results created before the error are returned.
...
# Sets the size of the {@link org.flowable.ldap.LDAPGroupCache}. This LRU cache stores groups for users and avoids calling the LDAP system each time user groups are needed.
