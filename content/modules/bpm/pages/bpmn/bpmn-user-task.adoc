...
In addition to general properties, user task has the following specific properties:

* *Due Date* to inform the performer when the task must be completed.
...
NOTE: Setting a due date does not start any automatic actions. It is necessary to manually configure any subsequent actions.
...
[horizontal]
Expression:: – an expression that result is a String containing username.
Process variable:: – a variable that refers to _User_ entity.
User provider:: – a Java class with the method returning a username.

image::bpmn-user-task/assignee-sources.png[,460]
...
image::bpmn-user-task/assign-by-expression.png[,460]
...
image::bpmn-user-task/assigne-by-expression-direct.png[,460]
...
image::bpmn-user-task/assignee-initiator.png[,460]
...
In this case, the system offers to select an existing user provider Java class:

image::bpmn-user-task/selecting-user-provider.png[,460]

The user provider class needs to implement one or more methods with the `String` return type.
A method gets parameter values that are specified in the modeler and returns a username of the user that becomes a task assignee.
...
It allows you to select one of the previously created Java beans provided with the `@UserGroupListProvider` annotation.
...
[[defining-task-forms]]
== User Task Process Forms

User task can be configured with a xref:forms.adoc[process form] to assist the performer in entering data required for this task. The collected data can then be passed along in the process.
...
image::bpmn-user-task/task-listeners-field.png[,460]
