...
NOTE: 设置截止日期不会启动任何自动操作。需要手动配置后续的操作。

[[setting-due-date]]
=== 设置到期时间
_Due date_ 支持两种类型的设置：
...
[horizontal]
Expression:: - 表达式。必须返回字符串，包含 username。
...
此时，系统会加载已有的 user provider Java 类以供选择：
...
用户任务可以配置一个 xref:forms.adoc[流程表单]，协助执行人输入任务所需的数据。收集的数据可以在之后的流程中传递。
