...
== Overview
In BPMN, a *swimming pool* is a graphical element used to represent a xref:bpmn/bpmn-concepts.adoc#participants[participant] in a xref:bpmn/bpmn-concepts.adoc#collaboration[collaboration].

[[pools]]
== Pools
A swimming pool may have *lanes*, which represent different xref:bpmn/bpmn-concepts.adoc#actors[actors] within the process. Each lane contains the activities (task or subprocesses).
...
In BPMN, *Lanes* are used to organize and categorize xref:bpmn/bpmn-concepts.adoc#activities[Activities] within a *Pool*.
