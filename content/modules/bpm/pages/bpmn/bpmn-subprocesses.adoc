...
.*Graphical Notation*
A subprocess is represented as a rounded rectangle. To create a subprocess, select image:bpmn-subprocesses/icon-subprocess.png[,30] from the palette and place it on the canvas.
...
All activities, gateways, events, and so on, that are part of the subprocess need to be enclosed within this element.
...
Embedded subprocess has no special properties, only general ones.

image::bpmn-subprocesses/subprocess-properties.png[,463]

A subprocess may be xref:bpm:bpmn/transactions.adoc#asynchronous-continuation[asynchronous]
...
A transaction is a logical unit of work that allows you to group a set of individual activities,
...
Call Activity allows you to call a reusable process or a global task from within another process.
...
<callActivity id="Activity_08ermzt" name="Call activity"
  ...
  <extensionElements>
    <flowable:in sourceExpression="${customer.name}" <3>
                 target="customerName" />
    <flowable:out source="customerName"  <4>
                  target="customerName" />
  </extensionElements>
...
You can configure a call activity by setting its properties:

image::bpmn-subprocesses/call-activity-properties.png[,463]

* *Called element*: This property refers to an existing process definition.
+
CAUTION: Avoid unmanaged recursion! Technically, it's possible to call another instance of the process from within itself.

* *Called element type*: by default, the _Key_ parameter is used, which calls the most recent version of the process. You can call a specific version of the process using its id.

* *Business key expression*: You can define it by expression or inherit from the parent process. See xref:bpm:bpm-concepts.adoc#business-key[business key] for more details.

* *Variables Mapping*:

** *Inheriting variables*: Call activity can inherit process variables from the parent process. When the called process starts, new instances of the process variables will be created, distinct from the original references.

** *In mapping*: You can pass parameters to the called process using existing process variables (source) to map them to variables within the called process (target):
+
image::bpmn-subprocesses/in-mapping.png[,463]
+
Alternatively, you can use expressions:
+
image::bpmn-subprocesses/in-mapping-expression.png[,463]

** *Out Mapping*: Similarly, you can map variables from the called process (source) on the variables in the parent process (target):
+
image::bpmn-subprocesses/out-mapping.png[,463]
