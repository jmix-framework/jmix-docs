...
.*图形表示法*
子流程展示为一个圆角矩形。如需创建子流程，从工具箱选择 image:bpmn-subprocesses/icon-subprocess.png[,30]，然后放置到画布中。
...
子流程可以是 xref:bpm:bpmn/transactions.adoc#asynchronous-continuation[异步]、
...
调用活动（Call Activity）支持调用可重用流程、或另一个流程中全局任务。
...
* *Called Element（调用元素）* 关联一个已有的流程定义。
+
CAUTION: 请避免未管理的递归！从技术上讲，可以从自身内部调用该流程的另一个实例。

* *Called element type（调用元素类型）*：默认情况下，使用 _key_ 参数，调用流程的最新版本。可以通过 id 指定调用流程的特定版本。

* *Business Key expression（业务键值表达式）*：可以用表达式定义或从父流程继承。参阅 xref:bpm:bpm-concepts.adoc#business-key[业务键值]。

* *Variables Mapping（变量映射）*：

** *Inheriting variables（继承变量）*：调用活动可以从父流程继承流程变量。当被调用的流程启动时，将在其中创建流程变量新实例，与之前的引用相区分。

** *In mapping（向内映射）*：可以使用已有流程变量（source）将参数传递到被调用流程（target）中的变量：
+
...
** *Out mapping（向外映射）*：类似的，可以将被调用流程（source）中的变量映射到父流程（target）中的变量：
+
