...
A collaboration does not have a distinct visual representation on its own. However, the presence of at least one xref:bpm:bpmn/bpmn-pools-lanes.adoc#pools[pool] indicates that it is part of a collaboration.
...
You can use collaborations in different scenarios. In the first case, using the collaboration allows you to place the main process and its subprocess on a single diagram:
