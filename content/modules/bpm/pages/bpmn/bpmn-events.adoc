...
[NOTE]
====
In the case of xref:bpm:bpmn/bpmn-collaboration.adoc[collaboration], select the desired xref:bpm-concepts.adoc#participants[participant] (xref:bpm:bpmn/bpmn-pools-lanes.adoc#pools[pool]).
====
...
* If a compensation boundary event is attached to an activity with xref:bpm:multi-instance.adoc[multiple instance] characteristics, a compensation event subscription is created for each instance.
