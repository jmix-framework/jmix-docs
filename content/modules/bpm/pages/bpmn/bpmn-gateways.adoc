...
如果流程中有一个带结果的用户任务，可以选择其中一个结果用于激活给定的流。

image::bpmn-gateways/exclisive-gateway-task-outcome.png[]

设置结果的条件，首先选择一个用户任务，然后选择结果。
...
使用这个参数可以实现某种形式的投票。
例如，可以只要一个用户完成任务并产生 'yes' 的结果即可选择该路径。
...
从形式上讲，BPMN 规范允许同一个网关扮演传入和传出两个角色，但是这种用法是不推荐的。
