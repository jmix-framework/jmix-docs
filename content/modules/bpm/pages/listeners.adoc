...
See https://www.flowable.com/open-source/docs/bpmn/ch07b-BPMN-Constructs/#execution-listener[Flowable documentation^].

[[task-listeners]]
== Task Listeners

See https://www.flowable.com/open-source/docs/bpmn/ch07b-BPMN-Constructs/#task-listener[Flowable documentation^].

[[spring-events-bpm]]
== Spring Events for BPM

Jmix BPM introduces a set of Spring application events. The following events are published:
...
<3> Gets `User` entity to get the user's first name and email address. The `UserTaskAssignedEvent` contains the `username` of the user to whom the task is assigned.
