...
There are several approaches to get notifications of some process engine events, for example, task assigned, activity completed and so on.
...
<4> The `UserTaskAssignedEvent` contains a `TaskData` object which holds information about the user task.
