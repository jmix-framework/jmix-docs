...
Each element on the canvas has its own properties. At the moment the only element present is `StartEvent`. It represents the <<start-form, Start form>> seen the user initiating the process. Let's configure it:
...
** *Form type*: `Input dialog`
** *Open mode*: `Dialog` (this will be set by default)
* Add input parameters. They must duplicate parameters of the `Approval` form but the respective fields should be non-editable:
+
[cols="1,1,1,1,1"]
|===
|Process variable | Caption | Type | Editable | Required
