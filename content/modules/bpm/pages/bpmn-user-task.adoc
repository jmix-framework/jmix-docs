...
In BPMN can be the only performer of the task. When we say "the task is assigned to group", it means there is a list of *potential performers*, not a *UserGroup* object.
One of them must become an actual task performer by xref:bpmn-user-task.adoc#claiming-group-tasks[claiming] the task on himself.
====
...
* _Due date_ parameter isn't validated during the  process deployment. But at runtime wrong value will cause an error.
====
...
When you have a *Business Calendars* add-on installed, the corresponding field appears in the _General_ properties section in the *Web BPM Modeler*:
...
See details in xref:../../business-calendar/index.adoc[Business Calendars] add-on.
...
In this case, Jmix BPM allows assigning the task to  _candidates list_ -- and any of its members can become a performer. But previously he or she must _claim_ the task.
Claiming is an action of taking task ownership by user.

The task with candidates appears in xref:my-tasks.adoc[*My tasks*] view in the *Group tasks* section for each candidate.
...
See the xref:user-groups.adoc[User groups] section for more information.
...
Similar to _User Groups Provider_, you can get a list of candidate users programmatically via calling a method of the Spring bean annotated by `@UserListProvider` annotation:
...
[source,java]
----
@UserListProvider(value = "jbt_UserListProviderDemo",
...
For xref:bpm:input-dialog.adoc[input dialog] process forms, outcomes are defined in the *Outcomes* section in the *BPMN Inspector* panel.
...
When outcomes are used with the input dialog form, a special _jmix:formOutcomes_ section is added. Here you can see your outcomes with _jmix:formOutcome_ attributes.
...
For xref:bpm:jmix-view.adoc#declare-task-outcomes[Jmix screen] forms, outcomes are defined in the screen controller.
...
When the user clicks one of the outcome buttons, the task is completed and the decision is written to a special process variable which name is built according to the following name pattern: `<user-task-id>_result`. You can find it in the list of variables in thr *Process instance edit* view.
...
User tasks can be multi-instanced. See details on this mode in the xref:multi-instance.adoc[Multi-Instance] section.
...
But all their decisions will be accumulated in the one *outcomes container*.
...
Originally _documentation_ field was intended to document a process model by analytics. But in the case of *User task* it can be considered as instruction for user, explaining what to do.

You can find this field at the very end of the properties panel.

TIP: You can update the _documentation_ field every time you change your process. Thus, it's the easiest and effective way to provide users with actual information.
...
[source,xml]
----
<userTask id="user-task-01" name="Important task" />
----

*Due date*

To represent due date in XML, attribute `flowable:dueDate` is used:

[source,xml]
----
    <userTask id="user-task-1" name="Task 1" flowable:dueDate="PT3H" /> <1>
    <userTask id="user-task-2" name="Task 2" flowable:dueDate="2024-10-21" /> <2>
    <userTask id="user-task-3" name="Task 3" flowable:dueDate="newDate" /> <3>
    <userTask id="user-task-4" name="Task 4" flowable:dueDate="${someDate}" /> <4>
----
<1> -- duration 3 hours
<2> -- deadline is 2024, October, 21
<3> -- variable `newDate`
<4> -- expression
...
=== Candidates XML Representation


[source,xml]
----
    <userTask id="Activity_0cmmbsk" name="Review document"
        flowable:candidateGroups="accountants"
        jmix:assigneeSource="expression"
        jmix:candidateGroupsValue="accountants">
----

[source,xml]
----
    <userTask id="Activity_0t0as8h" name="Review document"
        flowable:candidateUsers="bob, peter"
        flowable:candidateGroups="${allGroups.getAllUserGroups()}"
        jmix:assigneeSource="expression"
        jmix:candidateGroupsSource="userGroupsListProvider"
        jmix:candidateGroupsValue="allGroups"
        jmix:candidateUsersValue="bob, peter">
----
