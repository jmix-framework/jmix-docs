...
If you need to migrate all process instances, open the xref:bpm:admin-screens.adoc#process-definitions[Process Definitions] screen and click *View details*. In the editor, select a required version in the combo box field and click the *Migrate all instances* button. Before the migration is performed, all instances are validated, and if any instance cannot be migrated, an information window will be displayed.

To migrate a single instance, find it in the xref:bpm:admin-screens.adoc#process-instances[Process Instances] screen and click *Edit*. In the editor, click the *Migrate* button.
