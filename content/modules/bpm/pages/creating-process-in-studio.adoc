...
Каждый элемент на холсте обладает собственным набором свойств. Пока единственный элемент на холсте — `StartEvent`. Он соответствует xref:bpm:getting-started.adoc#start-form[стартовой форме], которую увидит пользователь при запуске процесса. Давайте его настроим:
...
Создадим пользовательскую задачу утверждения *Approval*. Она соответствует xref:bpm:getting-started.adoc#approval-form[форме утверждения заявки], которую увидит менеджер.
...
Вторая пользовательская задача в модели уведомляет бухгалтеров об одобрении заявки. Эта задача соответствует xref:bpm:getting-started.adoc#payment-form[форме оплаты], предназначенной только для бухгалтеров.
...
Теперь процесс можно запустить и протестировать. Инструкции приведены в разделе xref:bpm:getting-started.adoc#testing-process[Запуск процесса].
