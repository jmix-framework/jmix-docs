...
For information on the `EntityManager` methods refer to the https://jakarta.ee/specifications/persistence/2.2/apidocs/javax/persistence/entitymanager[javax.persistence.EntityManager^] API documentation.
...
. The following JPA features are not supported: named queries, `StoredProcedureQuery`, `CriteriaBuilder`, `EntityGraph`, `EntityTransaction`.