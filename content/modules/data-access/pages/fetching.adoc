...
当使用 xref:data-manager.adoc[DataManager] 加载 JPA 实体时，Jmix 支持实体中引用属性的懒加载。也就是说，如果使用了 `DataManager` 或 UI 的 xref:ui:data/data-loaders.adoc[数据加载器] 来加载实体，则可以直接访问其引用属性获得关联实体，Jmix 会按需加载。这个过程是递归的，因此可以通过访问引用属性遍历整个对象关系图。
...
一个 _fetch plan_ 定义在一次特定的操作中，需要从数据库预加载什么样的对象关系图。Fetch plan 可以用在 xref:data-manager.adoc[DataManager]、UI xref:ui:data-components.adoc[数据组件] 中以提高性能，或者用在 xref:rest:index.adoc[REST API] 中定义返回数据的格式，而无需创建单独的一组 DTO。

TIP: Fetch plan 与关联实体的懒加载完全兼容。也就是说，可以使用 fetch plan 加载实体关系图的其中一部分，当访问到某些未加载的引用时，再进行延迟加载。

TIP: 使用 xref:studio:fetch-plan-designer.adoc[Studio fetch plan 设计器] 创建 fetch plan。
...
[NOTE]
====
当用 xref:studio:fetch-plan-designer.adoc[Studio] 为一个实体创建界面时，界面创建向导默认会为主实体和选中的关联实体提供 `_base` fetch plan，因此 fetch plan 的定义会看上去有一点不同：
...
* `_local` fetch plan 包含所有本地属性（直接属性，非实体引用属性）。
* `_instance_name` fetch plan 包含所有构成 the xref:data-model:entities.adoc#instance-name[实例名称] 的属性。可以是本地属性或引用属性。如果实体未指定实例名称，则该 fetch plan 为空。
* `_base` fetch plan 包含所有 `_local` 和 `_instance_name` fetch plan 内的属性。因此，仅当 `_instance_name` fetch plan 包含引用属性时，该属性才与 `_local` 不同。

TIP: 除非加载大量“宽”实体遇到性能问题，否则都应使用 `_base` fetch plan。这样能避免未加载属性引起的问题，参阅 <<partial-entities,partial entities>> 了解详情。
...
首先，在项目的根包内的资源根目录创建 `fetch-plans.xml`。Studio 中通过 Jmix 工具窗口的 *New -> Advanced -> Fetch Plan Configuration File* 创建该文件。

创建好之后，Jmix 工具窗口的 *New -> Advanced* 菜单会包含一个 *Fetch Plan* 条目。这里可以通过设计器定义 fetch plan。
