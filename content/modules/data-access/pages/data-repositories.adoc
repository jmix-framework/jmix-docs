...
Интерфейс `JmixDataRepository` расширяет стандартный интейфейс https://docs.spring.io/spring-data/commons/docs/{spring-boot-version}/api/org/springframework/data/repository/PagingAndSortingRepository.html[PagingAndSortingRepository^] из Spring Data. Он предоставляет несколько собственных методов, учитывающих специфику Jmix:
...
Вы можете применить аннотацию `io.jmix.core.repository.ApplyConstraints` к своему репозиторию данных. Если значение аннотации равно `false`, репозиторий использует `UnconstrainedDataManager` вместо `DataManager`.
