...
Controls whether records with `null` values are included in the result of negative property conditions.

By default, when a negative property condition is used, records where the filtered property is `null` are not included in the result because of standard SQL three-valued logic. If this property is set to `true`, Jmix adds an `is null` clause for negative property condition operators.

The property affects the following operators:

[cols="1,2,2", options="header"]
|===
|Operator
|Default JPQL
|JPQL when the property is `true`
...
Element collections are not affected by this property. Negative conditions for element collections already use `not exists (...)`, which includes empty collections.
...
When an instance of an entity with an `Integer` or `Long` identifier annotated with xref:data-model:entities.adoc#jmix-generated-value[@JmixGeneratedValue] is created in memory via the `Metadata.create()` method, an identifier value is assigned to the entity right away. This value is obtained from the xref:features:sequences.adoc[] mechanism that fetches the next id from a database sequence. In order to reduce the number of database calls, the sequence’s increment is set by default to 100, which means that the framework obtains the range of ids on each invocation. So it "caches" this range and yields the ids without going for the next value to the database until the whole range is used.
