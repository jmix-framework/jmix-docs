...
无需为 `session_customerCode` 或 `current_user_email` 参数设置值，框架会在执行查询前自动为其赋值。因此，在框架中某些无法完全控制查询语句执行的地方这种机制会很有用，例如，行级角色的 xref:security:row-level-roles.adoc#jpql-policy[JPQL 策略] 或通用过滤器的 xref:ui:vcl/components/filter.adoc#jpql-conditions[JPQL 条件]。
