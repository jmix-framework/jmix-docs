...
[IMPORTANT]
====
属性条件在参数值为空的时候，会始终解析为 true。参数值为空是指 null，空串或者空集合。例如，下面的请求会返回同样的结果，即加载所有的实例：
...
这个行为与 UI 数据加载器中的 xref:flow-ui:data/data-loaders.adoc#query-conditions[query 条件] 以及 xref:flow-ui:vc/components/propertyFilter.adoc[] 和 xref:flow-ui:vc/components/genericFilter.adoc[] 组件的行为一致：如果用户没有输入任何值，则会忽略该条件。

尽管这个行为在 UI 层面是没有问题的，但是在业务逻辑层面其实是值得商榷的。因此，我们建议在参数值可能为空的时候，尽量使用 <<load-by-query,查询语句>> 加载数据。
====
...
首先，不要一个一个实体调用单独的 `save(entity)` 方法，而是可以将所有实体（数量小于 1000）放在一个会话中保存，如上面所介绍的，将实体添加到 `SaveContext` 后调用 `save(SaveContext)`。
