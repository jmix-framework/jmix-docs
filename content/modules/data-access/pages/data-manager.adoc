...
这个行为与 UI 数据加载器中的 xref:ui:data/data-loaders.adoc#query-conditions[query 条件] 以及 xref:ui:vcl/components/property-filter.adoc[] 和 xref:ui:vcl/components/filter.adoc[] 组件的行为一致：如果用户没有输入任何值，则会忽略该条件。
