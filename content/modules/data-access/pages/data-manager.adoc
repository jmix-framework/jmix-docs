...
Вы можете инжектировать `DataManager` в бин Spring или контроллер экрана Backoffice UI, например:
...
[IMPORTANT]
====
`PropertyCondition` принимает значение true, если значение его параметра пусто (null, пустая строка или пустая коллекция). Например, следующие запросы вернут одинаковый результат - будут загружены все имеющиеся в БД экземпляры:

[source,java,indent=0]
----
include::example$/ex1/src/main/java/dataaccess/ex1/bean/CustomerService.java[tags=empty-conditions]
----

Это поведение идентично xref:flow-ui:data/data-loaders.adoc#query-conditions[условиям запросов] в загрузчиках данных UI и компонентам xref:flow-ui:vc/components/propertyFilter.adoc[] и xref:flow-ui:vc/components/genericFilter.adoc[]: если пользователь не ввел значение, условие игнорируется.

В то время как  в UI такое поведение необходимо, при написании бизнес-логики оно обычно нежелательно. Поэтому если значение параметра может оказаться пустым, используйте загрузку с помощью <<load-by-query,запросов>>.
====
...
[source,java,indent=0]
----
include::example$/ex1/src/main/java/dataaccess/ex1/bean/CustomerService.java[tags=using-locks]
----
