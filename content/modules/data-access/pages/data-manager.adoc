...
This behaviour is identical to the xref:ui:data/data-loaders.adoc#query-conditions[query conditions] in UI data loaders and to the xref:ui:vcl/components/property-filter.adoc[] and xref:ui:vcl/components/filter.adoc[] components: if the user doesn't enter a value, the condition is ignored.

While it's necessary for UI, such a behaviour is usually not desired when writing business logic, so consider using <<load-by-query,query>> if the parameter values can possibly be empty.
====
...
Consider using locks for entities that can be updated concurrently. To set the lock mode type specify one the
`javafx.persistence.LockModeType` enum values as a parameter of the `lockMode()` method. For example:
