...
Это поведение идентично xref:ui:data/data-loaders.adoc#query-conditions[условиям запросов] в загрузчиках данных UI и компонентам xref:flow-ui:vc/components/propertyFilter.adoc[] и xref:flow-ui:vc/components/genericFilter.adoc[]: если пользователь не ввел значение, условие игнорируется.
