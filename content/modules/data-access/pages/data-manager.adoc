...
NOTE: Для операторов отрицательных условий свойства, таких как `NOT_EQUAL`, `NOT_CONTAINS` и `NOT_IN_LIST`, записи с `null` значениями по умолчанию исключаются. Вы можете изменить это поведение с помощью свойства приложения xref:data-properties.adoc#jmix.data.include-null-clause-in-not-conditions[jmix.data.include-null-clause-in-not-conditions].
...
Учитывайте следующую информацию при работе с xref:data-model:entities.adoc#element-collections[коллекциями элементов].

[[element-collection-queries]]
==== Запросы с коллекциями элементов

Используйте `join` для запроса сущностей по значению элемента:
...
Сокращенный запрос должен включать предложение `from`:
...
Так можно найти пустые коллекции:
...
Используйте `join` при загрузке коллекций элементов как скалярных значений. Следующий метод вернет список `KeyValueEntity` с парами "имя -> тег":
...
Все элементы коллекции можно загрузить с помощью следующего запроса:
...
* Коллекция элементов как скалярное значение. Следующий запрос вызовет ошибку: `select p.name, p.tags from sample_Product p`
* Оператор `in`. Следующий запрос вызовет ошибку: `select p from sample_Product p where :tag in (p.tags)`
====

[[element-collection-conditions]]
==== Условия с коллекциями элементов

При использовании `PropertyCondition` операции `[NOT_]EQUAL`, `GREATER[_OR_EQUAL]`, `LESS[_OR_EQUAL]`, `CONTAINS`, `[STARTS|ENDS]_WITH` сравнивают элементы коллекции.

Например, следующий метод фактически выполняет запрос `select e from sample_Product e join e.tags t where t = :tag` и возвращает продукты, содержащие заданное значение в своей коллекции `tags`:
...
Так можно найти пустые коллекции с помощью `PropertyCondition`:
