...
Если вы сохраняете большую коллекцию сущностей и сохраненные экземпляры не нужны вам немедленно, используйте метод `SaveContext.setDiscardSaved(true)`. Это повысит производительность, поскольку `DataManager` не будет извлекать сохраненные cущности из базы данных. Например:
