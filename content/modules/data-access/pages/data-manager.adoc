...
Это поведение идентично xref:ui:data/data-loaders.adoc#query-conditions[условиям запросов] в загрузчиках данных UI и компонентам xref:ui:vcl/components/property-filter.adoc[] и xref:ui:vcl/components/filter.adoc[]: если пользователь не ввел значение, условие игнорируется.
