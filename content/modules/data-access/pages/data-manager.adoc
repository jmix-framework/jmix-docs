...
This behaviour is identical to the xref:flow-ui:data/data-loaders.adoc#query-conditions[query conditions] in UI data loaders and to the xref:flow-ui:vc/components/propertyFilter.adoc[] and xref:flow-ui:vc/components/genericFilter.adoc[] components: if the user doesn't enter a value, the condition is ignored.
