...
====
If you provide a data container from the host screen to a fragment as described above, keep in mind that the loaders' identifiers for this container should correspond. Thus, you should either set the same `id` for both host and fragment container, or the data loader in the fragment should have no `id` at all.

If you set the same loader `id` as defined in the host screen, don't forget to set `provided="true"` for the loader too:
