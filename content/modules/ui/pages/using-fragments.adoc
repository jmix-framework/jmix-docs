...
[CAUTION]
====
If you provide a data container from the host screen to a fragment as described above, keep in mind that the loaders' identifiers for this container should correspond.

. If you set `provided="true"` for the fragment's container, it can have no loader definition at all.
. In case you define the loader in the fragment explicitly, you should set the same `id` for the host and the fragment container's loader. Make sure the fragment's loader has both `id` and `provided="true"`:
+
[source,xml,indent=0]
----
