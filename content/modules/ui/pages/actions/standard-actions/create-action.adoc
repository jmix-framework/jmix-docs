= CreateAction
:page-aliases: backoffice-ui:actions/standard-actions/create-action.adoc

`CreateAction` is a xref:actions/standard-actions.adoc#list-actions[list action] designed to create new xref:data-model:entities.adoc[entity] instances. It creates a new instance and opens the entity edit screen with the created instance. After the instance is saved to the database by the edit screen, the action adds it to the data container of the UI component.
