...
`ViewAction` is a xref:actions/standard-actions.adoc#list-actions[list action] designed to view and edit xref:data-model:entities.adoc[entity] instances. It opens the edit screen in the same way as xref:actions/standard-actions/edit-action.adoc[EditAction], but it makes all fields non-editable and disables commit actions. If you want to allow users to switch the screen to the editable mode, add a button and link it with the predefined `enableEditing` action:
...
For this and `enableEditing` actions to work properly, the screen should implement the `ReadOnlyAwareScreen` interface. The `StandardEditor` base class implements it.
