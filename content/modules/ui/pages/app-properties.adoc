...
当你能完全控制菜单项时，可以使用 `false`。这样可以很容易对菜单进行删除、修改和重排序，例如，修改扩展组件的菜单。
...
定义 xref:ui:vcl/components/pagination.adoc[] 和 xref:ui:vcl/components/simple-pagination.adoc[] 组件的默认每页的实体数。

默认值：`50`

参阅 <<jmix.ui.entity-page-size,jmix.ui.entity-max-page-size>>。
...
下面的示例中，配置了两个实体的最大加载数量。其他实体仍然使用 <<jmix.ui.default-max-fetch-size,jmix.ui.default-max-fetch-size>> 的值。
...
定义 xref:ui:vcl/components/pagination.adoc[] 和 xref:ui:vcl/components/simple-pagination.adoc[] 组件对于特定实体的每页实例数量。

下面的示例中，配置了两个实体的每页实例数。其他实体仍然使用 <<jmix.ui.default-page-size,jmix.ui.default-page-size>> 的值。
