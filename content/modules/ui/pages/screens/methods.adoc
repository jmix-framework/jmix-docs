...
* `preventUnsavedChanges()` вызывается при закрытии экрана для предотвращения потери несохраненных данных. Стандартная реализация вызывает либо метод `showUnsavedChangesDialog()`, либо `showSaveConfirmationDialog()` бина xref:screens/validation.adoc[ScreenValidation], в зависимости от значения свойства xref:ui:app-properties.adoc#jmix.ui.screen.use-save-confirmation[jmix.ui.screen.use-save-confirmation].
