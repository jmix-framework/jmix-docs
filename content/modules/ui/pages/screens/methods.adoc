...
* `show()` - shows the screen. This method is usually used after creating the screen as described in the xref:ui:opening-screens.adoc[Opening Screens] section.
...
----
+
The parameter value is propagated to the xref:ui:screens/screen-events.adoc#before-close-event[BeforeCloseEvent] and xref:ui:screens/screen-events.adoc#after-close-event[AfterCloseEvent], so the information about the reason why the screen was closed can be obtained in the listeners. For more information on using these listeners, see xref:ui:opening-screens.adoc#executing-code-after-close[Executing code after close and returning values].
+
* `getScreenData()` - returns the `ScreenData` object that serves as a registry for all xref:ui:data-components.adoc[data components] defined in the screen XML descriptor. You can use its `loadAll()` method to trigger all data loaders of the screen:
+
[source,java,indent=0]
----
...
* `getEditedEntity()` - when the screen is shown, returns an instance of the entity being edited. It’s the instance that is set in the data container specified in the xref:ui:screens/annotations.adoc#edited-entity-container[@EditedEntityContainer] annotation.
+
In the xref:ui:screens/screen-events.adoc#init-event[InitEvent] and xref:ui:screens/screen-events.adoc#after-init-event[AfterInitEvent] listeners, this method returns `null`. In the xref:ui:screens/screen-events.adoc#before-show-event[BeforeShowEvent] listener, this method returns the instance passed to the screen for editing (later in the screen opening process the entity is reloaded and a different instance is set to the data container).
...
If the screen has unsaved changes in xref:ui:data/data-context.adoc[DataContext], a dialog with a corresponding message will be displayed before the screen is closed. You can adjust the notification type using the xref:ui:app-properties.adoc#jmix.ui.screen.use-save-confirmation[jmix.ui.screen.use-save-confirmation] application property. If you use the `closeWithDiscard()` or `close(StandardOutcome.DISCARD)` methods, unsaved changes are ignored without any message.
...
* `preventUnsavedChanges()` method prevents the loss of data in case of closing the window with unsaved changes. The method invokes either `showUnsavedChangesDialog()`, or `showSaveConfirmationDialog()`, depending on the xref:ui:app-properties.adoc#jmix.ui.screen.use-save-confirmation[jmix.ui.screen.use-save-confirmation] property value.
+
[source,java,indent=0]
----
include::example$/ex1/src/main/java/ui/ex1/screen/screens/OrderEdit1.java[tags=prevent-unsaved]
----
<1> Shows the standard dialog for confirming saving changed data with *Save*, *Do not save*, and *Cancel* buttons.
<2> Shows the standard dialog for unsaved changes with *Yes* and *No* buttons.
