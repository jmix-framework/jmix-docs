...
* `preventUnsavedChanges()` method is called on screen close to prevent the loss of unsaved data. The standard implementation invokes either `showUnsavedChangesDialog()`, or `showSaveConfirmationDialog()` of xref:screens/validation.adoc[ScreenValidation] bean, depending on the xref:ui:app-properties.adoc#jmix.ui.screen.use-save-confirmation[jmix.ui.screen.use-save-confirmation] property value.
