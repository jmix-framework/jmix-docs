...
----
+
* `showUnsavedChangesDialog()` - 为没保存的数据显示标准对话框，带有 *Yes* 和 *No* 按钮。用在 `StandardEditor` 中。`showUnsavedChangesDialog()` 有响应用户操作的处理器。示例用法可以参考 `io.jmix.ui.screen.StandardEditor#preventUnsavedChanges` 方法。
* `showSaveConfirmationDialog()` - 为确认保存更改的数据显示标准对话框带有 *Save*，*Do not save* 和 *Cancel* 按钮。用在 `StandardEditor` 中。`showSaveConfirmationDialog()` 有响应用户操作的处理器。示例用法可以参考 `io.jmix.ui.screen.StandardEditor#preventUnsavedChanges` 方法。
+
可以用 xref:ui:app-properties.adoc#jmix.ui.screen.use-save-confirmation[jmix.ui.screen.use-save-confirmation] 应用程序属性调整对话框类型。
