...
Thus, changes in the `com.company.demo.widgets` package trigger the widgetset compilation. Other changes, for example, in screen controllers or XML descriptors do not trigger such compilation. So if you see unexpected re-compilation, check the particular reason (execute `compileWidgets` with the `--info` parameter).

The `excludePaths` and `includePaths` parameters accept the list of exclude and include patterns, respectively.
...
----
<1> Returns the `ColorButtonState` object for this connector.
<2> Reacts on server state change.
<3> Refreshes the widget's style if the value on server has changed.
