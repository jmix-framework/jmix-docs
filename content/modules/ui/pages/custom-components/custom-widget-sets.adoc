...
[NOTE]
====
The `excludePaths` parameter for the `compileWidgets` task is useful if you don't want to compile a widgetset if, for example, some screen is changed.
...
`includePaths` prevent re-compilation only issued by source code changes. If you see unexpected re-compilation, check the particular reason (execute `compileWidgets` with the `--info` param).

The `excludePaths` and `includePaths` params accept the list of exclude and include patterns, respectively. For pattern rules see: https://docs.gradle.org/current/javadoc/org/gradle/api/tasks/util/PatternFilterable.html[^].
====
...
The `ColorButtonState` state class defines what data are sent between the client and the server. It contains public fields that are automatically serialized on the server-side and deserialized on the client.
