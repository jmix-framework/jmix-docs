...
[NOTE]
====
You can recompile the theme without restarting the application:

* In Jmix Studio, open the *Gradle* tool window and double-click *Tasks* → *web* → *compileThemes*.
...
Then you should force the browser to reload the page.

Reloading a page in Chrome/Mozilla Firefox::

* Windows and Linux:
** Hold down `Ctrl` + `Shift` and click `R`.
** Or hold down `Ctrl` and click the *Reload* button.
** Or hold down `Ctrl` and click `F5`.
* Mac:
** Press the `⇧ Shift` key on your keyboard while clicking the *Reload* button.
** Or press `⌘ Cmd` + `⇧ Shift` + `R` on your keyboard.

Reloading a page in Microsoft Edge on Windows and Linux::

* Hold down `Ctrl` and click `F5` on your keyboard.
* Or hold down `Ctrl` on your keyboard while clicking the *Refresh* button in Microsoft Edge.

Reloading a page in Safari on Mac::

* Press `Shift` on your keyboard while clicking the *Refresh* button in Safari.
* Or press `⌘ Cmd` + `⌥ Option` + `R` on your keyboard.
====
...
. Restart the application or <<recompile-theme,recompile the theme>>. Now the user will see the application with the *Hover-dark* theme on the first login:
+
