...
* 在 Jmix Studio 中，打开 *Gradle* 工具窗口，双击 *Tasks* → *web* → *compileThemes*。
* 在命令行中，使用以下命令：
+
...
例如，对于 Google Chrome，可以按下 `Shift` + `Ctrl/Cmd` + `R`。
====
...
. 重启应用程序或 <<recompile-theme,重编译主题>>。用户第一次登录后即可查看使用 *Hover-dark* 主题的应用。
+
