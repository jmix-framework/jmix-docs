...
`<data>` 元素可以有 `readOnly="true"` 属性，此时会使用一个特殊的 “不操作” 的 `DataContext` 实现，该实现不跟踪实体的改动，因此不会影响性能。默认情况下，Studio 生成的实体浏览界面会有只读的数据上下文，所以如果需要在实体浏览界面跟踪实体改动并且提交改动的实体，需要再删除 XML 的 `readOnly="true"` 属性。
...
Jmix UI 组件的标准行为是对 JPA 实体进行 CRUD 操作。因此，如果数据绑定至 DTO 实体，则需要重写数据上下文内置的数据处理逻辑。

`commitDelegate` 处理器可以访问包含修改过实体实例的 `SaveContext` 对象。用这个对象自定义 xref:data-access:data-manager.adoc#save[保存逻辑]，示例：
