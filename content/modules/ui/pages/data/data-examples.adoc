...
----
<1> `data` 跟匀速定义 xref:data/data-context.adoc[DataContext] 实例。
...
这个例子中，`Employee` 实体包含了 `equipment` 属性，这个是一对多的集合。所以实现这个界面的最简单的方法就是使用带有 `equipment` 属性的 xref:data-access:fetching.adoc#fetch-plan[fetch plan] 加载 employees 列表，并且使用 xref:ui:data/property-containers.adoc[property container] 来保存 equiment 列表。然后绑定左边的表格到主容器，绑定右边的表格到属性容器。
...
<1> 主容器和主加载器。
<2> 依赖容器和加载器。
<3> 由于没有使用 xref:ui:facets/data-load-coordinator.adoc[DataLoadCoordinator] facet，数据加载器不会自动加载数据。
<4> 主表格。
<5> 从表格。
...
TIP: 使用 xref:ui:facets/data-load-coordinator.adoc[DataLoadCoordinator] facet 可以将数据组件通过声明式的方式连接，不需要写 Java 代码。
...
UI 表格中按照实体属性排序的功能是通过 `CollectionContainerSorter` 实现的，需要为 xref:ui:data/collection-container.adoc[CollectionContainer] 设置该排序器。标准的实现是，如果数据在一页以内能显示，则在内存做数据排序，否则会使用适当的 "order by" 语句发送一个新的数据库请求。"order by" 语句是由 `JpqlSortExpressionProvider` bean 创建。
