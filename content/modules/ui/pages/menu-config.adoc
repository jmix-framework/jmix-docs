...
* `caption` - a caption of the `menu` element. If not set, the caption will be determined by the rule explained <<defining-menu-caption,below>>.
...
* `icon` - an icon of the `menu` element. See xref:ui:icons.adoc[Icons] for details.
...
* `id` - a unique identifier of the element. If no `screen`, `bean`, `class` attributes are defined, the `id` is used to point to a screen with the same `id`. When the user clicks on the menu item, the corresponding screen will be opened on the xref:ui:screens/root-screens.adoc#main-screen[main screen].
+
[source,xml,indent=0]
----
...
* `caption` - a caption of the `item` element. If not set, the caption will be determined by the rule explained <<defining-menu-caption,below>>.
+
[source,xml,indent=0]
----
...
----
* `icon` - an icon of the `item` element. See xref:ui:icons.adoc[Icons] for details.
...
* `entityFetchPlan` - optional xref:data-access:fetching.adoc#fetch-plan[fetch plan] for loading an entity property.
