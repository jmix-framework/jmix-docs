...
[TIP]
====
如需在 Jmix Studio 中添加内部的 `validators` 元素，可以在界面 XML 或者 *Jmix UI* 层级结构面板中选择组件或者其内部元素，然后点击 *Jmix UI* 组件面板的 xref:studio:screen-designer.adoc#component-inspector-add-button[Add] 按钮。
====
