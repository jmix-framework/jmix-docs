...
[TIP]
====
To add a nested `validators` element in Jmix Studio, select the component, or it nested element in the screen descriptor XML or in the *Component Hierarchy* panel and click on the xref:studio:screen-designer.adoc#component-inspector-add-button[Add] button in the *Component Inspector* panel.
====
