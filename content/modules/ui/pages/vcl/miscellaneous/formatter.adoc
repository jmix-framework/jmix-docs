...
In a screen XML-descriptor, such component formatters can be defined in a nested `formatter` element.

[TIP]
====
To add a nested `formatter` element in Jmix Studio, select the component, or it nested element in the screen descriptor XML or in the *Component Hierarchy* panel and click on the xref:studio:screen-designer.adoc#component-inspector-add-button[Add] button in the *Component Inspector* panel.
====

Below is an example of adding a formatter to the `Table.Column` component:
