...
To set the `icon` value in Jmix Studio, select the `icon` attribute in the *Component Inspector* and open the xref:studio:screen-designer.adoc#icon-editor[Choose icon] dialog.
...
If different icons should be displayed depending on the user's xref:localization:index.adoc[locale], you can set paths to the icons in the message pack and specify a message key in the `icon` attribute, for example:
...
If at least one component in the xref:ui:vcl/containers.adoc[container] has the <<required,required>> attribute with `"true"` value and `requiredIndicatorVisible="true"` the required icon will appear in the top right corner of the container. Possible values − `true`, `false`. By default, `requiredIndicatorVisible="false"`.
