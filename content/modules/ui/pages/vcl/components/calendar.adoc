...
[[day-month-names]]
To change the names of days and months, use the `dayNames` and `monthNames` nested elements, for example:

[source,xml,indent=0]
----
include::example$/ex1/src/main/resources/ui/ex1/screen/component/calendar/calendar-screen.xml[tags=customized-calendar-day-names]
----

image::vcl/components/calendar-customized-captions.png[align="center"]

The `dayNames` or `monthNames` element does not extend but overrides a set of standard names. You should define the values of all days of week or month names explicitly.

[TIP]
====
To define `dayNames` or `monthNames` in Jmix Studio, select the component in the screen descriptor XML or in the *Component Hierarchy* panel and click on the xref:studio:screen-designer.adoc#component-inspector-add-button[Add] button in the *Component Inspector* panel.
====
