...
[TIP]
====
To add grouping columns in Jmix Studio, select the `GroupTable` component in the screen descriptor XML or in the *Component Hierarchy* panel and click on the xref:studio:screen-designer.adoc#add-column[Add]*->Group->Column* button in the *Component Inspector* panel.
====
