...
Данный компонент может использоваться как внутри компонента xref:ui:vcl/components/filter.adoc[Filter], так и отдельно от него.
...
[NOTE]
====
Вы можете использовать xref:dyn-attr:index.adoc[динамические атрибуты], например:

[source,xml,indent=0]
----
include::jmix:dyn-attr:example$/ex1/src/main/resources/dynattr/ex1/screen/car/car-browse-propf.xml[tags=filter]
----

Необязательно иметь на экране фасет xref:dyn-attr:dynattr-view.adoc#using-dynamicattributes-facet[dynamicAttributes].
...
Например, вы можете использовать xref:ui:vcl/components/entity-combo-box.adoc[EntityComboBox] вместо `EntityPicker`:
