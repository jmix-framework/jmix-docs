...
该组件可以在 xref:ui:vcl/components/filter.adoc[Filter] 组件内使用或者独立使用。
...
[NOTE]
====
也可以使用 xref:dyn-attr:index.adoc[动态属性]，示例：
...
例如，可以使用 xref:ui:vcl/components/entity-combo-box.adoc[EntityComboBox] 替换 `EntityPicker`：
...
也可以使用带自定义查找界面的 `EntityPicker`，示例：
