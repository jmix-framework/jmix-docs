...
----
<1> `parameterClass="java.lang.Void"` makes the filter parameterless, and `defaultValue="true"` defines that the `where` condition will be added to the data loader query by default.
<2> The `where` clause with a boolean value.
