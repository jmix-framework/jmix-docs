...
[TIP]
====
To add `action` in Jmix Studio, select the component in the screen descriptor XML or in the *Component Hierarchy* panel and click on the xref:studio:screen-designer.adoc#add-picker-action[Add] button in the *Component Inspector* panel.
====
...
To check values entered into the `ValuePicker` component, you can use a xref:vcl/miscellaneous/validator.adoc[validator] in a nested `validators` element.
