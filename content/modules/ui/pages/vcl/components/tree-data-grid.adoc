...
<div class="backoffice-ui-live-demo-container">
    <a href="https://demo.jmix.io/sampler/#main/sample?id=treedatagrid-simple" class="live-demo-btn" target="_blank">ДЕМОНСТРАЦИЯ</a>
</div>
++++

`TreeDataGrid` представляет собой гибридный компонент, сочетающий в себе способности и гибкость xref:vcl/components/data-grid.adoc[DataGrid] с иерархической структурой xref:vcl/components/tree.adoc[Tree]. Этот компонент используется для xref:data-model:entities.adoc[сущностей], которые имеют ссылки на самих себя. Например, это может быть структура каталога продукции или организационная структура компании.
...
Атрибут xref:vcl/xml.adoc#data-container[dataContainer] компонента `TreeDataGrid` должен содержать ссылку на xref:data/collection-container.adoc[контейнер коллекции].
...
См. соответствующий xref:vcl/components/data-grid.adoc#exporting-column-values[раздел] для компонента `DataGrid`.
...
См. xref:vcl/components/tree.adoc#collapse-event[Свернувшееся Событие].

[[column-collapsing-change-event]]
=== ColumnCollapsingChangeEvent

См. xref:vcl/components/data-grid.adoc#column-collapsing-change-event[ColumnCollapsingChangeEvent].
...
См. xref:vcl/components/data-grid.adoc#column-resize-event[ColumnResizeEvent].

[[context-click-event]]
=== ContextClickEvent

См. xref:vcl/components/data-grid.adoc#context-click-event[ContextClickEvent].

=== EditorCloseEvent

См. xref:vcl/components/data-grid.adoc#editor-close-event[EditorCloseEvent].

=== EditorOpenEvent

См. xref:vcl/components/data-grid.adoc#editor-open-event[EditorOpenEvent].

=== EditorPostCommitEvent

См. xref:vcl/components/data-grid.adoc#editor-post-commit-event[EditorPostCommitEvent].

=== EditorPreCommitEvent

См. xref:vcl/components/data-grid.adoc#editor-pre-commit-event[EditorPreCommitEvent].

[[expand-event]]
=== ExpandEvent

См. xref:vcl/components/tree.adoc#expand-event[ExpandEvent].

[[item-click-event]]
=== ItemClickEvent

См. xref:vcl/components/data-grid.adoc#item-click-event[ItemClickEvent].
...
См. xref:vcl/components/data-grid.adoc#sort-event[SortEvent].
...
См. xref:vcl/components/data-grid.adoc#item-details-generator[DetailsGenerator].
...
См. xref:vcl/components/data-grid.adoc#row-description-provider[RowDescriptionProvider].

[[row-style-provider]]
=== RowStyleProvider

См. xref:vcl/components/data-grid.adoc#row-style-provider[RowStyleProvider].
