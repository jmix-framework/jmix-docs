...
[TIP]
====
To add `query` in Jmix Studio, select the component in the screen descriptor XML or in the *Component Hierarchy* panel and click on the xref:studio:screen-designer.adoc#component-inspector-add-button[Add]*->Query* button in the *Component Inspector* panel.
====
...
To add a formatter programmatically, use the `setFormatter()` component method.
