...
`TextField` supports automatic case conversion. The `caseConversion` attribute can have one of the following values:
...
Also, you can use the `conversionErrorMessage` attribute to specify a custom conversion error message declaratively in the screen XML descriptor, for example:
