...
Не используйте `EntityComboBox`, если количество вариантов очень велико (тысячи или более), так как `EntityComboBox` загружает весь список вариантов в браузер пользователя и в память сервера. Для выбора из произвольно большого количества вариантов, используйте xref:vcl/components/entity-picker.adoc[EntityPicker] или xref:vcl/components/suggestion-field.adoc[SuggestionField].
