...
Don't use `EntityComboBox` if the number of options is large (thousands or more). `EntityComboBox` loads the whole list of options to the user's browser and into the server memory. For selection from an arbitrary large lists of options, use xref:vcl/components/entity-picker.adoc[EntityPicker] or xref:vcl/components/suggestion-field.adoc[SuggestionField].
