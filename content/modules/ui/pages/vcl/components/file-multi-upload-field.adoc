...
Компонент `FileMultiUploadField` позволяет загружать файлы во временное хранилище. После этого их можно обработать как объекты `File` в файловой системе сервера или переместить в xref:files:index.adoc[файловое хранилище].
...
<2> В методе `onInit()` добавим слушателей, которые будут реагировать на события успешной загрузки или ошибки.
...
<5> Далее для каждого элемента мэп создается соответствующий объект `FileRef` путем вызова `TemporaryStorage.putFileIntoStorage()`. Также этот метод используется для перемещения загружаемого файла из временного хранилища в FileStorage.
<6> После обработки необходимо очистить список файлов вызовом `clearUploads()` на случай повторной загрузки.
