...
The `FileMultiUploadField` component allows you to upload files to the xref:files:uploading-files.adoc#using-temporary-storage[temporary storage]. After that, you can process them as `File` objects in the server file system or move to the xref:files:index.adoc[file storage].
