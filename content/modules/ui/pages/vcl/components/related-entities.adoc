...
[TIP]
====
To add `property` in Jmix Studio, select the `RelatedEntities` component in the screen descriptor XML or in the *Component Hierarchy* panel and click on the xref:studio:screen-designer.adoc#component-inspector-add-button[Add]*->Property Option* button in the *Component Inspector* panel.
====
