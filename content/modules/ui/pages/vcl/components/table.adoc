...
[TIP]
====
To add `column` in Jmix Studio, select the `columns` element in the screen descriptor XML or in the *Component Hierarchy* panel and click on the xref:studio:screen-designer.adoc#add-column[Add] button in the *Component Inspector* panel.
====
...
[TIP]
====
To define `aggregation` in Jmix Studio, select the aggregated table column in the screen descriptor XML or in the *Component Hierarchy* panel and click on the xref:studio:screen-designer.adoc#component-inspector-add-button[Add] button in the *Component Inspector* panel.
====
...
If the column `id` specifies a related entity's attribute (that is contains dots), the `columnGeneration` handler should be defined as in the following example.

* A column associated with a related entity's attribute:
+
[source,xml,indent=0]
----
...
* The corresponding `columnGenerator` handler:
+
[source,java,indent=0]
----
