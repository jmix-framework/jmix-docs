...
会忽略小于等于 `0` 的选项值；大于实体的 <<max-fetch-size,最大读取数量>> 的选项会替换为该最大值。
...
`itemsPerPageUnlimitedOptionVisible` 属性设置每页数量 `ComboBox` 中的无限制选项（null）是否展示。该属性的默认值为 `true`。
...
[[max-fetch-size]]
====
所有实体的最大读取数量通过 xref:app-properties.adoc#jmix.ui.default-max-fetch-size[jmix.ui.default-max-fetch-size] 进行配置。默认值为 `10000`。如果某个实体需要设置不同的最大读取数，可以配置 xref:app-properties.adoc#jmix.ui.entity-max-fetch-size[jmix.ui.entity-max-fetch-size]。
====
