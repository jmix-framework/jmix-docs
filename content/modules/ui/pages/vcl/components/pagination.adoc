...
The `itemsPerPageUnlimitedOptionVisible` attribute sets the visibility of unlimited (null) option value in the items per page `ComboBox`. The default value is `true`.
...
[[max-fetch-size]]
====
The maximum fetch size for all entities is defined by the xref:app-properties.adoc#jmix.ui.default-max-fetch-size[jmix.ui.default-max-fetch-size] UI property. Its default value is `10000`. A particular entity may have different maximum fetch size, set with xref:app-properties.adoc#jmix.ui.entity-max-fetch-size[jmix.ui.entity-max-fetch-size].
====
