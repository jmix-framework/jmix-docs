...
With automatic configuration, the loader without parameters in its main query text (the static query not including xref:ui:data/data-loaders.adoc#query-conditions[query conditions]) is refreshed on `BeforeShowEvent` in `Screen` or `AttachEvent` in `ScreenFragment`.
...
TIP: `onScreenEvent` triggers are fired *after* listeners of the corresponding events defined in the screen controller with the `@Subscribe` annotation.
--
[[on-fragment-event]]
