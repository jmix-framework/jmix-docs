...
使用自动模式时，如果加载器的查询语句中没有参数（静态查询语句不包含 xref:ui:data/data-loaders.adoc#query-conditions[查询条件]），则该加载器会在 `Screen` 的 `BeforeShowEvent` 或 `ScreenFragment` 的 `AttachEvent` 中自动刷新。

[NOTE]
====
如果主查询语句中有自定义参数（非 `container_` 或 `component_`），则自动配置会忽略该加载器。

如需刷新被忽略的加载器，请通过编码的方式设置参数值并调用加载器的 `load()` 方法。
====
...
TIP: `onScreenEvent` 会在界面控制器中使用 `@Subscribe` 注解定义的对应消息监听器 *之后* 触发。
--
[[on-fragment-event]]
