...
При автоматической конфигурации, если у загрузчика нет параметров в основном тексте запроса (то есть в статическом запросе не включающем xref:ui:data/data-loaders.adoc#query-conditions[условия]), то он срабатывает по событию `BeforeShowEvent` в `Screen` или по событию `attachEvent` в `ScreenFragment`.
...
TIP: Триггеры `onScreenEvent` срабатывают *после* слушателей соответствующих событий, заданных в контроллере экрана аннотациями `@Subscribe`.
--
[[on-fragment-event]]
