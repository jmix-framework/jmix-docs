...
除了支持 `ScreenFacet` 的 xref:facets/screen-facet.adoc#xml[属性] 外，`EditorScreenFacet` 新增了下列自有属性：
...
参阅 xref:facets/screen-facet.adoc#screen-configurer[ScreenConfigurer]。

[[transformation]]
=== Transformation

在编辑界面中选择并验证实体后调用。请参阅 xref:actions/standard-actions/create-action.adoc#transformation[Transformation] 了解详情。
