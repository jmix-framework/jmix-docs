...
. 定义一个继承自 `BackgroundTask` 抽象类的任务。任务类的构造函数接收两个参数，一个是任务关联的界面控制器，另一个是用于任务构造函数的超时时限。
+
关闭界面将中断与其相关的任务。此外，任务将在指定的时限超时后自动中断。

. 在 `BackgroundTask.run()` 方法中实现任务逻辑。

. 通过将任务实例传递给 `BackgroundWorker` bean 的 `handle()` 方法，可以创建一个 `BackgroundTaskHandler` 类对象用于对任务进行控制。在界面控制器中注入或通过 `ApplicationContext` 类来获得对 `BackgroundWorker` 的引用。

. 调用 `BackgroundTaskHandler` 的 `execute()` 方法来运行任务。

[WARNING]
不要在 `BackgroundTask.run()` 方法中读取或更新 UI 组件的状态和数据容器，可以使用 `done()`、`progress()` 和 `canceled()` 回调方法替代。如果尝试从后台线程设置 UI 组件的值，则会抛出 `IllegalConcurrentAccessException`。
...
1. `T` - 显示任务进度的对象类型。在工作线程中调用 `TaskLifeCycle.publish()` 期间，将此类型对象传递给任务的 `progress()` 方法。
...
该方法应该支持外部中断。如需提供该功能，建议在长时间运行时定期检查 `TaskLifeCycle.isInterrupted()` 标识，并在需要时停止执行。另外，不应该忽略掉 `InterruptedException`（或任何其他异常） - 而应该正确退出方法或根本不处理异常（将异常暴露给调用方）
...
当发生任何异常时在 UI 线程中调用该方法。

[[notes-and-tips]]
== Notes & Tips

1. 用 `dialogs.createBackgroundWorkDialog()` 方法展示模态窗，包含一个进度条和 *Cancel* 按钮。该模态窗可以定义进度条的不同类型，以及允许或禁止取消后台任务。
+
...
3. 后台任务受 xref:app-properties.adoc#jmix.ui.background-task-timeout-check-interval[jmix.ui.background-task-timeout-check-interval] 和 xref:app-properties.adoc#jmix.ui.background-task.threads-count[jmix.ui.background-task.threads-count] 应用程序属性的影响。


[[usage-example]]
== 用例

通常，在启动后台任务时，你需要展示一个简单的用户界面：

1. 为用户展示请求的操作正在执行中。
2. 允许用户中止长时间的操作。
3. 如果可以确定执行的百分比，则显示操作的进度。

可以用 xref:dialogs.adoc[Dialogs] 接口的 `createBackgroundWorkDialog()` 方法完成。

考虑下面情形作为示例：

1. 给定的界面包含展示 customer 的表格，支持多选。
2. 当用户点击按钮时，系统给选定的 customer 发送提醒邮件，发邮件的过程不阻塞 UI，并支持中止操作。
