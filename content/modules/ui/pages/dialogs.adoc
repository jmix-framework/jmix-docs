...
Значение, переданное в метод `withHtmlSanitizer()`, имеет приоритет над значением глобального свойства xref:app-properties.adoc#jmix.ui.component.html-sanitizer-enabled[jmix.ui.component.html-sanitizer-enabled].
