...
NOTE: 使用 `DataLoadCoordinator` facet 以声明式的方式将数据加载器和数据容器、可视化组件、界面事件进行关联。除非在需要设置一些加载参数的时候，才使用编程的方法，如xref:ui:data/data-examples.adoc#using-screen-parameters-in-loaders[下面]例子所示。
