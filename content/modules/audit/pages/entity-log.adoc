...
实体日志是一种监控系统内 JPA 实体变更的机制。用于记录实体属性的改动并提供用户视图用于查找和展示变更的详细信息：
...
如果 JPA 实体是通过 xref:data-access:data-manager.adoc[DataManager] 或 xref:data-access:entity-manager.adoc[EntityManager] 保存，实体日志会在事务提交时自动记录 JPA 实体的变更。但对原生 SQL 无效。

另外，如果实体是通过 `EntityManager` 加载，可以在应用程序代码中直接使用 `EntityLog` bean 记录实体的变更：调用 `registerCreate()`、`registerModify()` 和 `registerDelete()` 方法，并设置 `auto` 参数为 `false`。当框架自动调用实体日志记录时，这个参数是 `true`。

NOTE: 只有在处理 _managed_ 实体的事务中才能使用 `EntityLog` 记录变更。也就是说，只有在使用 `EntityManager` 加载数据时，才需要从应用代码调用 `EntityLog`。通过 `DataManager` 加载的实体始终是 _detached_ 的，其更改无法通过手动调用 `EntityLog` 来记录。
...
* *Auto* 复选框配置当 JPA 子系统调用 `EntityLog` 提交事务时（`auto = true`），系统是否需要记录变更。

* *Manual* 复选框配置当应用程序代码调用 `EntityLog` 时（`auto = false`），系统是否需要记录变更。
...
在下面的示例中，在数据库初始化时，完成了对 `Customer` 实体 `phone` 属性的变更记录的配置：
...
在下面的示例中，`Order` 实体的详情视图展示实体日志内容的列表。这是视图 XML 的部分内容：
