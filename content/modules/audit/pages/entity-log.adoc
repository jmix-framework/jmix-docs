...
实体日志是一种跟踪 JPA 实体变更的机制。它记录实体属性的改动并提供用户视图用于查找和展示变更的信息：
...
如果 JPA 实体是通过 xref:data-access:data-manager.adoc[DataManager] 或 xref:data-access:entity-manager.adoc[EntityManager] 保存，则实体日志会自动跟踪实体的变更。但如果使用原生 SQL，则实体日志不会起作用。
...
你可以在应用程序的 *Audit -> Entity Log* 视图中配置实体日志。
...
打开 *Audit -> Entity Log* 视图即可查看实体日志。可以在过滤器中设置必要的条件查找日志记录。
