...
The user should have the capability to see next and previous dates or scroll to a specific time in the time-grid calendar display modes. Currently, our application does not offer this functionality.

[[date-navigation]]
== Date Navigation

`FullCalendar` provides methods for next and previous navigation. The "step" of next and previous navigation depends on the current calendar's display mode.
...
The `scrollTime` attribute takes the string that follows the ISO-8601 duration format, which is used by the `java.time.Duration` class. For instance:
