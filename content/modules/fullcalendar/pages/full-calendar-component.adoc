...
TIP: Selection mode can be enabled by <<selectionEnabled, selectionEnabled>> property.
...
| `io.jmix.fullcalendarflowui.component.event.WeekNavigationLinkClickEvent` fired when the user clicks on the week number navigation link. The navigation link can be activated
* by the <<navigationLinksEnabled, navigationLinksEnabled>> property.
...
TIP: The now-indicator can be enabled by <<nowIndicatorVisible, nowIndicatorVisible>> property.
