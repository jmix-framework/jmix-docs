...
- `acceptedFileExtensions` requires a leading dot (`.png`, not `png`). **Corrected by live testing**: the recorded symptom — loads fine, fails at the first upload — is wrong. It throws from `onAttach` when the view opens, so the user gets an error dialog and no chat at all. Fail-fast and unmissable, which is a different warning from a lurking trap. → `attachments.adoc`.
- **Attachments vanish after a reload unless the fetch plan names them.** An `@ElementCollection` is not in `_base`, so a chat whose messages are saved correctly renders no attachment when the conversation is loaded again — files on disk, rows in the database, nothing in the log. `<property name="attachments"/>` in the fetch plan is the fix, and it belongs beside the message factory snippet: a reader who writes the factory and stops there has a feature that works until the first reload. → `attachments.adoc`.
- **A provider that reads only `userMessage()` silently drops the files.** `LLMRequest` declares `attachments()` as well, returning `List<AIAttachment>` (`name`, `mimeType`, `byte[] data`), and the add-on populates it. The obvious one-line provider ignores it, so the files are stored and displayed while the model never sees them — observed answer: "I currently don't have access to any attached files." The documented provider maps them into Spring AI `Media`. → `attachments.adoc` and `getting-started.adoc`.
